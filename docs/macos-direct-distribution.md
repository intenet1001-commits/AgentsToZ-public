...
## 공증 인증 조회가 반복해서 실패할 때

빌드나 재등록 전에 기존 프로필을 읽기 전용으로 확인한다. 아래 명령은 Apple 앱 암호를 입력받거나 키체인 항목을 생성·삭제하지 않는다. 실제 `notarytool history`의 종료 코드와 JSON 응답까지 확인하므로 뒤에 실행한 다른 명령의 성공을 인증 성공으로 오인하지 않는다.

```bash
bun check-macos-notary.ts --profile YOUR_PROFILE
# 최초 저장할 때 --keychain을 지정했다면 같은 절대경로를 사용한다.
bun check-macos-notary.ts --profile YOUR_PROFILE --keychain /absolute/path/login.keychain-db
```

`--keychain`을 생략한 `notarytool`은 data protection keychain을 사용한다. 일반 `login.keychain-db`와 별개이므로 `security find-generic-password`에서 찾지 못했다고 기존 인증이 삭제됐다고 판단하지 않는다. 해당 저장소는 사용자 로그인 실행 환경에도 의존한다. [Apple TN3147](https://developer.apple.com/documentation/technotes/tn3147-migrating-to-the-latest-notarization-tool), [Apple TN3137](https://developer.apple.com/documentation/technotes/tn3137-on-mac-keychains)

- `ready`: 반환된 `authArgs`의 프로필과 저장소를 이후 `submit`, `wait`, `info`, `log`에도 그대로 사용한다. 저장소를 임의로 바꾸지 않는다.
- `session-locked-profile-unavailable`: 화면이 잠긴 상태에서 기존 프로필 조회가 실패했다. 잠금이 원인으로 확정됐다는 뜻은 아니다. 재등록 안내부터 띄우지 말고 사용자가 Mac을 정상적으로 잠금 해제한 뒤 같은 프로필을 재확인한다.
- `profile-unavailable`: 최초 저장 위치와 현재 로그인 실행 환경을 확인한다. 이 오류만으로 잘못된 Apple 암호나 자격 증명 삭제를 단정하지 않는다.
- `apple-auth-rejected`: Apple이 실제로 HTTP 401을 반환한 경우다. 로컬 항목 조회 실패와 분리하여 인증 정보를 점검한다.
- 그 외: 키체인 상호작용·도구 오류·시간 초과·네트워크를 먼저 확인한다. 자동 암호 재입력이나 무한 재시도는 하지 않는다.

반복 무인 빌드용으로 파일 기반 키체인을 선택한다면 최초 `store-credentials`와 모든 조회 명령에 동일한 `--keychain` 절대경로를 지정하고, 저장 직후 별도 프로세스에서 위 검사를 통과시킨다. 기존 data protection 항목이 파일 기반 키체인으로 자동 이전되는 것은 아니다. 실제 잠금 상태에서의 성공 여부도 별도로 검증한다. 화면 잠금이나 키체인 보호 설정을 해제해서 문제를 우회하지 않는다.
...
## Developer ID 직접 배포 명령

공식 명령은 감사된 `AgentsToZ-public`의 유일한 `origin`이 가리키는 clean 기본 브랜치 HEAD에서만 실행한다.
먼저 기존 공증 프로필을 읽기 전용으로 확인한다.

```bash
bun check-macos-notary.ts --profile YOUR_PROFILE
# 별도 파일 키체인에 저장했다면 모든 단계에서 같은 절대경로를 사용한다.
bun check-macos-notary.ts --profile YOUR_PROFILE --keychain /absolute/path/release.keychain-db
```

검사가 `ready`일 때 같은 profile·keychain 인자로 다음 명령을 실행한다.

```bash
bun run build:macos-direct-distribution --profile YOUR_PROFILE
# 또는
bun run build:macos-direct-distribution --profile YOUR_PROFILE --keychain /absolute/path/release.keychain-db
```

이 명령은 다음 순서만 수행한다.

1. `developer-id-base`의 공개 소스·Developer ID canary·inside-out 앱 서명 검증
2. 앱 ZIP 제출→submission ID 저장→wait/info/log의 `Accepted` 확인→앱 ticket staple
3. stapled 앱으로 arm64 DMG 생성→Developer ID로 DMG 서명
4. DMG 제출→별도 submission ID 저장→`Accepted` 확인→DMG ticket staple
5. DMG를 읽기 전용으로 마운트해 내부 앱 코드서명, app/DMG ticket, Gatekeeper와 앱 CDHash를 재검증
6. 최종 DMG SHA-256과 로컬 receipt 생성

공증 wait가 시간 초과되거나 응답이 유실되면 같은 파일을 자동 재제출하지 않는다. 출력 폴더의
`*-notary-<submissionId>.submission.json`에 기록된 ID로 상태를 조회한 뒤 명시적으로 복구한다.
완료 receipt의 `notarized-dmg-awaiting-install-test`는 공증 산출물이 생겼다는 뜻이며 설치·공개 게시
완료가 아니다. 새 Mac 다운로드·설치·첫 실행 검증 전에는 `installed:false`, `published:false`,
`ready:false`를 유지한다.
