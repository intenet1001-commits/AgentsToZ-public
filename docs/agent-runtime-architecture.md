...
- 기본 원격 프로토콜: `agentstoz-local-v9` (OPS 상태·열기·후보 조회, SAS 사람 검토 포함)
- Internet task transport: `agentstoz-local-v9` + explicit `tasks-v1` scope (구현, execution safety hold)
- Internet conversation transport: 같은 E2EE v9 + explicit `conversations-v1` scope (구현, execution safety hold)
- 최종 갱신: 2026-09-05
...
현재 `agentstoz-local-v9`은 기존 프로젝트·프로세스 제어에 OPS 상태·열기·대기 후보 조회를 더한다. 공유 OPS 후보의 저장/거절은 Internet SAS 승인 controller authority에서만 허용하며 LAN QR은 조회만 가능하다. Task route나 event를 기본 DTO에 끼워 넣지 않으며, 기존 pairing/session이 앱 업데이트만으로 agent 실행 권한을 얻지 않는다.

Internet 원격 Agent Runtime은 `agentstoz-local-v9` transport의 별도 `tasks-v1` scope로 구현했다. 기존 세션은 자동 승격되지 않으며 Mac의 SAS 승인 화면에서 이 모바일 기기에 작업 권한을 명시적으로 체크해야 한다. 다만 로컬 production containment gate가 닫혀 있어 end-to-end 실행은 safety hold 상태다.
...
4. **Internet E2EE `agentstoz-local-v9` + opt-in `tasks-v1` — transport/UI 구현**
