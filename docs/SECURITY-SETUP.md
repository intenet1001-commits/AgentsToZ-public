...
browser mode에서는 `http://127.0.0.1:9000/portal.html`의 Google OAuth가 root 앱과 같은
origin 및 `portmgr-auth` storage key를 사용한다. callback 뒤 root 앱의 Push/Pull에도 JWT가
붙는다. Tauri는 browser localStorage를 공유한다고 가정하지 말고 별도 OAuth 증거가 없으면
browser mode를 sync 경로로 사용한다.

```
1) 정본 MIGRATION_SQL 적용 + RLS enabled 확인
2) Redirect URLs에 browser root와 desktop callback/* 허용
3) browser/desktop Google 로그인 후 portmgr_is_member() = true 확인
4) portmgr_allowed_members에 허용 이메일 row 등록
5) 필요할 때 anon key 로테이션 및 모든 기기 설정 갱신
```
...
- [ ] **앱/포털에서 로그인 세션으로 Push/Pull 확인** → 0장. 세션이 없으면 RLS가 요청을 거부한다
