...
이 설명은 공개판 사용자가 **자기 Supabase와 자기 Vercel**로 독립된 개인 웹 포털을 만드는 순서입니다.
결과는 휴대폰·다른 브라우저에서 여는 `https://<내-프로젝트>.vercel.app` 주소입니다. 데스크톱 앱을 설치하거나 내 PC에 저장소를 clone하는 과정이 아니며, 원작자의 개인 포털이나 데이터베이스에도 연결하지 않습니다.

로컬 앱만 쓸 때는 이 작업이 필요 없습니다. 웹 포털을 휴대폰이나 다른 브라우저에서도 열고
싶을 때만 진행하세요. 아래 1번부터 한 단계씩 완료하고, 각 단계의 **성공 근거**를 확인한 뒤
다음으로 넘어가면 됩니다.

`Deploy with Vercel`은 설치 파일 다운로드 버튼이 아닙니다. Vercel 가입·로그인과 Git 제공자 연결을 거쳐, 연결한 GitHub·GitLab·Bitbucket 계정에 공개 소스 사본을 만들고 본인 Vercel 계정에 새 프로젝트·웹 주소를 만드는 버튼입니다. Vercel 계정이 없다면 버튼 안에서 가입할 수 있지만, 완료하려면 본인 소유의 Git 제공자·Vercel·Supabase·Google 계정이 모두 필요합니다. 각 서비스의 가입·요금제·사용량 비용은 본인이 확인하고 관리합니다.
...
| 허용 Google 이메일 | Supabase `portmgr_allowed_members` | 개인 정보이므로 Git·Vercel에는 넣지 않음 |
...
회원 권한의 단일 정본은 Supabase의 `portmgr_allowed_members`와 이를 확인하는
`portmgr_is_member()`입니다. 포털 빌드에는 이메일 목록을 복제하지 않습니다. 오류가 나도 RLS를 끄면 안 됩니다.
...
1. AgentsToZ 앱에서 **초기 설정 → 선택 기능 · 개인 웹 포털과 고급 설정 → 내 계정에 개인 웹 포털 만들기**를 엽니다.
...
[Vercel에 내 개인 포털 프로젝트 만들기](https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fintenet1001-commits%2FAgentsToZ-public)

Vercel은 이 흐름을 `Deploy with Vercel` 또는 Deploy Button이라고 부릅니다. 버튼에는 공개 GitHub 저장소 주소만 들어 있습니다. Supabase 값, 이메일, 토큰은 URL로 전달하지 않습니다. 이 과정은 내 PC의 clone이나 데스크톱 앱 설치를 대신하지 않습니다.

1. Vercel에 가입하거나 로그인하고, 사용할 GitHub·GitLab·Bitbucket 계정을 연결합니다.
2. 화면에 표시된 Git 제공자·Vercel 계정/팀이 본인 소유인지 확인합니다. 공개 소스 사본과 새 프로젝트 이름을 정한 뒤 생성·배포합니다.
3. 프로젝트 **Settings → Environment Variables**에서 **Production**에 아래 두 항목을 추가합니다.

   - `VITE_SUPABASE_URL`: 1번에서 확인한 본인 Supabase Project URL
   - `VITE_SUPABASE_ANON_KEY`: 1번에서 확인한 본인 anon 또는 publishable key
4. 회원 이메일 목록, `service_role`, Google Client Secret, GitHub/Vercel 로그인 토큰은 추가하지 않습니다. 회원은 Supabase `portmgr_allowed_members`에서만 관리합니다.
