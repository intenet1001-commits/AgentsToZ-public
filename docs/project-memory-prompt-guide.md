...
`세션 기억하기 필요` 상태를 표시합니다. 기본값에서는 실제 AI 호출이 사용자가
`세션 기억하기` 버튼이나 명령을 실행할 때 한 번 발생합니다.

사용자가 AI 사용량 패널에서 **Codex 자동 세션 기억**을 명시적으로 켜면 예외적으로
호스트 서비스가 50%·75%·90% 컨텍스트 체크포인트를 관찰합니다. `task_complete`로
현재 턴 종료가 증명되고, 등록 프로젝트의 canonical memory root가 정확히 해석되며,
위 활동 판정도 저장 필요일 때만 Codex 비대화형 기억 갱신을 한 번 실행합니다.
켜기 전의 완료 턴, subagent, 미등록 폴더, 임시 대화 폴더는 자동 저장하지 않습니다.
