...
외부 relay의 모바일 워크룸 입력은 결과 확인 1초 주기 때문에 안전 선택 키와 첫 출력 조회가 각각 최대 1초씩 추가 대기할 수 있다. 메뉴 숫자·y/n·Esc/Tab·화살표·제어 키·Enter에만 250ms 결과 확인을 최대 네 번 적용하고, 같은 세션의 첫 출력 조회도 입력 성공 후 5초 안에 한 번만 빠르게 확인한다. 일반 텍스트 입력, 이후 평상시 출력·목록·워크스페이스 조회는 기존 1초 주기를 유지해 지속 입력 때 relay RPC가 늘지 않는다. 연결 토큰이 교체되면 빠른 출력 조회 예약을 폐기한다. 전체 미확정 결과 대기 예산은 약 60초, 암호화 envelope·requestId·순서·재전송 금지 계약은 변하지 않는다. 체감 개선 폭은 Mac의 1초 수신 주기와 실제 네트워크 지연에 따라 달라지므로 실기기에서 별도 측정한다.
