      ...
      try {
        ...
        if (IS_WIN) {
          ...
          const filter = documentMode
            ? 'Document Files (*.html;*.htm;*.md;*.markdown;*.txt;*.log;*.json;*.csv;*.pdf)|*.html;*.htm;*.md;*.markdown;*.txt;*.log;*.json;*.csv;*.pdf|All Files (*.*)|*.*'
  ...
  return ""
    ...
    if (url.pathname === "/api/open-orca-localhost" && req.method === "POST") {
      try {
        ...
        if (!useFloating && wtFirst && hasHiddenOrcaPathSegment(wtFirst)) return hiddenOrcaWorktreeResponse(headers);
        ...
        return await withOrcaLock(async () => {
          ...
          // Windows fallback: floating 모드에서도 path selector 사용 (id:global-floating-terminal 미지원)
          let worktreeSelector = useFloating ? ORCA_FLOATING_WORKTREE_SELECTOR : `path:${cdPath}`;
          if (!useFloating) {
          ...
          let created = await nodeOrcaRunJsonRetry(
            cli,
          ...
          // orcaEnsureRepo 없이 path selector를 직접 시도 — 이미 Orca에 등록된 경로거나
          // folder-kind 워크트리(= git 아닌 폴더)여도 tab create는 동작한다.
          if (!created.ok && useFloating && String(created.error ?? '').includes('selector_not_found') && cdPath) {
            worktreeSelector = `path:${cdPath}`;
            const fallback = await nodeOrcaRunJsonRetry(
              cli,
              ['tab', 'create', '--url', localhostUrl, '--worktree', worktreeSelector],
              { attempts: 1, timeoutMs: 8000 },
            );
            if (fallback.ok) created = fallback;
          }
          if (!created.ok) {
          ...
          return new Response(JSON.stringify({
            ...
            message: useFloating
              ? `Orca 플로팅 워크스페이스에서 localhost:${portNumber} 열림`
                + (revealWarning ? `\n⚠ ${revealWarning} Orca에서 Cmd/Ctrl+Alt+A를 눌러 확인하세요.` : '')
              : `Orca 프로젝트에서 localhost:${portNumber} 열림`,
            revealWarning,
          }), { headers });
        });
      } catch (error: any) {
...
${evidence}`;
      ...
      try {
        ...
        if (fetchRemote) {
          ...
          if (originResult.ok) {
            ...
            remoteRefreshError = 'origin 원격 저장소가 없습니다';
          }
        }
    ...
    // ── VOC(개선 요청) ─────────────────────────────────────────────────────
    // 화면에서 바로 남긴 개선 요청을 앱 데이터 폴더의 voc/ 에 한 건당 파일 하나로 쌓는다.
    // Supabase를 쓰지 않는 이유: 로컬 폴더면 검토 세션에서 그냥 읽으면 되므로 내보내기
    // 경로가 필요 없고, 지금 RLS가 라이브 DB에 적용돼 있지 않아 솔직한 제품 메모를
    // 원격에 두는 것이 위험하다. 처리 완료는 voc/done/ 으로 옮기는 것으로 끝난다.
    if (url.pathname === "/api/voc" && req.method === "POST") {
      try {
        const body = await req.json() as { comment?: unknown; anchor?: unknown; tab?: unknown; appVersion?: unknown };
        ...
        return new Response(JSON.stringify({ success: true, file, id: record.id }), { headers });
      } catch (e: any) {
        return new Response(JSON.stringify({ error: e.message }), { status: 500, headers });
      }
    }
