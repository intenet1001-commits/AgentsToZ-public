...
import { resolveRegisteredProjectMemory, type ProjectMemoryProjectResolution } from "./src/projectMemoryProjectResolver";
...
import { parseAiMemoryAlias } from "./src/projectMemoryAiAlias";
...
async function runPortalDeployment(input: {
  ...
  try {
    ...
    for (const [name, value] of [
      ...
      ['VITE_ALLOWED_EMAIL', input.allowedEmail],
    ] as const) {
    ...
    deployStatus.url = persistPortalDeployUrl(productionUrl);
...
function persistPortalDeployUrl(value: unknown): string {
  ...
  const temporaryFile = `${PORTAL_DATA_FILE}.tmp-${process.pid}-${crypto.randomUUID()}`;
  try {
    writeFileSync(temporaryFile, JSON.stringify({ ...current, portalDeployUrl }, null, 2));
    renameSync(temporaryFile, PORTAL_DATA_FILE);
...
async function authenticatedCliDiagnosticAsync(
  ...
  if (auth.timedOut) {
    return { id, state: 'unknown', installed: true, version: compactVersionLabel(version.output), detail: '로그인 확인 시간이 초과됐습니다.' };
  }
  return auth.ok
    ? { id, state: 'ready', installed: true, authenticated: true, version: compactVersionLabel(version.output) }
    : { id, state: 'needs-login', installed: true, authenticated: false, version: compactVersionLabel(version.output) };
}
...
async function buildOnboardingToolsDiagnostics(): Promise<OnboardingToolsResponse> {
  ...
  const optionalAgentDiagnostics = await Promise.all([
    ...agentExecutables.map(({ id, executable }) => executableDiagnosticAsync(id, executable)),
...
function loadDesktopSupabaseProxyTarget(): { url: string; serviceRoleKey: string } {
  ...
  if (parsed.protocol !== 'https:' && parsed.protocol !== 'http:') throw new Error('지원하지 않는 Supabase URL입니다.');
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    const headers: Record<string, string> = {
      ...
      "Access-Control-Allow-Headers": "Content-Type, Authorization, apikey, X-Client-Info, Prefer, Range, Range-Unit, Accept-Profile, Content-Profile, X-Supabase-Api-Version",
    ...
    // 데스크톱 앱은 Google 로그인을 요구하지 않는다. WebView에는 service_role 키를 절대
    // 넘기지 않고, Tauri origin에서 온 PostgREST 요청만 portal.json의 고정 Supabase로
    // 전달한다. 배포 포털과 일반 웹페이지는 이 경로를 사용할 수 없다.
    if (url.pathname.startsWith(`${DESKTOP_SUPABASE_PROXY_PREFIX}/`) || url.pathname === DESKTOP_SUPABASE_PROXY_PREFIX) {
      if (!isTauriAppOrigin(requestOrigin)) {
        return new Response(JSON.stringify({
          code: 'DESKTOP_SUPABASE_PROXY_DENIED',
          message: '데스크톱 앱에서만 사용할 수 있는 Supabase 경로입니다.',
      ...
      try {
        ...
        if (request.action === "create-github-repository") {
          ...
          if (request.archiveMemory) {
            try {
              ...
              const archived = await runProjectMemoryPrivateGitHubArchive({
                folderPath: project.canonicalPath,
        ...
        if (request.action === "open-code-app") {
          const internalResponse = await fetch(`http://127.0.0.1:${Number(process.env.API_PORT) || 3001}/api/open-code-app`, {
            ...
            body: JSON.stringify({ agent: request.agent, folderPath: project.canonicalPath }),
          });
    ...
    if (url.pathname === "/api/project-memory/refresh-resolved-status" && req.method === "POST") {
      try {
        ...
        const refreshed = await remoteProjectMemoryStatus({
          portalDataFile: PORTAL_DATA_FILE,
          folderPath: resolution.canonicalPath,
        });
    ...
    if (url.pathname === "/api/supabase-service-key/from-cli" && req.method === "POST") {
      try {
        const body = await req.json().catch(() => ({})) as { supabaseUrl?: unknown };
        const portal = existsSync(PORTAL_DATA_FILE)
          ? JSON.parse(readFileSync(PORTAL_DATA_FILE, "utf8"))
          : {};
        // 추가 단말 마법사에서는 portal.json을 저장하기 전에 로컬 관리자 연결을
        // 검증한다. 초대에 들어 있던 URL을 이 요청에만 사용하고, 서비스 키나 CLI
        // 로그인 토큰은 응답/브라우저/다른 단말로 돌려보내지 않는다.
        const requestedUrl = typeof body.supabaseUrl === "string" ? body.supabaseUrl.trim() : "";
        const ref = supabaseProjectRefFromUrl(requestedUrl || portal?.supabaseUrl);
        if (!ref) {
          return new Response(JSON.stringify({
    ...
    if (url.pathname === "/api/project-memory/private-github-archive/status" && req.method === "POST") {
      try {
        const body = await readMemoryParams(req, url);
        const local = detectProjectMemory(body.folderPath);
        if (!local.exists || !local.config) {
          return new Response(JSON.stringify({
            error: "Private GitHub에 보관할 프로젝트 장기기억이 초기화되지 않았습니다.",
            code: "PROJECT_MEMORY_NOT_INITIALIZED",
          }), { status: 404, headers });
        }
        return new Response(JSON.stringify({
    ...
    if (url.pathname === "/api/project-memory/private-github-archive/enable" && req.method === "POST") {
      try {
        const body = await readMemoryParams(req, url);
        if (typeof body.githubUrl !== "string" || !body.githubUrl.trim()) {
          return new Response(JSON.stringify({
            error: "먼저 이 프로젝트에 Private GitHub 저장소를 만들거나 연결하세요.",
            code: "GITHUB_REPOSITORY_REQUIRED",
          }), { status: 400, headers });
        }
        // Enabling is itself the first verified archive. The setting is written
        // only after gh confirms PRIVATE + WRITE and the exact branch Push wins.
        const archived = await runProjectMemoryPrivateGitHubArchive({
          folderPath: body.folderPath,
          repositoryUrl: body.githubUrl,
        });
        if (!archived.result.success) {
          return new Response(JSON.stringify({
            enabled: false,
            archive: archived.result,
            error: archived.result.error,
            code: archived.result.errorCode,
          }), { status: 409, headers });
        }
        const status = enableProjectMemoryPrivateGitHubArchive({
          appDataDir: APP_DATA_DIR,
          projectRoot: archived.snapshot.projectRoot,
          memoryId: archived.snapshot.memoryId,
          repositoryUrl: body.githubUrl,
        ...
        return new Response(JSON.stringify({ ...status, archive: archived.result }), { headers });
      } catch (error: any) {
        return new Response(JSON.stringify({ error: error.message, code: error.code }), { status: 400, headers });
      }
    }

    if (url.pathname === "/api/project-memory/private-github-archive/run" && req.method === "POST") {
      try {
        const body = await readMemoryParams(req, url);
        const snapshot = projectMemoryPrivateGitHubArchiveSnapshot({ folderPath: body.folderPath });
        const record = readProjectMemoryPrivateGitHubArchiveRecord({
          appDataDir: APP_DATA_DIR,
        ...
        if (!record?.enabled) {
          return new Response(JSON.stringify({
            error: "이 프로젝트의 Private GitHub 장기 보관이 켜져 있지 않습니다.",
            code: "PRIVATE_GITHUB_ARCHIVE_DISABLED",
          }), { status: 409, headers });
        }
        const result = await archiveProjectMemoryToPrivateGitHub({
          ...snapshot,
          ...
          expectedRepositoryId: record.repositoryId,
          gitCommand: GIT_PATH,
          ghCommand: resolveGitHubCliPath() ?? "gh",
        });
        const status = recordProjectMemoryPrivateGitHubArchiveResult({
          appDataDir: APP_DATA_DIR,
          projectRoot: snapshot.projectRoot,
          memoryId: snapshot.memoryId,
          result,
        ...
        return new Response(JSON.stringify({
          ...
          archive: result,
          ...(!result.success ? {
            error: result.error ?? "Private GitHub 보관에 실패했습니다.",
            code: result.errorCode ?? "PRIVATE_GITHUB_ARCHIVE_FAILED",
          } : {}),
        }), {
    ...
    if (url.pathname === "/api/project-memory/private-github-archive/disable" && req.method === "POST") {
      try {
        const body = await readMemoryParams(req, url);
        const local = detectProjectMemory(body.folderPath);
        if (!local.exists || !local.config) {
          return new Response(JSON.stringify({
            error: "프로젝트 장기기억이 초기화되지 않았습니다.",
            code: "PROJECT_MEMORY_NOT_INITIALIZED",
          }), { status: 404, headers });
        }
        const status = disableProjectMemoryPrivateGitHubArchive({
          appDataDir: APP_DATA_DIR,
          projectRoot: local.projectRoot,
          memoryId: local.config.memoryId,
        });
        return new Response(JSON.stringify({
          ...status,
          remoteDeleted: false,
          message: "이 단말의 자동 보관만 껐습니다. GitHub 보관 branch는 삭제하지 않았습니다.",
        }), { headers });
      } catch (error: any) {
        return new Response(JSON.stringify({ error: error.message, code: error.code }), { status: 400, headers });
      }
    ...
    if (url.pathname === "/api/project-memory/recall" && req.method === "POST") {
      try {
        ...
        const recallLimit = typeof body.limit === "number"
          ? body.limit
    ...
    if (url.pathname === "/api/project-memory/push" && req.method === "POST") {
      try {
        ...
        if (result.success === true) queueEnabledProjectMemoryPrivateGitHubArchive(body.folderPath);
    ...
    if (url.pathname === "/api/project-memory/session-end" && req.method === "POST") {
      try {
        ...
        if (result.localSaved === true) queueEnabledProjectMemoryPrivateGitHubArchive(body.folderPath);
        return new Response(JSON.stringify(result), { headers });
      } catch (error: any) {
        return new Response(JSON.stringify({ error: error.message }), { status: 500, headers });
      }
    ...
    if (url.pathname === "/api/build-status" && req.method === "GET") {
      ...
      const requestedCursor = cursorParam !== null && /^\d+$/.test(cursorParam)
        ? Number(cursorParam)
        : null;
      const canSendDelta = requestedCursor !== null && Number.isSafeInteger(requestedCursor);
      const responseBase = canSendDelta
        && requestedCursor >= buildStatus.outputBase
        && requestedCursor <= buildStatus.outputCursor
        ? requestedCursor
        : buildStatus.outputBase;
      const responseOutput = canSendDelta
        ? buildStatus.output.slice(responseBase - buildStatus.outputBase)
        : buildStatus.output;
      return new Response(
        JSON.stringify({
          ...buildStatus,
          output: responseOutput,
          outputBase: responseBase,
        }),
        { headers }
      );
    }
    ...
    if (url.pathname === "/api/deploy-portal" && req.method === "POST") {
      try {
        ...
        const body = await req.json().catch(() => ({})) as { allowedEmail?: unknown };
        const allowedEmail = typeof body.allowedEmail === 'string' ? body.allowedEmail.trim() : '';
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(allowedEmail)) {
          return new Response(JSON.stringify({ error: 'Google 로그인 허용 이메일을 먼저 확인하세요.' }), { status: 400, headers });
        }
        const portal = existsSync(PORTAL_DATA_FILE)
          ? JSON.parse(readFileSync(PORTAL_DATA_FILE, 'utf8')) as Record<string, unknown>
          : {};
        const supabaseUrl = typeof portal.supabaseUrl === 'string' ? portal.supabaseUrl.trim() : '';
        const supabaseAnonKey = typeof portal.supabaseAnonKey === 'string' ? portal.supabaseAnonKey.trim() : '';
        const projectRef = supabaseProjectRefFromUrl(supabaseUrl);
        if (!projectRef || !isPublicSupabaseClientKey(supabaseAnonKey)) {
          return new Response(JSON.stringify({
            error: '첫 단말의 Supabase Project URL과 anon/publishable key를 앱에 먼저 연결하세요.',
          }), { status: 409, headers });
        }

        deployStatus = { isDeploying: true, output: [], exitCode: null, url: null, projectRef };
        devLog('[Deploy] Starting portal deployment...');
        void runPortalDeployment({ supabaseUrl, supabaseAnonKey, allowedEmail });

        return new Response(
          JSON.stringify({ success: true, message: "Vercel 환경 변수 저장과 배포를 시작했습니다" }),
          { headers }
        );
      } catch (error: any) {
    ...
    if (url.pathname === "/api/deploy-portal-reset" && req.method === "POST") {
      ...
      deployStatus = { isDeploying: false, output: ['⚠️ 배포가 강제 초기화되었습니다.'], exitCode: null, url: null, projectRef: null };
      return new Response(JSON.stringify({ success: true }), { headers });
    }
...
on error
  return ""
    ...
    if (url.pathname === "/api/open-hermes-telegram-setup" && req.method === "POST") {
      try {
        const body = await req.json().catch(() => ({})) as { profileName?: unknown };
        ...
        if (!/^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$/.test(profileName)) {
          return new Response(JSON.stringify({ success: false, error: '유효하지 않은 Hermes profile 이름입니다.' }), { status: 400, headers });
        }
        ...
        const hermesCli = terminalAgentCli('hermes', false);
        const launch = await openTerminalWithCmd(`${hermesCli} --profile ${profileName} gateway setup`, null, `Hermes ${profileName} Telegram 설정`, 'terminal');
        return new Response(JSON.stringify(launchPayload(`Hermes ${profileName} profile Telegram 설정 열기`, launch)), { headers });
      } catch (error: any) {
        return new Response(JSON.stringify({ success: false, error: error.message }), { status: 500, headers });
      }
    }
    ...
    if (url.pathname === "/api/build-windows" && req.method === "POST") {
      ...
      const readWinStream = async (stream: any) => {
        const decoder = new TextDecoder();
        for await (const chunk of stream) {
          pushBuildLog(decoder.decode(chunk, { stream: true }));
        }
      };
    ...
    if (url.pathname === "/api/install-windows-prereqs" && req.method === "POST") {
      ...
      (async () => {
        const log = (s: string) => pushBuildLog(s + '\n');
        let prereqTempDir = '';
        try {
...
${evidence}`;
    ...
    if (url.pathname === "/api/cleanup-stale-worktrees" && req.method === "POST") {
      try {
        ...
        // Some registered roots can be sleeping network volumes or stale mount points.
        // Synchronous stat/open blocks Bun's single HTTP thread and makes unrelated USE
        // control calls time out. Keep the cleanup request awaitable without freezing health.
        let folderStat;
        try {
          folderStat = await statAsync(folderPath);
        } catch {
        ...
        try {
          await statAsync(join(folderPath, '.git'));
        } catch {
...
${summaries}`;
    ...
    if (url.pathname === "/api/project-memory/suggest-display-name" && req.method === "POST") {
      try {
        ...
        const prompt = `다음 프로젝트 장기기억을 사용자가 한눈에 구분할 수 있는 한국어 별칭 하나만 만드세요.\n`
          + `규칙: 후보 여러 개 금지, 2~6단어, 최대 30자, 버전·UUID 금지.\n`
          + `반드시 다른 설명 없이 {"displayName":"별칭 하나"} JSON 한 줄만 출력하세요.\n`
        ...
        const displayName = parseAiMemoryAlias(run.text);
        if (!displayName) {
          return new Response(JSON.stringify({ error: "AI가 한 개의 짧은 별칭 형식으로 응답하지 않았습니다." }), { status: 502, headers });
        }
        return new Response(JSON.stringify({ displayName }), { headers });
      } catch (e: any) {
        return new Response(JSON.stringify({ error: e.message }), { status: 500, headers });
      }
    }
