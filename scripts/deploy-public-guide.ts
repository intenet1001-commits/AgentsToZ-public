...
export function normalizeGuideRemoteScript(distDirectory: string): void {
  const scriptPath = join(distDirectory, GUIDE_REMOTE_SCRIPT_PATH.slice(1));
  const source = readFileSync(scriptPath, 'utf8');
  const normalized = source.replace(/\r\n?/g, '\n');
  if (normalized !== source) writeFileSync(scriptPath, normalized, 'utf8');
}
...
export async function runPublicGuideDeployment(options: {
  ...
  normalizeGuideRemoteScript(distDirectory);
