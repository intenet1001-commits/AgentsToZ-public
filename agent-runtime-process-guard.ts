...
if (launchProtocol === 'agentstoz-onboarding-codex-auth-v1') {
  const { runCodexAuthGuard } = await import('./src/onboardingCodexAuthGuard');
  await runCodexAuthGuard(argv.slice(1));
}
if (launchProtocol === 'agentstoz-onboarding-github-auth-v1') {
  const { runGithubAuthGuard } = await import('./src/onboardingGithubAuthGuard');
  await runGithubAuthGuard(argv.slice(1));
}
