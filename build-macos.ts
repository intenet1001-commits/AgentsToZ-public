...
const gitAdd = await $`git add build-number.json src-tauri/tauri.conf.json src-tauri/Cargo.toml src-tauri/Cargo.lock src-tauri/icons/`.nothrow();
if (gitAdd.exitCode === 0) {
  const gitCommit = await $`git commit -m "chore: bump to v${newVersion}" -- build-number.json src-tauri/tauri.conf.json src-tauri/Cargo.toml src-tauri/Cargo.lock src-tauri/icons/`.nothrow();
