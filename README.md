...
- LAN·외부 QR은 모두 30일 동안 한 번만 사용할 수 있습니다. LAN 세션은 해당 WebSocket과 함께 끝나므로 앱·로컬 API 재시작 또는 사용자가 끄면 종료됩니다. 외부는 스캔 후 승인 대기가 최대 24시간이고, 승인된 연결은 승인 시점부터 최대 30일 유지됩니다. Mac의 외부 host identity·pairing secret·세션 cursor는 계정 전용 앱 데이터의 0600 vault에 저장되어 재시작 뒤 복원을 시도하며, 사용자가 외부 원격제어를 끄거나 기기 연결을 해제하면 폐기됩니다. 늦은 QR 승인에도 전체 기간을 보장하기 위해 내부 릴레이 호스트 행만 최대 62일 유지됩니다. 외부 인터넷 연결은 종단간 암호화되며, relay는 프로젝트나 실행 기능의 평문을 볼 수 없습니다.
- 등록 프로젝트와 Git에서 발견한 연결 워크트리를 구분해 표시합니다. 실행·중지·재실행, 로컬 브라우저, Orca의 Claude/Codex/AGY/Hermes, Codex/Hermes 앱 열기를 선택할 수 있습니다. **새 Codex 대화 만들기**는 최근 기록 조회와 무관하게 항상 유지하며, Codex 앱의 프로젝트 경로 새 대화로 고정 안전 안내를 전송한 뒤 해당 프로젝트 소속으로 실제 저장됐는지 확인하고 그 대화를 열도록 Codex 앱에 요청합니다. 로컬 세션 기록에서 정확한 프로젝트 연결이 확인됐을 때만 **최근 대화 다시 열기**를 추가하고, 확인할 수 없으면 다른 대화를 추측하지 않습니다. Orca 최초 로그인·약관 동의·폴더 신뢰는 Mac에서 사용자가 한 번 직접 완료합니다. 원격 Hermes는 프로젝트 폴더에서 보이는 Orca Floating 터미널로 열며, 워크트리 생성은 표준 Git형과 Orca 사이드바 등록형 중 하나를 고릅니다.
...
QR 원격제어는 What I Said 외부 앱 피드와 별개입니다. 서로 다른 서버 경로·인증 capability를 사용하며, 원격제어로 프롬프트나 장기기억을 읽을 수 없습니다.
