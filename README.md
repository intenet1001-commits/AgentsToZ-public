...
여러 프로젝트를 함께 지휘할 때는 독립된 `AgentsToZ-Control` 프로젝트를 사용할 수 있습니다. Codex Voice나 텍스트 대화에서 “AgentsToZ, 관제센터 만들어”라고 지시하면 설치된 MCP가 등록된 작업 루트를 조회한 뒤 관제 문서, Git 초기 이력, 프로젝트 장기기억을 함께 만듭니다. 데스크톱 앱의 **도구 및 설정 → Control 바로 열기**는 등록된 관제센터를 검색 없이 엽니다.

공개 저장소를 처음 clone하거나 공개 앱을 처음 설치한 사용자는 초기 설정의 **내 Control 자동 만들기** 안내를 받습니다. 작업 루트가 하나면 한 번의 선택으로 만들고, 여러 개면 저장 위치를 고른 뒤 만듭니다. 이미 다른 Mac에서 사용 중인 Control이 있으면 **다른 Mac의 Control 복원**에서 본인의 Private GitHub 주소를 넣어 clone합니다.

관제센터에는 프로젝트 별칭·관계·공통 결정처럼 오래 유지할 운영 맥락을 저장합니다. 실제 진행 상태와 터미널 이벤트는 AgentsToZ 미션·워크룸 런타임에 남고, 각 프로젝트의 설계·변경·검증 결과는 해당 프로젝트 장기기억에 남습니다.

다른 Mac에서 이어 쓰려면 관제센터를 별도 GitHub 저장소에 push한 뒤 새 Mac의 **새 프로젝트 → GitHub 주소**로 clone합니다. clone 흐름은 저장소에 포함된 `.agent-memory/config.json`의 `memoryId`와 원격 저장소 계보를 확인하고 Supabase 기억을 먼저 Pull합니다. Git은 관제 문서와 이력을, Supabase는 동일한 `memoryId`의 기억 리비전을 동기화하므로 새 Mac에서 기억을 새로 초기화하지 않습니다. 개인 관제 내용에는 Private 저장소를 권장하며 공개 범위는 생성할 때 사용자가 명시합니다.
