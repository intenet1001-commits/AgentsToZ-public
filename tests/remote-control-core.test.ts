...
describe('QR remote-control core security boundary', () => {
  ...
  test('bounds unique action ids, rate limits accepted work, and requires phone confirmation', async () => {
    ...
    const core = new RemoteControlCore(fixture.gateway, {
      ...
      // Reads carry their own allowance since a phone must follow the host's own nextPage chain
      // to see its projects. This test drives the limit with projects.list, so it has to set the
      // read budget; leaving it at the default would exercise nothing.
      maxReadsPerRateWindow: 2,
...
test('an action is bound to the connection that asked for it, not to whoever is connected', async () => {
  // The delivery predicate alone is not enough: when a second connection takes the session over it
  // installs its own predicate, and asking that one answers "connected" for work the first
  // connection walked away from. Both predicates below return true, so only the captured
  // generation can tell the two apart — remove that comparison and this dispatches.
  let releaseLookup!: () => void;
  const stalled = new Promise<void>(resume => { releaseLookup = resume; });
  const dispatched: string[] = [];
  let stall = false;
  const gateway: RemoteControlGateway = {
    listRegisteredProjects: () => {
      if (!stall) return [target()];
      stall = false;
      return stalled.then(() => [target()]);
    },
    executeRegisteredProjectAction: request => { dispatched.push(request.action); },
  };
  ...
  const pairing = core.enable('http://192.168.1.10:4300');
  const ready = await core.pair(pairingToken(pairing.pairingUrl));
  const controlId = ready.projects[0]!.controlId;

  core.bindSessionDelivery(ready.sessionToken, () => true);
  stall = true;
  const pending = core.perform(action({
    sessionToken: ready.sessionToken,
  ...
  // A different connection restores the same session while the first one's action is parked.
  core.bindSessionDelivery(ready.sessionToken, () => true);
  releaseLookup();

  // A cancelled action comes back as a failed action.result, not a rejection: the phone still
  // needs an answer for the actionId it sent.
...
test('a phone can finish enumerating its own projects without spending the mutating budget', async () => {
  // Measured on this Mac's 134 registered rows: the six pages the host itself hands out consumed
  // the whole 6-per-10s window, so the work-root lookup that follows was refused and the filter
  // came up empty. At 150 rows the seventh page was refused outright and the phone, which does not
  // retry, simply showed a short list. Reads commit nothing, so they get their own window; the
  // mutating budget is unchanged, which is the half that matters for safety.
  const count = 150;
  const targets = Array.from({ length: count }, (_, index) => target({
    internalId: 'project-' + index,
    name: '프로젝트 ' + index,
    actions: ['start'],
  }));
  const core = new RemoteControlCore({
    listRegisteredProjects: () => targets,
    executeRegisteredProjectAction: () => undefined,
    listWorkspaceRoots: () => [{ internalId: 'root-1', name: '작업 루트', path: '/Users/private/work' }],
  }, {
    hostName: '이 Mac',
    now: () => 1_000,
    randomBytes: sequentialRandom(),
  ...
  const pairing = core.enable('http://192.168.1.10:4300');
  const ready = await core.pair(pairingToken(pairing.pairingUrl));
  expect(ready.projectCount).toBe(count);

  let page = ready.nextPage;
  let seen = ready.projects.length;
  let requests = 0;
  while (page !== null) {
    requests += 1;
    const result = await core.perform(action({
      sessionToken: ready.sessionToken,
      actionId: 'page-' + requests + '-1111-4111-8111-11111111111'.slice(0, 30 - String(requests).length),
      action: 'projects.list',
      page,
    })) as { ok: boolean; projects?: unknown[]; nextPage: number | null };
    expect(result.ok).toBe(true);
    seen += result.projects!.length;
    page = result.nextPage;
  }
  expect(seen).toBe(count);
  expect(requests).toBeGreaterThan(6);

  // The page asks for work roots immediately after the last page, on the same window.
  const roots = await core.perform(action({
    sessionToken: ready.sessionToken,
    actionId: 'roots-01-1111-4111-8111-111111111111',
    action: 'workspace-roots.list',
  })) as { ok: boolean };
  expect(roots.ok).toBe(true);

  // None of that spent the mutating allowance, and that allowance is still six.
  const controlId = ready.projects[0]!.controlId;
  for (let attempt = 0; attempt < 6; attempt += 1) {
    const started = await core.perform(action({
      sessionToken: ready.sessionToken,
      actionId: 'act-' + attempt + '-1111-4111-8111-1111111111111',
      action: 'start',
      controlId,
      remoteConfirmed: true,
    })) as { ok: boolean };
  ...
  await expect(core.perform(action({
    sessionToken: ready.sessionToken,
    actionId: 'act-over-1111-4111-8111-111111111111',
    action: 'start',
    controlId,
    remoteConfirmed: true,
  }))).rejects.toMatchObject({ code: 'RATE_LIMITED' });
