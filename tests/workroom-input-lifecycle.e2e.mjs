...
const mobileTouch = process.env.WORKROOM_TOUCH_TEST === '1';
const browser = await (mobileTouch ? webkit : chromium).launch({ headless: true });
const results = [];
async function fixture(options = {}) {
  const context = await browser.newContext({ viewport: mobileTouch ? {width:393,height:852} : { width: 1000, height: 1050 }, hasTouch:mobileTouch, serviceWorkers: 'block' });
  const state = {
...
async function run(name, execute, options) {
  if(mobileTouch && name !== 'number-and-application-cursor-buttons') return;
...
try {
  await run('number-and-application-cursor-buttons', async ({page,state}) => {
    const from=state.requests.length;
    await page.getByRole('button',{name:'2',exact:true})[mobileTouch?'tap':'click']();
    await until(()=>inputs(state,from).length===1,'Number tap must send once');
    assert.equal(inputs(state,from)[0].data,'2');
    await page.evaluate(()=>new Promise(resolve=>window.__workroom.instances.at(-1).write('\x1b[?1h',resolve)));
    await page.getByRole('button',{name:'↑',exact:true})[mobileTouch?'tap':'click']();
    await until(()=>inputs(state,from).length===2,'Application arrow must send once');
    assert.equal(inputs(state,from)[1].data,'\x1bOA');
    await page.evaluate(()=>new Promise(resolve=>window.__workroom.instances.at(-1).write('\x1b[?1l',resolve)));
    await page.getByRole('button',{name:'↓',exact:true})[mobileTouch?'tap':'click']();
    await until(()=>inputs(state,from).length===3,'Normal arrow must send once');
    assert.equal(inputs(state,from)[2].data,'\x1b[B');
  });
