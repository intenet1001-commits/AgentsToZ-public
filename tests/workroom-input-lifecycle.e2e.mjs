...
try {
  ...
  await run('ended-session-lifecycle', async ({ page, state }) => {
    ...
    await until(async () => (await tabs.getByRole('tab').count()) === 0, 'A naturally exited session must also leave the default active list');
  ...
  await run('last-session-close-late-resize-error', async ({page, state}) => {
    ...
    state.beforeOperation = async body => {
      ...
      if (body.operation === 'close') closeSeen = true;
