...
describe('portal remote control offers QR scanning on this screen', () => {
  ...
  test('the scan control sits with the top navigation, not at the bottom', () => {
    const navIndex = PORTAL.indexOf('workspace-navigation');
  ...
  test('scanning uses the shared scanner without unloading the connection owner', () => {
    expect(PORTAL).toContain('onDetected={acceptPairing}');
    expect(PORTAL).not.toContain('href="/?scan=remote"');
  });
});
