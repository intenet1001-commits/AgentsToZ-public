...
describe('AI usage session rows', () => {
  ...
  test('the card separates by surface, not by a coloured bar down its edge', () => {
    ...
    expect(card.slice(0, 600)).toContain("background: 'rgb(var(--surface-highlight-rgb) / 0.032)'");
    expect(card.slice(0, 600)).toContain("border: '1px solid rgb(var(--surface-highlight-rgb) / 0.075)'");
  });
