...
describe("AgentsToZ project route API contract", () => {
  ...
  test("legacy project-worker routes cannot silently select the CS-CEO skill", () => {
    const dispatch = readFileSync(new URL("../src/agentstozProjectDispatch.ts", import.meta.url), "utf8");
    expect(dispatch).not.toContain(': "cs-ceo"');
    expect(dispatch).toContain('WORKER_PROFILE_REQUIRED');
    expect(api).not.toContain('workerProfile: "cs-ceo"');
  });
