...
describe('Control profile through real API and MCP',()=>{
 test('an unregistered user profile recalls and reviews memory via the same MCP from any cwd',async()=>{
  ...
  expect(proposal.result.structuredContent.saved).toBe(false);expect(proposal.result.structuredContent.proposal.state).toBe('pending');
  const reviewed=await f.post('/api/control-profile/review',{id:proposal.result.structuredContent.proposal.id,accept:true,expectedRevision:prepared.body.profile.revision});expect(reviewed.status).toBe(200);expect(reviewed.body.proposal.state).toBe('saved');expect(reviewed.body.backup.backedUp).toBe(false);
