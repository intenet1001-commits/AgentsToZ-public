...
test("git's worktree list decides ownership, and a missing git falls back", () => {
  const root = "/Users/gwanli/product_2026/portmanagement";
  const external = "/Users/gwanli/.codex/worktrees/portmanagement-orca";
  const outsider = "/Users/gwanli/product_2026/portmanagement-extra/worktrees/x";
  const punctuationSibling = `${root}..other`;
  const legacyShapedSibling = `${root}_worktrees_unlisted`;
  ...
  const porcelain = [
    ...
    "HEAD def",
    "",
    // Git, not lexical containment, proves this external sibling belongs here.
    `worktree ${external}`,
    "HEAD ghi",
    "",
  ].join("\n");
  ...
  expect(linked.has(claudeProjectSlug(external))).toBe(true);
  expect(linked.has(claudeProjectSlug(outsider))).toBe(false);
  expect(isProjectTranscriptDir(claudeProjectSlug(`${root}/worktrees/orca surface`), slug, linked)).toBe(true);
  expect(isProjectTranscriptDir(claudeProjectSlug(external), slug, linked)).toBe(true);
  expect(isProjectTranscriptDir(claudeProjectSlug(outsider), slug, linked)).toBe(false);
  // Once Git returned a family, only that exact set is authoritative. These
  // names deliberately collide with the legacy slug heuristics but are not
  // linked worktrees.
  expect(isProjectTranscriptDir(claudeProjectSlug(punctuationSibling), slug, linked)).toBe(false);
  expect(isProjectTranscriptDir(claudeProjectSlug(legacyShapedSibling), slug, linked)).toBe(false);
  ...
  const empty = parseWorktreePaths("");
  expect(empty).toHaveLength(0);
  expect(isProjectTranscriptDir(`${slug}-worktrees-feature-x`, slug)).toBe(true);
  expect(isProjectTranscriptDir(`${slug}--claude-worktrees-panel-e2e`, slug)).toBe(true);
  expect(isProjectTranscriptDir(`${slug}-extra`, slug)).toBe(false);
});
...
test("XML-shaped user prompts are kept unless they are a known host wrapper", () => {
  const claude = extractSessionExcerpts([
    line({ type: "user", timestamp: "2026-08-09T10:00:00Z", message: { content: "<project_context>실제 요구사항</project_context>" } }),
    line({ type: "user", timestamp: "2026-08-09T10:00:01Z", message: { content: "<section>HTML 예시</section>" } }),
    line({ type: "user", timestamp: "2026-08-09T10:00:02Z", message: { content: "<tool_result>host noise</tool_result>" } }),
  ], null);
  expect(claude.map(entry => entry.text)).toEqual([
    "<project_context>실제 요구사항</project_context>",
    "<section>HTML 예시</section>",
  ]);

  const codex = extractCodexExcerpts([
    line({ timestamp: "2026-08-09T10:00:00Z", type: "event_msg", payload: { type: "user_message", message: "<input>사용자 프롬프트</input>" } }),
    line({ timestamp: "2026-08-09T10:00:01Z", type: "event_msg", payload: { type: "user_message", message: "<system-reminder>host noise</system-reminder>" } }),
  ], null);
  expect(codex.map(entry => entry.text)).toEqual(["<input>사용자 프롬프트</input>"]);
});
...
test("Codex accepts an exact Git-listed external worktree but not an unlisted sibling", () => {
  const root = "/Users/gwanli/product_2026/portmanagement";
  const external = "/Users/gwanli/.codex/worktrees/portmanagement-orca";
  const outsider = "/Users/gwanli/.codex/worktrees/portmanagement-other";
  const family = [root, external];

  expect(isPathInsideProjectFamily(`${external}/src`, root, family)).toBe(true);
  expect(isPathInsideProjectFamily(outsider, root, family)).toBe(false);
  expect(isPathInsideProjectFamily(`${root}/src`, root, family)).toBe(true);
  expect(isPathInsideProjectFamily("C:\\worktrees\\orca\\src", "C:\\repo", ["C:\\repo", "C:\\worktrees\\orca"]))
    .toBe(true);
  expect(isPathInsideProjectFamily("C:\\worktrees\\other", "C:\\repo", ["C:\\repo", "C:\\worktrees\\orca"]))
    .toBe(false);
  ...
  expect(sweep).toContain("isPathInsideProjectFamily(cwd, root, worktreePaths)");
  expect(source).toContain("const worktreePaths = resolveProjectMemoryWorktreeRoots(root)");
  expect(sweep).toContain("collectSessionTranscriptFile({");
  const streaming = readFileSync(join(import.meta.dir, "..", "src/sessionTranscriptStream.ts"), "utf8");
  expect(streaming).toContain("extractOwnedClaudeSessionExcerpts([line], input.sinceIso, input.exactRoots)");
});
...
test('current Codex completed message items reach memory context without injected response items or tools', () => {
  const lines = [
    { type: 'response_item', payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text: 'injected context' }] } },
    { type: 'event_msg', payload: { type: 'item_completed', item: { type: 'UserMessage', content: [{ type: 'text', text: '새 요구사항' }] } } },
    { type: 'event_msg', payload: { type: 'item_completed', item: { type: 'AgentMessage', content: [{ type: 'Text', text: '검토 결과' }] } } },
    { type: 'event_msg', payload: { type: 'item_completed', item: { type: 'CommandExecution', content: [{ type: 'text', text: 'tool output' }] } } },
  ].map(record => JSON.stringify({ ...record, timestamp: '2026-09-07T14:20:00Z' }));
  expect(extractCodexExcerpts(lines, '2026-09-07T14:19:00Z').map(({ role, text }) => ({ role, text })))
    .toEqual([{ role: 'user', text: '새 요구사항' }, { role: 'assistant', text: '검토 결과' }]);
  expect(extractCodexExcerpts(lines, '2026-09-07T14:21:00Z')).toEqual([]);
});
