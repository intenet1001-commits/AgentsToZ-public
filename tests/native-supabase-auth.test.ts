...
describe('native Supabase OAuth relay', () => {
  ...
  test('legacy native relay stays nonce-bound while the desktop UI no longer invokes login', () => {
    ...
    expect(portalManagerSource).toContain('requiresUserSession && !isDeployedWeb()');
    ...
    expect(supabaseClientSource).toContain("LOCAL_WEB_PROXY_PATH = '/api/supabase-proxy'");
    expect(supabaseClientSource).toContain("runtime !== 'deployed-web'");
