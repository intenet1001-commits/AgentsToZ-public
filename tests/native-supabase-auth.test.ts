...
describe('native Supabase OAuth relay', () => {
  ...
  test('sidecar callback is nonce-bound and frontend exchanges PKCE code in the WebView', () => {
    for (const route of [
    ...
    expect(portalManagerSource).toContain('signInNativeSupabase(supabase');
    ...
    expect(portalManagerSource).toContain('api/auth/native/callback/*');
