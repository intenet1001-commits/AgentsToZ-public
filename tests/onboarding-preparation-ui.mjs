...
try {
 ...
 const handoffBefore=store.read();
 await page.getByRole('button',{name:'현재 단계 AI 인계문 복사',exact:true}).click();
 await page.getByRole('button',{name:'현재 단계 복사됨',exact:true}).waitFor();
 assert.equal(store.read().revision,handoffBefore.revision,'copy must not mutate progress');
 await page.getByRole('combobox',{name:'도움을 받을 AI'}).selectOption('claude');
 await page.evaluate(()=>{Object.defineProperty(navigator,'clipboard',{configurable:true,value:{writeText:async()=>{throw new Error('fixture denied');}}});document.execCommand=()=>false;});
 await page.getByRole('button',{name:'현재 단계 AI 인계문 복사',exact:true}).click();
 const fallback=page.getByRole('textbox',{name:'직접 복사할 AI 인계문'});
 await fallback.waitFor();
 assert.ok((await fallback.inputValue()).includes(handoffBefore.runId));
 assert.equal(store.read().revision,handoffBefore.revision,'fallback must not mutate progress');
 await page.reload();
 await page.getByText('Codex CLI · 설치 확인 · 로그인은 별도 확인',{exact:true}).waitFor();
