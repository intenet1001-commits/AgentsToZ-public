...
describe("project-memory local HTTP API", () => {
  test("resolves an allowlisted project and closes recall -> feedback -> verified recall", async () => {
    ...
    writeFileSync(join(appData, "project-memory-mention-aliases.json"), JSON.stringify({
      schemaVersion: 1,
      aliases: [{
        memoryId: status.config!.memoryId,
        primaryAlias: "demo-project",
        redirects: [],
    ...
    const savedMention = await post(baseUrl, "/api/project-memory/mentions/save", {
      memoryId: status.config!.memoryId,
      alias: "Demo Project",
    });
    expect(savedMention.response.status).toBe(503);
    expect(savedMention.body.code).toBe("MENTION_SHARED_UNAVAILABLE");

    const mentionListResponse = await fetch(`${baseUrl}/api/project-memory/mentions`);
    const mentionList = await mentionListResponse.json() as any;
    expect(mentionListResponse.status).toBe(200);
    expect(mentionList.mentions).toHaveLength(1);

    const mentionResolved = await post(baseUrl, "/api/project-memory/mentions/resolve", { alias: "#demo-project" });
    expect(mentionResolved.response.status).toBe(200);
    expect(mentionResolved.body.memoryId).toBe(status.config!.memoryId);
    expect(mentionResolved.body.canonicalPath).toBe(project);
    ...
    const mentionSavePreflight = await fetch(`${baseUrl}/api/project-memory/mentions/save`, {
      method: "OPTIONS",
      headers: {
        Origin: portalOrigin,
        "Access-Control-Request-Method": "POST",
    ...
    expect(mentionSavePreflight.status).toBe(200);
    expect(mentionSavePreflight.headers.get("access-control-allow-origin")).toBe(portalOrigin);
    expect(mentionSavePreflight.headers.get("access-control-allow-private-network")).toBe("true");
