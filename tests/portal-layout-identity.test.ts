...
describe('기기 신원과 조회 선택은 다른 값이다', () => {
  ...
  test('배포 북마크 설정은 프로젝트 조회 기기와 독립이다', () => {
    expect(manager).toContain('data-testid="deployed-settings-bookmark-context"');
    expect(manager).not.toContain('data-testid="deployed-settings-device-context"');
    expect(manager).not.toContain('data-testid="settings-change-project-device"');
    expect(manager).toContain('data-testid="deployed-settings-connection"');
    expect(manager).toContain("const targetDeviceId = isDeployedWeb() ? ownDeviceId");
    expect(portal).not.toContain('selectedProjectDevice={selectedDevice');
    expect(portal).toContain("activeTab==='ports'&&<div><span>조회 기기</span>");
  });
...
describe('포털 번들', () => {
  ...
  test('개발 HMR이 같은 DOM 컨테이너에 React root를 중복 생성하지 않는다', () => {
    const shell = readFileSync(new URL('../src/remote-control-portal-main.tsx', import.meta.url), 'utf8');
    expect(shell).toContain('workspaceWindow.__agentstozWorkspaceRoot ?? createRoot');
    expect(shell).toContain('workspaceWindow.__agentstozWorkspaceRoot = root;');
    expect(portal).not.toContain('createRoot(');
  });
...
describe('모바일 조작성', () => {
  ...
  test('저장된 full 선호도 모바일 폭에서는 compact로 강제하고 폭 복귀 시 선호를 보존한다', () => {
    ...
    expect(portal).toContain("const isFullLayout = false && windowWidth >= 768");
  ...
  test('카드 metadata와 로그인 보안 문구는 장식용 faint 색을 쓰지 않는다', () => {
    expect(css).toContain('.portal-bookmark-card__visits { color: var(--text-secondary);');
    expect(css).toContain('.portal-bookmark-card__external { display: inline-flex; color: var(--text-secondary);');
    expect(css).toContain('.portal-bookmark-card__url { color: var(--text-secondary);');
    expect(css).toContain('.portal-login-card__security { margin: 15px 0 0; color: var(--text-secondary); font-size: 11px;');
  });
