...
describe('provider-neutral hosted remote controller', () => {
  ...
  test('keeps an approved session resumable while the user moves through portal pages', () => {
    ...
    expect(remote).toContain('navigateWorkspace(tab)');
    expect(remote).toContain('workspace-navigation');
    expect(remote).not.toContain('href="/?tab=memories"');
  ...
  test('keeps app opening, fresh Codex creation, and both worktree owners distinct', () => {
    ...
    expect(projectCard).toContain("'codex.thread.start': 'Mac의 Codex 앱에서 열기'");
    ...
    expect(api).toContain('prepareProjectCodexApp(workingPath, target.name)');
    expect(api).toContain('new FileCodexFirstConversationPendingStore(APP_DATA_DIR)');
    expect(api).toContain('verifyRecoveredProjectCodexThread(folderPath, threadId)');
