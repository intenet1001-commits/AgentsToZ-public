...
async function verifyCurrentGitWorktreeUi(browser) {
  ...
  try {
    ...
    await worktreePage.getByRole('button',{name:'워크룸에서 이어서 확인',exact:true}).waitFor({state:'visible'});
    ...
    await worktreePage.getByTestId('ai-work-request').waitFor({state:'visible'});
    check('VOC: 세션 기억하기는 AI 작업에 요청을 준비하고 자동 실행하지 않음',
      terminalStarts.length===3 && (await worktreePage.getByLabel('AI 작업 요청',{exact:true}).inputValue()).includes('remember-session')
      && await worktreePage.getByLabel('AI 작업 프로젝트',{exact:true}).inputValue()===projectId);
    check('VOC: 125% 기본 창에서 AI 요청 영역 가로 넘침 없음',await worktreePage.getByTestId('ai-work-request').evaluate(element=>element.scrollWidth<=element.clientWidth+1));
    await worktreePage.screenshot({path:'output/playwright/voc-ai-request-125.png'});
    await worktreePage.getByTestId('ai-work-request-start').click();
    await worktreePage.getByTestId('ai-terminal-panel').waitFor({state:'visible'});
