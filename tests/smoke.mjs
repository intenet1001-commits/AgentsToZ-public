...
import { verifyPortAutoUploadUi } from './port-auto-upload-smoke.mjs';
import { verifyAiProjectNamesUi } from './ai-project-names-smoke.mjs';
import { verifyPromptGuideUi } from './prompt-guide-smoke.mjs';
...
async function verifyCurrentGitWorktreeUi(browser) {
  ...
  await isolated.route('**/api/**', async route => {
    ...
    if (pathname === '/api/agent-runtime/terminals') {
      ...
      if (body.operation === 'start') {
        ...
        terminalSession = {id: `smoke-terminal-session-${terminalStarts.length}`, targetId: body.targetId, agent: body.agent, state: 'running', createdAt: new Date().toISOString(), exitCode: null, cols: 100, rows: 28};
        terminalSessions.push(terminalSession);
      }
      if (body.operation === 'close') {
        const index = terminalSessions.findIndex(session => session.id === body.sessionId);
        if (index >= 0) {
          terminalSession = {...terminalSessions[index], state:'exited', exitCode:0};
          terminalSessions[index] = terminalSession;
        }
      }
      if (body.operation === 'list') return json({sessions: terminalSessions});
      if (body.operation === 'read') {
        const session = terminalSessions.find(candidate => candidate.id === body.sessionId) ?? terminalSession;
        return json({session, chunks: [{seq: 1, text: 'Fixture CLI ready'}], nextCursor: 1, truncated: false, hasMore: false});
      }
  ...
  try {
    ...
    const launches = [
      {launcher: worktreePage.getByTestId('detail-project-launch'), targetId: projectId, agent: 'claude'},
      {launcher: worktreePage.locator(`[data-testid="worktree-action-groups"][data-worktree-path="${projectPath}"]`).getByTestId('worktree-project-launch'), targetId: projectId, agent: 'codex'},
      {launcher: worktreePage.locator(`[data-testid="worktree-action-groups"][data-worktree-path="${linkedPath}"]`).getByTestId('worktree-project-launch'), targetId: linkedTargetId, agent: 'hermes'},
    ];
    for (const [index, launch] of launches.entries()) {
      ...
      await launch.launcher.getByRole('button', {name: '워크룸에서 작업', exact: true}).click();
      await worktreePage.getByTestId('ai-terminal-panel').waitFor({state: 'visible'});
      const selectedSession = worktreePage.getByTestId('ai-terminal-panel').locator('[role="tab"][aria-selected="true"]');
      await selectedSession.waitFor({state: 'visible'});
      check('Local: 선택한 AI와 정확한 프로젝트/워크트리로 한 번만 실행 ' + index,
        ...
        && ((await selectedSession.textContent()) ?? '').includes('실행 중'));
    }
    ...
    await tools.locator(':scope > summary').click();
    await worktreePage.getByTestId('terminal-agent-panel').locator(':scope > summary').click();
    await Promise.all([
      worktreePage.waitForResponse(response => {
        if (new URL(response.url()).pathname !== '/api/agent-runtime/terminals') return false;
        return response.request().postDataJSON()?.operation === 'list';
      }),
      worktreePage.getByTestId('detail-codex-run').click(),
    ]);
    await worktreePage.getByTestId('ai-terminal-panel').waitFor({state:'visible'});
    check('Local: 기존 AI 실행 버튼도 선택한 정확한 워크룸 세션을 재사용',terminalStarts.length===3 && terminalStarts[1].agent==='codex' && terminalStarts[1].targetId===projectId);
    await worktreePage.screenshot({path:'output/playwright/workroom-session-125.png'});
    await worktreePage.getByRole('button',{name:'새 AI로 원인 조사 요청',exact:true}).waitFor({state:'visible'});
    check('Local: 종료 저장 실패는 새 AI 조사 초안으로 확인 가능',await worktreePage.getByRole('button',{name:'새 AI로 원인 조사 요청',exact:true}).isVisible());
    await worktreePage.getByRole('button',{name:'새 AI로 원인 조사 요청',exact:true}).click();
    check('Local: 저장 실패 복구는 대상과 검토 요청을 전달하고 자동 실행하지 않음',
      (await worktreePage.getByLabel('터미널에 전달할 작업').inputValue()).includes('세션 기억 저장')&&terminalStarts.length===3);
    await worktreePage.getByTestId('workroom-remember-session').click();
    await worktreePage.getByLabel('터미널에 전달할 작업').waitFor({state:'visible'});
    check('VOC: 세션 기억하기는 워크룸에 요청을 준비하고 자동 실행하지 않음',
      terminalStarts.length===3 && (await worktreePage.getByLabel('터미널에 전달할 작업').inputValue()).includes('remember-session')
      && await worktreePage.getByLabel('터미널 프로젝트').inputValue()===projectId);
    check('VOC: 125% 기본 창에서 워크룸 요청 영역 가로 넘침 없음',await worktreePage.getByTestId('ai-terminal-panel').evaluate(element=>element.scrollWidth<=element.clientWidth+1));
    await worktreePage.screenshot({path:'output/playwright/voc-workroom-request-125.png'});
    await worktreePage.getByRole('button',{name:'선택한 AI로 시작',exact:true}).click();
    await worktreePage.getByTestId('ai-terminal-panel').waitFor({state:'visible'});
    check('VOC: 명시적 실행은 대상과 기억 요청을 한 번 전달하고 워크룸에 연결',
      terminalStarts.length===4 && terminalStarts[3].targetId===projectId && terminalStarts[3].prompt.includes('remember-session'));
    await worktreePage.getByRole('button',{name:'세션 종료',exact:true}).click();
    await worktreePage.getByRole('dialog',{name:'세션 종료 전 저장'}).getByRole('button',{name:'저장 없이 종료',exact:true}).click();
    await worktreePage.getByRole('button',{name:/종료된 세션 보기/}).click();
    await worktreePage.getByRole('tab',{name:/codex.*종료/}).click();
    await worktreePage.getByTestId('workroom-dismiss-session').click();
    await worktreePage.getByRole('tab',{name:/codex.*종료/}).waitFor({state:'hidden'});
    await worktreePage.getByTestId('workroom-memory-attention').waitFor({state:'visible'});
    check('VOC: 종료된 세션 목록 제거는 실행·저장 기록을 삭제하지 않음',
      await worktreePage.getByRole('tab',{name:/codex.*종료/}).count()===0
      && terminalSession.state==='exited'
      && await worktreePage.getByTestId('workroom-memory-attention').isVisible());
...
async function verifyIndeterminateConversationUi(browser) {
  ...
  try {
    ...
    await page.getByTestId('ai-structured-runtime-toggle').click();
...
(async () => {
  ...
  try {
    ...
    if (isPortalOnly) {
      ...
      // prove authentication. The public workspace home also renders before
      // sign-in, so classify its shell separately from the legacy management UI.
      const surface = classifyPortalSmokeSurface({
        ...
        hasWorkspaceShell: (
            await page.getByText('내 작업 공간', { exact: true }).count() === 1
            && await page.getByRole('navigation').count() === 1
            && await page.getByText('프로젝트 현황', { exact: true }).count() === 1
            && await page.getByText('원격 작업 이어가기', { exact: true }).count() === 1
        ),
      });
      if (surface === 'password-gate') {
        ...
        check('Portal: "프로젝트 추가" 버튼 없음 (포털 전용)',
          await page.getByRole('button', { name: /프로젝트 추가/ }).count() === 0);
      } else if (surface === 'workspace-shell') {
        check('Portal: 기본 작업 홈 정상 렌더 (로그인·원격 작업 검증은 별도)', true);
        check('Portal: "프로젝트 추가" 버튼 없음 (포털 전용)',
          await page.getByRole('button', { name: /프로젝트 추가/ }).count() === 0);
      } else {
      ...
      if (await page.getByTestId('top-level-runtime-tab').count()) {
      await page.getByTestId('top-level-runtime-tab').click();
      await page.getByTestId('ai-structured-runtime-toggle').click();
    const runtimePanel = page.getByTestId('agent-runtime-panel');
      await runtimePanel.waitFor({ state: 'visible', timeout: 10_000 });
      ...
      for (const steps of [-5, 0, 5]) {
        ...
        const composerSummary = composer.locator(':scope > summary');
      ...
      check(
        ...
        check('Local: 중복 AI 작업 탭이 없고 워크룸은 유지됨',
          await page.getByTestId('top-level-runtime-tab').count()===0
          && await page.locator('[data-top-level-tab="terminal"]').count()===1);
      }
      ...
      await verifyPortAutoUploadUi(browser, TARGET, check);
      await verifyAiProjectNamesUi(browser, TARGET, check);
      await verifyPromptGuideUi(browser, TARGET, check);
