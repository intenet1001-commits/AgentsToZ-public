...
describe("codex rollout scan reuses unchanged parses", () => {
  ...
  test("the scan goes through the cache instead of reading the file directly", () => {
    const cachedReader = sliceFrom(
      apiServer,
      "function cachedCodexRolloutFileSummary",
      "\n/** Content-free observations",
    );
    expect(cachedReader).toContain("readCodexRolloutSummary(");
    expect(cachedReader).toContain("mtimeMs: file.mtimeMs, size: file.size");

    const scan = sliceFrom(
      apiServer,
      "const files = await recentCodexRolloutFiles(96);",
      "if (codexSessions.length >= 24) break;",
    );
    expect(scan).toContain("cachedCodexRolloutFileSummary(file)");
    expect(scan).not.toContain("readFileSync(file.full");
  });
