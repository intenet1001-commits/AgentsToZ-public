...
import {canMoveMemoryToHistory,memoryHistoryKey,moveMemoryToHistory,readMemoryHistory,reuseUnchangedMemoryJobs} from '../src/terminalMemoryHistory';
...
test('unchanged Workroom receipt polls reuse the same list, but a state change becomes visible',()=>{
 const rows:{sessionId:string;targetId:string;state:'failed'|'recovery-required'}[]=[{sessionId:'s',targetId:'p',state:'failed'}];
 expect(reuseUnchangedMemoryJobs(rows,[{...rows[0]!}])).toBe(rows);
 expect(reuseUnchangedMemoryJobs(rows,[{...rows[0]!,state:'recovery-required' as const}])).not.toBe(rows);
});
