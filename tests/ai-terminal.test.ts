...
import {AiTerminalService, bindAiTerminalTarget, aiTerminalRegistrationEvidence, writeAiTerminalInput, type AiTerminalResolvedTarget} from '../src/aiTerminalService';
...
test('explicit skip suppresses exit and shutdown memory hooks without deleting records',async()=>{
...
test('save-before-close keeps CLI alive on failed verification and closes once after verified input revision',async()=>{
