...
describe("app project-memory save and sync separation", () => {
  ...
  test("shows a copy action for the exact project name and memory ID pair", () => {
    expect(panel).toContain('data-testid="project-memory-project-id-copy"');
    expect(panel).toContain('프로젝트명 + memory ID 복사');
    expect(panel).toContain('프로젝트명: ${projectName.trim()}');
    expect(panel).toContain('memory_id: ${memoryId}');
  });
  ...
  test("offers every Hermes skill the installer writes, with a one-line purpose", () => {
    const templates = readdirSync(join(import.meta.dir, "../templates/hermes"), { withFileTypes: true })
      // Installer discovery is SKILL.md-based.  A prior compatibility test may
      // leave an empty legacy directory behind; that is not an installed skill.
      .filter(entry => entry.isDirectory()
        && existsSync(join(import.meta.dir, "../templates/hermes", entry.name, "SKILL.md")))
  ...
  // 설치·마이그레이션 프롬프트는 일상 채팅 명령과 섞이면 안 된다. 그런데 프로젝트 패널에
  // 두니 프로젝트 단위 작업으로 읽혔다 — 이제 장기기억 탭의 환경 단위 상자 한 곳에 있다.
  test("keeps setup and migration prompts out of routine chat commands", () => {
    ...
    expect(directory).toContain('다른 환경에 설치·연결');
