...
describe("generated Claude/Codex remember-session safety", () => {
  ...
  test("synchronizes before editing and obeys autoBackup", () => {
    expect(skill).toContain("Synchronize safely before editing");
    expect(skill).toContain("/api/project-memory/sync");
    expect(skill).not.toContain("/api/project-memory/pull");
