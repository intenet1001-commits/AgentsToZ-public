...
describe('sidecar build plan', () => {
  test('development builds contain no production Team ID define', () => {
    ...
    expect(plan.nativeCommands).toHaveLength(1);
    expect(plan.nativeCommands[0]!.args).toContain('/usr/bin/clang');
    expect(plan.nativeCommands[0]!.outfile).toEndWith('/agentstoz-codex-accessibility-helper');
