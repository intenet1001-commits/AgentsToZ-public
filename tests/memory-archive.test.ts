...
describe('정리 워크플로', () => {
  test('보관 → 원격 삭제 → 로컬 삭제 순서다', () => {
    ...
    const removeAt = appSource.indexOf('removeProjectFamilyLocally(item.id);');
  ...
  test('원격 삭제가 실패하면 로컬도 남긴다', () => {
    ...
    expect(appSource).toContain('Supabase 삭제 실패 — 원격 변경 전이므로 로컬 항목을 남겼습니다');
    expect(appSource).toContain('Supabase 삭제 확인 실패 — 숨김 표시는 보존했습니다');
  });

  test('원격 삭제는 캡처한 전체 신원을 서버의 단일 원자 RPC에서 대조한다', () => {
    const cleanup = appSource.slice(
      appSource.indexOf('const cleanupProject = async'),
      appSource.indexOf('const handleSaveMemo'),
    );
    expect(cleanup).toContain('remoteDeletionCandidates(deletion.family, cfg.deviceId)');
    ...
    expect(cleanup).not.toContain('verifiedRemoteDeletionIds');
  });
