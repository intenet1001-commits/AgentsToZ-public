...
describe('AI 장기기억 별칭 응답', () => {
  ...
  test('display name과 안전한 command mention alias를 함께 파싱한다', () => {
    expect(parseAiMemoryRecommendation('{"displayName":"기억 허브","mentionAlias":"Memory Hub"}'))
      .toEqual({ displayName: '기억 허브', mentionAlias: 'memory-hub' });
    expect(parseAiMemoryRecommendation('{"displayName":"기억 허브","mentionAlias":"한글"}'))
      .toEqual({ displayName: '기억 허브', mentionAlias: '한글' });
  });
});
