...
describe('local API version check', () => {
  test('health response uses the shared capability contract instead of a drifting copy', () => {
    ...
    expect(REQUIRED_CONTEXT_API_CAPABILITIES).toContain('project-memory.mentions');
    expect(REQUIRED_CONTEXT_API_CAPABILITIES).toContain('agent-runtime.management-v1');
