...
describe('mobile remote-control accessibility boundaries', () => {
  ...
  test('the six-digit internet approval code remains visible on the portal panel', () => {
    expect(portalCss).toMatch(/\.remote-sas \{[^}]*color: var\(--accent\)/);
    expect(portalCss).not.toMatch(/\.remote-sas \{[^}]*color: var\(--on-accent\)/);
  });
});
