...
describe('실행 실패 대신 미설치를 먼저 말한다', () => {
  test('최근 앱 대화는 CLI 추측 대신 정확한 Desktop 실행 파일을 요구한다', () => {
    expect(apiSource).toContain('const executable = installedHermesDesktopExecutable();');
    expect(apiSource).toContain("'HERMES_EXACT_REOPEN_UNAVAILABLE'");
    expect(apiSource).toContain('hermesSessionDeepLink(recentSession.id)');
...
describe('버튼은 숨지 않고, 누르면 이유를 말한다', () => {
  test('미설치가 확인된 경우에만 막고, 미확인이면 서버 판정에 맡긴다', () => {
    ...
    expect(appSource).toContain('Hermes CLI가 이 기기에 설치되어 있지 않습니다.');
  });

  test('Desktop 앱과 CLI 설치 판정을 독립적으로 유지한다', () => {
    // Hermes Desktop은 설치됐지만 CLI가 없는 조합도 정상이다. CLI 판정은
    // openHermesMain에만 적용하고 앱은 서버의 Desktop 실행 파일 판정에 맡긴다.
    expect(appSource).toContain('const blockedByMissingHermesCli = (): boolean =>');
    expect(appSource).toContain('if (blockedByMissingHermesCli()) return false;');
    expect(appSource).not.toContain("if (agent === 'hermes' && blockedByMissingHermesCli()) return;");
  });
