...
describe('buildGitMergeSyncPrompt', () => {
  ...
  test('연결된 공개 저장소의 메인 행에만 공개 업데이트 복사 버튼을 둔다', () => {
    const appSource = readFileSync(resolve(import.meta.dir, '../src/App.tsx'), 'utf8');
    const selfHostingSource = readFileSync(resolve(import.meta.dir, '../src/selfHosting.ts'), 'utf8');
    ...
    expect(appSource).toContain('AGENTSTOZ_PUBLIC_REPOSITORY_URL = publicGitHubRepositoryUrl(import.meta.env.VITE_REPO_URL)');
    expect(selfHostingSource).toContain('AgentsToZ-public');
