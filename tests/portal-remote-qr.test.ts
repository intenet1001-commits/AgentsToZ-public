...
describe('portal PWA remote-control QR intake', () => {
  ...
  test('allows camera and local photo blobs on both entries of the common workspace shell', () => {
    ...
    expect(header('/remote/(.*)', 'Permissions-Policy')).toContain('camera=(self)');
    expect(header('/remote/(.*)', 'Content-Security-Policy')).toContain("img-src 'self' data: blob:");
    expect(header('/remote/(.*)', 'Content-Security-Policy')).toContain("frame-ancestors 'none'");
  });
});
