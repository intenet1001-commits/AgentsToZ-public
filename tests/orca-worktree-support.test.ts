...
describe('Orca exact-path action notices', () => {
  ...
  for (const action of actions.filter(action => action !== 'browser')) {
    test(`${action} hidden-path uses the exact folder in visible Floating compatibility mode`, () => {
      ...
      expect(notice).toContain('워크트리와 브랜치는 바꾸지 않았습니다');
      expect(notice).toContain('성공했습니다');
    });

    test(`${action} hidden-path attempted state reports the compatible surface`, () => {
      ...
      expect(notice).toContain('Floating Terminal');
      expect(notice).toContain('시도했습니다');
    });
  }

  test('browser hidden-path remains blocked because Floating is a terminal-only surface', () => {
    const notice = formatOrcaWorktreeActionNotice({ action: 'browser', visibility: 'hidden-path' });
    expect(notice).toContain(actionText.browser);
...
describe('Orca launch compatibility across worktree creators', () => {
  test('keeps the primary checkout on the Orca worktree surface even when Codex changed its branch', () => {
    expect(resolveOrcaLaunchSurface({
      requestedFloating: false,
      repositoryPath: '/repo',
      worktreePath: '/repo',
      listingAvailable: true,
      listedPaths: ['/repo'],
  ...
  test('keeps an Orca-created listed worktree in the sidebar', () => {
    expect(resolveOrcaLaunchSurface({
      requestedFloating: false,
      repositoryPath: '/repo',
      worktreePath: '/Users/me/orca/workspaces/repo/task',
      listingAvailable: true,
      listedPaths: ['/repo', '/Users/me/orca/workspaces/repo/task'],
    }).floating).toBe(false);
  });

  test('opens an AgentsToZ-created unlisted worktree visibly in Floating at the same path', () => {
    expect(resolveOrcaLaunchSurface({
      requestedFloating: false,
      repositoryPath: '/repo',
  ...
  test('opens a Codex hidden-path worktree in Floating instead of rejecting it', () => {
    expect(resolveOrcaLaunchSurface({
      requestedFloating: false,
      repositoryPath: '/repo',
      worktreePath: '/Users/me/.codex/worktrees/task',
      listingAvailable: false,
    })).toEqual({
      floating: true,
      visibility: 'hidden-path',
  ...
  test('does not infer absence when Orca listing is unavailable', () => {
    expect(resolveOrcaLaunchSurface({
      requestedFloating: false,
      repositoryPath: '/repo',
      worktreePath: '/elsewhere/task',
      listingAvailable: false,
    })).toEqual({
      floating: false,
      visibility: 'unknown',
