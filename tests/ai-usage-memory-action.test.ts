...
describe('AI usage session-memory action', () => {
  ...
  test('keeps the context warning separate from whether project memory still needs saving', () => {
    expect(source).toContain('const CONTEXT_WARNING_THRESHOLD = 75');
    ...
    expect(source).toContain('data-testid="codex-auto-remember-status"');
    expect(source).toContain('data-testid="codex-auto-remember-toggle"');
    expect(source).toContain('/api/project-memory/auto-checkpoint/status');
    expect(source).toContain('/api/project-memory/auto-checkpoint/settings');
    expect(source).toContain('Codex 프로젝트 기억 자동 체크포인트');
    expect(source).toContain('원문 저장 아님');
    expect(source).toContain('자동 체크포인트 켜기');
    expect(source).toContain('앱을 업데이트하고 완전히 다시 열어 주세요.');
  });
