...
describe('VOC — 새 Orca Hermes 입력창에 OSC 색상 응답이 글자로 들어오던 문제', () => {
  test('macOS/Linux AI는 터미널 생성과 시작 명령을 하나의 Orca 호출로 보낸다', () => {
    const apiCreate = apiSource.slice(
      apiSource.indexOf('async function createOrcaFloatingTerminal('),
      apiSource.indexOf('type OrcaListedTerminal'),
    );
    const apiHandler = apiSource.slice(
      apiSource.indexOf('const agentCommandForSurface ='),
      apiSource.indexOf('// `terminal switch` points Orca'),
    );
    expect(apiCreate).toContain("...(initialCommand ? ['--command', initialCommand] : [])");
    expect(apiHandler).toContain('const initialAgentCommand = !IS_WIN');
    expect(apiHandler).toContain('if (!initialTerminalCommand)');
  });

  test('Tauri가 실제로 쓰는 Rust 실행 경로도 같은 원자적 시작 규칙을 쓴다', () => {
    const rustCreate = rustSource.slice(
      rustSource.indexOf('fn orca_create_floating_terminal('),
      rustSource.indexOf('fn normalize_orca_floating_terminal_path('),
    );
    const rustHandler = rustSource.slice(rustSource.indexOf('fn open_orca_agent('));
    expect(rustCreate).toContain('args.extend(["--command", command])');
    expect(rustHandler).toContain('let initial_terminal_command = if !cfg!(target_os = "windows") && cmd.is_some()');
    expect(rustHandler).toContain('if initial_terminal_command.is_none()');
  });
});
