...
describe('AI terminal through real remote transports',()=>{
 ...
 test('LAN WebSocket: pairs, rejects cross-socket tokens, runs PTY and revokes on disconnect',async()=>{
  ...
  phone.socket.close();await Bun.sleep(50);expect(server.status().sessions).toHaveLength(0);
  const retry=await connect();expect((await retry.exchange({type:'session.restore',protocolVersion:REMOTE_CONTROL_PROTOCOL_VERSION,sessionToken:ready.sessionToken})).type).toBe('error');
