...
import {mkdtempSync,chmodSync,writeFileSync,rmSync,realpathSync,statSync} from 'node:fs';
...
function fixture(withUnresolvedProject=false){
 ...
 let present=true;
 const gateway:RemoteControlGateway={listRegisteredProjects:()=>present?[{internalId:'private-project',name:'Fixture project',port:null,command:null,kind:'main',folderPath:dir,status:'unknown',actions:['folder.open']},...(withUnresolvedProject?[{internalId:'unresolved-project',name:'Unresolved project',port:null,command:null,kind:'main' as const,folderPath:tmpdir(),status:'unknown' as const,actions:[] }]:[])]:[],executeRegisteredProjectAction:()=>{}};
 const directory=(path:string):RemoteTargetDirectory=>{try{const canonical=realpathSync(path),info=statSync(canonical);return info.isDirectory()?{status:'found',path:canonical,dev:info.dev,ino:info.ino}:{status:'missing'};}catch{return{status:'unknown'};}};
 const remote=createAiTerminalRemoteGateway({service,active:()=>present,resolve:async bindings=>resolveVerifiedRemoteTargetAuthorities(bindings,[{targetId:'project-runtime-123',cwd:dir}],directory,(a,b)=>a.path===b.path&&a.dev===b.dev&&a.ino===b.ino)});
 return{service,gateway,remote,remove(){present=false}};
}

describe('AI terminal through real remote transports',()=>{
 test('an unresolved registration cannot block a verified project over the encrypted relay',async()=>{
  const f=fixture(true),h=await terminalRelayHarness(f.gateway,f.remote);disposals.push(async()=>{await h.host.disable()});
  await h.connect();f.service.setRemoteAccess(h.owner,true);
  const projects=h.controller.status().projects,valid=projects.find(p=>p.name==='Fixture project')!,unresolved=projects.find(p=>p.name==='Unresolved project')!;
  const started=await h.controller.sendTerminal(request({operation:'start',targetId:valid.controlId,agent:'codex',cols:80,rows:24}));
  const listed=await h.controller.sendTerminal(request({operation:'list'}));
  expect(listed.sessions?.map(s=>s.targetId)).toEqual([valid.controlId]);
  await expect(h.controller.sendTerminal(request({operation:'start',targetId:unresolved.controlId,agent:'codex',cols:80,rows:24}))).rejects.toThrow('프로젝트의 폴더 또는 Git 연결');
  expect((await f.service.perform(request({operation:'list'}))).sessions).toHaveLength(1);
  await h.controller.sendTerminal(request({operation:'close',sessionId:started.session!.id}));
 },15000);
 ...
 for(const delivered of [false,true])test(`expired uncertain input retains approval without replay (delivered=${delivered})`,async()=>{
  ...
  if(delivered){
    let completed=false;
    for(let n=0;n<60;n++){
      const local=await f.service.perform(request({operation:'read',sessionId:id,after:0}));
      if(local.chunks?.some(chunk=>chunk.text.includes('RESULT:UNCERTAIN_PROBE'))){completed=true;break;}
      await Bun.sleep(20);
    }
