...
try{
 ...
 for(const theme of ['gray','dark']){
  ...
  if(css){assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await page.screenshot({path:`/tmp/onboarding-first-project-${theme}.png`,fullPage:true});}
	 ...
	 await page.getByRole('button',{name:'내 AgentsToZ OPS 준비',exact:true}).click();await page.getByRole('status').filter({hasText:'control:create'}).waitFor();
	 await open();await page.getByRole('button',{name:'다른 Mac의 AgentsToZ OPS 복원',exact:true}).click();await page.getByRole('status').filter({hasText:'control:restore'}).waitFor();
	 await open();
