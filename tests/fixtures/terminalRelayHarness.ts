...
export async function terminalRelayHarness(gateway:RemoteControlGateway,terminalGateway:RemoteTerminalGateway) {
 ...
 const hostTransport:RemoteControlInternetHostTransport={
  ...
  async receiveEnvelopes(_h,_s,after){return hostInbox.filter(e=>BigInt(e.relaySequence)>BigInt(after))},
 ...
 const host=new RemoteControlInternetAgent({gateway,terminalGateway,transport:hostTransport,controllerOrigin:'https://terminal-fixture.example.test',hostName:'AI terminal test Mac',onRecordChanged:()=>{},autoPoll:false});
 ...
 const phoneTransport:RemoteControlRelayControllerTransport={
  ...
  async sendEnvelope(_h,_s,envelope){if(!hostInbox.some(e=>e.envelope.messageId===envelope.messageId))hostInbox.push({relaySequence:String(hostInbox.length+1),envelope});if(losePhoneReply){losePhoneReply=false;throw new Error('fixture phone reply lost')}},
  async receiveEnvelopes(_h,_s,after){await host.pollNow();return phoneInbox.filter(e=>BigInt(e.relaySequence)>BigInt(after))},
  async acknowledge(){},async revoke(){await host.revokeSession(sessionId)},
 };
 const controller=new RemoteControlRelayController({pairingUrl:pairing.pairingUrl,transport:phoneTransport,controllerName:'Fixture phone',sleep:async()=>{await Bun.sleep(5)}});
 return{host,controller,phoneTransport,pairing,sessionId,hostInbox,phoneInbox,owner:'internet:'+sessionId,
  ...
  restore(snapshot:unknown){return new RemoteControlRelayController({restoredSession:snapshot,transport:phoneTransport,controllerName:'Fixture phone',sleep:async()=>{await Bun.sleep(5)}})},
