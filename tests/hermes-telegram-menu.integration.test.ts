...
describe('Hermes Telegram command menu compatibility', () => {
  ...
  liveRendererTest('keeps all eight memory commands visible when a real gateway home has more than 100 commands', () => {
    ...
    const renderArgs = ['-c', [
      ...
      'required = {"remember-session", "memory-link", "memory-sync", "memory-status", "memory-unlink", "memory-start", "project-start", "hermes-open"}',
