...
describe('public self-hosting flow', () => {
  ...
  test('gives any AI a self-hosting prompt with the authenticated-RLS boundary', () => {
    ...
    expect(prompt).toContain('DB 목록이 유일한 회원 권한 정본');
    expect(prompt).not.toContain('VITE_ALLOWED_EMAIL');
  ...
  test('requires an explicit Vercel account confirmation before automatic production deploy', () => {
    ...
    expect(setup).toContain('Vercel에 내 개인 포털 프로젝트 만들기');
    expect(setup).toContain('데스크톱 앱 설치가 아니라 선택형 개인 웹 포털 생성');
    expect(setup).toContain('GitHub·GitLab·Bitbucket 계정에 공개 소스 사본');
  ...
  test('keeps the standalone beginner manual actionable and in dependency order', () => {
    ...
    expect(manual).toContain('portmgr_is_member()');
    expect(manual).not.toContain('VITE_ALLOWED_EMAIL');
