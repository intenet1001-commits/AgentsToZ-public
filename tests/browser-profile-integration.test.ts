...
describe('deployment Chrome profile integration', () => {
  test('offers a persisted local profile selector but not on deployed web', () => {
    ...
    const terminalOnlyGuard = app.indexOf("{activeTab !== 'portal' && (", controlsStart);
  ...
  test('never depends on a sidebar selection that is empty on launch', () => {
    ...
    expect(region).toContain("ports\n              .filter(p => p.deployUrl");
    // 비활성은 정말로 열 것이 하나도 없을 때뿐이다.
    expect(region).toContain('const empty = !target && deployable.length === 0');
    expect(region).toContain('배포 주소가 등록된 프로젝트가 없습니다');
  });

  test('passes the selected profile only to deployment links', () => {
    ...
    expect(app).toContain('void openBrowserWithDiagnostics(item, url)');
  });
  ...
  test('routes only automatic deploy bookmarks through the selected profile', () => {
    ...
    expect(portalManager).toContain('const portalUrl = `https://portmanager-portal.vercel.app?${p}`');
