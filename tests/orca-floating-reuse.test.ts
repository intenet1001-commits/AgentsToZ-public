...
describe('Orca Floating agent tab reuse', () => {
  ...
  test('uses a verified persisted handle before title matching and fails closed when Orca is unavailable', () => {
    ...
    expect(apiSource).toContain("'--limit', String(ORCA_CONTEXT_TERMINAL_LIMIT)");
    expect(apiSource).not.toContain("'terminal', 'list',\n    '--worktree', ORCA_FLOATING_WORKTREE_SELECTOR");
    ...
    expect(apiSource).toContain('if (effectiveFloating && newWindow !== true)');
    ...
    expect(tauriSource).toContain('"terminal", "list",\n            "--limit", "1000"');
