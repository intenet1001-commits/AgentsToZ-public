...
import {
  ...
  REMOTE_CONTROL_INTERACTIVE_TERMINAL_POLL_MS,
...
describe('a request the Mac never answers must not wedge the controller', () => {
  ...
  test('concurrent Workroom callers serialize without dropping or replaying keys', async () => {
    const connection=await connected();
    const {controller,transport,deliver,hostReceive}=connection;
    await deliver('1',1,ready);await controller.refresh();
    await advertiseFeatures(connection,['workspace-v1']);
    const sent:string[]=[];
    let sequence=2;
    transport.onSend=async envelope=>{
      const message=JSON.parse(decoder.decode(await decryptRemoteControlRelayEnvelope({key:hostReceive,envelope,now})));
      sent.push(message.request.requestId);
      sequence++;
      await deliver(String(sequence),sequence,{type:'terminal.result',requestId:message.request.requestId,ok:true,body:message.request.operation==='workspace'?{kind:'workspace',action:'workroom.status'}:{sessions:[]}});
    };
    const responses=await Promise.all([
      controller.sendTerminal({operation:'workspace',requestId:'status-poll',targetId:'test-project',workspace:{action:'workroom.status',sessionId:'test-session'}}),
      controller.sendTerminal({operation:'input',requestId:'arrow-key',sessionId:'test-session',data:'\x1b[A'}),
      controller.sendTerminal({operation:'input',requestId:'number-key',sessionId:'test-session',data:'2'}),
    ]);
    expect(responses).toHaveLength(3);
    expect(sent).toEqual(['status-poll','arrow-key','number-key']);
  });

  test('a Workroom key and its first output read use bounded fast result polls only', async () => {
    const sleeps:number[]=[];
    const connection=await connected({sleep:async ms=>{sleeps.push(ms);}});
    const {controller,transport,deliver,hostReceive,setClock}=connection;
    await deliver('1',1,ready);await controller.refresh();
    let sequence=1;let receiveCount=0;
    const session={id:'test-session',targetId:'test-project',agent:'codex',state:'running',createdAt:new Date(now).toISOString(),exitCode:null,cols:100,rows:28};
    transport.onSend=async envelope=>{
      const message=JSON.parse(decoder.decode(await decryptRemoteControlRelayEnvelope({key:hostReceive,envelope,now})));
      expect(message.type).toBe('terminal.request');
      receiveCount=0;
      transport.pollHook=async()=>{
        if(++receiveCount!==4)return;
        sequence++;
        const body=message.request.operation==='read'
          ?{session,chunks:[],nextCursor:0,truncated:false,hasMore:false}
          :{session};
        await deliver(String(sequence),sequence,{type:'terminal.result',requestId:message.request.requestId,ok:true,body});
      };
    };
    const key={operation:'input' as const,requestId:'fast-key',sessionId:'test-session',data:'2'};
    await controller.sendTerminal(key);
    expect(sleeps).toEqual(Array(3).fill(REMOTE_CONTROL_INTERACTIVE_TERMINAL_POLL_MS));
    sleeps.length=0;
    await controller.sendTerminal({operation:'read',requestId:'first-output',sessionId:'test-session',after:0});
    expect(sleeps).toEqual(Array(3).fill(REMOTE_CONTROL_INTERACTIVE_TERMINAL_POLL_MS));
    sleeps.length=0;
    await controller.sendTerminal({operation:'read',requestId:'ordinary-output',sessionId:'test-session',after:0});
    expect(sleeps).toEqual(Array(3).fill(REMOTE_CONTROL_ACTION_RESULT_POLL_MS));
    sleeps.length=0;
    await controller.sendTerminal({operation:'input',requestId:'ordinary-text',sessionId:'test-session',data:'hello'});
    expect(sleeps).toEqual(Array(3).fill(REMOTE_CONTROL_ACTION_RESULT_POLL_MS));
    sleeps.length=0;
    await controller.sendTerminal({operation:'input',requestId:'second-key',sessionId:'test-session',data:'3'});
    expect(sleeps).toEqual(Array(3).fill(REMOTE_CONTROL_INTERACTIVE_TERMINAL_POLL_MS));
    sleeps.length=0;
    setClock(now+10_000);
    await controller.sendTerminal({operation:'read',requestId:'other-output',sessionId:'test-session',after:0});
    expect(sleeps).toEqual(Array(3).fill(REMOTE_CONTROL_ACTION_RESULT_POLL_MS));
    expect(transport.sent).toHaveLength(7); // pair + six unique terminal requests
  });

  test('an unanswered Workroom key keeps the one-minute uncertainty budget and is never replayed', async () => {
    const sleeps:number[]=[];
    const {controller,transport,deliver}=await connected({sleep:async ms=>{sleeps.push(ms);}});
    await deliver('1',1,ready);await controller.refresh();
    await expect(controller.sendTerminal({operation:'input',requestId:'uncertain-key',sessionId:'test-session',data:'1'}))
      .rejects.toThrow('입력은 전달되었을 수 있으므로');
    expect(sleeps.reduce((total,ms)=>total+ms,0)).toBe(60_000);
    expect(sleeps.filter(ms=>ms===REMOTE_CONTROL_INTERACTIVE_TERMINAL_POLL_MS)).toHaveLength(4);
    expect(transport.sent).toHaveLength(2); // pair + one key, no replay after timeout
  });

  test('tester requests require host capability and retain the start identity through encrypted relay envelopes', async()=>{
    const legacy=await connected();await legacy.deliver('1',1,ready);await legacy.controller.refresh();await advertiseFeatures(legacy,['workspace-v1']);
    await expect(legacy.controller.sendTerminal({operation:'workspace',requestId:'tester-status-legacy',targetId:'test-project',workspace:{action:'tester.status'}})).rejects.toThrow('업데이트');
    const connection=await connected();const {controller,transport,deliver,hostReceive}=connection;
    await deliver('1',1,ready);await controller.refresh();await advertiseFeatures(connection,['workspace-v1','tester-v1']);
    const received:any[]=[];let sequence=2;
    transport.onSend=async envelope=>{const m=JSON.parse(decoder.decode(await decryptRemoteControlRelayEnvelope({key:hostReceive,envelope,now})));received.push(m.request);sequence++;
      await deliver(String(sequence),sequence,{type:'terminal.result',requestId:m.request.requestId,ok:true,body:{kind:'workspace',action:'tester.start',tester:{canRun:true,canCancel:true,run:{id:'20260914T060000Z-1234abcd',state:'running',profileId:'quick',createdAt:'2026-09-14T06:00:00Z',origin:'app',checkCount:0,checks:[]}}}});
    };
    const workspace={action:'tester.start' as const,profileId:'quick',revisionHash:'a'.repeat(64),testRequestId:'test_1789358400000_12345678-1234-1234-1234-123456789012'};
    for(const requestId of ['tester-envelope-one','tester-envelope-two'])expect((await controller.sendTerminal({operation:'workspace',requestId,targetId:'test-project',workspace})).tester!.run!.id).toBe('20260914T060000Z-1234abcd');
    expect(received.map(r=>r.workspace.testRequestId)).toEqual([workspace.testRequestId,workspace.testRequestId]);
    expect(JSON.stringify(transport.sent)).not.toContain('tester.start');
  });

  test('terminal admission is bounded and a failed response releases queued callers', async () => {
    const {controller,transport,deliver,hostReceive}=await connected();
    await deliver('1',1,ready);await controller.refresh();
    let release!:()=>void;
    ...
    transport.onSend=async envelope=>{
      const message=JSON.parse(decoder.decode(await decryptRemoteControlRelayEnvelope({key:hostReceive,envelope,now})));
      await gate;sequence++;
      await deliver(String(sequence),sequence,{type:'terminal.result',requestId:message.request.requestId,
        ...(sequence===2?{ok:false,error:'fixture rejection'}:{ok:true,body:{sessions:[]}})});
    };
    const all=Promise.allSettled(Array.from({length:33},(_,i)=>controller.sendTerminal({operation:'list',requestId:'queued-'+i})));
    release();const results=await all;
    expect(results[0]?.status).toBe('rejected');
    expect(results[32]?.status).toBe('rejected');
    expect(results.slice(1,32).every(r=>r.status==='fulfilled')).toBe(true);
    await expect(controller.sendTerminal({operation:'list',requestId:'after-error'})).resolves.toEqual({sessions:[]});
  });

  test('a waiting terminal close gets the next relay slot before another background refresh', async () => {
    let resumeWait!: () => void;
    const {controller,transport,deliver,hostReceive}=await connected({
      sleep:()=>new Promise<void>(resolve=>{resumeWait=resolve;}),
    });
    await deliver('1',1,ready);await controller.refresh();
    const originalStatus=transport.status.bind(transport);
    let releasePoll!:()=>void;
    transport.status=async()=>{await new Promise<void>(resolve=>{releasePoll=resolve;});return originalStatus();};
    const poll=controller.refresh();
    const close=controller.sendTerminal({operation:'close',requestId:'terminal-close-458',sessionId:'terminal-session-458'});
    releasePoll();await poll;
    transport.status=originalStatus;
    const statusCalls=transport.statusCalls;
    const competingRefresh=controller.refresh();
    await competingRefresh;
    // The periodic reader must not retake the released slot ahead of the user's waiting tap.
    expect(transport.statusCalls).toBe(statusCalls);
    let sent=0;
    transport.onSend=async envelope=>{
      const message=JSON.parse(decoder.decode(await decryptRemoteControlRelayEnvelope({key:hostReceive,envelope,now})));
      expect(message.request.operation).toBe('close');sent++;
      await deliver('2',2,{type:'terminal.result',requestId:message.request.requestId,ok:true,body:{sessions:[]}});
    };
    resumeWait();await expect(close).resolves.toEqual({sessions:[]});expect(sent).toBe(1);
    // A successful command releases its reservation; later refreshes still run.
    await controller.refresh();expect(transport.statusCalls).toBe(statusCalls+1);
  });
