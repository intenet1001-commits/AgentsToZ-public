...
describe('ChatGPT desktop Codex integration', () => {
  ...
  test('keeps the codex deep link registered by the ChatGPT desktop app', () => {
    ...
    expect(apiSource).toContain("if (mode === 'new')");
    expect(apiSource).toContain('openChatGptDeepLink(`codex://threads/${latest.sessionId}`)');
    expect(rustSource).not.toContain('codex://threads/new?path={}');
  });
