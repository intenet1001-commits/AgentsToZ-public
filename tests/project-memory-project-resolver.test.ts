...
import { createRegisteredProjectMemoryResolver, resolveRegisteredProjectMemory } from "../src/projectMemoryProjectResolver";
...
describe("registered project memory resolver", () => {
  ...
  test("collapses separate parent and generated worktree rows into one canonical representative", () => {
    const folder = project("folder");
    const worktree = project("worktree", false);
    const rows = [
      { id: "parent", name: "demo", folderPath: folder },
      { id: "parent_wt_feature", name: "demo feature", folderPath: worktree, worktreePath: worktree },
    ];
    const resolveRoot = (alias: string) => alias === folder || alias === worktree ? folder : null;

    for (const query of ["m", "demo", folder, worktree, "parent_wt_feature"]) {
      expect(resolveRegisteredProjectMemory(query, rows, resolveRoot)).toMatchObject({
        ok: true,
        id: "parent",
        name: "demo",
  ...
  test("prefers the canonical-root row over an earlier manual external alias", () => {
    const folder = project("folder");
    const external = project("external", false);
    const rows = [
      { id: "manual-alias", name: "external alias", folderPath: external },
      { id: "parent", name: "demo", folderPath: folder },
    ];
    const resolveRoot = (alias: string) => alias === folder || alias === external ? folder : null;

    expect(resolveRegisteredProjectMemory("manual-alias", rows, resolveRoot)).toMatchObject({
      ok: true,
      id: "parent",
      name: "demo",
  ...
  test("accepts an external absolute alias only when it resolves to the registered canonical memory", () => {
    const folder = project("folder");
    const external = project("external-worktree", false);
    const outsider = project("outsider");
    const row = [{ id: "p", name: "demo", folderPath: folder }];
    const resolveRoot = (alias: string) => {
      if (alias === folder || alias === external) return folder;
      if (alias === outsider) return outsider;
      return null;
    };

    expect(resolveRegisteredProjectMemory(external, row, resolveRoot)).toMatchObject({
      ok: true,
      requestedPath: external,
      canonicalPath: folder,
      matchedBy: "path",
    });
    expect(resolveRegisteredProjectMemory(outsider, row, resolveRoot))
      .toMatchObject({ ok: false, code: "PROJECT_NOT_REGISTERED" });
    expect(resolveRegisteredProjectMemory(external, row))
      .toMatchObject({ ok: false, code: "PROJECT_NOT_REGISTERED" });
  });

  test("does not dynamically allow an external alias when the shared canonical root is uninitialized", () => {
    const folder = project("folder", false);
    const external = project("external-worktree", false);
    const resolveRoot = (alias: string) => alias === folder || alias === external ? folder : null;

    expect(resolveRegisteredProjectMemory(
      external,
      [{ id: "p", name: "demo", folderPath: folder }],
      resolveRoot,
...
test("one enumeration snapshot detects each registered source only once across all memory lookups", () => {
  const rows = Array.from({ length: 25 }, (_, i) => {
    const root = project(`snapshot-${i}`);
    writeFileSync(join(root, '.agent-memory/config.json'), JSON.stringify({ memoryId: `memory-${i}` }));
    return { id: `p-${i}`, folderPath: root };
  });
  let probes = 0;
  const lookup = createRegisteredProjectMemoryResolver(rows, path => { probes++; return path; });
  for (let i = 0; i < rows.length; i++) expect(lookup(`memory-${i}`)).toMatchObject({ ok: true, id: `p-${i}` });
  expect(probes).toBe(rows.length);
});
