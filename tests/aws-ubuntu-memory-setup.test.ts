...
// 환경 단위 설치·연결 버튼은 장기기억 탭 한 곳으로 모았다(VOC 2026-08-28).
const memoryDirectory = readFileSync(join(root, "src", "PortalMemoryDirectory.tsx"), "utf8");
const standalonePrompt = readFileSync(join(root, "src", "externalMemorySetupPrompts.ts"), "utf8");
...
describe("AWS Ubuntu project-memory bootstrap", () => {
  ...
  test("keeps standalone and Supabase-connected Ubuntu flows separate", () => {
    expect(memoryDirectory).toContain("이 앱이 없는 PC에 장기기억 만들기");
    expect(memoryDirectory).not.toContain("앱 없는 PC·AWS Ubuntu용");
    expect(memoryDirectory).toContain('data-testid="portal-memory-copy-aws-ubuntu-setup"');
    expect(memoryDirectory).toContain("AWS Ubuntu 서버를 이 Supabase에 연결");
    expect(memoryDirectory).not.toContain("AWS Ubuntu · Supabase 연결");
    expect(standalonePrompt).toContain("외부 앱이나 서버, 네트워크는 쓰지 않고");
  });

  // 두 갈래가 프로젝트 패널과 장기기억 탭 양쪽에 있어서 프로젝트 단위 작업으로 읽혔다.
  test("no longer offers environment-level setup inside the project panel", () => {
    expect(panel).not.toContain('data-testid="copy-aws-ubuntu-memory-setup"');
    expect(panel).not.toContain('data-testid="copy-standalone-init-prompt"');
    expect(panel).not.toContain('data-testid="copy-manual-init-prompt"');
  });
