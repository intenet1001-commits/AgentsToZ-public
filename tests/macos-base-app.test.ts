...
import { MACOS_BASE_API_SIDECAR_ENTITLEMENTS, MACOS_BASE_CODE, MACOS_BASE_EMPTY_ENTITLEMENTS, MACOS_BASE_REQUIRED_FILES, inspectMacOSBaseBundle, planMacOSBaseAppSigning, signMacOSBaseApp, verifyMacOSBaseSignatureInspection } from '../src/macOSBaseAppSigning';
...
function fixture() {
  ...
  const entitlements = { apiSidecar: join(root, 'api.plist'), empty: join(root, 'empty.plist') };
  writeFileSync(entitlements.apiSidecar, MACOS_BASE_API_SIDECAR_ENTITLEMENTS);
  writeFileSync(entitlements.empty, MACOS_BASE_EMPTY_ENTITLEMENTS);
  return { root, app, entitlements };
}
function inspection(app: string, code: typeof MACOS_BASE_CODE[number]) {
  const entitlements = code.path.endsWith('/agentstoz-api-sidecar')
  ...
  return [`Executable=${join(app, code.path)}`, `Identifier=${code.identifier}`,
    ...
    'TeamIdentifier=Q1W2E3R4T5', 'Timestamp=Sep 7, 2026 at 12:00:00', entitlements].join('\n');
}
...
test('base signing is inside-out, never ad-hoc, and requires actual canary evidence', () => {
  ...
  expect(plan.filter(c => c.operation === 'sign').map(c => c.args.at(-1)))
    .toEqual([...MACOS_BASE_CODE.slice(0, 4).map(c => join(app, c.path)), app]);
  for (const c of plan.filter(c => c.operation === 'sign')) { expect(c.args).not.toContain('--deep'); expect(c.args).toContain('--timestamp'); }
  const signs = plan.filter(c => c.operation === 'sign');
  expect(signs.filter(c => c.entitlements === 'apiSidecar')).toHaveLength(1);
  expect(signs.find(c => c.entitlements === 'apiSidecar')!.args).toContain(entitlements.apiSidecar);
  expect(signs.filter(c => c.entitlements === 'empty').every(c => c.args.includes(entitlements.empty))).toBeTrue();
  expect(() => planMacOSBaseAppSigning({ ...identity(), identity: null }, app, entitlements)).toThrow();
  expect(() => planMacOSBaseAppSigning(identity(), app, { ...entitlements, empty: join(app, 'entitlements') })).toThrow();
  expect(() => planMacOSBaseAppSigning(identity(), app + '/../AgentsToZ_byCS.app', entitlements)).toThrow();
});

test('inspection requires only API-sidecar JIT and empty entitlements on every other binary', () => {
  const { app } = fixture();
  for (const code of MACOS_BASE_CODE) {
    ...
    for (const invalid of [
      ...
      code.path.endsWith('/agentstoz-api-sidecar')
        ? valid.replace('com.apple.security.cs.allow-jit', 'com.apple.security.get-task-allow')
        : valid.replace('<dict/>', '<dict><key>com.apple.security.get-task-allow</key><true/></dict>'),
      code.path.endsWith('/agentstoz-api-sidecar')
        ? valid.replace('<true/></dict>', '<true/><false/></dict>')
        : valid.replace('<dict/>', '<dict/><dict/>'),
...
test('verified signing creates a digest and remains explicitly unnotarized and uninstalled', async () => {
  ...
  expect(result).toMatchObject({ mode: 'developer-id-base', nestedCodeSigned: 4, enhancedRuntimeEnabled: false, notarized: false, installed: false, ready: false });
...
test('base pipeline rejects unpublished/private/moving sources before issuing a release receipt', async () => {
  for (const fault of ['unpublished', 'private', 'changed-before-sign', 'changed-after-sign', 'sign-failed', 'stale-app', 'canary', 'none']) {
    ...
    const dependencies: MacOSBasePipelineDependencies = {
      ...
      sign: async () => { signCalls++; if (fault === 'sign-failed') throw Error('sign failed'); return { schemaVersion: 1, mode: 'developer-id-base', result: 'signed-awaiting-notarization', appSigned: true, nestedCodeSigned: 4, bundleDigest: 'a'.repeat(64), enhancedRuntimeEnabled: false, notarized: false, installed: false, ready: false }; },
    };
    if (fault === 'none') {
      ...
      expect(receipt.signingIdentity).toEqual(identity().identity!);
