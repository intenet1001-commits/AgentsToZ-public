...
describe("port kill paths target listeners only", () => {
  ...
  test("Unix listener checks resolve both macOS and Ubuntu lsof locations", () => {
    expect(apiServer).toContain("['/usr/sbin/lsof', '/usr/bin/lsof'].find(candidate => existsSync(candidate)) ?? 'lsof'");
    expect(apiServer).not.toContain("cmd: ['/usr/sbin/lsof'");
    expect(apiServer).not.toContain("Bun.spawnSync(['/usr/sbin/lsof'");
    // Follow each caller and the extracted discovery helper. Counting inline
    // argv literals misses whether a delegated probe retains the same guard.
    for (const name of ['getPidsByPort', 'getListeningPortsSnapshot']) {
      const fn = sliceFrom(apiServer, `async function ${name}`, '\n}\n');
      expect(fn).toContain('[LSOF_EXECUTABLE,');
      expect(fn).toContain("'-sTCP:LISTEN'");
    }
    expect(apiServer).toContain('createWorktreePortDiscovery({ executable: LSOF_EXECUTABLE })');
    const discovery = readFileSync(join(root, 'src', 'worktreePortDiscovery.ts'), 'utf8');
    expect(discovery).toContain('runWorktreeLsof(input.executable, args)');
    expect(discovery).toContain("'-iTCP:10001-59999', '-sTCP:LISTEN'");
  });
