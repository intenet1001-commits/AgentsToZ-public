...
describe('remote control mobile page reconnection', () => {
  ...
  test('preserves session token in sessionStorage for restoration', () => {
    // sessionStorage를 사용한 세션 토큰 저장/복원
    expect(REMOTE_CONTROL_MOBILE_JS).toContain('sessionStorage');
    expect(REMOTE_CONTROL_MOBILE_JS).toContain('SESSION_STORAGE_KEY');
  });
