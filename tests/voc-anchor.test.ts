...
describe('배선', () => {
  ...
  test('오버레이는 lazy — 끄면 청크를 내려받지 않는다', () => {
    ...
    expect(appSource).toContain("{vocMode && typeof document !== 'undefined' && createPortal(");
  ...
  test('저장 실패 시 쓴 글을 지우지 않는다', () => {
    // 성공했을 때만 비운다. 저장 뒤 목록 갱신이 붙었어도 이 조건은 그대로여야 한다.
    expect(overlaySource).toContain('if (ok) {\n      reset();');
    expect(overlaySource).not.toContain('reset();\n    const ok = await onSubmit');
  });
