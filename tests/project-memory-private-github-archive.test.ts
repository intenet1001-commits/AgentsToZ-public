...
describe("Private GitHub project-memory archive", () => {
  ...
  test("archives 25,000 verified journal entries within a generous long-term budget", async () => {
    const { root, runner, input } = fixture();
    try {
      ...
      // Include this large fixture's cleanup in its existing 60s test budget,
      // not Bun 1.3.1's fixed 5s hook budget. The measured 45s archive limit above
      // still excludes cleanup and is unchanged.
