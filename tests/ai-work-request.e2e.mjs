...
const projects=${JSON.stringify(options.noControl?projects.filter(p=>p.targetId!==CONTROL):projects)};
...
async function fixture(options={}){
 ...
 await context.route('**/*',async route=>{
  ...
  if(url.pathname==='/api/control-profile/status')return route.fulfill({json:{success:true,profile:options.profile??{state:'unprepared'}}});
...
try{
 ...
 await run('partial inventory preserves selection; complete removal requires an explicit new choice',async({page,prompt,start})=>{
  ...
  assert.match(await page.getByRole('status').innerText(),/마지막 확인값이라 선택할 수 있으며.*실행 직전에 실제 등록·경로를 다시 검사/s);
 ...
 await run('mission mode launches a visible Control coordinator with four selectable workers',async({page,prompt,start,state})=>{
  ...
  assert.equal(await page.getByLabel('AI 작업 실행 방식').locator('option:checked').innerText(),'AgentsToZ OPS 미션 관제');
 ...
 await run('a user-wide profile starts orchestration without a Control folder and uses its saved policy',async({page,prompt,start,state})=>{
  await page.getByLabel('AI 작업 실행 방식').selectOption('mission');
  await until(async()=>await page.getByLabel('AI 작업 관제 전략').inputValue()==='cs-ceo','Profile policy is loaded');
  await prompt.fill('아젠투지, 공통 운영 기억을 확인하고 진행해');await start.click();
  await page.waitForFunction(()=>window.__work.started.length===1);assert.equal(state.starts[0].targetId,A);assert.match(state.starts[0].prompt,/agentstoz_use_get_control_profile/);assert.match(state.starts[0].prompt,/agentstoz_use_recall_control_context/);
 },{noControl:true,profile:{state:'ready',projectId:null,coordinationPolicy:'cs-ceo'},targetModes:[{targets:projects.filter(p=>p.targetId!==CONTROL),complete:true}]});
 await run('1000px at 125% keeps controls within the workspace and keyboard reaches start',async({page,panel,prompt})=>{
  assert.match(await panel.getByRole('heading',{name:/AI 작업 시작/}).innerText(),/AI 작업 시작.*요청 1/);
