...
test('v11 upgrade backs up and stabilizes legacy entry identities exactly once', () => {
  ...
  expect(agentInstructions).toContain('memory-agent-version:11');
...
test('ordinary AI update uses the last known-good identity document before replacing it', () => {
  ...
  expect(body).toContain('stabilizeProjectMemoryEntryIds(stripMarkdownFence(raw), previousIdentity)');
});
