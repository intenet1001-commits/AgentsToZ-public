...
async function runtimeTab(page) {
  if (await page.getByTestId('quick-project-names').isVisible().catch(()=>false)) return;
  await page.getByTestId('legacy-name-work-reopen').click();
}
  ...
  for (const scenario of ['batch', 'edit-review', 'edit-conflict', 'edit-persisted-conflict', 'partial', 'apply-failure', 'refresh-complete', 'refresh-stop']) {
    ...
    try {
      ...
      if (['batch', 'partial', 'apply-failure'].includes(scenario)) {
        ...
        check(`Local: ${scenario} 이름 추천은 독립 대화상자에 표시`,
          await card.getAttribute('role')==='dialog' && await page.getByTestId('top-level-runtime-tab').count()===0);
        ...
        await eventually(async () => /전체 추천 완료/.test(await page.getByTestId('quick-project-names-progress').textContent()), 'The naming dialog did not finish all batches.');
        check(`Local: ${scenario} 이름 추천 완료 전 원본은 불변`,
