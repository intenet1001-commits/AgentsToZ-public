...
describe('Windows command launch planning', () => {
  ...
  test('can bind a supervised remote runtime to its sidecar parent lifetime', () => {
    const plan = buildWindowsSupervisedLaunch(
      'C:\\Program Files\\AgentsToZ\\windows-process-supervisor.ps1',
      { cmd: ['claude.exe', 'remote-control', '--spawn=session'], env: {} },
      { parentPid: 43_210 },
    );
    expect(plan.env.AGENTSTOZ_SUPERVISOR_PARENT_PID).toBe('43210');
    expect(() => buildWindowsSupervisedLaunch(
      'C:\\supervisor.ps1',
      { cmd: ['claude.exe'], env: {} },
      { parentPid: 1 },
    )).toThrow('safe process identifier');
    expect(() => buildWindowsSupervisedLaunch(
      'C:\\supervisor.ps1',
      { cmd: ['claude.exe'], env: {} },
      { parentPid: 0x1_0000_0000 },
    )).toThrow('safe process identifier');
  });
