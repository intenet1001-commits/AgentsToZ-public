...
describe('buildCodeAppDeepLink', () => {
  ...
  test('routes desktop and mobile Claude project opens through one verified session manager', async () => {
    ...
    // All three providers preserve the sidecar's structured outcome in Tauri.
    expect(app).not.toContain("await invoke<string>('open_code_app', { agent, folderPath })");
    expect(app).toContain("API.openCodeApp('codex', context.workingPath!, 'prepare')");
