...
describe('user-wide Control profile',()=>{
 ...
 test('local-only proposals save immediately without pretending that a shared review occurred',async()=>{
  const f=fixture();await f.store.prepare();const input={requestId:'request-local',title:'Local preference',body:'Keep this on this Mac.',evidence:'The user requested this.',expectedRevision:f.store.status().revision};
  const p=await f.store.propose(input);expect(p.state).toBe('saved');expect(f.saveCalls()).toBe(1);expect(f.store.pending()).toEqual([]);
  expect((await f.store.propose(input)).id).toBe(p.id);expect(f.saveCalls()).toBe(1);
 });
 test('shared-folder proposal is not a save, retries deduplicate, review saves once and conflicts preserve candidates',async()=>{
  const f=fixture(),root=join(f.dir,'Control'),memoryId=randomUUID();mkdirSync(root);f.memories.set(root,{memoryId,document:'# Shared operating memory\n',savedAt:'2026-09-14'});f.setCandidates([{root,memoryId,projectId:'control'}]);
  await f.store.prepare();const input={requestId:'request-1',title:'Operating preference',body:'Prefer verified results.',evidence:'The user requested this.',expectedRevision:f.store.status().revision};
