...
afterEach(async () => {
  for (const child of children.splice(0)) {
    ...
    try { await child.exited.catch(() => undefined); } finally { clearTimeout(forceExit); }
  }
...
async function fixture(initialized = true) {
  ...
  children.push(child);
  return { home, project, memory, baseUrl, appData, portsFile: join(appData, "ports.json") };
}

const route = { platform: "telegram", chatId: "8175665017", threadId: "6884" };

describe("project-memory thread HTTP API", () => {
  test("shared workspace authority blocks writers while leaving local-only worktree inventory readable", async () => {
    const { baseUrl, project, appData } = await fixture(false);
    const lease = await acquireWorkspaceLease({ workspacePath: project, appDataDir: appData });
    try {
      const readOnly = await post(baseUrl, "/api/list-git-worktrees", {
        folderPath: project,
        fetchRemote: false,
      });
      expect(readOnly.response.status).toBe(200);
      expect(readOnly.body.success).toBe(true);

      const refreshing = await post(baseUrl, "/api/list-git-worktrees", {
        folderPath: project,
        fetchRemote: true,
      });
      expect(refreshing.response.status).toBe(409);
      expect(refreshing.body).toMatchObject({
        success: false,
        ok: false,
        code: "WORKSPACE_LEASE_BUSY",
      });

      const start = await post(baseUrl, "/api/project-memory/thread/start", {
        ...route,
        project: "Demo Memory",
      });
      expect(start.response.status).toBe(409);
      expect(start.body.code).toBe("WORKSPACE_LEASE_BUSY");
      expect(existsSync(join(project, ".agent-memory"))).toBe(false);
    } finally {
      expect(lease.release()).toBe(true);
    }

    const projectRoot = dirname(project);
    const rootLease = await acquireWorkspaceLease({ workspacePath: projectRoot, appDataDir: appData });
    try {
      const create = await post(baseUrl, "/api/project-memory/thread/create-project", {
        ...route,
        threadId: "lease-blocked-create",
        name: "동시 생성 차단",
        rootPath: projectRoot,
      });
      expect(create.response.status).toBe(409);
      expect(create.body.code).toBe("WORKSPACE_LEASE_BUSY");
      expect(existsSync(join(projectRoot, "동시-생성-차단"))).toBe(false);
    } finally {
      expect(rootLease.release()).toBe(true);
    }
  }, 20_000);

  test("rejects query/body workspace target disagreement before any mapped mutation", async () => {
    const { baseUrl, project, home } = await fixture(false);
    const other = join(home, "projects", "other");
    mkdirSync(other, { recursive: true });
    const response = await fetch(
      `${baseUrl}/api/project-memory/update?folderPath=${encodeURIComponent(project)}`,
      {
        method: "POST",
        // Deliberately omit Content-Type: legacy memory parsing would otherwise
        // use the query while a generic JSON parser could select the body.
        body: JSON.stringify({ folderPath: other, agent: "codex" }),
      },
    );
    ...
    expect(body).toMatchObject({
      success: false,
      ok: false,
      code: "WORKSPACE_LEASE_UNSAFE",
    });
    expect(existsSync(join(project, ".agent-memory"))).toBe(false);
    expect(existsSync(join(other, ".agent-memory"))).toBe(false);
  }, 20_000);
