...
ReactDOM.createRoot(document.getElementById('root')).render(React.createElement(Harness));</script>`});
     ...
     if(body.operation==='workspace'){
      assert.equal(body.targetId,session.targetId);
      assert.deepEqual(body.workspace,{action:'workroom.status',sessionId:session.id});
      return route.fulfill({json:{kind:'workspace',action:'workroom.status',workroom:{sessionId:session.id,initialized:true,lastSavedAt:null,context:{usedPercent:null,observedAt:null,source:'unavailable'},save:{requestId:null,state:'idle',localSaved:false,backupSaved:false,message:''}}}});
     }
    ...
    await waitForOutput();
    await page.getByTestId('workroom-session-footer').getByRole('button',{name:'지금 저장',exact:true}).waitFor();
    await page.waitForFunction(()=>!document.querySelector('[data-testid="workroom-session-footer"] button')?.disabled);
    assert.equal(await page.getByTestId('workroom-session-footer').getByRole('alert').count(),0);
    // Enabled save controls must remain legible in both app themes. A fixed
    // dark footer combined with light-theme button tokens previously failed.
    for(const theme of ['gray','dark']){
     await page.evaluate(theme=>document.documentElement.dataset.appTheme=theme,theme);
     const checkContrast=async selector=>{
      const ratio=await page.locator(selector).evaluate(container=>{
       ...
       const text=luminance(getComputedStyle(button).color),bg=luminance(getComputedStyle(container).backgroundColor);
       return (Math.max(text,bg)+.05)/(Math.min(text,bg)+.05);
      });
      assert.ok(ratio>=4.5,`${name}/${remote}/${theme}: save button contrast ${ratio}`);
     };
     await checkContrast('[data-testid="workroom-session-footer"]');
     await page.getByRole('button',{name:'세션 종료',exact:true}).click();
     await page.getByRole('dialog',{name:'세션 종료 전 저장'}).waitFor();
     await checkContrast('.workroom-exit-dialog');
     await page.getByRole('button',{name:'취소',exact:true}).click();
    }
    await page.evaluate(()=>document.documentElement.dataset.appTheme='gray');
    ...
    if(remote){
     assert.ok(before.hostTop>=0&&before.hostBottom<=page.viewportSize().height,'selected mobile terminal is revealed without manual scrolling');
     await page.evaluate(()=>window.scrollTo(0,0));await page.waitForTimeout(300);
     assert.equal(await page.evaluate(()=>window.scrollY),0,'background output does not override deliberate scrolling');
    }
