...
describe('build information', () => {
  ...
  test('embeds published provenance and visibly marks an explicit unpublished test build', () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), 'agentstoz-build-info-'));
    const oldSha = process.env.AGENTSTOZ_RELEASE_SOURCE_SHA;
    const oldStatus = process.env.AGENTSTOZ_RELEASE_SOURCE_STATUS;
    try {
      mkdirSync(join(fixtureRoot, 'src-tauri'));
      writeFileSync(join(fixtureRoot, 'build-number.json'), '{"buildNumber":130}\n');
      writeFileSync(join(fixtureRoot, 'src-tauri', 'tauri.conf.json'), '{"version":"130.0.0"}\n');
      process.env.AGENTSTOZ_RELEASE_SOURCE_SHA = 'a'.repeat(40);
      process.env.AGENTSTOZ_RELEASE_SOURCE_STATUS = 'published';
      expect(createBuildInfo({ root: fixtureRoot })).toMatchObject({
        sourceCommit: 'a'.repeat(40),
        sourcePublished: true,
      });
      process.env.AGENTSTOZ_RELEASE_SOURCE_STATUS = 'unpublished';
      expect(createBuildInfo({ root: fixtureRoot }).sourcePublished).toBe(false);
    } finally {
      if (oldSha === undefined) delete process.env.AGENTSTOZ_RELEASE_SOURCE_SHA;
      else process.env.AGENTSTOZ_RELEASE_SOURCE_SHA = oldSha;
      if (oldStatus === undefined) delete process.env.AGENTSTOZ_RELEASE_SOURCE_STATUS;
      else process.env.AGENTSTOZ_RELEASE_SOURCE_STATUS = oldStatus;
      rmSync(fixtureRoot, { recursive: true, force: true });
    }
  });
  ...
  test('injects shared metadata and renders both public headers', () => {
    ...
    expect(appSource).toContain('<BuildInfoBadge className="workspace-build-info" />');
    ...
    expect(badgeSource).toContain('미공개 테스트 빌드');
    expect(badgeSource).toContain('>TEST</span>');
  });
});
