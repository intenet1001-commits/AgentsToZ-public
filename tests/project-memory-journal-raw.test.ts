...
describe("a journal body may contain anything", () => {
  ...
  test("new records encode the full entry in one inert v2 frame", () => {
    ...
    const parser = source.slice(
      source.indexOf("function parseProjectMemoryJournalFiles"),
      source.indexOf("function projectMemoryJournalSnapshot"),
    );
    expect(parser).toContain("JOURNAL_V2_FRAME");
    expect(parser).not.toContain("text.split(/^## /m)");
  });
});
