...
describe("project memory size budget", () => {
  ...
  // 모델이 확인할 수 있는 지시다.
  test("the prompt names one over-budget section, with its size and target", () => {
    const body = updateProjectMemoryBody();
    expect(body).toContain("COMPACTION TARGET");
    expect(body).toContain("${overweight.title}");
    expect(body).toContain("${overweight.bytes} bytes");
    expect(body).toContain("${MEMORY_NOTE_BUDGET_BYTES}-byte per-section budget");
  });

  test("compaction is demanded only when a section is actually over budget", () => {
    const body = updateProjectMemoryBody();
    expect(body).toContain("selectMemoryNoteToCompact");
    expect(body).toContain("Leave every other section unchanged.");
    // The no-op branch must not ask for a rewrite of anything.
    expect(body).toContain("no section is over its");
  });
  ...
  test("the over-budget section is measured from the document, not from the stored manifest", () => {
    const body = updateProjectMemoryBody();
    expect(body).toContain("selectMemoryNoteToCompact(buildMemoryNoteManifest(splitMemoryDocument(current))");
  ...
  test("prompt supplies today's date instead of letting the agent guess it", () => {
    const body = updateProjectMemoryBody();
    expect(body).toContain("Set the Last Updated date to ${today}");
