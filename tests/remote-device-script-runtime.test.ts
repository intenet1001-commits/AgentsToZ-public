...
`);
  ...
  // curl is mocked above; isolate Python's inventory request as well so this
  // minimal-host fixture never reads the developer's live projects on port 3001.
  const pythonFixtures = join(root, 'python-fixtures');
  mkdirSync(pythonFixtures);
  writeFileSync(join(pythonFixtures, 'sitecustomize.py'), `
import io, os, urllib.request
def fixture_urlopen(url, **kwargs):
    if url != 'http://127.0.0.1:3001/api/ports':
        raise RuntimeError('Unexpected fixture request')
    with open(os.environ['REPORT_LOG'], 'a') as log:
        log.write(url + '\\n')
    return io.BytesIO(b'[]')
...
`);
  ...
  expect(readFileSync(reportLog, 'utf8')).toContain('http://127.0.0.1:3001/api/ports');
