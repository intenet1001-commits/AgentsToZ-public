...
for(const reason of ['disconnect','rate overflow','queue overflow'] as const){
  test(`LAN ${reason} revokes an input awaiting checkout proof even while the old terminal grant remains set`,async()=>{
    ...
    const remote=createAiTerminalRemoteGateway({service,
      // Mirrors production (aiTerminalConnections): a listed-but-disconnected session is not
      // an active terminal owner. Presence in the list alone must never authorize a write.
      active:owner=>server?.status().sessions.some(session=>session.connected&&'lan:'+session.id===owner)??false,
    ...
    try {
      ...
      // A plain disconnect keeps the session listed and restorable; a policy close (rate or
      // queue overflow, code 1008) still ends it outright. Either way the owner stops being an
      // active terminal target, which is what gates the write below.
      const remaining=f.server.status().sessions;
      if(reason==='disconnect'){expect(remaining).toHaveLength(1);expect(remaining[0]!.connected).toBe(false);}
