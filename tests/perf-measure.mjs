...
// Maintainer measurements exercise an owned static build. No external request
// (fonts, auth, metrics or a developer's API) is allowed to leave the fixture.
const offline = process.argv.includes('--offline');
if (offline && target.hostname !== '127.0.0.1') throw new Error('--offline requires an owned loopback server');
...
async function sample(context, mode, index, capture = false) {
  ...
  if (offline) await page.route('**/*', route => new URL(route.request().url()).origin === target.origin ? route.continue() : route.abort());
