...
describe("project service Agent onboarding UI", () => {
  test("has one creation entry in the shared memory directory", () => {
    ...
    expect(directorySource).toContain("Buzz 앱용 프로젝트 Agent");
    expect(directorySource).toContain("Buzz 앱용 USE Agent 만들기");
    expect(directorySource).toContain("기존 DEV 프로젝트 기억은 그대로 두고 USE 운영기억을 별도로 준비합니다");
  ...
  test("separates service Agent creation from channel assignment", () => {
    ...
    expect(dialogSource).toContain("Buzz 앱용 USE 서비스 Agent 만들기");
  ...
  test("uses dedicated credential-free local endpoints", () => {
    ...
    expect(apiSource).toContain('LEGACY_CS_CEO_SETUP_REMOVED');
    expect(dialogSource).not.toContain('CS-CEO');
