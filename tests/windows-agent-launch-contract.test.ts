...
const supervisorSource = readFileSync(
  new URL('../src-tauri/resources/windows-process-supervisor.ps1', import.meta.url),
  'utf8',
);
...
describe('native Windows agent launch contract', () => {
  ...
  test('closes a remote runtime Job Object when its API-sidecar owner dies', () => {
    expect(apiSource).toContain('{ parentPid: process.pid }');
    expect(apiSource).toContain('spawn: spawnContainedClaudeRemoteControl');
    expect(supervisorSource).toContain('AGENTSTOZ_SUPERVISOR_PARENT_PID');
    expect(supervisorSource).toContain('OpenProcess(SYNCHRONIZE,false,parentPid)');
    expect(supervisorSource).toContain('WaitForSingleObject(p,0)');
    expect((supervisorSource.match(/if\(ParentGone\(parent\)\)return 70/g) ?? []).length).toBeGreaterThanOrEqual(3);
    expect(supervisorSource.indexOf('if(ParentGone(parent))return 70')).toBeLessThan(
      supervisorSource.indexOf('CreateProcessW(program'),
    );
    expect(supervisorSource).toContain('if(job!=IntPtr.Zero)CloseHandle(job)');
  });
});
