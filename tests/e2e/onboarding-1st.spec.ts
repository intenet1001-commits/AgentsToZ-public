...
async function main() {
  ...
  try {
    ...
    await assert(await page.getByText('선택 기능 · 개인 웹 포털과 고급 설정', { exact: true }).isVisible(), '개인 웹 포털은 닫힌 선택 기능으로 표시');
    ...
    await assert(
      await page.getByText('기존 PC 앱 또는 개인 포털의 연결 정보 붙여넣기', { exact: true }).isVisible(),
      '기존 앱 우선 연결 카드 타이틀 노출'
    );
    await assert(
