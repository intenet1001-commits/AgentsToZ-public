...
describe("journal entry construction", () => {
  ...
  test("an accidental huge narrative is bounded before it reaches decades of storage", () => {
    const entry = buildProjectMemoryJournalEntry({
      recordedAt: "2026-08-09T10:00:00.000Z",
      narrative: "장기기억".repeat(10_000),
    });
    expect(Buffer.byteLength(entry.body, "utf8")).toBeLessThanOrEqual(8 * 1024);
...
describe("journal file is append-only", () => {
  ...
  test("a retry starts a valid frame after an interrupted non-newline tail", () => {
    const isolated = mkdtempSync(join(tmpdir(), "agentstoz-journal-interrupted-"));
    try {
      mkdirSync(join(isolated, ".agent-memory/journal"), { recursive: true });
      writeFileSync(
        join(isolated, ".agent-memory/journal/2026-08.md"),
        "# 세션 일지 2026-08\n\n<!-- entry-v2:interrupted",
      );
      const valid = entry("재시도에서 보존된 기록");
      appendProjectMemoryJournal(isolated, valid);
      expect(readProjectMemoryJournal(isolated).map(item => item.entryHash)).toEqual([valid.entryHash]);
    } finally {
      rmSync(isolated, { recursive: true, force: true });
    }
  });
...
describe("remember writes a journal entry", () => {
  ...
  test("a file fsync failure leaves the baseline retryable and a duplicate retry re-fsyncs", () => {
    ...
    try {
      expect(() => markProjectMemoryRemembered({ folderPath: root, narrative: "fsync 뒤에만 완료" }))
        .toThrow(/injected fsync failure/);
    } finally {
      __setProjectMemoryDurabilityFaultForTests(null);
    }
    const afterFailure = readProjectMemoryDeviceState(root);
    expect(afterFailure.lastRememberedAt).toBe(before.lastRememberedAt);
    expect(afterFailure.lastRememberedActivityFingerprint).toBe(before.lastRememberedActivityFingerprint);

    // The bytes may already be visible after a failed fsync. The retry must
    // recognize the duplicate, fsync the existing month, and only then advance.
    const retry = markProjectMemoryRemembered({ folderPath: root, narrative: "fsync 뒤에만 완료" });
    expect(retry.markedRemembered).toBe(true);
    expect(readProjectMemoryJournal(root).filter(entry => entry.summary === "fsync 뒤에만 완료")).toHaveLength(1);
    expect(readProjectMemoryDeviceState(root).lastRememberedAt).not.toBe(before.lastRememberedAt);
  });
