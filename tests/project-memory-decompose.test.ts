...
import { mkdtempSync, mkdirSync, rmSync, existsSync, readFileSync, writeFileSync, unlinkSync, symlinkSync, statSync, utimesSync } from "node:fs";
...
test("regenerating unchanged notes and index preserves file timestamps", () => {
  const { root, memoryPath } = project();
  const doc = bigDocument();
  writeMemoryDocument(root, memoryPath, doc);
  const paths = [memoryPath, ...["manifest.json", "01-key-decisions.md", "02-recurring-issues.md"]
    .map(file => join(root, ".agent-memory/notes", file))];
  const oldTime = new Date("2020-01-01T00:00:00Z");
  for (const path of paths) utimesSync(path, oldTime, oldTime);
  writeMemoryDocument(root, memoryPath, doc);
  expect(readMemoryDocument(root, memoryPath)).toBe(doc);
  for (const path of paths) expect(statSync(path).mtimeMs).toBe(oldTime.getTime());
  expect(existsSync(join(root, ".agent-memory/backups/.pending-document-write.json"))).toBe(false);
});
