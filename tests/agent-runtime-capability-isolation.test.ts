...
describe('Agent Runtime bundled-sidecar capability isolation', () => {
  test('unreadable Git metadata leaves other projects and the API responsive',async()=>{
    if(process.platform==='win32')return;
    const home=mkdtempSync(join(tmpdir(),'agentstoz-unreadable-head-'));roots.push(home);
    const good=join(home,'local-project'),blocked=join(home,'unavailable-project');
    mkdirSync(good);runGit(good,['init','-q']);
    mkdirSync(join(blocked,'.git'),{recursive:true});
    // A FIFO is reproducibly unreadable without requesting an actual cloud
    // download. Discovery must reject it before opening or invoking Git.
    expect(Bun.spawnSync(['mkfifo',join(blocked,'.git','HEAD')]).exitCode).toBe(0);
    const env=bundledEnvironment(home);mkdirSync(env.APP_DATA_DIR!,{recursive:true});
    writeFileSync(join(env.APP_DATA_DIR!,'ports.json'),JSON.stringify([
      {id:'project_local_12345678',name:'Local project',folderPath:good},
      {id:'project_unavailable_12345678',name:'Unavailable project',folderPath:blocked},
    ]));
    ...
    const [inventory,ports]=await Promise.all([
      runtimeRequest({baseUrl,path:'/api/agent-runtime/targets'}),
      runtimeRequest({baseUrl,path:'/api/ports'}),
    ]);
    expect(ports.response.status).toBe(200);expect(inventory.response.status).toBe(200);
    expect(inventory.body.complete).toBe(false);
    const targets=inventory.body.targets as Array<Record<string,unknown>>;
    expect(targets.find(t=>t.targetId==='project_local_12345678')).toBeDefined();
    expect(targets.find(t=>t.targetId==='project_unavailable_12345678')?.worktreeCapable).toBe(false);
  },15_000);
