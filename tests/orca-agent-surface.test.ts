...
import { remoteOrcaAgentLaunchSurface } from '../src/orcaWorktreeSupport';
...
describe('Orca localhost 탭 표면', () => {
  test('원격 Hermes는 repo 등록과 무관한 보이는 Floating 표면을 사용한다', () => {
    expect(remoteOrcaAgentLaunchSurface('hermes')).toBe('floating');
    expect(remoteOrcaAgentLaunchSurface('claude')).toBe('worktree');
    expect(remoteOrcaAgentLaunchSurface('codex')).toBe('worktree');
    expect(remoteOrcaAgentLaunchSurface('agy')).toBe('worktree');
  });
