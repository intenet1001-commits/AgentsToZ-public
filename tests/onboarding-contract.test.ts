...
test('first-run and agent guidance preserve the authenticated Supabase contract', () => {
  ...
  expect(completion).toContain('signInNativeSupabase(supabase');
  ...
  expect(completion.indexOf("localStorage.setItem(SETUP_WIZARD_SEEN_KEY, '1')"))
    .toBeGreaterThan(completion.indexOf('signInNativeSupabase(supabase'));
  ...
  expect((setup.match(/const \[allowedEmail, setAllowedEmail\]/g) ?? [])).toHaveLength(3);
...
test('every local setup surface requires the same-origin OAuth session before sync', () => {
  const setup = source('src/SetupWizard.tsx');
  ...
  for (const guide of [setup, portal, readme, security]) {
    expect(guide).not.toContain('앱 안에는 로그인 화면이 없다');
    expect(guide).not.toContain('앱에는 별도 로그인 단계가 없다');
    expect(guide).not.toMatch(/앱에는 별도 로그인 단계가 없/);
    expect(guide).not.toMatch(/앱에는 로그인 화면이 없으므로/);
    expect(guide).not.toContain('저장한 URL + anon key로 Push/Pull이 바로 동작한다');
  }
  expect((setup.match(/const \[authRequired, setAuthRequired\]/g) ?? [])).toHaveLength(2);
  expect(setup).toContain('parseOnboardingHandoff(raw)');
  expect(setup).toContain('useState(false); // paste된 device_id는 명시적으로 선택할 때만 이어받는다');
