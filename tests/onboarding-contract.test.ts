...
test('first-run keeps web authentication while desktop completion has no user login', () => {
  ...
  expect(completion).toContain('if (isDeployedWeb())');
...
test('web setup keeps same-origin OAuth while the desktop app uses its local service proxy', () => {
  ...
  expect(client).toContain("export const LOCAL_WEB_PROXY_PATH = '/api/supabase-proxy'");
  expect(client).toContain("const proxy = runtime !== 'deployed-web'");
  expect(client).toContain('proxy ? LOCAL_PROXY_OPTIONS : AUTH_OPTIONS');
  expect(api).toContain("const DESKTOP_SUPABASE_PROXY_PREFIX = '/api/supabase-proxy'");
  expect(api).toContain('isLocalSupabaseProxyOrigin(localWebRequestOrigin(req.headers, requestOrigin))');
  ...
  expect(portal).toContain('requiresUserSession && !isDeployedWeb()');
  expect(portal).toContain('로컬 앱은 sidecar 연결을 사용하므로 Google 로그인이 필요하지 않습니다.');
...
test('server membership is the only hosted portal authority and has no build-time mirror', () => {
  const envExample = source('.env.example');
  const portal = source('src/portal-main.tsx');
  const membership = source('src/portalMembership.ts');
  const guide = source('docs/user-guide/GUIDE.md');
  expect(envExample).toContain('portmgr_allowed_members');
  expect(guide).toContain('단일 정본');
  expect(membership).toContain("rpc('portmgr_is_member')");
  expect(portal).toContain('verifyPortalMembership');
  for (const text of [envExample, portal, guide]) expect(text).not.toContain('VITE_ALLOWED_EMAIL');
  expect(portal).not.toContain("localStorage.getItem('portal_google_verified')");
});
