...
describe("AgentsToZ project worker dispatch plan", () => {
  test("binds hermes chat to the resolved project and memory", () => {
    ...
    expect(planAgentsToZProjectWorker({
      ...
      task: "로그인 버그를 조사해줘",
      hermesCli: "/opt/hermes/bin/hermes",
    })).toMatchObject({ok:false,code:"WORKER_PROFILE_REQUIRED"});
    expect(planAgentsToZProjectWorker({requestId:"r",project:"p",memoryId:"m",canonicalPath:"/p",task:"work",workerProfile:"cs-ceo"}))
      .toMatchObject({ok:false,code:"WORKER_PROFILE_REQUIRED"});
  });
