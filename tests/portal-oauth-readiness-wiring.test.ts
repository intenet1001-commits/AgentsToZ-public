...
describe('recoverable hosted Google login', () => {
  test('main portal proves Auth readiness and validates PKCE before leaving the page', () => {
    ...
    expect(login).toContain('canContinuePortalGoogleOAuth(recovery?.error)');
    expect(login).toContain('loginInFlight.current');
    expect(login).toContain('await signInWithNativePortalOAuth');
  });
