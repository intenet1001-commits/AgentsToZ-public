...
describe('public guide deploy artifact boundary', () => {
  ...
  // Regression: Windows checkouts can copy CRLF into the Linux installer script.
  // Found by /qa on 2026-08-29.
  test('dry-run normalizes the remote installer script to LF', async () => {
    const root = makeRoot();
    writeFileSync(join(root, 'vercel.guide.json'), `${JSON.stringify(staticConfig)}\n`);
    ...
    expect(readFileSync(join(root, 'dist-guide', 'agentstoz-remote-device.sh'), 'utf8')).toBe(
      '#!/usr/bin/env bash\nAGENT_VERSION="4"\n',
    );
  });
});
