...
createRoot(document.getElementById('root')).render(React.createElement(InternetQrRemoteControlDialog,{open:true,onClose(){},onOpenWorkroom(){fixture.openedWorkroom=true;},api}));
</script></html>`;
...
try{
 for(const width of [375,1024])for(const scope of ['target','root','all','all-incomplete']){
  const context=await browser.newContext({viewport:{width,height:900},serviceWorkers:'block'}),requests=[],unexpected=[],errors=[];let failGrant=scope==='root',grantAllowed=false;
  await context.route('**/*',async route=>{
   ...
   if(url.pathname==='/api/agent-runtime/terminals/access'){
    const body=route.request().postDataJSON();if(body.owner){requests.push(body);if(failGrant)return route.fulfill({status:500,json:{error:'fixture grant disk failure'}});grantAllowed=true;}
    return route.fulfill({json:{connections:grantAllowed?[{id:'internet:session-fixture',allowed:true,durable:true,expiresAt:new Date(Date.now()+29*86400000).toISOString()}]:[],workspaceRoots:[{workspaceRootId:'root-fixture',name:'Fixture root'}]}});
   }
  ...
  const workroomStatus=page.getByTestId('approved-internet-workroom-status');
  await workroomStatus.getByText(/허용됨.*앱 업데이트 후에도 유지/).waitFor();
  assert.equal(await page.getByTestId('update-internet-remote-task-scope').count(),0,'unavailable managed tasks do not look like a pending Workroom setup');
  assert.equal(await page.getByText('안전 격리 준비 중',{exact:true}).count(),0);
  await page.getByText('지속형 Codex 대화·기록 (읽기 전용)',{exact:true}).waitFor();
  await workroomStatus.getByRole('button',{name:'워크룸 열기 · 기기 권한 관리',exact:true}).click();
  assert.equal(await page.evaluate(()=>fixture.openedWorkroom),true);
