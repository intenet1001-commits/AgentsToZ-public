...
test('CI JS job은 listener 상태 통합 테스트에 필요한 lsof를 준비한다', () => {
  const workflow = readWorkflows().find((item) => item.file === 'verify.yml')?.body ?? '';
  expect(workflow).toContain('apt-get install -y lsof');
  expect(workflow.indexOf('apt-get install -y lsof')).toBeLessThan(workflow.lastIndexOf('bun test'));
});
