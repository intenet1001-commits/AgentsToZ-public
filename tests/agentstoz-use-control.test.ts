...
async function postControlCenter(baseUrl: string, workspaceRootId: string) {
...
`);
  ...
  const fakeAgentDirectory = join(home, ".local", "bin");
  mkdirSync(fakeAgentDirectory, { recursive: true });
  for (const agent of ["codex", "claude", "hermes", "agy"]) {
    const path = join(fakeAgentDirectory, agent);
...
`);
  ...
  const apiEnv = {
    ...
    PORTMGR_ORCHESTRATION_MISSION_TEST_KEY: "42".repeat(32),
  };
  ...
  const { baseUrl, child } = await startTestApiServer({
    ...
    env: apiEnv,
    entrypoint: 'tests/fixtures/agentstoz-use-api.ts',
  });
  children.push(child);
  return { baseUrl, home, controllerMemory, targetMemory, manualPath, channelId, apiEnv };
}
...
describe("AgentsToZ USE action contract", () => {
  test("accepts only the bounded ID-based action schema", () => {
    ...
    for (const agent of ["codex", "claude", "hermes", "agy"] as const) {
      expect(parseAgentsToZUseActionRequest({
        ...
        portId: "study-id",
        agent,
        requestId: `request_${agent}_start`,
      }).agent).toBe(agent);
    }
  });
  ...
  test("teaches the agent to fail closed and never send paths, commands, or credentials", () => {
    ...
    expect(prompt).toContain("agentstoz_use_list_workroom_sessions");
    expect(prompt).toContain("agentstoz_use_read_workroom_session");
    expect(prompt).toContain("agentstoz_use_start_workroom_session");
    expect(prompt).toContain("agentstoz_use_send_workroom_instruction");
...
describe("AgentsToZ USE local action API", () => {
  test("lists and inspects registered projects without returning local paths", async () => {
    ...
    const sessions = await post(baseUrl, {
    ...
    expect(typeof sessions.body.complete).toBe("boolean");
    expect(JSON.stringify(sessions.body)).not.toContain("folderPath");
  }, 30_000);
  ...
  test("lists path-free workspace choices and creates a registered project with app defaults", async () => {
    ...
    const registered = await fetch(`${baseUrl}/api/ports`).then(r => r.json()) as any[];
    expect(registered.find(row => row.id === created.body.project.projectId)?.role).toBe('managed');
    expect(created.body.project).not.toHaveProperty('role'); // MCP DTO remains unchanged.
    expect(JSON.stringify(created.body)).not.toContain("/tmp/");
    const createdPath = join(home, "projects", "새-프로젝트");
  ...
  test("creates a portable control center with one stable memory identity", async () => {
    const { baseUrl, home } = await fixture();
    const created = await post(baseUrl, {
    ...
    const root = join(home, "projects", "AgentsToZ-Control");
    const config = JSON.parse(readFileSync(join(root, ".agent-memory", "config.json"), "utf8"));
    const registered = await fetch(`${baseUrl}/api/ports`).then(r => r.json()) as any[];
    expect(registered.find(row => row.id === created.body.project.projectId)?.role).toBe('ops');
    expect(created.body.project).not.toHaveProperty('role');
    expect(config.memoryId).toBe(created.body.project.memoryId);
    expect(readFileSync(join(root, "README.md"), "utf8")).toContain("원격 기억을 먼저 Pull");
    expect(readFileSync(join(root, "CONTROL.md"), "utf8")).toContain("AgentsToZ 미션·워크룸 런타임");
    expect(Bun.spawnSync(["git", "status", "--porcelain=v1", "--untracked-files=all"], {
      cwd: root, stdout: "pipe", stderr: "pipe",
    }).stdout.toString().trim()).toBe("");
  }, 30_000);

  test("lets first-run onboarding create the same bounded control center without a controller project", async () => {
    const { baseUrl, home } = await fixture();
    const created = await postControlCenter(baseUrl, "main-root");
    ...
    const root = join(home, "projects", "AgentsToZ-Control");
    expect(readFileSync(join(root, "CONTROL.md"), "utf8")).toContain("## 기억 경계");

    const repeated = await postControlCenter(baseUrl, "main-root");
    ...
    expect(repeated.body.project.projectId).toBe(created.body.project.projectId);
  }, 30_000);

  test("maps the dedicated MCP tool without accepting paths", () => {
    expect(AGENTSTOZ_USE_MCP_TOOLS.some(tool => tool.name === "agentstoz_use_create_control_center")).toBe(true);
    expect(agentsToZUseMcpActionForTool("agentstoz_use_create_control_center", {
      workspaceRootId: "main-root",
      folderPath: "/tmp/ignored",
  ...
  test("starts and instructs all four agents through the same bounded Workroom contract", async () => {
    const { baseUrl, home } = await fixture();
    for (const agent of ["codex", "claude", "hermes", "agy"] as const) {
      const started = await post(baseUrl, {
        action: "start-workroom-session", controllerPortId: "controller-id", portId: "study-id",
        agent, requestId: `request_${agent}_start`,
      });
      expect(started.response.status).toBe(201);
      expect(started.body).toMatchObject({
        success: true, performed: true, effect: "started-workroom-session",
        projectId: "study-id", session: { agent, state: "running" },
      });
      const sessionId = started.body.session.id;
      const sent = await post(baseUrl, {
        action: "send-workroom-instruction", controllerPortId: "controller-id", portId: "study-id",
        sessionId, instruction: "상태를 확인해 주세요.", requestId: `request_${agent}_input`,
      });
      expect(sent.response.status).toBe(200);
      expect(sent.body).toMatchObject({ success: true, performed: true, session: { id: sessionId, agent } });
    }
    const opens = readFileSync(join(home, 'dashboard-opens.jsonl'), 'utf8').trim().split('\n').map(line => JSON.parse(line));
    expect(opens).toEqual(Array.from({ length: 4 }, () => ['/usr/bin/open', '/Applications/AgentsToZ_byCS.app']));
  }, 30_000);

  test("requires an explicit mission resume after an API runtime restart before starting Workroom", async () => {
    const first = await fixture();
    const created = await post(first.baseUrl, {
      action: "create-mission", controllerPortId: "controller-id",
      title: "원격 계속하기", goal: "재시작 뒤 명시적으로 이어가기", requestId: "mission_restart_create",
    });
    expect(created.response.status).toBe(201);
    ...
    const restarted = await startTestApiServer({ cwd: join(import.meta.dir, ".."), env: first.apiEnv, entrypoint: 'tests/fixtures/agentstoz-use-api.ts' });
    children.push(restarted.child);

    const interrupted = await post(restarted.baseUrl, {
      action: "read-mission", controllerPortId: "controller-id", missionId,
    });
    expect(interrupted.response.status).toBe(200);
    expect(interrupted.body.mission.state).toBe("interrupted");
    expect(interrupted.body.events.at(-1)).toMatchObject({ kind: "mission-interrupted" });

    const blocked = await post(restarted.baseUrl, {
      action: "start-workroom-session", controllerPortId: "controller-id", portId: "study-id",
      agent: "agy", missionId, requestId: "mission_restart_blocked",
    });
    expect(blocked.response.status).toBe(409);
    expect(blocked.body).toMatchObject({
      success: false, performed: false, code: "AGENTSTOZ_USE_MISSION_RESUME_REQUIRED",
    });
    const beforeResume = await post(restarted.baseUrl, {
      action: "list-workroom-sessions", controllerPortId: "controller-id", portId: "study-id",
    });
    expect(beforeResume.body.sessionCount).toBe(0);

    const resumed = await post(restarted.baseUrl, {
      action: "transition-mission", controllerPortId: "controller-id", missionId,
      transition: "resume", requestId: "mission_restart_resume",
    });
    expect(resumed.response.status).toBe(200);
    expect(resumed.body.mission.state).toBe("active");
    const continued = await post(restarted.baseUrl, {
      action: "start-workroom-session", controllerPortId: "controller-id", portId: "study-id",
      agent: "agy", missionId, requestId: "mission_restart_continue",
    });
    expect(continued.response.status).toBe(201);
    expect(continued.body).toMatchObject({
      success: true, performed: true, session: { agent: "agy" }, missionRecording: { recorded: true },
    });
  }, 60_000);
...
describe("AgentsToZ USE Codex MCP bridge", () => {
  test("advertises the spoken invocation contract during MCP initialization", async () => {
    const response = await handleAgentsToZUseMcpRequest({
      jsonrpc: "2.0", id: 1, method: "initialize", params: {},
    });
    expect(response?.result).toMatchObject({
        serverInfo: { name: "agentstoz-use", version: "1.18.0" },
      instructions: AGENTSTOZ_USE_MCP_INSTRUCTIONS,
    });
    expect(AGENTSTOZ_USE_MCP_INSTRUCTIONS).toContain("아젠투지");
    expect(AGENTSTOZ_USE_MCP_INSTRUCTIONS).toContain("agentstoz_use_list_workspace_roots");
    expect(AGENTSTOZ_USE_MCP_INSTRUCTIONS).toContain("agentstoz_use_create_control_center");
  });

  test("exposes only the bounded tools and maps IDs server-side", async () => {
    expect(AGENTSTOZ_USE_MCP_TOOLS.map(tool => tool.name)).toEqual([
      "agentstoz_use_get_tester",
      "agentstoz_use_plan_tester_setup",
      "agentstoz_use_apply_tester_setup",
      "agentstoz_use_start_tester",
      "agentstoz_use_read_tester_run",
      "agentstoz_use_cancel_tester_run",
      "agentstoz_use_prepare_tester_handoff",
      "agentstoz_use_list_tester_results",
      "agentstoz_use_get_control_profile",
      "agentstoz_use_recall_control_context",
      "agentstoz_use_list_control_memory_candidates",
      "agentstoz_use_propose_control_memory",
      "agentstoz_use_list_projects",
      "agentstoz_use_list_workspace_roots",
      "agentstoz_use_create_project",
      "agentstoz_use_create_control_center",
      ...
      "agentstoz_use_start_workroom_session",
      "agentstoz_use_send_workroom_instruction",
      "agentstoz_use_list_missions",
      "agentstoz_use_create_mission",
      "agentstoz_use_read_mission",
      "agentstoz_use_transition_mission",
      "agentstoz_use_link_mission_utterance",
      "agentstoz_use_record_mission_project_result",
    ...
    expect(agentsToZUseMcpActionForTool("agentstoz_use_list_workroom_sessions", {
      portId: "study-id",
      folderPath: "/tmp/ignored",
    }, "controller-id")).toEqual({
      action: "list-workroom-sessions",
    ...
    expect(agentsToZUseMcpActionForTool("agentstoz_use_read_workroom_session", {
      portId: "study-id",
      sessionId: "session_12345678",
      after: 12,
      command: "whoami",
    ...
    expect(agentsToZUseMcpActionForTool("agentstoz_use_start_workroom_session", {
      portId: "study-id", agent: "codex", requestId: "request_12345678", missionId: "mission_12345678",
    }, "controller-id")).toEqual({
      action: "start-workroom-session", controllerPortId: "controller-id", portId: "study-id",
      agent: "codex", requestId: "request_12345678", missionId: "mission_12345678",
    });
    for (const agent of ["hermes", "agy"]) {
      expect(agentsToZUseMcpActionForTool("agentstoz_use_start_workroom_session", {
        portId: "study-id", agent, requestId: `request_${agent}_start`,
      }, "controller-id")).toMatchObject({ action: "start-workroom-session", agent });
    }
    expect((AGENTSTOZ_USE_MCP_TOOLS.find(tool => tool.name === "agentstoz_use_start_workroom_session")!
      .inputSchema.properties as any).agent.enum).toEqual(["codex", "claude", "hermes", "agy"]);
    expect(agentsToZUseMcpActionForTool("agentstoz_use_read_mission", {
      missionId: "mission_12345678", afterEventId: "event_00000000-0000-4000-8000-000000000000", eventLimit: 25,
    }, "controller-id")).toEqual({
      action: "read-mission", controllerPortId: "controller-id", missionId: "mission_12345678",
      afterEventId: "event_00000000-0000-4000-8000-000000000000", eventLimit: 25,
    });
    expect(agentsToZUseMcpActionForTool("agentstoz_use_send_workroom_instruction", {
      portId: "study-id", sessionId: "session_12345678", instruction: "계속 진행해 주세요.", requestId: "request_abcdefgh", missionId: "mission_12345678",
    }, "controller-id")).toEqual({
      action: "send-workroom-instruction", controllerPortId: "controller-id", portId: "study-id",
      sessionId: "session_12345678", instruction: "계속 진행해 주세요.", requestId: "request_abcdefgh", missionId: "mission_12345678",
    });
    ...
    expect((listed as any).result.tools).toHaveLength(32);
  });
