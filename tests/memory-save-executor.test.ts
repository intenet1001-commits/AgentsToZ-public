...
test('completed JSONL passes through encrypted prompt and one proposal into document, journal and receipt',async()=>{
 ...
 expect(f.originalInput()).toContain('Prefer verified project evidence.');
 expect(f.originalInput()).toContain('Confirmed.');
 expect(f.originalInput()).not.toContain('"jsonl"');
...
test('provider failure is permanently fenced and is never retried automatically',async()=>{
 ...
 const job=f.c.saves.openJob(f.memoryId)!,diagnostic=f.c.saves.failure(job.saveId);
 expect(diagnostic).toMatchObject({version:1,stage:'provider-call',code:'UNKNOWN',providerCallPossible:true});
 await expect(executeAutomaticMemorySave(f.c)).rejects.toThrow();expect(calls).toBe(1);
 expect(f.c.saves.failure(job.saveId)).toEqual(diagnostic);
...
test('post-reservation readiness failures record no possible provider call while preserving coverage',async()=>{
 const f=await fixture();let readiness=0;f.c.provider.ready=async()=>++readiness<3;
 await expect(executeAutomaticMemorySave(f.c)).rejects.toThrow('POLICY_CHANGED');
 const job=f.c.saves.openJob(f.memoryId)!;expect(job.phase).toBe('recovery-required');expect(f.calls()).toBe(0);
 expect(f.c.saves.failure(job.saveId)).toMatchObject({stage:'provider-readiness',code:'POLICY_CHANGED',providerCallPossible:false});
});

test('a reserved job that fails before attempt admission keeps prepared coverage with a pre-call diagnostic',async()=>{
 const f=await fixture(),error=new Error('fixture input binding refused');
 f.c.saves.bindInput=()=>{throw error;};
 await expect(executeAutomaticMemorySave(f.c)).rejects.toBe(error);
 const job=f.c.saves.openJob(f.memoryId)!;expect(job.phase).toBe('prepared');expect(job.attemptId).toBeNull();expect(f.calls()).toBe(0);
 expect(f.c.saves.failure(job.saveId)).toMatchObject({stage:'input-binding',code:'UNKNOWN',providerCallPossible:false});
 expect(f.c.saves.pending(f.memoryId,1).items).toHaveLength(0);
});

test('diagnostic storage failure cannot replace the original provider failure or remove its fence',async()=>{
 const f=await fixture(),original=new Error('fixture original provider error');f.c.provider.propose=async()=>{throw original;};
 const db=new Database(f.c.saves.path);
 db.run("CREATE TRIGGER reject_failure_diagnostic BEFORE INSERT ON save_failure_diagnostics BEGIN SELECT RAISE(ABORT,'fixture diagnostic failure'); END");db.close();
 await expect(executeAutomaticMemorySave(f.c)).rejects.toBe(original);
 const job=f.c.saves.openJob(f.memoryId)!;expect(job.phase).toBe('recovery-required');expect(f.c.saves.failure(job.saveId)).toBeNull();
 expect(f.c.saves.pending(f.memoryId,1).items).toHaveLength(0);
});
...
test('unsafe model output, policy revocation and concurrent document edits never receive a success receipt',async()=>{
 for(const kind of ['secret','policy','edit']){const f=await fixture();const propose=f.c.provider.propose;
  ...
  const diagnostic=f.c.saves.failure(f.c.saves.openJob(f.memoryId)!.saveId);
  expect(diagnostic).toMatchObject({stage:'proposal-validation',providerCallPossible:true});
  expect(JSON.stringify(diagnostic)).not.toContain('ghp_');
