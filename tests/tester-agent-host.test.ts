...
async function finish(host:TesterAgentHost,id:string){for(let i=0;i<100;i++){const r=(await host.perform(req('read',{runId:id}))).run!;if(!['queued','starting','running','canceling'].includes(r.state))return r;await Bun.sleep(50);}throw Error('Test run did not finish');}
