...
describe("targeted project-memory updates", () => {
  ...
  test("large-memory path sends one section and Claude receives the prompt on stdin", () => {
    ...
    const updater = source.slice(source.indexOf("export function prepareProjectMemoryUpdate"), source.indexOf("\nfunction loadPortalConfig"));

    expect(runner).toContain('[resolveAgentBin("claude"), "--safe-mode", "-p", "--no-session-persistence", "--tools", "", "--strict-mcp-config"]');
    expect(runner).toContain("300_000,\n      prompt,");
    expect(runner).not.toContain('.codex-memory-');
    expect(runner).not.toContain('"-o"');
    expect(runner).toContain('return result.stdout;');
