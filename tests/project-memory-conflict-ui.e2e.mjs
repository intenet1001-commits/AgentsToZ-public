...
try {
  ...
  await page.route('**/api/agent-runtime/targets', route=>route.fulfill({json:{protocolVersion:'agentstoz-tasks-v2',complete:true,targets:[{targetId:'conflict-project',projectTargetId:'conflict-project',label:'충돌 검증 프로젝트',scope:'main',branch:'main',locked:false,worktreeCapable:true}]}}));
  await page.route('**/api/agent-runtime/terminals', route=>{
    const body=route.request().postDataJSON();
    if(body.operation==='start') {terminalRequest=body;terminalSession={id:'conflict-terminal-session',targetId:body.targetId,agent:body.agent,state:'running',createdAt:new Date().toISOString(),exitCode:null,cols:100,rows:28};}
    return route.fulfill({json:body.operation==='list'?{sessions:terminalSession?[terminalSession]:[]}:{session:terminalSession}});
  });
  await page.goto(APP_URL, { waitUntil: 'domcontentloaded', timeout: 30_000 });
  await page.locator('.workspace-sidebar-footer > .workspace-tools > summary').click();
  ...
  await sessionEnd.click();
  await resolver.waitFor({state:'visible'});
  const launcher=page.getByTestId('memory-conflict-terminal-launcher');
  await launcher.getByRole('combobox').selectOption('claude');
  await launcher.getByRole('button').click();
  await page.getByTestId('ai-terminal-panel').waitFor({state:'visible'});
  assert(terminalRequest?.agent==='claude' && terminalRequest?.targetId==='conflict-project','conflict review runs the selected CLI in the registered project');
  assert(terminalRequest?.prompt.includes('preview-revision') && terminalRequest.prompt.includes('원본 파일·Supabase를 덮어쓰거나'),'review carries read-only revision instructions');
  assert(!terminalRequest.prompt.includes('Keep the remote note.'),'full memory bodies never enter CLI arguments');
  assert(sessionEndCalls===2,'opening review does not rerun memory saving');
} finally {
  await browser.close();
}
