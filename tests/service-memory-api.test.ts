...
describe("USE service memory HTTP API", () => {
  test("rejects the retired global CS-CEO setup without touching service memory", async () => {
    const { appData, baseUrl } = await fixture();
    const legacy = await post(baseUrl, "/api/buzz-agent-bootstrap/status", { scope: "global", deviceName: "Test Mac" });
    expect(legacy.response.status).toBe(410);
    expect(legacy.body.code).toBe("LEGACY_CS_CEO_SETUP_REMOVED");
