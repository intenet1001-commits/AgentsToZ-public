...
describe('automatic port upload content', () => {
  test('local role metadata does not expand the cloud row or schedule a schema write', () => {
    const rolePort = {...port, role:'ops'};
    expect(buildPortUploadRow(rolePort, {deviceId:'mac', deviceName:'Mac'})).not.toHaveProperty('role');
    expect(portAutoUploadChangeKey([rolePort], {})).toBe(portAutoUploadChangeKey([port], {}));
  });
