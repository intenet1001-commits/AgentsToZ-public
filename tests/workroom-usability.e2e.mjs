...
const A='fixture-project-alpha',B='fixture-worktree-bravo',SA='fixture-session-alpha',SE='fixture-session-ended';
...
const attention=['failed','pending','saving','backup-pending','unavailable','failed','pending','unavailable'].map((state,i)=>({sessionId:`fixture-attention-${i}`,targetId:i%2?B:A,state}));
...
window.__workroomActions=[];const {AiTerminalPanel}=await import('/src/AiTerminalPanel.tsx');
function Harness(){const [visible,setVisible]=React.useState(true);window.__setWorkroomVisible=setVisible;
 const workroom=React.createElement(AiTerminalPanel,{projects:${JSON.stringify(projects)},visible,remote:${!!options.remote},entry:${JSON.stringify(options.entry??null)},sessionScope:'workroom-usability-fixture',onManageProject:id=>window.__workroomActions.push({action:'manage',id}),onWhatISaid:()=>window.__workroomActions.push({action:'what-i-said'})});
...
async function fixture(options={}){
  ...
  await context.route('**/*',async route=>{
    ...
    if(url.pathname==='/api/agent-runtime/terminals'){
      ...
      if(body.operation==='start'){
        ...
        const created=session(`fixture-started-${state.sessions.length}`,body.targetId,'running',body.agent);state.sessions.push(created);return respond({session:created});
      }
  ...
  await until(()=>state.requests.some(r=>r.operation==='list')&&(options.remote||state.requests.some(r=>r.path.endsWith('/memory'))),'Initial fixture reads must occur');
...
async function run(name,execute,options){let f;try{f=await fixture(options);const details=await execute(f);
...
try{
  ...
  await run('unselected-and-ended-sessions-remain-compact-and-read-only',async({panel,state})=>{
    ...
    assert.ok(await panel.getByText('종료된 세션의 기록입니다.',{exact:true}).isVisible());
  ...
  await run('attention-is-visible-and-history-is-progressively-revealed',async({page,panel})=>{
    ...
    await until(async()=>await urgent.getByRole('listitem').count()===5,'Attention must begin with five rows');
    ...
    assert.match(await history.locator('summary').innerText(),/완료된 저장 기록 45건/);assert.equal(await history.getByRole('listitem').count(),0);
    await urgent.getByRole('button',{name:/확인할 저장 더 보기/}).click();assert.equal(await urgent.getByRole('listitem').count(),8);
    for(const label of ['종료 후 저장 대기','세션 기억 저장 중','장기기억 초기화 필요','백업 확인 필요'])assert.ok((await urgent.innerText()).includes(label));
    await history.locator('summary').click();await until(async()=>await history.getByRole('listitem').count()===20,'Opening history should render only its first 20 rows');
    ...
    await history.getByRole('button',{name:/저장 기록 더 보기/}).click();assert.equal(await history.getByRole('listitem').count(),40);
    await history.getByRole('button',{name:/저장 기록 더 보기/}).click();assert.equal(await history.getByRole('listitem').count(),45);
  ...
  await run('bounded-app-shell-keeps-keys-and-memory-separate-and-reachable',async({page,panel,state})=>{
    ...
    for(const [width,height] of [[1440,1050],[1000,1050],[800,1050],[1000,700]]){
      ...
      await until(async()=>await history.getByRole('listitem').count()===20,'Memory history must remain clickable below the keys');
...
}finally{await browser.close();await writeFile(new URL('workroom-usability-results.json',output),JSON.stringify({generatedAt:new Date().toISOString(),results},null,2)+'\n');}
