...
import { supabaseCliCandidates } from '../project-memory-server';
...
describe('Windows release workflow', () => {
  ...
  test('runs only after an explicit manual request with a reason', () => {
  ...
  test('blocks Windows packages from dirty or unpublished source before versioning', () => {
    expect(wrapper).toContain('verifyReleaseSource');
    expect(wrapper).toContain('AGENTSTOZ_RELEASE_SOURCE_SHA');
    expect(wrapper).toContain('--allow-unpublished-source');
    expect(wrapper.indexOf('const releaseSource = verifyReleaseSource')).toBeLessThan(wrapper.indexOf('await $`bun update-version.ts`'));
  });
  ...
  test('discovers the default Scoop installation under the Windows user profile', () => {
    const user = 'C:/Users/example-user';
    const candidates = supabaseCliCandidates('win32', { USERPROFILE: user }, 'C:/different-home');
    expect(candidates).toContain(join(user, 'scoop', 'apps', 'supabase', 'current', 'supabase.exe'));
    expect(candidates).toContain(join(user, 'scoop', 'shims', 'supabase.exe'));
    expect(candidates).not.toContain(join('C:/different-home', 'scoop', 'shims', 'supabase.exe'));
    const route = apiSource.slice(apiSource.indexOf('if (url.pathname === "/api/supabase-cli/status"'), apiSource.indexOf('if (url.pathname === "/api/supabase-cli/apikeys"'));
    expect(route).toContain('resolveSupabaseCli()');
  });
  ...
  test('installs and validates the packaged artifact before upload', () => {
    ...
    expect(nativeSpec).toContain("apiPost('/api/execute-command'");
    expect(nativeSpec).toContain("apiPost('/api/force-restart-command'");
    expect(nativeSpec).toContain("apiPost('/api/stop-command'");
    expect(nativeSpec).not.toContain("invokeNative('execute_command'");
    expect(nativeSpec).not.toContain("invokeNative('force_restart_command'");
    expect(nativeSpec).not.toContain("invokeNative('stop_command'");
