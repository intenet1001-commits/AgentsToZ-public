...
describe('clone 주소 판정', () => {
  ...
  test('https와 ssh 표기를 모두 받고 같은 값으로 정규화한다', () => {
    const https = parseGitCloneRequest('https://github.com/example-owner/example-agentstoz');
    const ssh = parseGitCloneRequest('git@github.com:example-owner/example-agentstoz.git');
    expect(https).toEqual({ url: 'https://github.com/example-owner/example-agentstoz', repositoryName: 'example-agentstoz' });
...
describe('두 백엔드가 같은 방어를 한다', () => {
  ...
  test('실패하면 부분 생성물을 지우고 최종 폴더를 비워 둔다', () => {
    // API와 보존된 native helper 모두 숨김 staging에 clone한 뒤 성공할 때만
    // publish한다. 어느 쪽도 실패 후 재시도를 막아서는 안 된다.
    expect(apiSource).toContain('rmSync(stagingPath, { recursive: true, force: true })');
    expect(apiSource).toContain('renameSync(stagingPath, target)');
    expect(rustSource).toContain('let _ = fs::remove_dir_all(&staging_path);');
    expect(rustSource).toContain('fs::rename(&staging_path, path)');
  });

  test('앱도 sidecar의 단일 Workspace Lease 경로를 쓴다', () => {
    expect(appSource).not.toContain("invoke<string>('clone_repository'");
    expect(appSource).toContain("fetch(`${baseUrl}/api/clone-repository`");
    const handler = rustSource.slice(
      rustSource.indexOf('.invoke_handler(tauri::generate_handler!['),
      rustSource.indexOf('])', rustSource.indexOf('.invoke_handler(tauri::generate_handler![')),
    );
    expect(handler).not.toContain('clone_repository,');
    expect(apiSource).toContain("withManagedWorkspaceLease(parentPath, 'repository clone parent reservation'");
  });

  test('git 실행 파일을 PATH desert에서도 찾는다', () => {
    expect(rustSource).toContain('let git_bin = resolve_bin("git");\n    let output = Command::new(&git_bin)');
    expect(rustSource).toContain('.arg(&clone_url)');
    expect(rustSource).toContain('.arg(&staging_path)');
  });
});
