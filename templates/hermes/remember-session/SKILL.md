...
1. When automatic backup is enabled, Synchronize safely before editing. Call `POST http://127.0.0.1:3001/api/project-memory/sync` once with JSON `folderPath`.
   - Require HTTP success and `ok: true`. Local-only changes are pushed, remote-only changes are pulled, and matching content is left unchanged. Re-read memory after synchronization; never force a Pull. Stop on other API errors as well.
