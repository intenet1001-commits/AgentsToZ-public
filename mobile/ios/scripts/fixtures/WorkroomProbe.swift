...
@MainActor final class WorkroomProbe: ObservableObject {
    ...
    private func report(_ current: String, done: Bool = false, failed: Bool = false) throws {
        stage = current
        let data = try JSONSerialization.data(withJSONObject: ["stage":current,"checks":checks,"done":done,"failed":failed,"testerRunID":testerRunID])
    ...
    private func checkNativeDialogs() async throws {
        for (kind, accepted) in [("confirm", true), ("confirm", false), ("prompt", true), ("prompt", false), ("alert", true)] {
            try report("present-native-" + kind + (accepted ? "-accept" : "-cancel"))
            let invocation = kind == "prompt" ? "prompt('새 프로젝트 이름', '초기 이름')" : kind + "('프로젝트 생성 확인')"
            _ = try await js("window.__dialogResult='pending'; setTimeout(()=>{window.__dialogResult=" + invocation + ";},0); true")
            for _ in 0..<100 { if session?.isJavaScriptDialogPresented == true { break }; try await pause() }
            guard let dialog = session?.activeJavaScriptDialog else { throw NSError(domain: "FixtureDialogMissing", code: 1) }
            try check(session?.isJavaScriptDialogPresented == true, "native-dialog-visible-" + kind)
            session?.completeJavaScriptDialog(dialog.id, response: accepted ? "한글 프로젝트" : nil)
            // A delayed second button event must not approve a cancelled dialog.
            session?.completeJavaScriptDialog(dialog.id, response: "duplicate")
            let expected = kind == "confirm" ? (accepted ? "true" : "false") : kind == "prompt" ? (accepted ? "'한글 프로젝트'" : "null") : "undefined"
            try await waitJS("window.__dialogResult === " + expected, label: "native-" + kind + (accepted ? "-accepted" : "-cancelled"))
            try check(session?.activeJavaScriptDialog == nil, "native-dialog-completed-once-" + kind)
        }
    }
    func run() async {
        ...
        do {
            ...
            _ = try await js("document.querySelector('[aria-label=\"테스터 프로젝트\"]').selectedIndex=1; document.querySelector('[aria-label=\"테스터 프로젝트\"]').dispatchEvent(new Event('change')); true")
            try await waitJS("Array.from(document.querySelectorAll('#tester-panel button')).some(b=>b.textContent==='테스트 실행' && !b.disabled && !b.hidden)", label: "tester-ready")
            _ = try await js("Array.from(document.querySelectorAll('#tester-panel button')).find(b=>b.textContent==='테스트 실행').click(); true")
            try await waitJS("document.querySelector('#tester-panel strong')?.textContent==='선택한 검사 통과'", label: "tester-passed")
            testerRunID = try await session!.webView!.evaluateJavaScript("document.querySelector('#tester-panel [role=status]').textContent.match(/[0-9]{8}T[0-9]{6}Z-[a-f0-9]{8}/)[0]") as! String
            try check(true, "iphone-python-tester-result-visible")
            try report("tester-completed")
            _ = try await command("tester-checked")
            session?.selectTab("workroom")
            ...
            if let web = session?.webView,
               let diagnostic = try? await web.evaluateJavaScript("JSON.stringify({selected:Boolean(document.querySelector('#terminal-session')?.value),hasError:Boolean(document.querySelector('#terminal-error')?.textContent),ready:Boolean(document.querySelector('.xterm-rows')?.textContent.includes('REMOTE_READY')),rows:Boolean(document.querySelector('.xterm-rows')),targetError:Boolean(document.querySelector('#terminal-error')?.textContent.includes('프로젝트')),dialogType:typeof window.__dialogResult,dialogTrue:window.__dialogResult===true,dialogFalse:window.__dialogResult===false,dialogPending:window.__dialogResult==='pending',timeoutError:Boolean(document.querySelector('#terminal-error')?.textContent.includes('시간')),permissionError:Boolean(document.querySelector('#terminal-error')?.textContent.includes('허용')),inFlight:Boolean(window.inFlight)})") as? String {
