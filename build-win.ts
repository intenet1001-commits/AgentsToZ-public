...
const allowUnpublishedSource = process.argv.includes("--allow-unpublished-source");
...
if (releaseSource.unpublishedOverride) {
  console.warn(`[build-win] ⚠️ 미공개 소스 테스트 빌드: ${releaseSource.headSha}`);
  console.warn("[build-win] --allow-unpublished-source 산출물은 공식 설치·배포본으로 취급하지 마세요.");
} else {
  console.log(
    `[build-win] release source verified: ${releaseSource.remote}/${releaseSource.defaultBranch} @ ${releaseSource.headSha}`,
  );
}
...
if (needsStaging) {
  ...
  const mcpName = "agentstoz-use-mcp.exe";
  const mcpSrc = join(projectRoot, "src-tauri", "resources", mcpName);
  const guardName = "agentstoz-agent-runtime-guard.exe";
  const guardSrc = join(projectRoot, "src-tauri", "resources", guardName);
  ...
  const mcpDst = join(stagingDir, mcpName);
  const guardDst = join(stagingDir, guardName);
  const supervisorDst = join(stagingDir, supervisorName);
  mkdirSync(stagingDir, { recursive: true });
  copyFileSync(sidecarSrc, sidecarDst);
  copyFileSync(mcpSrc, mcpDst);
  copyFileSync(guardSrc, guardDst);
  ...
  const stagedMcp = mcpDst.replace(/\\/g, "/");
  const stagedGuard = guardDst.replace(/\\/g, "/");
  const stagedSupervisor = supervisorDst.replace(/\\/g, "/");
  conf.bundle.resources = {
    [stagedGlob]: sidecarName,
    [stagedMcp]: mcpName,
    [stagedGuard]: guardName,
