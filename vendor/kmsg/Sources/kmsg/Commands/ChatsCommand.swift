...
struct ChatsCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        guard let navigation = mainWindow.findFirst(identifier: "chatrooms"), belongsToWindow(navigation, mainWindow) else {
            throw KakaoTalkError.elementNotFound("Open the KakaoTalk Chats list before discovering rooms.")
        }
