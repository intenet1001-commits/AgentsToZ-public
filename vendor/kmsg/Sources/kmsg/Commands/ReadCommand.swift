...
struct ReadCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        let expectedTitle = chatID.flatMap { ChatIdentityRegistryStore.shared.record(for: $0)?.displayName } ?? chat ?? ""
        guard window.title == expectedTitle, kakao.windows.filter({ $0.title == expectedTitle }).count == 1 else {
            throw KakaoTalkError.elementNotFound("Exact unique target window is required")
        }
