...
struct SendCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        do {
            ...
            let expected = chatID.flatMap { ChatIdentityRegistryStore.shared.record(for: $0)?.displayName } ?? recipient ?? ""
            guard resolution.window.title == expected, kakao.windows.filter({ $0.title == expected }).count == 1 else {
                throw KakaoTalkError.actionFailed("Target chat window is not unique or changed")
            }
    ...
    private func isLikelyMessageInputElement(_ element: UIElement, in window: UIElement? = nil) -> Bool {
        guard let window, belongsToWindow(element, window) else { return false }
        guard isWritableMessageInput(element) else { return false }
    ...
    private func sendMessageToWindow(_ window: UIElement, kakao: KakaoTalkApp, runner: AXActionRunner) throws {
        ...
        guard let input = resolveMessageInputField(chatWindow: window, kakao: kakao, runner: runner) else {
            throw KakaoTalkError.actionFailed("Target chat input is unavailable; no typing fallback")
        }
        guard belongsToWindow(input, window) else { throw KakaoTalkError.actionFailed("Input belongs to another window") }
        ...
        guard belongsToWindow(input, window), let focused = kakao.applicationElement.focusedUIElement,
              CFEqual(focused.axElement, input.axElement), let focusedWindow = kakao.focusedWindow,
              CFEqual(focusedWindow.axElement, window.axElement) else { throw KakaoTalkError.actionFailed("Focus changed before send") }
        let sendSucceeded = runner.pressEnterWithVerification(
            on: input,
            label: "message input",
            attempts: 1,
    ...
    private func collectFocusedElementLineageCandidates(_ focusedElement: UIElement) -> [UIElement] {
        ...
        while let element = cursor, hops < 4 {
            ...
            let textDescendants = element.findAll(where: { node in
                guard isWritableMessageInput(node) else { return false }
    ...
    private func pickMessageInputField(from fields: [UIElement], in window: UIElement) -> UIElement? {
        fields.filter { belongsToWindow($0, window) && isLikelyMessageInputElement($0, in: window) }.sorted { lhs, rhs in
