...
struct KakaoTalkTranscriptReader {
    ...
    func readSnapshot(
        ...
        let messageRows = try collectTranscriptRows(
    ...
    private func collectTranscriptRows(
        ...
        // AXChildren of the one transcript table are in conversation order. In an
        // unfocused window newly appended rows can be below the composer's screen
        // coordinates, even though their text is fully exposed. Geometry is neither
        // a visibility test nor a chronology key: never filter/sort rows by frame.
        let containers = deduplicateElements([transcriptRoot] + transcriptRoot.findAll(
            where: { element in
                element.role == kAXTableRole || element.role == kAXOutlineRole || element.role == kAXListRole
            }, limit: 8, maxNodes: 900
        ))
        let rowLists = containers.map { directRowChildren(from: $0) }.filter { !$0.isEmpty }
        guard rowLists.count == 1, let rows = rowLists.first, rows.count <= 10_000 else {
            throw TranscriptReadError.transcriptContextUnavailable
        }
        let ordered = deduplicateElements(rows)
        let recentRows = Array(ordered.suffix(max(messageLimit * 6, 80)))
        runner.log("read: transcript rows raw=\(rows.count), unique=\(ordered.count), filtered=\(ordered.count), recent=\(recentRows.count)")
        return recentRows
    }
