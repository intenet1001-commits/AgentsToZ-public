...
struct MessageContextResolver {
    ...
    func resolve(in chatWindow: UIElement) -> MessageTranscriptContext? {
        ...
        guard belongsToWindow(inputElement, chatWindow), let transcriptRoot = resolveTranscriptRoot(chatWindow: chatWindow, paneRoot: paneRoot, inputElement: inputElement), belongsToWindow(transcriptRoot, chatWindow) else {
    ...
    private func resolveTranscriptRoot(chatWindow: UIElement, paneRoot: UIElement?, inputElement: UIElement) -> UIElement? {
        ...
        candidates = deduplicateElements(candidates).filter { belongsToWindow($0, chatWindow) }
    ...
    private func preferredChatPaneRoot(for inputElement: UIElement, in chatWindow: UIElement) -> UIElement? {
        ...
        let filtered = ancestors.filter { candidate in
            guard belongsToWindow(candidate, chatWindow) else { return false }
