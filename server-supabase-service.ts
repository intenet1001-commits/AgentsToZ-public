...
const LOOPBACK_SUPABASE_HOSTS = new Set(["localhost", "127.0.0.1", "::1", "[::1]"]);

/** Prevent a service key from crossing plaintext HTTP except to exact loopback. */
export function normalizeServerSupabaseUrl(rawUrl: string): string {
  const parsed = new URL(rawUrl.trim());
  if (parsed.username || parsed.password) throw new Error("Supabase URL에 사용자 정보가 포함될 수 없습니다.");
  if (parsed.protocol !== "https:"
    && !(parsed.protocol === "http:" && LOOPBACK_SUPABASE_HOSTS.has(parsed.hostname))) {
    throw new Error("Supabase URL은 HTTPS 또는 정확한 loopback HTTP 주소여야 합니다.");
  }
