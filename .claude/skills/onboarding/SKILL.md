---
name: onboarding
description: 이 프로젝트(AgentsToZ_byCS)를 처음 설치하는 사용자를 위한 대화형 온보딩. 환경 진단, CLI 설치, 계정 가입 안내, Supabase 테이블 생성, 첫 실행까지 단계별로 진행한다. 사용자가 "온보딩 해줘", "처음 설치", "환경 설정 도와줘", onboarding, setup 이라고 말할 때 사용한다.
---
...
1. 앱 화면(`localhost:9000`) → **설정 → 초기 설정 마법사** →
   `Supabase URL`, `anon key`, `기기 이름` 입력 → 저장.
   - 값은 `portal.json`(앱 데이터 디렉터리)에 저장된다.
   - **기기 이름을 입력하지 않으면** 마법사가 "이 기기 이름을 먼저 입력하세요" 에러로 멈춘다.
   - 에이전트는 사용자에게 키를 받아 파일에 기록하지 마라. 반드시 UI 입력으로 유도한다.
...
검증: Push 버튼 → Supabase Table Editor에서 `portmgr_ports`에 행이 생겼는지 확인.
