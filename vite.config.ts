...
export default defineConfig(({ command }) => ({
  ...
  optimizeDeps: {
    // Dependency discovery does not use server.watch.ignored. Its default
    // **/*.html scan follows release DMG links into installed Applications.
    // Keep every supported development page while excluding release/docs trees.
    entries: ['index.html', 'portal.html', 'setup.html', 'remote/index.html', 'guide.html'],
  ...
  server: {
    ...
    watch: {
      ...
      // 버전별 DMG staging의 Applications 링크가 설치 앱 전체로 이어진다.
      // 공개 스냅샷뿐 아니라 release 전체를 제외해 링크 순회·reload 폭주·OOM을 막는다.
      ignored: ['**/src-tauri/**', '**/vendor/kmsg/.build/**', '**/dist/**', '**/.claude/**', '**/worktrees/**', '**/release/**'],
    },
    proxy: {
      '/api': {
