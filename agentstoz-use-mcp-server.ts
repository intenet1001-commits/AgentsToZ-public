...
export const AGENTSTOZ_USE_MCP_INSTRUCTIONS = [
  ...
  "On invocation, call agentstoz_use_get_control_profile. Recall relevant user-wide operating memory through agentstoz_use_recall_control_context, independently of the current folder or AI. Read target project memory separately. For a name-only greeting, report profile status without creating a mission.",
  ...
  "To open AgentsToZ OPS itself, pass target=ops without portId to open_dashboard, open_code_app, the four Workroom tools, connect_buzz_channel, or open_buzz_dev. The profile token resolves OPS independently of cwd; never substitute the DEV project. A local-only app-data profile supports the OPS panel and memory calls, but needs an explicitly connected registered Control folder for app/Workroom/Orca/Buzz launches; never create one automatically. For Orca use open_code_app with surface=orca-floating or orca-worktree; agy is supported only on Orca, not as a desktop app. Buzz currently opens the app and returns the exact connected channel identity, but cannot deep-link the exact channel. Bypass must be explicitly requested; Codex/Hermes desktop apps cannot receive that permission mode. Report warnings and fallback notices, not an unverified exact launch.",
...
export const AGENTSTOZ_USE_MCP_TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description: "Bring the installed AgentsToZ desktop app to the foreground. Pass target=ops to focus its operating profile panel.",
    inputSchema: { type: "object", properties: {target: surfaceTargetProperties.target}, additionalProperties: false },
  },
  {
    name: "agentstoz_use_open_code_app",
    description: "Open a registered project in Codex, Claude, or Hermes. Default Codex behavior reopens an existing conversation without sending a message. For an explicitly requested first Codex conversation, use mode=prepare: it reopens an existing project conversation, or creates one and sends a fixed no-tools connection-test message. This is not a Workroom session. Report preparation errors without claiming a separate fallback fixed them.",
    inputSchema: {
      type: "object",
      properties: {
        ...surfaceTargetProperties,
        agent: { type: "string", enum: ["codex", "claude", "hermes", "agy"], description: "agy requires an Orca surface." },
        ...
        bypass: {type: 'boolean', description: 'Only after an explicit user request. Not supported by Codex/Hermes desktop apps; warnings are returned.'},
        mode: { type: "string", enum: ["reopen", "prepare"], description: "Codex only. prepare may send the fixed first connection-test message; use only when requested." },
      },
      required: ["agent"],
      oneOf: surfaceTargetChoice,
      additionalProperties: false,
...
export function agentsToZUseMcpActionForTool(
  ...
  if (surfaceAction !== 'open-code-app' && (args.surface !== undefined || args.mode !== undefined || args.bypass !== undefined)) {
...
async function callLocalControl(
  ...
  const response = await fetch(resolveAgentsToZUseMcpEndpoint(env), {
    method: "POST",
    headers: { "Content-Type": "application/json", ...(access ? {[CONTROL_PROFILE_HEADER]:access.token} : {}) },
