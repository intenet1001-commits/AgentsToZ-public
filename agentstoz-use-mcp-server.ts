...
import { AGENTSTOZ_USE_CONTROL_ENDPOINT, parseAgentsToZUseCodeAppMode } from "./src/agentstozUseControl";
...
export const AGENTSTOZ_USE_MCP_INSTRUCTIONS = [
  ...
  "Use agentstoz_use_propose_control_memory for a reviewed operating-memory candidate; candidate acceptance is not a saved memory. Never create a replacement profile on missing memory or a connection error.",
...
export const AGENTSTOZ_USE_MCP_TOOLS: ToolDefinition[] = [
  ...
  {name: "agentstoz_use_propose_control_memory", description: "Submit a compact, evidence-backed operating-memory candidate for review in AgentsToZ. Returns candidate status, not proof of a completed save. Use the current profile revision; retries must retain the same request ID and content.",inputSchema:{type:"object",properties:{requestId:{type:"string",minLength:1,maxLength:200},title:{type:"string",minLength:1,maxLength:200},body:{type:"string",minLength:1,maxLength:4000},evidence:{type:"string",minLength:1,maxLength:1000},expectedRevision:{type:"string",minLength:64,maxLength:64}},required:["requestId","title","body","evidence","expectedRevision"],additionalProperties:false}},
...
export function agentsToZUseMcpActionForTool(
  ...
  if (toolName === "agentstoz_use_open_code_app") {
    ...
    return {
      ...
      portId: requiredId(args.portId, "portId"),
      agent,
      ...(args.mode !== undefined ? { mode: parseAgentsToZUseCodeAppMode(agent, args.mode) } : {}),
    };
  }
