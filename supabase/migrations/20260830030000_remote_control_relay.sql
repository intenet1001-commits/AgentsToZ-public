-- Ephemeral Internet QR remote control.
-- All relay rows expire, browser callers use authenticated RPCs only, and the
-- Mac sidecar uses service_role RPCs plus a fresh per-enable in-memory secret.
...
begin
  ...
  where h.host_id = p_host_id
    and (h.owner_user_id is null or h.owner_user_id = v_user_id)
    and s.host_id = h.host_id and s.session_id = p_session_id and s.auth_user_id = v_user_id;
  if not found then
    raise exception using errcode = '42501', message = 'REMOTE_CONTROL_SESSION_ACCESS_DENIED';
  end if;
  delete from public.portmgr_remote_control_messages m
...
-- Mac sidecar: service_role plus a fresh in-memory host secret on every host RPC.
