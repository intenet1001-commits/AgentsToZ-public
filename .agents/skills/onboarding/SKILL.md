---
name: onboarding
description: AgentsToZ_byCS의 로컬 첫 실행, 첫 동기화 단말, 추가 Mac·Windows, Ubuntu/AWS 호스트 등록을 상황별로 안전하게 안내하는 Codex용 대화형 온보딩. 사용자가 "온보딩 해줘", "처음 설치", "새 기기 연결", "AWS 단말 등록", onboarding, setup 이라고 말할 때 사용한다.
---

# 온보딩 에이전트 (AgentsToZ_byCS)

설치된 AgentsToZ 앱 또는 소스를 직접 실행하는 사용자를 **첫 작업을 확인한 상태**까지 안내한다.

## 가장 먼저 실행 형태 확인

**설치된 앱에서는 로컬 프로젝트·포트 관리에 클라우드 계정·Bun·GitHub clone이 필요 없다.**
앱의 “내 기기와 연결”에서 설치·로그인·동기화 상태를 읽고 현재 단계 하나부터 진행한다.
Bun과 `bun install`은 소스 checkout을 직접 실행할 때만 필요하다. Git은 Git 작업을 선택한 경우 준비한다.
앱 UI가 열리지 않으면 설치 앱을 다시 실행하고 내장 서비스 상태를 확인한다. 이를 소스 설치로 바꾸지 않는다.

| 실행 형태/선택 기능 | 필요한 준비 | 실제 확인 |
|---|---|---|
| 설치 앱의 로컬 사용 | 앱 설치·실행 | 프로젝트 등록·열기 |
| 소스 직접 실행 | Bun·Git·`bun install`, 프로젝트의 실행 명령 | UI와 API 응답 |
| AI 작업 | 선택한 CLI의 설치·인증 | 첫 프로젝트에서 실제 응답 |
| Supabase 동기화 | 본인의 프로젝트·이 OS 사용자의 인증 | 단말 등록·실제 Push/Pull |
| 개인 모바일 포털 | 본인의 Supabase Google OAuth·Vercel | 로그인·기기 조회·원격 승인·첫 작업 |
| GitHub 보관 | 사용할 저장소의 권한 | 선택한 저장소에 실제 반영 |

### AI 핸드오프와 이어서 준비하기

- 앱이 만든 인계문에는 기존 준비 기록의 runId·revision·현재 단계가 들어간다. 복사 시점의 힌트이므로 먼저 앱에서 최신 상태를 읽는다.
- 진행 중인 확인은 결과를 재조회한다. AI를 바꾸거나 재부팅했다고 설치·리소스 생성을 반복하지 않는다.
- “AI 작업 후 다시 확인”은 실제 진단 결과로 기록을 갱신한다. AI의 완료 문구나 복사를 성공 증거로 쓰지 않는다.
- 설치 버전, 저장된 로그인 정보, 인증 확인, 프로젝트 권한, 첫 실행을 구분한다. unknown은 미설치·미로그인으로 바꾸지 않는다.
- 사용자가 이미 승인한 범위는 진행한다. 새 과금·공개 범위·덮어쓰기 결정만 구체적 대상을 먼저 확인한다.
- Google Cloud 프로젝트와 OAuth client는 다른 자원이다. 비밀번호·2FA·Client Secret은 공식 화면에서 직접 입력하고 AI 대화나 인계문에 복제하지 않는다.
- 같은 Mac의 다른 OS 사용자는 별도 등록 환경이다. 동일 hostname을 근거로 ID를 합치지 않는다. 현재 고정 포트 3001 구조의 다중 사용자 동시 실행은 미검증이다.
- Mac 전체 호스트의 TestFlight 배포 적합성은 아직 확인되지 않았다. 설치 앱 사용 절차와 배포 채널의 지원 여부를 혼동하지 않는다.
...
3. Codex의 `playwright` 스킬이 있으면 그 `SKILL.md`를 먼저 읽고 wrapper를 사용한다.
