...
별도의 AI 작업 탭과 앱 자체 실행 화면은 제공하지 않는다. 개발 실행은 워크룸의 Codex·Claude·Hermes·agy 터미널이나 외부 AI 앱으로 시작한다. OPS 운영기억은 프로젝트·장기기억 화면에서 관리하며 CS-CEO 자동 연동은 제공하지 않는다. 아래 관리형 런타임 API 설명은 기존 저장·원격 호환 계약을 설명하며, 현재 앱 화면에 독립 실행 탭이 있다는 뜻이 아니다.
...
interface PortInfo {
  id: string;
  name: string;
  role?: 'ops' | 'dev' | 'managed'; // 로컬 등록 역할. 미지정 레거시 행은 표시 시 해석하며 강제 재저장하지 않음
...
struct PortInfo {
    ...
    role: Option<ProjectRole>, // ops/dev/managed. 저장 merge는 누락된 role을 보존한다.
    #[serde(default)]
