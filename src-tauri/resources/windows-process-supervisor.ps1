...
$parentPidText = $env:AGENTSTOZ_SUPERVISOR_PARENT_PID
...
$parentPid = [uint32]0
if (-not [string]::IsNullOrWhiteSpace($parentPidText) -and -not [uint32]::TryParse($parentPidText,[ref]$parentPid)) { throw 'Supervisor parent PID is invalid.' }
foreach ($name in @('AGENTSTOZ_SUPERVISOR_PROGRAM','AGENTSTOZ_SUPERVISOR_ARGS_JSON','AGENTSTOZ_SUPERVISOR_CWD','AGENTSTOZ_SUPERVISOR_PARENT_PID')) { [Environment]::SetEnvironmentVariable($name,$null,'Process') }
...
public static class AgentsToZWindowsJobSupervisor {
 ...
 const uint SYNCHRONIZE=0x00100000, WAIT_OBJECT_0=0, WAIT_TIMEOUT=258, WAIT_FAILED=0xffffffff;
 ...
 static bool ParentGone(IntPtr p){if(p==IntPtr.Zero)return false;uint wait=WaitForSingleObject(p,0);if(wait==WAIT_OBJECT_0)return true;if(wait==WAIT_FAILED)Fail("WaitForSingleObject(parent)");if(wait!=WAIT_TIMEOUT)throw new InvalidOperationException("Unexpected parent wait state");return false;}
 public static int Run(string program,string[] args,string cwd,uint parentPid){
  IntPtr job=Job(),parent=IntPtr.Zero;var pi=new PROCESS_INFORMATION();
  try{if(parentPid>1){parent=OpenProcess(SYNCHRONIZE,false,parentPid);if(parent==IntPtr.Zero)Fail("OpenProcess(parent)");}
   if(ParentGone(parent))return 70;
   var si=new STARTUPINFO();si.cb=(uint)Marshal.SizeOf(typeof(STARTUPINFO));si.flags=STARTF_USESTDHANDLES;si.input=GetStdHandle(-10);si.output=GetStdHandle(-11);si.error=GetStdHandle(-12);
   ...
   while(Active(job)>0){if(ParentGone(parent))return 70;Thread.Sleep(50);}return 0;
  }finally{if(pi.thread!=IntPtr.Zero)CloseHandle(pi.thread);if(pi.process!=IntPtr.Zero)CloseHandle(pi.process);if(parent!=IntPtr.Zero)CloseHandle(parent);if(job!=IntPtr.Zero)CloseHandle(job);}
 }
}
'@
Add-Type -TypeDefinition $source -Language CSharp
exit [AgentsToZWindowsJobSupervisor]::Run($childProgram,[string[]]$childArgs,$childWorkingDirectory,$parentPid)
