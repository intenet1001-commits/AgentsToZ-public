...
#[cfg(target_os = "macos")]
mod macos_runtime_client_bridge;

fn default_port_sync_generation() -> String {
    "0".to_string()
}

fn canonical_port_sync_generation(value: serde_json::Value) -> Option<String> {
    match value {
        // Older local files can contain a safe JSON integer. Normalize it to
        // decimal text so a PostgreSQL bigint is never rounded by JavaScript.
        serde_json::Value::Number(number) => number.as_u64()
            .filter(|value| *value <= i64::MAX as u64)
            .map(|value| value.to_string()),
        serde_json::Value::String(value) => {
            let canonical_shape = value == "0"
...
fn deserialize_port_sync_generation<'de, D>(deserializer: D) -> Result<String, D::Error>
where
    D: serde::Deserializer<'de>,
{
    let value = serde_json::Value::deserialize(deserializer)?;
    canonical_port_sync_generation(value)
        .ok_or_else(|| serde::de::Error::custom("syncGeneration must be a non-negative PostgreSQL bigint"))
}

fn deserialize_optional_port_sync_generation<'de, D>(deserializer: D) -> Result<Option<String>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    let value = serde_json::Value::deserialize(deserializer)?;
    ...
    canonical_port_sync_generation(value)
        .map(Some)
        .ok_or_else(|| serde::de::Error::custom("sourcePortSyncGeneration must be a non-negative PostgreSQL bigint"))
}

#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "lowercase")]
enum ProjectRole { Ops, Dev, Managed }
...
struct PortInfo {
    ...
    #[serde(rename = "worktreeParentId", default, skip_serializing_if = "Option::is_none")]
    worktree_parent_id: Option<String>,
    ...
    #[serde(rename = "sourcePortDeviceId", default, skip_serializing_if = "Option::is_none")]
    source_port_device_id: Option<String>,
    #[serde(
        rename = "sourcePortSyncGeneration",
        default,
        deserialize_with = "deserialize_optional_port_sync_generation",
        skip_serializing_if = "Option::is_none"
    )]
    source_port_sync_generation: Option<String>,
    // Durable Supabase deletion-fence generation. Keeping it in ports.json is
    // essential: after an explicit restore, a pre-delete renderer still holds
    // generation 0 and the database must be able to reject that stale write.
    #[serde(
        rename = "syncGeneration",
        default = "default_port_sync_generation",
        deserialize_with = "deserialize_port_sync_generation"
    )]
...
static NEXT_PORTAL_FILE_WRITE_GENERATION: AtomicU64 = AtomicU64::new(1);
...
struct AppState {
    ...
    what_i_said_capability: Mutex<Option<String>>,
    remote_control_capability: Mutex<Option<String>>,
    onboarding_capability: Mutex<Option<String>>,
    agent_runtime_capability: Mutex<Option<String>>,
}

fn new_what_i_said_capability() -> String {
    let mut bytes = [0u8; 32];
    getrandom::fill(&mut bytes).expect("secure randomness for What I said capability");
    bytes.iter().map(|byte| format!("{:02x}", byte)).collect()
}

fn read_existing_what_i_said_capability(path: &std::path::Path) -> Result<Option<String>, String> {
    ...
    if path_metadata.file_type().is_symlink() || !path_metadata.is_file() {
        return Err("What I said 보안 연결 파일이 안전하지 않습니다.".to_string());
    }
    ...
    #[cfg(unix)]
    {
        ...
        if metadata.uid() != unsafe { libc::geteuid() } || metadata.mode() & 0o077 != 0 {
            return Err("What I said 보안 연결 파일 권한이 안전하지 않습니다.".to_string());
        }
    }
    let mut value = String::new();
    file.read_to_string(&mut value).map_err(|error| error.to_string())?;
    let token = value.trim();
    if token.len() != 64 || !token.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase()) {
        return Err("What I said 보안 연결 파일이 올바르지 않습니다.".to_string());
    }
    Ok(Some(token.to_string()))
}

fn load_or_create_what_i_said_capability(app_handle: &tauri::AppHandle) -> Result<String, String> {
    let app_data_dir = ensure_app_data_dir(app_handle)?;
    let path = app_data_dir.join("what-i-said-management.capability");
    if let Some(existing) = read_existing_what_i_said_capability(&path)? {
        return Ok(existing);
    }

    let token = new_what_i_said_capability();
    let mut options = fs::OpenOptions::new();
    ...
    match options.open(&path) {
        Ok(mut file) => {
            file.write_all(format!("{}\n", token).as_bytes()).map_err(|error| error.to_string())?;
            file.sync_all().map_err(|error| error.to_string())?;
            Ok(token)
        }
        Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => read_existing_what_i_said_capability(&path)?
            .ok_or_else(|| "What I said 보안 연결 파일을 확인하지 못했습니다.".to_string()),
        Err(error) => Err(error.to_string()),
    }
}

fn new_remote_control_capability() -> String {
    let mut bytes = [0u8; 32];
    getrandom::fill(&mut bytes).expect("secure randomness for remote control capability");
    bytes.iter().map(|byte| format!("{:02x}", byte)).collect()
}

fn read_existing_remote_control_capability(path: &std::path::Path) -> Result<Option<String>, String> {
    let path_metadata = match fs::symlink_metadata(path) {
        Ok(value) => value,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(None),
        Err(error) => return Err(error.to_string()),
    ...
    if path_metadata.file_type().is_symlink() || !path_metadata.is_file() {
        return Err("원격 제어 보안 연결 파일이 안전하지 않습니다.".to_string());
    }
    ...
    #[cfg(unix)]
    {
        use std::os::unix::fs::MetadataExt;
        if metadata.uid() != unsafe { libc::geteuid() } || metadata.mode() & 0o077 != 0 {
            return Err("원격 제어 보안 연결 파일 권한이 안전하지 않습니다.".to_string());
        }
    }
    let mut value = String::new();
    file.read_to_string(&mut value).map_err(|error| error.to_string())?;
    let token = value.trim();
    if token.len() != 64 || !token.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase()) {
        return Err("원격 제어 보안 연결 파일이 올바르지 않습니다.".to_string());
    }
    Ok(Some(token.to_string()))
}

fn load_or_create_remote_control_capability(app_handle: &tauri::AppHandle) -> Result<String, String> {
    let app_data_dir = ensure_app_data_dir(app_handle)?;
    let path = app_data_dir.join("remote-control-management.capability");
    if let Some(existing) = read_existing_remote_control_capability(&path)? {
        return Ok(existing);
    }

    let token = new_remote_control_capability();
    let mut options = fs::OpenOptions::new();
    options.write(true).create_new(true);
    #[cfg(unix)]
    {
    ...
    match options.open(&path) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => read_existing_remote_control_capability(&path)?
            .ok_or_else(|| "원격 제어 보안 연결 파일을 확인하지 못했습니다.".to_string()),
        Err(error) => Err(error.to_string()),
    }
}

fn new_onboarding_capability() -> String {
    let mut bytes = [0u8; 32];
    getrandom::fill(&mut bytes).expect("secure randomness for onboarding capability");
    bytes.iter().map(|byte| format!("{:02x}", byte)).collect()
}

fn read_existing_onboarding_capability(path: &std::path::Path) -> Result<Option<String>, String> {
    let path_metadata = match fs::symlink_metadata(path) {
        Ok(value) => value,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(None),
        Err(error) => return Err(error.to_string()),
    ...
    if path_metadata.file_type().is_symlink() || !path_metadata.is_file() {
        return Err("설치 도우미 보안 연결 파일이 안전하지 않습니다.".to_string());
    }
    ...
    #[cfg(unix)]
    {
        use std::os::unix::fs::MetadataExt;
        if metadata.uid() != unsafe { libc::geteuid() } || metadata.mode() & 0o077 != 0 {
            return Err("설치 도우미 보안 연결 파일 권한이 안전하지 않습니다.".to_string());
        }
    }
    let mut value = String::new();
    file.read_to_string(&mut value).map_err(|error| error.to_string())?;
    let token = value.trim();
    if token.len() != 64 || !token.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase()) {
        return Err("설치 도우미 보안 연결 파일이 올바르지 않습니다.".to_string());
    }
    Ok(Some(token.to_string()))
}

fn load_or_create_onboarding_capability(app_handle: &tauri::AppHandle) -> Result<String, String> {
    let app_data_dir = ensure_app_data_dir(app_handle)?;
    let path = app_data_dir.join("onboarding-management.capability");
    if let Some(existing) = read_existing_onboarding_capability(&path)? {
        return Ok(existing);
    }

    let token = new_onboarding_capability();
    let mut options = fs::OpenOptions::new();
    options.write(true).create_new(true);
    #[cfg(unix)]
    {
    ...
    match options.open(&path) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => read_existing_onboarding_capability(&path)?
            .ok_or_else(|| "설치 도우미 보안 연결 파일을 확인하지 못했습니다.".to_string()),
        Err(error) => Err(error.to_string()),
    }
}

fn new_agent_runtime_capability() -> String {
    let mut bytes = [0u8; 32];
    getrandom::fill(&mut bytes).expect("secure randomness for Agent Runtime capability");
    bytes.iter().map(|byte| format!("{:02x}", byte)).collect()
}

fn read_existing_agent_runtime_capability(path: &std::path::Path) -> Result<Option<String>, String> {
    let path_metadata = match fs::symlink_metadata(path) {
        Ok(value) => value,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(None),
        Err(error) => return Err(error.to_string()),
    ...
    if path_metadata.file_type().is_symlink() || !path_metadata.is_file() {
        return Err("에이전트 런타임 보안 연결 파일이 안전하지 않습니다.".to_string());
    }
    ...
    #[cfg(unix)]
    {
        use std::os::unix::fs::MetadataExt;
        if metadata.uid() != unsafe { libc::geteuid() } || metadata.mode() & 0o077 != 0 {
            return Err("에이전트 런타임 보안 연결 파일 권한이 안전하지 않습니다.".to_string());
        }
    }
    let mut value = String::new();
    file.read_to_string(&mut value).map_err(|error| error.to_string())?;
    let token = value.trim();
    if token.len() != 64
        || !token.bytes().all(|byte| byte.is_ascii_hexdigit() && !byte.is_ascii_uppercase())
    {
        return Err("에이전트 런타임 보안 연결 파일이 올바르지 않습니다.".to_string());
    }
    Ok(Some(token.to_string()))
}

fn load_or_create_agent_runtime_capability(app_handle: &tauri::AppHandle) -> Result<String, String> {
    let app_data_dir = ensure_app_data_dir(app_handle)?;
    let path = app_data_dir.join("agent-runtime-management.capability");
    if let Some(existing) = read_existing_agent_runtime_capability(&path)? {
        return Ok(existing);
    }

    let token = new_agent_runtime_capability();
    let mut options = fs::OpenOptions::new();
    options.write(true).create_new(true);
    #[cfg(unix)]
    {
    ...
    match options.open(&path) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => read_existing_agent_runtime_capability(&path)?
            .ok_or_else(|| "에이전트 런타임 보안 연결 파일을 확인하지 못했습니다.".to_string()),
        Err(error) => Err(error.to_string()),
    }
}

#[tauri::command]
fn what_i_said_management_request(
    path: String,
    method: String,
    body: Option<serde_json::Value>,
    state: State<'_, AppState>,
) -> Result<serde_json::Value, String> {
    let capability = state.what_i_said_capability
        .lock()
        .unwrap_or_else(|error| error.into_inner())
        .clone()
        .ok_or_else(|| "What I said 보안 연결을 준비하지 못했습니다.".to_string())?;
    authenticated_what_i_said_management_request(
        &capability,
        &path,
        &method,
...
#[tauri::command]
fn remote_control_management_request(
    path: String,
    method: String,
    body: Option<serde_json::Value>,
    state: State<'_, AppState>,
) -> Result<serde_json::Value, String> {
    let capability = state.remote_control_capability
        .lock()
        .unwrap_or_else(|error| error.into_inner())
        .clone()
        .ok_or_else(|| "원격 제어 보안 연결을 준비하지 못했습니다.".to_string())?;
    authenticated_remote_control_management_request(
        &capability,
        &path,
        &method,
        body.unwrap_or_else(|| serde_json::json!({})),
...
#[tauri::command]
async fn onboarding_management_request(
    body: serde_json::Value,
    tool: Option<String>,
    state: State<'_, AppState>,
) -> Result<serde_json::Value, String> {
    let capability = state.onboarding_capability.lock()
        .unwrap_or_else(|error| error.into_inner()).clone()
        .ok_or_else(|| "설치 도우미 보안 연결이 준비되지 않았습니다.".to_string())?;
    tauri::async_runtime::spawn_blocking(move || {
        authenticated_onboarding_management_request(&capability, body, tool.as_deref().unwrap_or("github"))
    }).await.map_err(|_| "설치 도우미 연결에 실패했습니다.".to_string())?
}

#[tauri::command]
async fn agent_runtime_request(
    path: String,
    method: String,
    body: Option<serde_json::Value>,
    state: State<'_, AppState>,
) -> Result<serde_json::Value, String> {
    let capability = state.agent_runtime_capability
        .lock()
        .unwrap_or_else(|error| error.into_inner())
        .clone()
        .ok_or_else(|| "에이전트 런타임 보안 연결을 준비하지 못했습니다.".to_string())?;
    // TcpStream intentionally has strict read/write deadlines, but it is still
    // blocking I/O. Keep a slow/half-open sidecar off Tauri's main thread.
    tauri::async_runtime::spawn_blocking(move || {
        authenticated_agent_runtime_request(&capability, &path, &method, body)
    })
    .await
    .map_err(|error| format!("에이전트 런타임 요청 작업을 완료하지 못했습니다: {}", error))?
}
...
#[serde(rename_all = "camelCase")]
struct LocalApiContract {
    schema_version: u64,
    required_capabilities: Vec<String>,
    sidecar_health_proof: LocalApiHealthProofContract,
    remote_control_health_proof: LocalApiHealthProofContract,
    agent_runtime_health_proof: LocalApiHealthProofContract,
    onboarding_health_proof: LocalApiHealthProofContract,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct LocalApiHealthProofContract {
    algorithm: String,
    encoding: String,
    nonce_bytes: usize,
...
fn local_api_contract() -> Option<LocalApiContract> {
    let contract: LocalApiContract = serde_json::from_str(LOCAL_API_CONTRACT_JSON).ok()?;
    let safe_name = |value: &str| {
        !value.is_empty()
            && value.bytes().all(|byte| byte.is_ascii_alphanumeric() || byte == b'-' || byte == b'_')
    };
    let valid_proof = |proof: &LocalApiHealthProofContract| {
        proof.algorithm == "hmac-sha256"
            && proof.encoding == "lowercase-hex"
            && proof.nonce_bytes == 32
            && safe_name(&proof.query_parameter)
            && safe_name(&proof.response_field)
            && proof.domain.ends_with('\0')
    };
    let sidecar_proof = &contract.sidecar_health_proof;
    let remote_control_proof = &contract.remote_control_health_proof;
    let agent_runtime_proof = &contract.agent_runtime_health_proof;
    let onboarding_proof = &contract.onboarding_health_proof;
    if !valid_proof(onboarding_proof)
        || onboarding_proof.query_parameter != sidecar_proof.query_parameter
        || [sidecar_proof, remote_control_proof, agent_runtime_proof].iter().any(|p| p.domain == onboarding_proof.domain || p.response_field == onboarding_proof.response_field)
        || !valid_proof(sidecar_proof)
        || !valid_proof(remote_control_proof)
        || !valid_proof(agent_runtime_proof)
        || remote_control_proof.query_parameter != sidecar_proof.query_parameter
        || agent_runtime_proof.query_parameter != sidecar_proof.query_parameter
        || remote_control_proof.domain != sidecar_proof.domain
        || remote_control_proof.response_field == sidecar_proof.response_field
        || agent_runtime_proof.response_field == sidecar_proof.response_field
        || agent_runtime_proof.response_field == remote_control_proof.response_field
        || agent_runtime_proof.domain == sidecar_proof.domain
    {
        return None;
    }
    Some(contract)
...
fn local_api_health_proof_is_valid(
    capability: &str,
    nonce: &[u8],
    proof: &str,
    contract: &LocalApiHealthProofContract,
) -> bool {
    if nonce.len() != contract.nonce_bytes {
        return false;
    }
    let Some(key) = decode_lower_hex(capability, 32) else {
        return false;
    };
    let Some(received) = decode_lower_hex(proof, 32) else {
        return false;
    };
    let Ok(mut mac) = Hmac::<Sha256>::new_from_slice(&key) else {
        return false;
    };
    mac.update(contract.domain.as_bytes());
    mac.update(nonce);
    // verify_slice performs the equality check in constant time.
...
fn classify_local_api_health(
    response: Option<&str>,
    capability: &str,
    nonce: &[u8],
) -> LocalApiHealth {
    classify_local_api_health_with_proof(
        response,
        capability,
        nonce,
        LocalApiProofAuthority::WhatISaid,
    )
}

fn classify_remote_control_api_health(
    response: Option<&str>,
    capability: &str,
    nonce: &[u8],
) -> LocalApiHealth {
    classify_local_api_health_with_proof(
        ...
        capability,
        nonce,
        LocalApiProofAuthority::RemoteControl,
    )
}

fn classify_agent_runtime_api_health(
    response: Option<&str>,
    capability: &str,
    nonce: &[u8],
) -> LocalApiHealth {
    classify_local_api_health_with_proof(
        ...
        capability,
        nonce,
        LocalApiProofAuthority::AgentRuntime,
    )
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum LocalApiProofAuthority {
    WhatISaid,
    RemoteControl,
    AgentRuntime,
    Onboarding,
...
fn classify_local_api_health_with_proof(
    response: Option<&str>,
    capability: &str,
    nonce: &[u8],
    authority: LocalApiProofAuthority,
) -> LocalApiHealth {
    ...
    let Some(contract) = local_api_contract() else {
        return LocalApiHealth::Incompatible;
    };
    let proof_contract = match authority {
        LocalApiProofAuthority::WhatISaid => &contract.sidecar_health_proof,
        LocalApiProofAuthority::RemoteControl => &contract.remote_control_health_proof,
        LocalApiProofAuthority::AgentRuntime => &contract.agent_runtime_health_proof,
        LocalApiProofAuthority::Onboarding => &contract.onboarding_health_proof,
    };
    let Some(proof) = health
        .get(&proof_contract.response_field)
        .and_then(|value| value.as_str())
    else {
        return LocalApiHealth::Foreign;
    };
    if !local_api_health_proof_is_valid(capability, nonce, proof, proof_contract) {
        return LocalApiHealth::Foreign;
    }
    let Some(detected_schema) = health.get("schemaVersion").and_then(|value| value.as_u64()) else {
        return LocalApiHealth::Incompatible;
    };
    if detected_schema < contract.schema_version {
        return LocalApiHealth::Incompatible;
    }
    ...
    let all_required = contract.required_capabilities
        .iter()
        .all(|required| has_capability(required));
...
fn new_local_api_health_nonce() -> Option<[u8; 32]> {
    let mut nonce = [0u8; 32];
    getrandom::fill(&mut nonce).ok()?;
    Some(nonce)
}

fn local_api_health_request_with_connection(nonce: &[u8], connection: &str) -> Option<String> {
    let contract = local_api_contract()?;
    if nonce.len() != contract.sidecar_health_proof.nonce_bytes {
        return None;
    }
    ...
    Some(format!(
        ...
        contract.sidecar_health_proof.query_parameter,
...
fn local_api_health_request(nonce: &[u8]) -> Option<String> {
    local_api_health_request_with_connection(nonce, "close")
}

/// Reads the complete health response. The request carries only a fresh public
/// nonce; the capability is used locally after the response and never sent.
fn local_api_health_response(nonce: &[u8]) -> Option<String> {
    ...
    let request = local_api_health_request(nonce)?;
...
fn authenticated_local_api_health(capability: &str) -> LocalApiHealth {
    let Some(nonce) = new_local_api_health_nonce() else {
        return LocalApiHealth::Unavailable;
    };
    let response = local_api_health_response(&nonce);
    classify_local_api_health(response.as_deref(), capability, &nonce)
}

const WHAT_I_SAID_MANAGEMENT_RESPONSE_MAX_BYTES: usize = 16 * 1024 * 1024;
const WHAT_I_SAID_MANAGEMENT_REQUEST_MAX_BYTES: usize = 1024 * 1024;
const REMOTE_CONTROL_MANAGEMENT_RESPONSE_MAX_BYTES: usize = 1024 * 1024;
const REMOTE_CONTROL_MANAGEMENT_REQUEST_MAX_BYTES: usize = 4 * 1024;
const AGENT_RUNTIME_RESPONSE_MAX_BYTES: usize = 3 * 1024 * 1024;
// A 32 KiB UTF-8 prompt can expand close to 6x when JSON escapes control
// characters. Keep this aligned with the sidecar's bounded HTTP reader.
const AGENT_RUNTIME_REQUEST_MAX_BYTES: usize = 256 * 1024;

struct LocalApiJsonResponse {
    status: u16,
    body: serde_json::Value,
    raw: String,
}

fn read_local_api_json_response(
    stream: &mut TcpStream,
    max_body_bytes: usize,
) -> Result<LocalApiJsonResponse, String> {
    ...
    let header_end = loop {
        ...
        if response.len() >= MAX_HEADER_BYTES {
            return Err("AgentsToZ 로컬 API 응답 헤더가 너무 큽니다.".to_string());
        }
        let read = stream.read(&mut scratch).map_err(|error| error.to_string())?;
        if read == 0 {
            return Err("AgentsToZ 로컬 API 응답이 중간에 종료됐습니다.".to_string());
        }
        response.extend_from_slice(&scratch[..read]);
    };

    let head = std::str::from_utf8(&response[..header_end])
        .map_err(|_| "AgentsToZ 로컬 API 응답 헤더가 올바르지 않습니다.".to_string())?;
    ...
    for line in head.lines().skip(1) {
        let Some((name, value)) = line.split_once(':') else {
            return Err("AgentsToZ 로컬 API 응답 헤더가 올바르지 않습니다.".to_string());
        };
        if name.eq_ignore_ascii_case("transfer-encoding") {
            return Err("AgentsToZ 로컬 API의 chunked 응답은 허용되지 않습니다.".to_string());
        }
        if name.eq_ignore_ascii_case("content-length") {
            let parsed = value.trim().parse::<usize>()
                .map_err(|_| "AgentsToZ 로컬 API Content-Length가 올바르지 않습니다.".to_string())?;
            if content_length.replace(parsed).is_some() {
                return Err("AgentsToZ 로컬 API Content-Length가 중복됐습니다.".to_string());
            }
        }
    }
    let content_length = content_length
        .ok_or_else(|| "AgentsToZ 로컬 API Content-Length가 없습니다.".to_string())?;
    if content_length > max_body_bytes {
        return Err("AgentsToZ 로컬 API 응답이 너무 큽니다.".to_string());
    }
    let body_start = header_end + 4;
    let expected = body_start.checked_add(content_length)
        .ok_or_else(|| "AgentsToZ 로컬 API 응답 크기가 올바르지 않습니다.".to_string())?;
    while response.len() < expected {
        let read = stream.read(&mut scratch).map_err(|error| error.to_string())?;
        if read == 0 {
            return Err("AgentsToZ 로컬 API JSON이 중간에 종료됐습니다.".to_string());
        }
        response.extend_from_slice(&scratch[..read]);
        if response.len() > expected {
            return Err("AgentsToZ 로컬 API 응답 경계가 올바르지 않습니다.".to_string());
        }
    }
    let body = if content_length == 0 {
        serde_json::Value::Null
    } else {
        serde_json::from_slice(&response[body_start..expected])
            .map_err(|error| format!("AgentsToZ 로컬 API JSON 오류: {}", error))?
    };
    let raw = String::from_utf8(response)
        .map_err(|_| "AgentsToZ 로컬 API 응답이 UTF-8이 아닙니다.".to_string())?;
    Ok(LocalApiJsonResponse { status, body, raw })
}

fn is_allowed_what_i_said_management_route(method: &str, path: &str) -> bool {
    matches!((method, path),
        ("POST", "/api/what-i-said/status")
        | ("POST", "/api/what-i-said/global-status")
        | ("POST", "/api/what-i-said/configure")
        | ("POST", "/api/what-i-said/configure-all")
        | ("POST", "/api/what-i-said/capture")
        | ("POST", "/api/what-i-said/sync")
        | ("POST", "/api/what-i-said/prompt-origin/register")
        | ("POST", "/api/what-i-said/guides/list")
        | ("POST", "/api/what-i-said/guides/save")
        | ("POST", "/api/what-i-said/list")
        | ("POST", "/api/what-i-said/delete")
        | ("POST", "/api/what-i-said/source/status")
        | ("POST", "/api/what-i-said/source/enable")
        | ("POST", "/api/what-i-said/source/rotate")
        | ("DELETE", "/api/what-i-said/source")
        | ("POST", "/api/what-i-said/remote/status")
        | ("POST", "/api/what-i-said/remote/configure")
        | ("POST", "/api/what-i-said/remote/exclude")
        | ("POST", "/api/what-i-said/remote-key/status")
        | ("POST", "/api/what-i-said/remote-key/issue")
        | ("POST", "/api/what-i-said/remote-key/revoke")
    )
}

fn authenticated_what_i_said_management_request(
    capability: &str,
    path: &str,
    method: &str,
    payload: serde_json::Value,
) -> Result<serde_json::Value, String> {
    if !is_allowed_what_i_said_management_route(method, path) || !payload.is_object() {
        return Err("허용되지 않은 What I said 관리 요청입니다.".to_string());
    }
    let body = serde_json::to_vec(&payload).map_err(|error| error.to_string())?;
    if body.len() > WHAT_I_SAID_MANAGEMENT_REQUEST_MAX_BYTES {
        return Err("What I said 관리 요청이 너무 큽니다.".to_string());
    }
    let addr: SocketAddr = LOCAL_API_ADDR.parse()
        .map_err(|error| format!("로컬 API 주소 오류: {}", error))?;
    let mut stream = TcpStream::connect_timeout(&addr, Duration::from_secs(2))
        .map_err(|error| format!("AgentsToZ 로컬 API 연결 실패: {}", error))?;
    ...
    // Authenticate the exact TCP peer before this connection ever receives the
    // management capability. Reconnecting after this challenge would restore a
    // probe/use race where another process could take port 3001 in between.
    let nonce = new_local_api_health_nonce()
        .ok_or_else(|| "What I said 보안 nonce를 만들지 못했습니다.".to_string())?;
    let health_request = local_api_health_request_with_connection(&nonce, "keep-alive")
        .ok_or_else(|| "What I said 보안 확인 요청을 만들지 못했습니다.".to_string())?;
    stream.write_all(health_request.as_bytes()).map_err(|error| error.to_string())?;
    let health = read_local_api_json_response(&mut stream, 1024 * 1024)?;
    if classify_local_api_health(Some(&health.raw), capability, &nonce) != LocalApiHealth::Compatible {
        return Err("What I said 로컬 API의 보안 연결을 확인하지 못했습니다.".to_string());
    }
    ...
    let response = read_local_api_json_response(
        &mut stream,
        WHAT_I_SAID_MANAGEMENT_RESPONSE_MAX_BYTES,
    )?;
    Ok(serde_json::json!({
...
fn is_allowed_remote_control_management_route(method: &str, path: &str) -> bool {
    matches!((method, path),
        ("POST", "/api/remote-control/status")
        | ("POST", "/api/remote-control/interfaces")
        | ("POST", "/api/remote-control/enable")
        | ("POST", "/api/remote-control/pairing/rotate")
        | ("POST", "/api/remote-control/sessions/revoke")
        | ("POST", "/api/remote-control/disable")
        | ("POST", "/api/remote-control/internet/status")
        | ("POST", "/api/remote-control/internet/enable")
        | ("POST", "/api/remote-control/internet/pairing/issue")
        | ("POST", "/api/remote-control/internet/sessions/approve")
        | ("POST", "/api/remote-control/internet/sessions/scopes")
        | ("POST", "/api/remote-control/internet/sessions/revoke")
        | ("POST", "/api/remote-control/internet/disable")
    )
}

fn remote_control_management_http_request(
    method: &str,
    path: &str,
    capability: &str,
    body_len: usize,
    ...
    format!(
        "{} {} HTTP/1.1\r\nHost: {}\r\nOrigin: http://tauri.localhost\r\nAccept: application/json\r\nContent-Type: application/json\r\nX-AgentsToZ-Remote-Control-Capability: {}\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
        method,
        path,
        LOCAL_API_ADDR,
        capability,
...
fn authenticated_remote_control_management_request(
    capability: &str,
    path: &str,
    method: &str,
    payload: serde_json::Value,
) -> Result<serde_json::Value, String> {
    if !is_allowed_remote_control_management_route(method, path) || !payload.is_object() {
        return Err("허용되지 않은 원격 제어 관리 요청입니다.".to_string());
    }
    let body = serde_json::to_vec(&payload).map_err(|error| error.to_string())?;
    if body.len() > REMOTE_CONTROL_MANAGEMENT_REQUEST_MAX_BYTES {
        return Err("원격 제어 관리 요청이 너무 큽니다.".to_string());
    }
    let addr: SocketAddr = LOCAL_API_ADDR.parse()
        .map_err(|error| format!("로컬 API 주소 오류: {}", error))?;
    let mut stream = TcpStream::connect_timeout(&addr, Duration::from_secs(2))
    ...
    // Prove the exact TCP peer with the remote-control capability before that
    // capability is sent. The What I said proof/header are intentionally not
    // accepted for this independent authority domain.
    let nonce = new_local_api_health_nonce()
        .ok_or_else(|| "원격 제어 보안 nonce를 만들지 못했습니다.".to_string())?;
    let health_request = local_api_health_request_with_connection(&nonce, "keep-alive")
        .ok_or_else(|| "원격 제어 보안 확인 요청을 만들지 못했습니다.".to_string())?;
    stream.write_all(health_request.as_bytes()).map_err(|error| error.to_string())?;
    let health = read_local_api_json_response(&mut stream, 1024 * 1024)?;
    if classify_remote_control_api_health(Some(&health.raw), capability, &nonce)
        != LocalApiHealth::Compatible
    {
        return Err("원격 제어 로컬 API의 보안 연결을 확인하지 못했습니다.".to_string());
    }

    let request = remote_control_management_http_request(
        method,
        path,
        capability,
        body.len(),
    ...
    let response = read_local_api_json_response(
        &mut stream,
        REMOTE_CONTROL_MANAGEMENT_RESPONSE_MAX_BYTES,
    )?;
    Ok(serde_json::json!({
        "status": response.status,
        "body": response.body,
...
fn onboarding_management_path(tool: &str) -> Result<&'static str, String> {
    match tool {
        "github" => Ok("/api/onboarding/github"),
        "codex" => Ok("/api/onboarding/codex"),
        "codex-login" => Ok("/api/onboarding/codex-login"),
        _ => Err("지원하지 않는 설치 도우미입니다.".to_string()),
    }
}

fn authenticated_onboarding_management_request(
    capability: &str,
    payload: serde_json::Value,
    tool: &str,
) -> Result<serde_json::Value, String> {
    let path = onboarding_management_path(tool)?;
    let method = "POST";
    if !payload.is_object() {
        return Err("허용되지 않은 설치 도우미 관리 요청입니다.".to_string());
    }
    let body = serde_json::to_vec(&payload).map_err(|error| error.to_string())?;
    if body.len() > 4096 {
        return Err("설치 도우미 관리 요청이 너무 큽니다.".to_string());
    }
    let addr: SocketAddr = LOCAL_API_ADDR.parse()
        .map_err(|error| format!("로컬 API 주소 오류: {}", error))?;
    let mut stream = TcpStream::connect_timeout(&addr, Duration::from_secs(2))
    ...
    // Prove the exact TCP peer with the onboarding capability before that
    // capability is sent. The What I said proof/header are intentionally not
    // accepted for this independent authority domain.
    let nonce = new_local_api_health_nonce()
        .ok_or_else(|| "설치 도우미 보안 nonce를 만들지 못했습니다.".to_string())?;
    let health_request = local_api_health_request_with_connection(&nonce, "keep-alive")
        .ok_or_else(|| "설치 도우미 보안 확인 요청을 만들지 못했습니다.".to_string())?;
    stream.write_all(health_request.as_bytes()).map_err(|error| error.to_string())?;
    let health = read_local_api_json_response(&mut stream, 1024 * 1024)?;
    if classify_local_api_health_with_proof(Some(&health.raw), capability, &nonce, LocalApiProofAuthority::Onboarding)
        != LocalApiHealth::Compatible
    {
        return Err("설치 도우미 로컬 API의 보안 연결을 확인하지 못했습니다.".to_string());
    }

    let request = format!("{} {} HTTP/1.1\r\nHost: 127.0.0.1:3001\r\nX-AgentsToZ-Onboarding-Capability: {}\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n", method, path, capability, body.len());
    stream.write_all(request.as_bytes()).map_err(|error| error.to_string())?;
    stream.write_all(&body).map_err(|error| error.to_string())?;
    let response = read_local_api_json_response(
        &mut stream,
...
fn is_agent_runtime_task_id(value: &str) -> bool {
...
fn is_allowed_agent_runtime_route(method: &str, path: &str) -> bool {
    if path == "/api/agent-runtime/tester" { return method == "POST"; }
    if matches!(path, "/api/agent-runtime/terminals" | "/api/agent-runtime/terminals/access" | "/api/agent-runtime/terminals/memory" | "/api/agent-runtime/quick-labels" | "/api/agent-runtime/cs-duty") { return method == "POST"; }
    if matches!(path,
        "/api/agent-runtime/capabilities" | "/api/agent-runtime/readiness" |
        "/api/agent-runtime/targets" | "/api/agent-runtime/tasks"
    ) {
        return method == "GET";
    }
    if path == "/api/agent-runtime/tasks/start" {
        return method == "POST";
    }
    if matches!(path,
        "/api/agent-runtime/conversations" |
        "/api/agent-runtime/conversations/archived" |
        "/api/agent-runtime/conversations/capabilities"
    ) {
        return method == "GET";
    }
    if path == "/api/agent-runtime/conversations/start" {
        return method == "POST";
    }
    if let Some(remainder) = path.strip_prefix("/api/agent-runtime/conversations/") {
        if method == "GET" {
            if let Some(conversation_id) = remainder.strip_suffix("/question") {
                return is_agent_runtime_task_id(conversation_id);
            }
            let Some((route, query)) = remainder.split_once('?') else {
                return false;
            };
            let Some(conversation_id) = route.strip_suffix("/events") else {
                return false;
            };
            let Some(after) = query.strip_prefix("after=") else {
                return false;
            };
            return is_agent_runtime_task_id(conversation_id)
                && !after.is_empty()
                ...
                && (after == "0" || !after.starts_with('0'))
                && after.parse::<u64>().is_ok_and(|value| value <= 9_007_199_254_740_991);
        }
        if method != "POST" || remainder.contains('?') {
            return false;
        }
        let Some((conversation_id, action)) = remainder.split_once('/') else {
            return false;
        };
        return is_agent_runtime_task_id(conversation_id)
            && matches!(action,
                "continue" | "history" | "steer" | "interrupt" |
                "archive" | "unarchive" | "delete" | "question/answer"
            );
    }
    let Some(remainder) = path.strip_prefix("/api/agent-runtime/tasks/") else {
        return false;
    };
    if method == "POST" {
        let Some(task_id) = remainder.strip_suffix("/cancel") else {
            return false;
        };
        return is_agent_runtime_task_id(task_id);
    }
    if method != "GET" {
        return false;
    }
    ...
    let Some(task_id) = route.strip_suffix("/events") else {
        return false;
    };
    let Some(after) = query.strip_prefix("after=") else {
        return false;
    };
    is_agent_runtime_task_id(task_id)
        && !after.is_empty()
        && after.len() <= 16
        && after.bytes().all(|byte| byte.is_ascii_digit())
        && (after == "0" || !after.starts_with('0'))
...
fn agent_runtime_http_request(
    ...
    format!(
        "{} {} HTTP/1.1\r\nHost: {}\r\nOrigin: http://tauri.localhost\r\nAccept: application/json\r\n{}X-AgentsToZ-Agent-Runtime-Capability: {}\r\nContent-Length: {}\r\nConnection: close\r\n\r\n",
        method,
        path,
        LOCAL_API_ADDR,
        content_type,
...
fn agent_runtime_response_timeout(method: &str, path: &str, body: Option<&serde_json::Value>) -> Duration {
    // Both walk the same KakaoTalk probes, so the default 20s window is not enough for either.
    // Diagnose deliberately keeps going after a failed check, which makes it the slower of the two.
    if method == "POST" && path == "/api/agent-runtime/cs-duty" && matches!(body.and_then(|v| v.get("operation")).and_then(|v| v.as_str()), Some("enable") | Some("diagnose")) { return Duration::from_secs(75); }
    if method != "POST" || path != "/api/agent-runtime/terminals/memory" {
        return Duration::from_secs(20);
    }
    let operation = body.and_then(|value| value.as_object())
        .and_then(|value| value.get("automaticOperation"))
        .and_then(|value| value.as_str());
    Duration::from_secs(match operation {
        Some("execute-recovery") => 375,
        Some("prepare-provider" | "revalidate-provider" | "review-recovery" | "review-recovery-provider" | "verify-recovery-provider") => 75,
        _ => 20,
    })
}

fn authenticated_agent_runtime_request(
    capability: &str,
    path: &str,
    method: &str,
    payload: Option<serde_json::Value>,
) -> Result<serde_json::Value, String> {
    if !is_allowed_agent_runtime_route(method, path) {
        return Err("허용되지 않은 에이전트 런타임 요청입니다.".to_string());
    }
    let read_timeout = agent_runtime_response_timeout(method, path, payload.as_ref());
    let body = match (method, payload) {
        ("GET", None) => Vec::new(),
        ("POST", Some(value)) if value.is_object() => {
            serde_json::to_vec(&value).map_err(|error| error.to_string())?
        }
        _ => return Err("에이전트 런타임 요청 본문이 올바르지 않습니다.".to_string()),
    };
    if body.len() > AGENT_RUNTIME_REQUEST_MAX_BYTES {
        return Err("에이전트 런타임 요청이 너무 큽니다.".to_string());
    }
    let addr: SocketAddr = LOCAL_API_ADDR.parse()
        .map_err(|error| format!("로컬 API 주소 오류: {}", error))?;
    let mut stream = TcpStream::connect_timeout(&addr, Duration::from_secs(2))
        .map_err(|error| format!("AgentsToZ 로컬 API 연결 실패: {}", error))?;
    stream.set_read_timeout(Some(Duration::from_secs(20))).map_err(|error| error.to_string())?;
    stream.set_write_timeout(Some(Duration::from_secs(5))).map_err(|error| error.to_string())?;

    // Bind proof and use to one TCP peer. This authority is independent from
    // both What I said and QR remote-control capabilities.
    let nonce = new_local_api_health_nonce()
        .ok_or_else(|| "에이전트 런타임 보안 nonce를 만들지 못했습니다.".to_string())?;
    let health_request = local_api_health_request_with_connection(&nonce, "keep-alive")
        .ok_or_else(|| "에이전트 런타임 보안 확인 요청을 만들지 못했습니다.".to_string())?;
    stream.write_all(health_request.as_bytes()).map_err(|error| error.to_string())?;
    let health = read_local_api_json_response(&mut stream, 1024 * 1024)?;
    if classify_agent_runtime_api_health(Some(&health.raw), capability, &nonce)
        != LocalApiHealth::Compatible
    {
        return Err("에이전트 런타임 로컬 API의 보안 연결을 확인하지 못했습니다.".to_string());
    }

    // Keep the health proof at 20 seconds. Only an exact explicit provider
    // canary receives the longer response window after authenticating the peer.
    stream.set_read_timeout(Some(read_timeout)).map_err(|error| error.to_string())?;
    let request = agent_runtime_http_request(method, path, capability, body.len());
    stream.write_all(request.as_bytes()).map_err(|error| error.to_string())?;
    if !body.is_empty() {
        stream.write_all(&body).map_err(|error| error.to_string())?;
    }
    let response = read_local_api_json_response(&mut stream, AGENT_RUNTIME_RESPONSE_MAX_BYTES)?;
    Ok(serde_json::json!({
        "status": response.status,
        "body": response.body,
    }))
...
fn local_api_post_json(path: &str, payload: &serde_json::Value) -> Result<serde_json::Value, String> {
    ...
    // Claude Code remote-control may legitimately spend 30s establishing the
    // verified session and another few seconds opening its exact deep link.
    stream.set_read_timeout(Some(Duration::from_secs(45))).map_err(|e| e.to_string())?;
...
fn spawn_bundled_api_sidecar(app_handle: &tauri::AppHandle) -> Result<Child, String> {
    ...
    let what_i_said_capability = state
        .what_i_said_capability
        .lock()
        .unwrap_or_else(|error| error.into_inner())
        .clone()
        .ok_or_else(|| "What I said 보안 연결을 준비하지 못했습니다.".to_string())?;
    let remote_control_capability = state
        .remote_control_capability
        .lock()
        .unwrap_or_else(|error| error.into_inner())
        .clone()
        .ok_or_else(|| "원격 제어 보안 연결을 준비하지 못했습니다.".to_string())?;
    let agent_runtime_capability = state
    ...
    let onboarding_capability = state.onboarding_capability.lock()
        .unwrap_or_else(|error| error.into_inner()).clone()
        .ok_or_else(|| "설치 도우미 보안 연결이 준비되지 않았습니다.".to_string())?;
    let mut command = Command::new(&executable);
    command
        .env("PORTMGR_ONBOARDING_CAPABILITY", onboarding_capability)
        .env("API_PORT", "3001")
        .env("APP_DATA_DIR", &app_data_dir)
        .env("PORTMGR_BUNDLED_SIDECAR", "1")
        .env("PORTMGR_WHAT_I_SAID_CAPABILITY", what_i_said_capability)
        .env("PORTMGR_REMOTE_CONTROL_CAPABILITY", remote_control_capability)
        .env("PORTMGR_AGENT_RUNTIME_CAPABILITY", agent_runtime_capability)
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        command
            ...
            // Ordinary request-scoped Git and memory children inherit this
            // group. Agent Runtime deliberately creates its own guarded group
            // and is drained through the sidecar's parent-pipe shutdown.
            .process_group(0);
    }
    ...
    {
        let pid = child.id();
        let actual_process_group = i32::try_from(pid)
            .ok()
            .map(|pid| unsafe { libc::getpgid(pid) })
            .unwrap_or(-1);
        if pid <= 1 || actual_process_group != pid as i32 {
            let _ = child.kill();
            let _ = child.wait();
            return Err(format!(
                "번들된 로컬 API의 독립 프로세스 그룹을 확인하지 못했습니다 (pid={}, pgid={})",
                pid, actual_process_group
            ));
        }
    }
...
fn maintain_local_api_sidecar(app_handle: &tauri::AppHandle) {
    ...
    let capability = state.what_i_said_capability
        ...
        .unwrap_or_else(|error| error.into_inner())
        .clone();
    let health = capability.as_deref()
        .map(authenticated_local_api_health)
        .unwrap_or(LocalApiHealth::Unavailable);
    let owned_child_blocks_spawn = {
        let mut child_guard = state.api_sidecar.lock().unwrap_or_else(|e| e.into_inner());
        let mut clear_owned_child = false;
        let blocks_spawn = if let Some(child) = child_guard.as_mut() {
            ...
            match child.try_wait() {
                ...
                Ok(Some(status)) => {
                    #[cfg(not(target_os = "windows"))]
                    {
                        match terminate_and_confirm_local_api_process_group(process_group_id, None) {
                            Ok(()) => {
                                clear_owned_child = true;
                                false
                            }
                            Err(error) => {
                                eprintln!(
                                    "[API sidecar] exited with {}; old process group {} remains quarantined: {}",
                                    status, process_group_id, error
                                );
                                true
                            }
                        }
                ...
                Err(wait_error) => {
                    #[cfg(not(target_os = "windows"))]
                    {
                        match terminate_and_confirm_local_api_process_group(
                            process_group_id,
                            Some(child),
                        ) {
                            Ok(()) => {
                            ...
                            Err(cleanup_error) => {
                                eprintln!(
                                    "[API sidecar] could not inspect pid {} ({}); its process group remains quarantined: {}",
                                    process_group_id, wait_error, cleanup_error
                                );
                                true
                            }
                        }
    ...
    match local_api_supervisor_decision(health, owned_child_blocks_spawn, port_open) {
    ...
    // Serialize the final stop check and child installation with app shutdown.
    // If shutdown wins first, no replacement is created; if this block wins,
    // shutdown waits for the mutex and then terminates the exact new child.
    let mut child_guard = state.api_sidecar.lock().unwrap_or_else(|e| e.into_inner());
    if state.api_supervisor_stop.load(Ordering::SeqCst) || child_guard.is_some() {
        return;
    }
...
// The Bun sidecar owns a 24s hard ceiling that includes an in-flight runtime
// resolver plus provider-tree cleanup and durable journal close. Native must
// remain the outer backstop, never the earlier deadline.
const LOCAL_API_SIDECAR_SHUTDOWN_GRACE: Duration = Duration::from_secs(27);

#[cfg(not(target_os = "windows"))]
const LOCAL_API_PROCESS_GROUP_TERM_GRACE: Duration = Duration::from_millis(250);
#[cfg(not(target_os = "windows"))]
const LOCAL_API_PROCESS_GROUP_CONFIRM_GRACE: Duration = Duration::from_secs(2);

#[cfg(not(target_os = "windows"))]
fn wait_for_local_api_process_group_exit(
    process_group_id: u32,
    leader: &mut Option<&mut Child>,
    timeout: Duration,
) -> bool {
    let deadline = Instant::now() + timeout;
    loop {
        if let Some(child) = leader.as_mut() {
...
/// Remove the old sidecar's ordinary descendants and prove the negative PGID
/// is absent before the supervisor may create another writer authority.
#[cfg(not(target_os = "windows"))]
fn terminate_and_confirm_local_api_process_group(
    ...
    if process_group_id <= 1 {
        return Err("Refusing to clean an unsafe local API process group ID".to_string());
    }
    if wait_for_local_api_process_group_exit(
        process_group_id,
        &mut leader,
        Duration::ZERO,
    ) {
    ...
    if wait_for_local_api_process_group_exit(
        process_group_id,
        &mut leader,
        LOCAL_API_PROCESS_GROUP_TERM_GRACE,
    ) {
        return Ok(());
    }
    ...
    if wait_for_local_api_process_group_exit(
        process_group_id,
        &mut leader,
        LOCAL_API_PROCESS_GROUP_CONFIRM_GRACE,
    ) {
        return Ok(());
    }

    Err(format!(
        "old local API process group {} did not disappear after SIGKILL",
        process_group_id
    ))
}
...
fn terminate_local_api_sidecar(child: &mut Child) -> Result<(), String> {
    #[cfg(not(target_os = "windows"))]
    {
        // Give Bun's SIGTERM handler time to cancel supervised agent trees,
        // durably record their terminal state, close SQLite, and release its
        // exact-owner supervisor lease. Child::kill() sends SIGKILL on Unix
        // and previously made every normal app exit look like a crash.
        let pid = child.id();
        match child.try_wait() {
            Ok(Some(_)) => {
                return terminate_and_confirm_local_api_process_group(pid, None);
            }
            Err(_) => {
                return terminate_and_confirm_local_api_process_group(pid, Some(child));
            }
            Ok(None) => {}
        }
        let term_sent = pid > 1
        ...
        if term_sent {
            let deadline = Instant::now() + LOCAL_API_SIDECAR_SHUTDOWN_GRACE;
            loop {
                match child.try_wait() {
                    Ok(Some(_)) => {
                        return terminate_and_confirm_local_api_process_group(pid, None);
    ...
    #[cfg(target_os = "windows")]
    {
        // Windows has no portable SIGTERM equivalent for this no-window child.
        // Full sidecar containment there requires the native host to own a Job
        // Object; do not claim POSIX process-group guarantees on this branch.
...
fn shutdown_local_api_sidecar(app_handle: &tauri::AppHandle) {
    let state = app_handle.state::<AppState>();
    state.api_supervisor_stop.store(true, Ordering::SeqCst);
    let mut child_guard = state.api_sidecar.lock().unwrap_or_else(|e| e.into_inner());
    if let Some(mut child) = child_guard.take() {
        #[cfg(not(target_os = "windows"))]
        while let Err(error) = terminate_local_api_sidecar(&mut child) {
            // App exit is fail-closed: keep retrying the old group rather than
            // abandoning a request-scoped writer that could outlive its lease.
            eprintln!("[API sidecar] shutdown remains quarantined: {}", error);
            std::thread::sleep(Duration::from_millis(250));
        }
        #[cfg(target_os = "windows")]
        if let Err(error) = terminate_local_api_sidecar(&mut child) {
            eprintln!("[API sidecar] Windows sidecar shutdown failed: {}", error);
        }
    }
}
...
fn lock_owner_pid(owner: &str) -> Option<u32> {
    ...
    if parts.next()? != "v2" { return None; }
    let pid = parts.next()?.parse::<u32>().ok()?;
    let nonce = parts.next()?;
    if pid == 0 || parts.next().is_some() || nonce.len() != 32
...
#[cfg(unix)]
fn lock_owner_process_alive(owner: &str) -> bool {
    let Some(pid) = lock_owner_pid(owner) else { return false; };
    let result = unsafe { libc::kill(pid as libc::pid_t, 0) };
    if result == 0 { return true; }
    match std::io::Error::last_os_error().raw_os_error() {
        Some(libc::ESRCH) => false,
        Some(libc::EPERM) => true,
        // An unknown process-query failure is not permission to delete a lock.
        _ => true,
    }
}
...
fn lock_owner_process_alive(owner: &str) -> bool {
        ...
        .output()
    {
        ...
        // If Windows cannot answer, fail closed and leave recovery to restart.
        _ => true,
    }
}
...
fn acquire_ports_file_lock(lock_path: &std::path::Path) -> Result<PortsFileLock, String> {
    ...
    getrandom::fill(&mut owner_bytes)
        .map_err(|error| format!("ports.json 잠금 소유자 생성 실패: {}", error))?;
    let nonce: String = owner_bytes.iter().map(|byte| format!("{:02x}", byte)).collect();
    let owner = format!("v2:{}:{}", std::process::id(), nonce);
    for _ in 0..150 {
        match fs::OpenOptions::new().write(true).create_new(true).open(lock_path) {
            Ok(mut file) => {
                if let Err(error) = file.write_all(owner.as_bytes()).and_then(|_| file.sync_all()) {
                    let _ = fs::remove_file(lock_path);
                    return Err(format!("ports.json 잠금 소유자 저장 실패: {}", error));
                }
                return Ok(PortsFileLock { path: lock_path.to_path_buf(), owner });
            }
            Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
                let observed_owner = fs::read_to_string(lock_path).ok();
                let stale = fs::metadata(lock_path)
                    ...
                    .unwrap_or(false);
                if stale && observed_owner.as_deref()
                    .map(|owner| !lock_owner_process_alive(owner))
                    .unwrap_or(false)
                {
                    if observed_owner.is_some()
                        && fs::read_to_string(lock_path).ok() == observed_owner {
                        let _ = fs::remove_file(lock_path);
                    }
                    continue;
                }
...
fn record_changed_from_base(base: &serde_json::Value, desired: &serde_json::Value) -> bool {
    ...
    keys.into_iter().any(|key| {
        key != "id" && !(key == "role" && !desired_object.contains_key(key))
            && base_object.get(key) != desired_object.get(key)
    })
}
...
fn merge_record_fields(
    ...
    let keys: HashSet<&String> = base_object.keys().chain(desired_object.keys()).collect();
    for key in keys {
        if key == "role" && !desired_object.contains_key(key) { continue; }
...
fn merge_port_snapshots(
    ...
    for desired_value in desired {
        ...
        match (base_by_id.get(id), current_by_id.get(id)) {
            (None, current_value) => {
                merged_by_id.insert(id.to_string(), preserve_omitted_role(desired_value, current_value.copied()));
            }
...
fn merge_legacy_port_save(
    ...
    let current_by_id: HashMap<&str, &serde_json::Value> =
        current.iter().filter_map(|value| port_id(value).map(|id| (id, value))).collect();
    desired.iter().map(|value| preserve_omitted_role(
        value, port_id(value).and_then(|id| current_by_id.get(id).copied()),
    ))
...
fn preserve_omitted_role(desired: &serde_json::Value, current: Option<&serde_json::Value>) -> serde_json::Value {
    let mut result = desired.clone();
    if let Some(object) = result.as_object_mut() {
        if !object.contains_key("role") {
            if let Some(role) = current.and_then(|value| value.get("role")) {
                object.insert("role".into(), role.clone());
            }
        }
    }
    result
...
fn validate_role_changes(merged: &[serde_json::Value], current: &[serde_json::Value]) -> Result<(), String> {
    let current_by_id: HashMap<&str, &serde_json::Value> =
        current.iter().filter_map(|value| port_id(value).map(|id| (id, value))).collect();
    for value in merged {
        if let Some(role) = value.get("role") {
            let previous = port_id(value).and_then(|id| current_by_id.get(id)).and_then(|row| row.get("role"));
            if !matches!(role.as_str(), Some("ops" | "dev" | "managed")) && Some(role) != previous {
                return Err("PROJECT_ROLE_INVALID".into());
            }
        }
    }
    Ok(())
...
fn save_ports(app_handle: tauri::AppHandle, ports: Vec<serde_json::Value>) -> Result<(), String> {
    ...
    validate_role_changes(&merged, &current)?;
...
fn load_portal(app_handle: tauri::AppHandle) -> Result<serde_json::Value, String> {
    let app_data_dir = ensure_app_data_dir(&app_handle)?;
    let _lock = acquire_portal_file_lock(&app_data_dir)?;
    ...
    let metadata_file = app_data_dir.join("portal-local-metadata.json");
    if metadata_file.exists() {
        let content = fs::read_to_string(&metadata_file).map_err(|e| e.to_string())?;
        let metadata: serde_json::Value = serde_json::from_str(&content).map_err(|e| e.to_string())?;
        if !metadata.is_object() {
            return Err("portal-local-metadata.json 형식이 객체가 아닙니다.".into());
        }
        merge_portal_local_metadata(&mut value, &metadata);
    }
    Ok(value)
}

fn normalized_portal_local_ids(value: Option<&serde_json::Value>) -> serde_json::Value {
    ...
    let values: Vec<serde_json::Value> = value
        ...
        .collect();
    // These IDs are deletion authority, not an LRU cache. Dropping an older
    // marker just because the list grew lets a later Pull/auto-push recreate a
    // project the user removed. A separate verified compactor may eventually
    // discard IDs that are absent both locally and remotely; normalization
    // must preserve every valid unique marker.
    serde_json::Value::Array(values)
}

fn merge_portal_local_metadata(target: &mut serde_json::Value, metadata: &serde_json::Value) {
    ...
    for field in [
        ...
        target.insert(
            field.to_string(),
            normalized_portal_local_ids(metadata.get(field)),
        );
    }
}

fn mutate_portal_local_metadata(
    ...
    ) || !matches!(mode, "add" | "remove")
    {
        return Err("포털 로컬 메타데이터 변경 요청이 올바르지 않습니다.".into());
    }
    let requested = ids
        .as_array()
        .filter(|values| values.len() <= 2_048)
        .ok_or_else(|| "포털 로컬 메타데이터 ID 목록이 올바르지 않습니다.".to_string())?;
    let requested = normalized_portal_local_ids(Some(&serde_json::Value::Array(requested.clone())))
        .as_array()
        .cloned()
        .unwrap_or_default();
    let mut current = normalized_portal_local_ids(metadata.get(field))
        .as_array()
        .cloned()
        .unwrap_or_default();
    if mode == "add" {
        ...
        let removed: HashSet<String> = requested
            .into_iter()
            .filter_map(|value| value.as_str().map(str::to_string))
            .collect();
        current.retain(|value| value.as_str().map(|id| !removed.contains(id)).unwrap_or(false));
    }
    metadata.as_object_mut()
        .ok_or_else(|| "포털 로컬 메타데이터 형식이 객체가 아닙니다.".to_string())?
        .insert(field.to_string(), serde_json::Value::Array(current));
    Ok(())
}

fn write_json_atomic(path: &std::path::Path, value: &serde_json::Value) -> Result<(), String> {
    let parent = path.parent().ok_or_else(|| "portal.json 경로가 올바르지 않습니다.".to_string())?;
    let file_name = path.file_name().and_then(|name| name.to_str()).unwrap_or("portal.json");
    let temporary = parent.join(format!(
        ".{}.{}.{}.tmp",
        file_name,
        std::process::id(),
        NEXT_PORTAL_FILE_WRITE_GENERATION.fetch_add(1, Ordering::Relaxed),
    ));
...
fn acquire_portal_file_lock(app_data_dir: &std::path::Path) -> Result<PortsFileLock, String> {
    acquire_ports_file_lock(&app_data_dir.join("portal.json.lock"))
        .map_err(|error| error.replace("ports.json", "portal.json"))
}

#[tauri::command]
fn save_portal(app_handle: tauri::AppHandle, mut data: serde_json::Value) -> Result<(), String> {
    let app_data_dir = ensure_app_data_dir(&app_handle)?;
    let _lock = acquire_portal_file_lock(&app_data_dir)?;
    let file = app_data_dir.join("portal.json");
    let metadata_file = app_data_dir.join("portal-local-metadata.json");
    let authoritative = if metadata_file.exists() {
        let content = fs::read_to_string(&metadata_file).map_err(|e| e.to_string())?;
        serde_json::from_str(&content).map_err(|e| e.to_string())?
    } else if file.exists() {
        let content = fs::read_to_string(&file).map_err(|e| e.to_string())?;
    ...
    merge_portal_local_metadata(&mut data, &authoritative);
    write_json_atomic(&file, &data)
}

#[tauri::command]
fn save_portal_local_metadata(
    app_handle: tauri::AppHandle,
    field: String,
    ids: serde_json::Value,
    mode: String,
    ...
    let _lock = acquire_portal_file_lock(&app_data_dir)?;
    let metadata_file = app_data_dir.join("portal-local-metadata.json");
    let portal_file = app_data_dir.join("portal.json");
    let authoritative = if metadata_file.exists() {
        let content = fs::read_to_string(&metadata_file).map_err(|e| e.to_string())?;
        serde_json::from_str(&content).map_err(|e| e.to_string())?
    } else if portal_file.exists() {
        let content = fs::read_to_string(&portal_file).map_err(|e| e.to_string())?;
        serde_json::from_str(&content).map_err(|e| e.to_string())?
    } else {
        serde_json::json!({})
    };
    ...
    merge_portal_local_metadata(&mut metadata, &authoritative);
    mutate_portal_local_metadata(&mut metadata, &field, &ids, &mode)?;
    write_json_atomic(&metadata_file, &metadata)?;

    let mut portal = if portal_file.exists() {
        let content = fs::read_to_string(&portal_file).map_err(|e| e.to_string())?;
        serde_json::from_str(&content).map_err(|e| e.to_string())?
    } else {
        serde_json::json!({ "items": [], "categories": [] })
    };
    merge_portal_local_metadata(&mut portal, &metadata);
    write_json_atomic(&portal_file, &portal)?;
    Ok(metadata)
}
...
fn clone_staging_path(target: &std::path::Path) -> Result<std::path::PathBuf, String> {
    let parent = target
        .parent()
        .filter(|candidate| candidate.is_dir())
        .ok_or_else(|| "대상 폴더의 상위 작업 폴더가 존재하지 않습니다".to_string())?;
    let name = target
        .file_name()
        .and_then(|value| value.to_str())
        .filter(|value| !value.is_empty())
        .ok_or_else(|| "대상 폴더 이름이 올바르지 않습니다".to_string())?;
...
fn clone_repository(repository_url: String, folder_path: String) -> Result<String, String> {
    ...
    // Clone into a hidden sibling and publish with one same-filesystem rename.
    // If the app/host is killed mid-clone, the user-facing final path remains free
    // so retry is possible; only an unmistakable staging residue may remain.
    let staging_path = clone_staging_path(path)?;
    ...
    if let Err(error) = fs::rename(&staging_path, path) {
        let _ = fs::remove_dir_all(&staging_path);
        return Err(format!("복제는 끝났지만 대상 폴더로 확정하지 못했습니다: {}", error));
    }
...
fn open_code_app(agent: String, folder_path: String) -> Result<String, String> {
    ...
    // All three app routes go through the localhost sidecar. Codex and Hermes
    // must first resolve the newest session bound to this exact project; a
    // generic `threads/new?path=` or project-only launch would silently create
    // a new task or restore whichever tab happened to be frontmost.
    let response = local_api_post_json(
        "/api/open-code-app",
        &serde_json::json!({ "agent": agent, "folderPath": folder_path }),
    )?;
    if response.get("success").and_then(|value| value.as_bool()) != Some(true) {
        return Err(response.get("error").and_then(|value| value.as_str())
            .unwrap_or("코드 앱 실행을 확인하지 못했습니다.").to_string());
    }
    if agent == "claude" {
        let reused = response.get("reusedActiveSession").and_then(|value| value.as_bool()) == Some(true);
        return Ok(if reused {
            format!("기존 Claude Code 대화를 다시 열었습니다: {}", folder_path)
        } else {
            format!("Claude Code 대화를 시작하고 앱에서 열었습니다: {}", folder_path)
        });
    }
    if agent == "hermes"
        && response.get("selectionVerified").and_then(|value| value.as_bool()) == Some(false)
    {
        return Ok(format!(
            "Hermes Desktop 실행을 확인하고 최근 프로젝트 대화를 여는 요청을 전달했습니다: {}",
            folder_path
        ));
    }
    Ok(format!("가장 최근 {} 프로젝트 대화를 다시 열었습니다: {}", agent, folder_path))
}
...
#[tauri::command]
fn open_preferred_browser(url: String, browser: String, profile_directory: Option<String>) -> Result<String, String> {
    let parsed = tauri::Url::parse(&url).map_err(|_| "올바른 웹 주소가 필요합니다.".to_string())?;
    if !matches!(parsed.scheme(), "http" | "https") || url.chars().any(|c| c.is_control()) {
        return Err("http 또는 https 주소만 열 수 있습니다.".to_string());
    }
    if !matches!(browser.as_str(), "ego-lite" | "chrome" | "system") {
        return Err("지원하지 않는 브라우저입니다.".to_string());
    }
    if profile_directory.is_some() && browser != "chrome" {
        return Err("Chrome 프로필은 Chrome에서만 사용할 수 있습니다.".to_string());
    }
    if profile_directory.is_some() {
        return open_in_chrome(url, profile_directory);
    }
    #[cfg(target_os = "macos")]
    {
        let mut command = Command::new("/usr/bin/open");
        match browser.as_str() {
            "ego-lite" => { command.args(["-a", "ego lite"]); }
            "chrome" => { command.args(["-a", "Google Chrome"]); }
            _ => {}
        }
        let output = command.arg(parsed.as_str()).output().map_err(|e| e.to_string())?;
        if !output.status.success() {
            return Err("선택한 브라우저를 열지 못했습니다. 설치 여부를 확인하거나 앱 설정에서 브라우저를 변경해주세요.".to_string());
        }
    }
    #[cfg(not(target_os = "macos"))]
    {
        if browser == "ego-lite" {
            return Err("Ego Lite 연결은 현재 macOS에서 지원합니다. 앱 설정에서 Chrome 또는 시스템 기본 브라우저를 선택해주세요.".to_string());
        }
        if browser == "chrome" { return open_in_chrome(url, None); }
        #[cfg(target_os = "windows")]
        let mut command = { let mut cmd = Command::new("rundll32.exe"); cmd.arg("url.dll,FileProtocolHandler"); cmd };
        #[cfg(not(target_os = "windows"))]
        let mut command = Command::new("xdg-open");
        command.arg(parsed.as_str()).spawn().map(reap_detached).map_err(|e| e.to_string())?;
    }
    Ok("브라우저에서 열었습니다.".to_string())
}
...
struct WorktreeInfo {
    ...
    /// `no-origin` is a successful local refresh with no remote capability, not an error.
    #[serde(rename = "remoteRefreshState", default, skip_serializing_if = "Option::is_none")]
    remote_refresh_state: Option<String>,
...
fn git_worktree_add(folder_path: String, branch_name: String, worktree_path: Option<String>, orca_managed: Option<bool>) -> Result<serde_json::Value, String> {
    ...
    if orca_managed.unwrap_or(false) {
        ...
        if !visible {
            ...
            // Hooks or a newly opened terminal can write after creation but
            // before this rollback. Keep Orca's final dirty-file guard active.
            let _ = orca_run_json(&cli, &["worktree", "rm", "--worktree", &selector], ORCA_TIMEOUT_MS);
...
fn git_worktree_remove(folder_path: String, worktree_path: String, orca_managed: Option<bool>) -> Result<(), String> {
    ...
    if orca_managed.unwrap_or(false) {
        let removed_by_orca = resolve_orca_cli().is_some_and(|cli| {
            ...
            orca_run_json_retry(
                &cli,
                // The status check above is intentionally only a friendly
                // early error. Orca must perform the final non-force guard so
                // files created in the intervening window are not discarded.
                &["worktree", "rm", "--worktree", &selector],
                2,
                20_000,
                500,
            )
...
fn fetch_origin_with_timeout(folder_path: &str) -> Result<&'static str, String> {
    ...
    loop {
        match child.try_wait() {
            Ok(Some(status)) if status.success() => return Ok("fetched"),
...
fn list_git_worktrees(folder_path: String, fetch_remote: Option<bool>) -> Result<Vec<WorktreeInfo>, String> {
    let (remote_refresh_state, remote_refresh_error) = if fetch_remote.unwrap_or(false) {
        match fetch_origin_with_timeout(&folder_path) {
            Ok(state) => (Some(state.to_string()), None),
        ...
        if line.starts_with("worktree ") {
            if let Some(path) = current_path.take() {
                ...
                worktrees.push(WorktreeInfo {
                    ...
                    remote_refresh_state: remote_refresh_state.clone(),
                });
...
fn ensure_cmux_window(cli: &str) -> Result<(), String> {
    let out = Command::new(cli)
        .args(["list-windows"])
        .output()
        .map_err(|error| cmux_access_help_msg(&format!("cmux 창 목록 확인 실패: {}", error)))?;
    if !out.status.success() {
        let stderr = String::from_utf8_lossy(&out.stderr).trim().to_string();
        return Err(cmux_access_help_msg(&format!("cmux 창 목록 확인 실패: {}", stderr)));
    }
    if String::from_utf8_lossy(&out.stdout).trim() != "No windows" {
        return Ok(());
    }
    let created = Command::new(cli)
        .args(["new-window"])
        .output()
        .map_err(|error| cmux_access_help_msg(&format!("cmux 창 생성 실패: {}", error)))?;
    if !created.status.success() {
        let stderr = String::from_utf8_lossy(&created.stderr).trim().to_string();
        return Err(cmux_access_help_msg(&format!("cmux 창 생성 실패: {}", stderr)));
    }
    std::thread::sleep(std::time::Duration::from_millis(500));
    Ok(())
}
...
fn orca_paths_match(left: &str, right: &str) -> bool {
    let left = resolve_orca_project_path(left.to_string());
    let right = resolve_orca_project_path(right.to_string());
...
/// A valid Git worktree may be selectable by Orca while remaining hidden from
/// its sidebar. A terminal created against that selector can then exist only in
/// the background. `None` listing means the Orca lookup failed, not that the
/// worktree is absent, so callers retain the exact-selector attempt in that case.
fn orca_external_worktree_requires_floating(
    repo_path: Option<&str>,
    worktree_path: Option<&str>,
    listing: Option<&serde_json::Value>,
) -> bool {
    let Some(worktree_path) = worktree_path else { return false; };
    if repo_path.is_some_and(|repo_path| orca_paths_match(repo_path, worktree_path)) {
        return false;
    }
    if has_hidden_orca_path_segment(worktree_path) {
        return true;
    }
    let Some(listing) = listing else { return false; };
    !listing.pointer("/result/worktrees")
        .and_then(|value| value.as_array())
        .map(|worktrees| worktrees.iter().any(|worktree| {
            worktree.get("path").and_then(|value| value.as_str())
                .is_some_and(|path| orca_paths_match(path, worktree_path))
        }))
        .unwrap_or(false)
}

fn orca_floating_compatibility_notice(worktree_path: &str) -> String {
    format!(
        "⚠ Orca가 이 외부 워크트리를 사이드바 카드로 관리하지 않아, 정확한 폴더의 Floating Terminal로 열었습니다. ({})\n워크트리와 브랜치는 바꾸지 않았습니다.",
        worktree_path,
    )
}
...
fn open_cmux_claude(name: String, folder_path: Option<String>, worktree_path: Option<String>, bypass: bool) -> Result<String, String> {
    ...
    let claude_bin = resolve_agent_bin("claude");
    if !std::path::Path::new(&claude_bin).is_file() {
        return Err("Claude CLI 실행 파일을 찾을 수 없습니다. 설치 후 다시 시도해주세요.".into());
    }
    let claude_cli = cmux_agent_command(
        &claude_bin,
...
fn open_cmux_codex(name: String, folder_path: Option<String>, worktree_path: Option<String>, bypass: bool) -> Result<String, String> {
    let agent_bin = resolve_agent_bin("codex");
    if !std::path::Path::new(&agent_bin).is_file() {
        return Err("Codex CLI 실행 파일을 찾을 수 없습니다. 설치 후 다시 시도해주세요.".into());
    }
...
fn open_cmux_agy(name: String, folder_path: Option<String>, worktree_path: Option<String>, bypass: bool) -> Result<String, String> {
    let agent_bin = resolve_agent_bin("agy");
    if !std::path::Path::new(&agent_bin).is_file() {
        return Err("Antigravity CLI 실행 파일을 찾을 수 없습니다. 설치 후 다시 시도해주세요.".into());
    }
    let agent_cli = cmux_agent_command(
        &agent_bin,
        if bypass { "--dangerously-skip-permissions" } else { "" },
...
fn open_cmux_hermes(name: String, folder_path: Option<String>, worktree_path: Option<String>, bypass: bool) -> Result<String, String> {
    let _ = bypass;
    let agent_bin = resolve_agent_bin("hermes");
    if !std::path::Path::new(&agent_bin).is_file() {
        return Err("Hermes CLI 실행 파일을 찾을 수 없습니다. 설치 후 다시 시도해주세요.".into());
    }
    let agent_cli = cmux_agent_command(&agent_bin, "");
...
fn open_cmux_tmux(name: String, folder_path: Option<String>, worktree_path: Option<String>, bypass: bool, fresh: bool) -> Result<String, String> {
    ...
    let tmux_bin = resolve_agent_bin("tmux");
    if !std::path::Path::new(&tmux_bin).is_file() {
        return Err("tmux 실행 파일을 찾을 수 없습니다. tmux를 설치한 뒤 다시 시도하세요.".into());
    }
    let claude_cli = cmux_agent_command(
        &claude_bin,
        if bypass { "--dangerously-skip-permissions" } else { "" },
    );
    let tmux_cli = shell_quote_if_needed(&tmux_bin);
    ...
    let escaped_session = escape_sq(&session_name);
    let escaped_cd_path = escape_sq(&cd_path);
    let tmux_cmd = if fresh {
        format!("{tmux_cli} kill-session -t '{escaped_session}' 2>/dev/null; {tmux_cli} new-session -s '{escaped_session}' -c '{escaped_cd_path}' {claude_cli}")
    } else {
        format!("{tmux_cli} new-session -A -s '{escaped_session}' -c '{escaped_cd_path}' {claude_cli}")
    };
...
/// `{ok:false}`는 exit 0으로도, exit 1 + stdout으로도 온다(Orca 1.4.x) — 두 경우 모두 JSON 오류를 읽는다.
...
fn orca_run_json(cli: &str, args: &[&str], timeout_ms: u64) -> Result<serde_json::Value, String> {
    ...
    if !out.status.success() {
        return Err(orca_cli_failure_message(&stdout, &String::from_utf8_lossy(&out.stderr)));
    }
    ...
    if parsed.get("ok").and_then(|v| v.as_bool()) == Some(false) {
        return Err(orca_json_error_message(&parsed));
    }
    Ok(parsed)
}

/// Keep `code` next to `message`: callers match on codes such as
/// `selector_not_found` and `terminal_handle_stale`, which a message may omit.
fn orca_json_error_message(parsed: &serde_json::Value) -> String {
    let field = |pointer: &str| parsed.pointer(pointer)
        .and_then(|value| value.as_str())
        .map(str::trim)
        .filter(|value| !value.is_empty());
    match (field("/error/code"), field("/error/message")) {
        (Some(code), Some(message)) if message.contains(code) => message.to_string(),
...
/// Orca 1.4.x exits non-zero *and* prints its `{ok:false}` document on stdout,
/// leaving stderr empty. Read that document first: collapsing it to "unknown"
/// hid `terminal_handle_stale`, so one closed floating tab blocked every later
/// launch of that agent with "탭을 확인하지 못했습니다 (unknown)".
fn orca_cli_failure_message(stdout: &str, stderr: &str) -> String {
    if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(stdout.trim()) {
        if parsed.get("ok").and_then(|v| v.as_bool()) == Some(false) {
            return orca_json_error_message(&parsed);
        }
    }
    let stderr = stderr.trim();
    if stderr.is_empty() { "unknown".to_string() } else { stderr.to_string() }
}
...
fn orca_create_floating_terminal(cli: &str, title: &str, initial_command: Option<&str>) -> Result<String, String> {
    let mut args = vec![
        "terminal", "create",
    ...
    // Orca 1.4.180 requires explicit focus. Otherwise the terminal is
    // created behind the currently selected project's Floating tab.
    args.push("--focus");
    let created = orca_run_json_retry(
        cli,
        &args,
...
fn orca_managed_floating_terminal_marker(agent: &str, folder_path: &str) -> String {
    ...
    // Keep this in byte-for-byte parity with src/orcaFloatingTerminal.ts. The
    // Hermes revision prevents an already-running pre-fix tab (whose process
    // environment cannot be changed) from being reused after an update.
    let launch_revision = if agent == "hermes" { "\0no-osc11-v1" } else { "" };
    let payload = format!("{}\0{}{}", agent, normalized_path, launch_revision);
...
fn find_orca_floating_terminal_handle_in_value(value: &serde_json::Value, handle: &str) -> bool {
    match value {
        ...
        serde_json::Value::Object(fields) => {
            ...
            let is_live = fields.get("connected").and_then(|value| value.as_bool()) != Some(false)
                && fields.get("orphaned").and_then(|value| value.as_bool()) != Some(true);
            if matches_handle && is_floating && is_live {
                return true;
            }
            fields
                .values()
    ...
    match value {
        ...
        serde_json::Value::Object(fields) => {
            ...
            if let Some(handle) = handle.filter(|value| !value.trim().is_empty()) {
                if worktree_id == Some("global-floating-terminal") && is_live {
...
fn orca_create_worktree_terminal(cli: &str, title: &str, worktree_path: &str, initial_command: Option<&str>) -> Result<String, String> {
    let selector = format!("path:{}", worktree_path);
    let mut args = vec!["terminal", "create", "--worktree", &selector, "--title", title];
    if let Some(command) = initial_command {
        args.extend(["--command", command]);
    }
    args.push("--focus");
...
fn open_orca_agent(agent: Option<String>, name: Option<String>, folder_path: Option<String>, worktree_path: Option<String>, bypass: Option<bool>, floating: Option<bool>, new_window: Option<bool>) -> Result<String, String> {
    ...
    let (label, managed_agent): (&str, &str) = match agent_kind {
        "codex" => ("Codex", "codex"),
        "agy" => ("Antigravity", "agy"),
    ...
    // Git owns worktree identity. Orca-created worktrees remain in the sidebar;
    // AgentsToZ/Codex/plain-git worktrees that Orca hides or omits open in the
    // visible Floating surface while preserving the exact selected cwd.
    if !use_floating {
        let listing = if wt_first.as_deref().is_some_and(has_hidden_orca_path_segment) {
            None
        } else {
            orca_run_json_retry(&cli, &["worktree", "list"], 2, 8_000, 400).ok()
        };
        if orca_external_worktree_requires_floating(
            repo_path.as_deref(),
            wt_first.as_deref(),
            listing.as_ref(),
        ) {
            use_floating = true;
            fallback_notice = Some(orca_floating_compatibility_notice(&cd_path));
        }
    }
    ...
    let command_for_surface = |use_wsl_shell: bool| -> Option<String> {
        let agent_cmd = |bin: &str, flag: &str| orca_agent_command(bin, flag, use_bypass, use_wsl_shell);
        let command = match agent_kind {
            "codex" => agent_cmd("codex", "--dangerously-bypass-approvals-and-sandbox"),
            "agy" => agent_cmd("agy", "--dangerously-skip-permissions"),
            "hermes" => agent_cmd("hermes", ""),
            "agents" => agent_cmd("claude", "--dangerously-skip-permissions").map(|command| format!("{} agents", command)),
            "terminal" => None,
            _ => agent_cmd("claude", "--dangerously-skip-permissions"),
        };
        // Hermes' documented theme override short-circuits its OSC 11 probe.
        // Orca's delayed OSC reply otherwise appears as `]11;rgb:...` in the
        // prompt. Windows worktree terminals use cmd; Floating uses WSL.
        command.map(|value| {
            if agent_kind != "hermes" {
                value
            } else if cfg!(target_os = "windows") && !use_wsl_shell {
                format!("set \"HERMES_TUI_THEME=dark\" && {}", value)
            } else {
                format!("env HERMES_TUI_THEME=dark {}", value)
            }
        })
    };
    let mut use_wsl_shell = cfg!(target_os = "windows") && use_floating;
    let mut cmd = command_for_surface(use_wsl_shell);
        ...
        if let Some(record) = remembered {
            match validate_remembered_orca_floating_terminal(&cli, &record.handle) {
                RememberedOrcaFloatingTerminalValidation::Valid => {
                    ...
                    let fallback_suffix = fallback_notice.as_ref()
                        .map(|notice| format!("\n{}", notice))
                        .unwrap_or_default();
                    return Ok(format!(
    ...
    // `terminal send` can overtake a new shell's startup OSC colour query. Its
    // delayed OSC 11 response then becomes visible text in Hermes. On Unix,
    // give Orca the agent command during terminal creation so it starts only
    // after shell bootstrap. Windows keeps the post-create path because the
    // returned terminal host decides whether quoting must target cmd or WSL.
    let initial_terminal_command = if !cfg!(target_os = "windows") && cmd.is_some() {
        Some(build_orca_terminal_command(&cd_path, cmd.as_deref(), use_wsl_shell))
    } else {
        None
    };
    let handle = if use_floating {
        orca_create_floating_terminal(&cli, &terminal_title, initial_terminal_command.as_deref())?
    } else {
        match orca_create_worktree_terminal(&cli, &terminal_title, &cd_path, initial_terminal_command.as_deref()) {
            Ok(h) => h,
            Err(e) if orca_should_fall_back_to_floating(&e) => {
                use_floating = true;
                use_wsl_shell = cfg!(target_os = "windows");
                cmd = command_for_surface(use_wsl_shell);
    ...
    if initial_terminal_command.is_none() {
...
fn open_orca_localhost(port: u16, folder_path: Option<String>, worktree_path: Option<String>, floating: Option<bool>) -> Result<String, String> {
    ...
    // global-floating-terminal은 터미널 전용이라 browser `tab create` selector로는
    // selector_not_found가 난다. 브라우저는 항상 사용자가 누른 구체 경로에 연결한다.
    let requested_floating = floating.unwrap_or(false);
    let repo_path = folder_path.as_deref().map(str::trim).filter(|s| !s.is_empty())
        .map(expand).map(resolve_orca_project_path);
    let wt_first = first_worktree(&worktree_path).map(|s| resolve_orca_project_path(expand(&s)));
    ...
    Ok(format!(
        "Orca 프로젝트에서 localhost:{} 열림{}",
        port,
        if requested_floating {
...
pub fn run() {
  ...
  tauri::Builder::default()
    .manage(AppState {
        ...
        what_i_said_capability: Mutex::new(None),
        remote_control_capability: Mutex::new(None),
        onboarding_capability: Mutex::new(None),
        agent_runtime_capability: Mutex::new(None),
    })
    .invoke_handler(tauri::generate_handler![
        ...
        // Mutating Git/worktree commands intentionally are not exposed as
        // direct Tauri invokes. The React client routes them through the
        // localhost sidecar so they share its cross-process Workspace Lease
        // with managed Agent Runtime tasks. Keeping the native helpers below
        // preserves their unit-tested implementation while preventing a
        // second, uncoordinated mutation surface.
        check_file_exists,
        suggest_name,
        suggest_names_batch,
        open_cmux_claude,
        ...
        set_global_shortcut,
        get_platform,
        what_i_said_management_request,
        remote_control_management_request,
        agent_runtime_request,
        onboarding_management_request,
        #[cfg(target_os = "macos")]
        macos_runtime_client_bridge::agent_runtime_native_broker_status,
        #[cfg(target_os = "macos")]
        macos_runtime_client_bridge::agent_runtime_native_broker_register,
        #[cfg(target_os = "macos")]
        macos_runtime_client_bridge::agent_runtime_native_broker_unregister,
        #[cfg(target_os = "macos")]
        macos_runtime_client_bridge::agent_runtime_native_broker_probe,
        #[cfg(target_os = "macos")]
        macos_runtime_client_bridge::agent_runtime_native_broker_dedicated_identity_fixture,
        #[cfg(target_os = "macos")]
        macos_runtime_client_bridge::agent_runtime_native_broker_provision_dedicated_identity,
    ])
    ...
    .setup(|app| {
      ...
      let capability = load_or_create_what_i_said_capability(app.handle())
        .map_err(|message| std::io::Error::new(std::io::ErrorKind::Other, message))?;
      *app.state::<AppState>()
        .what_i_said_capability
        .lock()
        .unwrap_or_else(|error| error.into_inner()) = Some(capability);
      let remote_control_capability = load_or_create_remote_control_capability(app.handle())
        .map_err(|message| std::io::Error::new(std::io::ErrorKind::Other, message))?;
      *app.state::<AppState>()
        .remote_control_capability
        .lock()
        .unwrap_or_else(|error| error.into_inner()) = Some(remote_control_capability);
      let onboarding_capability = load_or_create_onboarding_capability(app.handle())
        .map_err(|message| std::io::Error::new(std::io::ErrorKind::Other, message))?;
      *app.state::<AppState>().onboarding_capability.lock()
        .unwrap_or_else(|error| error.into_inner()) = Some(onboarding_capability);
      let agent_runtime_capability = load_or_create_agent_runtime_capability(app.handle())
        .map_err(|message| std::io::Error::new(std::io::ErrorKind::Other, message))?;
      *app.state::<AppState>()
        .agent_runtime_capability
        .lock()
        .unwrap_or_else(|error| error.into_inner()) = Some(agent_runtime_capability);
      // `tauri dev` already owns one supervised source API through dev.ts.
      // That process intentionally does not receive an installed-app capability,
      // so a second Rust supervisor would only classify it as foreign and pollute
      // logs. Release bundles remain the sole owner of the authenticated sidecar.
      if cfg!(debug_assertions) {
        println!("[API sidecar] source development API is managed by dev.ts");
      } else {
...
#[cfg(test)]
mod tests {
    use super::{merge_legacy_port_save, merge_port_snapshots, validate_role_changes};
    use super::agent_runtime_response_timeout;
    use super::{
        chrome_profile_directory_is_safe, cmux_agent_command, discover_chrome_profiles_at,
        find_orca_floating_terminal_handle_in_value, find_orca_managed_floating_terminal_in_value,
        is_orca_stale_terminal_error, orca_cli_failure_message, orca_should_fall_back_to_floating,
        clone_staging_path, git_worktree_add, git_worktree_remove,
        inspect_orca_floating_visibility, is_absolute_path, list_git_worktrees, normalized_path_key,
        collect_descendant_pids_with, orca_external_worktree_requires_floating,
        parse_windows_netstat_listeners, reject_hidden_orca_worktree,
        claim_port_launch, classify_agent_runtime_api_health, classify_local_api_health,
        classify_remote_control_api_health,
        reject_live_tracked_process,
        force_kill_managed_process_group, register_managed_process,
        agent_runtime_http_request, is_allowed_agent_runtime_route,
        is_allowed_remote_control_management_route, is_allowed_what_i_said_management_route,
        local_api_health_request,
        local_api_supervisor_decision, spawn_port_env, stop_targets,
        terminate_managed_process_group,
        lock_owner_pid, lock_owner_process_alive,
        merge_portal_local_metadata, mutate_portal_local_metadata, normalized_portal_local_ids,
        read_existing_agent_runtime_capability, read_existing_remote_control_capability,
        read_existing_what_i_said_capability,
        remote_control_management_http_request,
        ...
        PortsFileLock, AGENT_RUNTIME_REQUEST_MAX_BYTES,
    };
    #[cfg(not(target_os = "windows"))]
    use super::{
        managed_process_group_alive, terminate_and_confirm_local_api_process_group,
    };
    use std::process::Command;
    use std::sync::{Arc, Mutex};
    ...
    #[test]
    fn cs_duty_uses_only_exact_native_post_route_and_bounded_enable_wait() {
        let path = "/api/agent-runtime/cs-duty";
        assert!(is_allowed_agent_runtime_route("POST", path));
        for (method, route) in [("GET", path), ("POST", "/api/agent-runtime/cs-duty?operation=enable"), ("POST", "/api/agent-runtime/cs-duty/send")] {
            assert!(!is_allowed_agent_runtime_route(method, route));
        }
        let enable = serde_json::json!({"operation":"enable"});
        let status = serde_json::json!({"operation":"status"});
        assert_eq!(agent_runtime_response_timeout("POST", path, Some(&enable)), Duration::from_secs(75));
        assert_eq!(agent_runtime_response_timeout("POST", path, Some(&status)), Duration::from_secs(20));
    }

    #[test]
    fn stale_file_lock_owner_cannot_remove_a_replacement_lock() {
        let root = temp_test_dir("owned-file-lock");
        std::fs::create_dir_all(&root).unwrap();
        let path = root.join("portal.json.lock");
        std::fs::write(&path, "replacement-owner").unwrap();
        drop(PortsFileLock {
            path: path.clone(),
            owner: "old-owner".to_string(),
        });
        assert_eq!(std::fs::read_to_string(&path).unwrap(), "replacement-owner");
        drop(PortsFileLock {
            path: path.clone(),
            owner: "replacement-owner".to_string(),
        });
        assert!(!path.exists());
    ...
    #[test]
    fn pid_lock_owner_proof_distinguishes_live_and_legacy_owners() {
        let owner = format!("v2:{}:{}", std::process::id(), "a".repeat(32));
        assert_eq!(lock_owner_pid(&owner), Some(std::process::id()));
        assert!(lock_owner_process_alive(&owner));
        assert_eq!(lock_owner_pid("legacy-owner-without-pid"), None);
        assert!(!lock_owner_process_alive("legacy-owner-without-pid"));
        assert_eq!(lock_owner_pid(&format!("v2:{}:{}", std::process::id(), "A".repeat(32))), None);
    }

    #[test]
    fn portal_deletion_markers_are_never_silently_evicted() {
        let all_ids = (0..2_100)
            .map(|index| serde_json::Value::String(format!("deleted-{index}")))
            .collect::<Vec<_>>();
        let normalized = normalized_portal_local_ids(Some(&serde_json::Value::Array(all_ids)));
        let normalized = normalized.as_array().unwrap();
        assert_eq!(normalized.len(), 2_100);
        assert_eq!(normalized.first().and_then(|value| value.as_str()), Some("deleted-0"));

        let mut metadata = serde_json::json!({
            "localOnlyDeletedPortIds": normalized,
        });
        mutate_portal_local_metadata(
            &mut metadata,
            "localOnlyDeletedPortIds",
            &serde_json::json!(["deleted-2100"]),
            "add",
        ).unwrap();
        let persisted = metadata["localOnlyDeletedPortIds"].as_array().unwrap();
        assert_eq!(persisted.len(), 2_101);
        assert_eq!(persisted.first().and_then(|value| value.as_str()), Some("deleted-0"));
        assert_eq!(persisted.last().and_then(|value| value.as_str()), Some("deleted-2100"));
    }

    #[test]
    fn remote_deletion_markers_survive_rust_storage_roundtrip_beyond_request_limit() {
        let root = temp_test_dir("portal-remote-deletion-roundtrip");
        std::fs::create_dir_all(&root).unwrap();
        let path = root.join("portal-local-metadata.json");
        let remote_ids = (0..2_100)
            .map(|index| serde_json::Value::String(format!("remote-{index}")))
            .collect::<Vec<_>>();
        let metadata = serde_json::json!({
            "localOnlyDeletedPortIds": ["local-only"],
            "remoteDeletedPortIds": remote_ids,
            "verifiedLegacyGeneratedWorktreeIds": ["verified-worktree"],
        });

        super::write_json_atomic(&path, &metadata).unwrap();
        let stored: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&path).unwrap()).unwrap();
        let mut portal = serde_json::json!({
            "items": [],
            "localOnlyDeletedPortIds": ["stale-local"],
            "remoteDeletedPortIds": ["stale-remote"],
            "verifiedLegacyGeneratedWorktreeIds": ["stale-verified"],
        });
        merge_portal_local_metadata(&mut portal, &stored);
        assert_eq!(
            portal["localOnlyDeletedPortIds"],
            serde_json::json!(["local-only"])
        );
        assert_eq!(
            portal["verifiedLegacyGeneratedWorktreeIds"],
            serde_json::json!(["verified-worktree"])
        );
        let roundtripped = portal["remoteDeletedPortIds"].as_array().unwrap();
        assert_eq!(roundtripped.len(), 2_100);
        assert_eq!(
            roundtripped.first().and_then(|value| value.as_str()),
            Some("remote-0")
        );
        ...
        mutate_portal_local_metadata(
            &mut portal,
            "remoteDeletedPortIds",
            &serde_json::json!(["remote-2100"]),
            "add",
        )
        .unwrap();
        super::write_json_atomic(&path, &portal).unwrap();
        let persisted: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&path).unwrap()).unwrap();
        let persisted = persisted["remoteDeletedPortIds"].as_array().unwrap();
        assert_eq!(persisted.len(), 2_101);
        assert_eq!(
            persisted.first().and_then(|value| value.as_str()),
            Some("remote-0")
        );
        assert_eq!(
            persisted.last().and_then(|value| value.as_str()),
            Some("remote-2100")
        );
    ...
    #[test]
    fn what_i_said_capability_requires_private_regular_file_permissions() {
        let root = temp_test_dir("what-i-said-capability");
        std::fs::create_dir_all(&root).unwrap();
        ...
        #[cfg(unix)]
        {
            ...
            assert_eq!(read_existing_what_i_said_capability(&path).unwrap(), Some(token.clone()));
            std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o644)).unwrap();
            assert!(read_existing_what_i_said_capability(&path).is_err());
            std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o600)).unwrap();
            let link = root.join("capability-link");
            symlink(&path, &link).unwrap();
            assert!(read_existing_what_i_said_capability(&link).is_err());
        }
        #[cfg(not(unix))]
        assert_eq!(read_existing_what_i_said_capability(&path).unwrap(), Some(token));
        std::fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn remote_control_capability_requires_private_regular_file_permissions() {
        let root = temp_test_dir("remote-control-capability");
        std::fs::create_dir_all(&root).unwrap();
        let path = root.join("capability");
        ...
        #[cfg(unix)]
        {
            use std::os::unix::fs::{symlink, PermissionsExt};
            std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o600)).unwrap();
            assert_eq!(read_existing_remote_control_capability(&path).unwrap(), Some(token.clone()));
            std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o640)).unwrap();
            assert!(read_existing_remote_control_capability(&path).is_err());
            std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o600)).unwrap();
            let link = root.join("capability-link");
            symlink(&path, &link).unwrap();
            assert!(read_existing_remote_control_capability(&link).is_err());
        }
        #[cfg(not(unix))]
        assert_eq!(read_existing_remote_control_capability(&path).unwrap(), Some(token));
        std::fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn agent_runtime_capability_requires_private_regular_file_permissions() {
        let root = temp_test_dir("agent-runtime-capability");
        std::fs::create_dir_all(&root).unwrap();
        let path = root.join("capability");
        ...
        #[cfg(unix)]
        {
            use std::os::unix::fs::{symlink, PermissionsExt};
            std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o600)).unwrap();
            assert_eq!(read_existing_agent_runtime_capability(&path).unwrap(), Some(token.clone()));
            std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o644)).unwrap();
            assert!(read_existing_agent_runtime_capability(&path).is_err());
            std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o600)).unwrap();
            let link = root.join("capability-link");
            symlink(&path, &link).unwrap();
            assert!(read_existing_agent_runtime_capability(&link).is_err());
        }
        #[cfg(not(unix))]
        assert_eq!(read_existing_agent_runtime_capability(&path).unwrap(), Some(token));
        std::fs::remove_dir_all(root).unwrap();
    }

    #[test]
    fn what_i_said_management_proxy_has_an_exact_route_allowlist() {
        for (method, path) in [
            ("POST", "/api/what-i-said/status"),
            ("POST", "/api/what-i-said/global-status"),
            ("POST", "/api/what-i-said/configure"),
            ...
            ("POST", "/api/what-i-said/source/rotate"),
            ("DELETE", "/api/what-i-said/source"),
            // 웹에서는 라우터가 바로 받지만 앱은 이 목록을 거친다. 빠뜨리면
            // 그 화면이 앱에서만 조용히 비어 보인다.
            ("POST", "/api/what-i-said/remote/status"),
            ("POST", "/api/what-i-said/remote/configure"),
            ("POST", "/api/what-i-said/remote/exclude"),
            ("POST", "/api/what-i-said/remote-key/status"),
            ...
            assert!(is_allowed_what_i_said_management_route(method, path));
        }
        for (method, path) in [
            ("GET", "/api/what-i-said/feed"),
            ("POST", "/api/ports"),
            ("POST", "/api/what-i-said/status?x=1"),
            ("DELETE", "/api/what-i-said/delete"),
            ("post", "/api/what-i-said/status"),
            ("GET", "/api/what-i-said/guides/list"),
            ("DELETE", "/api/what-i-said/guides/save"),
            ("POST", "/api/what-i-said/guides/list?limit=1"),
            ("POST", "/api/what-i-said/guides/save?"),
            ("POST", "/api/what-i-said/guides/save/"),
        ] {
            assert!(!is_allowed_what_i_said_management_route(method, path));
    ...
    #[test]
    fn remote_control_management_proxy_is_path_free_and_has_an_exact_route_allowlist() {
        for path in [
            ...
            assert!(is_allowed_remote_control_management_route("POST", path));
        }
        for (method, path) in [
            ("GET", "/api/remote-control/status"),
            ("DELETE", "/api/remote-control/sessions/revoke"),
            ("POST", "/api/remote-control/status?path=/tmp/project"),
            ("POST", "/api/execute-command"),
            ("POST", "/api/ports"),
            ("post", "/api/remote-control/enable"),
            ("GET", "/api/remote-control/internet/status"),
            ("DELETE", "/api/remote-control/internet/sessions/revoke"),
            ("POST", "/api/remote-control/internet/status?host=secret"),
            ("POST", "/api/remote-control/internet/arbitrary"),
        ] {
            assert!(!is_allowed_remote_control_management_route(method, path));
        }
        ...
        assert!(!request.contains("X-AgentsToZ-What-I-Said-Capability"));
    }

    #[test]
    fn agent_runtime_provider_probe_timeout_is_exact_and_keeps_other_requests_bounded() {
        for operation in ["prepare-provider", "revalidate-provider", "review-recovery", "execute-recovery", "review-recovery-provider", "verify-recovery-provider"] {
            let body = serde_json::json!({"automaticOperation": operation});
            assert_eq!(agent_runtime_response_timeout("POST", "/api/agent-runtime/terminals/memory", Some(&body)), Duration::from_secs(if operation == "execute-recovery" {375} else {75}));
            for (method, path) in [("GET", "/api/agent-runtime/terminals/memory"), ("POST", "/api/agent-runtime/terminals"),
                ("POST", "/api/agent-runtime/terminals/memory?probe=1"), ("POST", "/api/agent-runtime/terminals/memory/")] {
                assert_eq!(agent_runtime_response_timeout(method, path, Some(&body)), Duration::from_secs(20));
            }
        }
        for body in [serde_json::json!({}), serde_json::json!({"automaticOperation":"status"}),
            serde_json::json!({"automaticOperation":"enable"}), serde_json::json!({"keyOperation":"prepare"}),
            serde_json::json!([{"automaticOperation":"prepare-provider"}]), serde_json::Value::Null] {
            assert_eq!(agent_runtime_response_timeout("POST", "/api/agent-runtime/terminals/memory", Some(&body)), Duration::from_secs(20));
        }
    }

    #[test]
    fn agent_runtime_proxy_has_only_the_bounded_semantic_route_shapes() {
        for (method, path) in [
            ...
            ("GET", "/api/agent-runtime/tasks/task_12345678/events?after=9007199254740991"),
            ...
            ("GET", "/api/agent-runtime/conversations/capabilities"),
            ...
            ("POST", "/api/agent-runtime/conversations/conversation_12345678/unarchive"),
            ...
            ("POST", "/api/agent-runtime/terminals"),
            ("POST", "/api/agent-runtime/terminals/access"),
            ("POST", "/api/agent-runtime/terminals/memory"),
            ("POST", "/api/agent-runtime/quick-labels"),
            ("POST", "/api/agent-runtime/tester"),
        ] {
            assert!(is_allowed_agent_runtime_route(method, path), "{method} {path}");
        }
        for (method, path) in [
            ("POST", "/api/agent-runtime/capabilities"),
            ("POST", "/api/agent-runtime/readiness"),
            ("GET", "/api/agent-runtime/readiness?detail=1"),
            ("POST", "/api/agent-runtime/targets"),
            ("GET", "/api/agent-runtime/targets?cwd=/tmp"),
            ("GET", "/api/agent-runtime/tasks/start"),
            ("GET", "/api/agent-runtime/tasks/task/events?after=0"),
            ("GET", "/api/agent-runtime/tasks/task_12345678/events"),
            ("GET", "/api/agent-runtime/tasks/task_12345678/events?after=00"),
            ("GET", "/api/agent-runtime/tasks/task_12345678/events?after=9007199254740992"),
            ("GET", "/api/agent-runtime/tasks/task_12345678/events?after=0&cwd=/tmp"),
            ("POST", "/api/agent-runtime/tasks/task_12345678/cancel?x=1"),
            ("POST", "/api/agent-runtime/tasks/task_12345678/shell"),
            ("POST", "/api/agent-runtime/conversations"),
            ("GET", "/api/agent-runtime/conversations/start"),
            ("GET", "/api/agent-runtime/conversations/conversation_12345678/history"),
            ("GET", "/api/agent-runtime/conversations/conversation_12345678/events"),
            ("GET", "/api/agent-runtime/conversations/conversation_12345678/events?after=00"),
            ("GET", "/api/agent-runtime/conversations/conversation_12345678/events?after=0&cwd=/tmp"),
            ("POST", "/api/agent-runtime/conversations/conversation_12345678/continue?cwd=/tmp"),
            ("GET", "/api/agent-runtime/conversations/conversation_12345678/question?cwd=/tmp"),
            ("POST", "/api/agent-runtime/conversations/conversation_12345678/question"),
            ("GET", "/api/agent-runtime/conversations/conversation_12345678/question/answer"),
            ("POST", "/api/agent-runtime/conversations/conversation_12345678/shell"),
            ("POST", "/api/execute-command"),
            ("GET", "/api/agent-runtime/tester"),
            ("POST", "/api/agent-runtime/tester?path=/tmp"),
            ("POST", "/api/agent-runtime/tester/run"),
            ("post", "/api/agent-runtime/tasks/start"),
        ] {
            assert!(!is_allowed_agent_runtime_route(method, path), "{method} {path}");
        }
        ...
        let request = agent_runtime_http_request(
            "POST",
            "/api/agent-runtime/tasks/start",
            &capability,
            2,
        );
        assert!(request.contains(&format!(
        ...
        assert!(!request.contains("X-AgentsToZ-Remote-Control-Capability"));
        assert!(!request.contains("X-AgentsToZ-What-I-Said-Capability"));

        let maximally_escaped_prompt = "\u{0001}".repeat(32 * 1024);
        let payload = serde_json::to_vec(&serde_json::json!({
            "prompt": maximally_escaped_prompt,
        })).unwrap();
        assert!(payload.len() < AGENT_RUNTIME_REQUEST_MAX_BYTES);
        assert_eq!(AGENT_RUNTIME_REQUEST_MAX_BYTES, 256 * 1024);
    }

    #[test]
    fn remote_control_health_proof_is_distinct_from_what_i_said_authority() {
        ...
        assert_eq!(contract["schemaVersion"], 14);
        assert!(contract["requiredCapabilities"].as_array().unwrap().iter()
            .any(|value| value.as_str() == Some("remote-control.management-v1")));
        assert_eq!(
            contract["remoteControlHealthProof"]["responseField"],
            "remoteControlSidecarProof",
        );
        assert_eq!(
            contract["remoteControlHealthProof"]["domain"],
            contract["sidecarHealthProof"]["domain"],
        );
        assert_ne!(
            contract["remoteControlHealthProof"]["responseField"],
            contract["sidecarHealthProof"]["responseField"],
        );

        let capability = "c".repeat(64);
        let nonce = [0x11u8; 32];
        let proof = "e5906a92c66f8e34cb3c9d66ef95d58e8aaee69bedf172a30b13cea700bf5d74";
        let health = health_response(serde_json::json!({
            "service": "agentstoz-api",
            "schemaVersion": contract["schemaVersion"],
            "capabilities": contract["requiredCapabilities"],
            "remoteControlSidecarProof": proof,
        }));
        assert_eq!(
            classify_remote_control_api_health(Some(&health), &capability, &nonce),
            LocalApiHealth::Compatible,
        );
        assert_eq!(
            classify_local_api_health(Some(&health), &capability, &nonce),
            LocalApiHealth::Foreign,
        );

        let what_i_said_health = health_response(serde_json::json!({
            "service": "agentstoz-api",
            "schemaVersion": contract["schemaVersion"],
            "capabilities": contract["requiredCapabilities"],
            "sidecarProof": proof,
        ...
        assert_eq!(
            classify_remote_control_api_health(Some(&what_i_said_health), &capability, &nonce),
            LocalApiHealth::Foreign,
        );
    }

    #[test]
    fn onboarding_proof_rejects_other_authority_and_changed_nonce() {
        assert_eq!(super::onboarding_management_path("github").unwrap(), "/api/onboarding/github");
        assert_eq!(super::onboarding_management_path("codex").unwrap(), "/api/onboarding/codex");
        assert_eq!(super::onboarding_management_path("codex-login").unwrap(), "/api/onboarding/codex-login");
        assert!(super::onboarding_management_path("../../api/execute-command").is_err());
        assert!(super::onboarding_management_path("codex?command=whoami").is_err());
        let contract: serde_json::Value = serde_json::from_str(super::LOCAL_API_CONTRACT_JSON).unwrap();
        let cap = "c".repeat(64);
        let nonce = [0x11u8; 32];
        let proof = "784f9c5213aa9c70c6cf2a5822b5f884392bcdbfa565b2aa9ce77210e2cc9a2b";
        let response = health_response(serde_json::json!({
            "service":"agentstoz-api", "schemaVersion":contract["schemaVersion"],
            "capabilities":contract["requiredCapabilities"], "onboardingSidecarProof":proof,
        }));
        assert_eq!(super::classify_local_api_health_with_proof(Some(&response), &cap, &nonce, super::LocalApiProofAuthority::Onboarding), LocalApiHealth::Compatible);
        assert_eq!(classify_agent_runtime_api_health(Some(&response), &cap, &nonce), LocalApiHealth::Foreign);
        assert_eq!(super::classify_local_api_health_with_proof(Some(&response), &cap, &[0x12;32], super::LocalApiProofAuthority::Onboarding), LocalApiHealth::Foreign);
    }

    #[test]
    fn agent_runtime_health_proof_has_an_independent_field_key_and_domain() {
        let contract: serde_json::Value = serde_json::from_str(include_str!("../../context-api-contract.json"))
            .expect("context API contract should parse");
        assert!(contract["requiredCapabilities"].as_array().unwrap().iter()
            .any(|value| value.as_str() == Some("agent-runtime.management-v1")));
        assert_eq!(
            contract["agentRuntimeHealthProof"]["responseField"],
        ...
        assert_ne!(
            contract["agentRuntimeHealthProof"]["domain"],
            contract["sidecarHealthProof"]["domain"],
        );

        let capability = "c".repeat(64);
        let nonce = [0x11u8; 32];
        let proof = "90c857c10fd6cf8a4f2d362f9fc1041e41a2252dc9db5a64eedc73c224351748";
        let health = health_response(serde_json::json!({
            "service": "agentstoz-api",
            "schemaVersion": contract["schemaVersion"],
            "capabilities": contract["requiredCapabilities"],
            "agentRuntimeSidecarProof": proof,
        }));
        assert_eq!(
            classify_agent_runtime_api_health(Some(&health), &capability, &nonce),
            LocalApiHealth::Compatible,
        );
        assert_eq!(
            classify_local_api_health(Some(&health), &capability, &nonce),
    ...
    #[test]
    fn local_api_health_requires_a_fresh_authenticated_proof_and_the_shared_contract() {
        let contract: serde_json::Value = serde_json::from_str(include_str!("../../context-api-contract.json"))
            .expect("context API contract should parse");
        let schema = contract["schemaVersion"].as_u64().expect("schemaVersion");
        let capability = "c".repeat(64);
        let wrong_capability = "d".repeat(64);
        let nonce_a = [0x11u8; 32];
        let nonce_b = [0x22u8; 32];
        let proof_a = "e5906a92c66f8e34cb3c9d66ef95d58e8aaee69bedf172a30b13cea700bf5d74";
        let proof_b = "7a04c2513057a34bbea07a9b82423304d8f61b590c8b06324da9eefb59eea51a";
        ...
        let current = health_response(serde_json::json!({
            ...
            "sidecarProof": proof_a,
        }));
        assert_eq!(
            classify_local_api_health(Some(&current), &capability, &nonce_a),
            LocalApiHealth::Compatible,
        );

        let request = local_api_health_request(&nonce_a).expect("health request");
        assert!(request.contains(&format!("?nonce={}", "11".repeat(32))));
        assert!(!request.contains(&capability));

        let missing_proof = health_response(serde_json::json!({
            "service": "agentstoz-api",
            "schemaVersion": schema,
            "capabilities": contract["requiredCapabilities"],
        }));
        assert_eq!(
            classify_local_api_health(Some(&missing_proof), &capability, &nonce_a),
            LocalApiHealth::Foreign,
        );

        let wrong_proof = health_response(serde_json::json!({
            "service": "agentstoz-api",
            "schemaVersion": schema,
            "capabilities": contract["requiredCapabilities"],
            "sidecarProof": proof_b,
        }));
        assert_eq!(
            classify_local_api_health(Some(&wrong_proof), &capability, &nonce_a),
            LocalApiHealth::Foreign,
        );
        assert_eq!(
            classify_local_api_health(Some(&current), &capability, &nonce_b),
            LocalApiHealth::Foreign,
        );
        assert_eq!(
            classify_local_api_health(Some(&current), &wrong_capability, &nonce_a),
            LocalApiHealth::Foreign,
        );
        assert_eq!(
            classify_local_api_health(Some(&current), &capability, &[0x11u8; 31]),
            LocalApiHealth::Foreign,
        );
        ...
        assert_eq!(
            classify_local_api_health(Some(&old), &capability, &nonce_a),
            LocalApiHealth::Incompatible,
        );
        ...
        assert_eq!(
            classify_local_api_health(Some(&incomplete), &capability, &nonce_a),
            LocalApiHealth::Incompatible,
        );
        assert_eq!(
            classify_local_api_health(Some("HTTP/1.1 200 OK\r\n\r\nnot-json"), &capability, &nonce_a),
            LocalApiHealth::Foreign,
        );
        assert_eq!(
        ...
        assert_eq!(
            classify_local_api_health(None, &capability, &nonce_a),
            LocalApiHealth::Unavailable,
        );
    }
    ...
    #[test]
    fn clone_staging_is_a_hidden_sibling_so_interruption_never_claims_the_final_path() {
        let root = temp_test_dir("clone-staging");
        std::fs::create_dir_all(&root).expect("create clone root");
        let target = root.join("project");
        let staging = clone_staging_path(&target).expect("staging path");
        assert_eq!(staging.parent(), target.parent());
        assert!(staging
            .file_name()
            .unwrap()
            .to_string_lossy()
            .starts_with(".agentstoz-staging-project-"));
    ...
    fn managed_process_group_termination_reaches_term_ignoring_grandchildren() {
        ...
        command
            .arg("-c")
            .arg("trap '' TERM; sleep 30 & echo $! > \"$CHILD_PID_FILE.tmp\"; mv \"$CHILD_PID_FILE.tmp\" \"$CHILD_PID_FILE\"; wait")
    ...
    #[cfg(not(target_os = "windows"))]
    #[test]
    fn local_api_restart_waits_until_a_crashed_leaders_group_is_gone() {
        use std::os::unix::process::CommandExt;
        ...
        command
            .args([
                "-c",
                r#"trap "" TERM; /bin/sh -c 'trap "" TERM; while :; do sleep 1; done' & exit 23"#,
            ])
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .process_group(0);
        let mut leader = command.spawn().expect("spawn isolated local API group");
        let process_group_id = leader.id();
        let status = leader.wait().expect("reap crashed local API leader");

        assert_eq!(status.code(), Some(23));
        assert!(
            managed_process_group_alive(process_group_id),
            "the request-scoped child should survive the leader"
        );
        assert_eq!(
            local_api_supervisor_decision(LocalApiHealth::Unavailable, true, false),
            LocalApiSupervisorDecision::WaitForOwnedChild,
        );

        terminate_and_confirm_local_api_process_group(process_group_id, None)
            .expect("confirm old local API group exit");
        assert!(!managed_process_group_alive(process_group_id));
        assert_eq!(
            local_api_supervisor_decision(LocalApiHealth::Unavailable, false, false),
            LocalApiSupervisorDecision::Spawn,
        );
...
"#;
    ...
    #[test]
    fn project_role_merge_matches_bun_golden_fixtures() {
        let fixtures: serde_json::Value = serde_json::from_str(include_str!("../../tests/fixtures/project-role-merge.json")).unwrap();
        for case in fixtures.as_array().unwrap() {
            let desired = case["desired"].as_array().unwrap();
            let current = case["current"].as_array().unwrap();
            let merged = if let Some(base) = case["base"].as_array() {
                merge_port_snapshots(base, desired, current)
            } else { merge_legacy_port_save(desired, current) };
            let result = validate_role_changes(&merged, current);
            if case["error"].as_bool() == Some(true) {
                assert_eq!(result.unwrap_err(), "PROJECT_ROLE_INVALID", "{}", case["name"]);
            } else {
                result.unwrap();
                assert_eq!(serde_json::json!(merged), case["expected"], "{}", case["name"]);
            }
        }
    }

    #[test]
    fn port_info_role_round_trip_is_optional_and_strict() {
        for role in ["ops", "dev", "managed"] {
            let value: PortInfo = serde_json::from_value(serde_json::json!({"id":"p", "name":"P", "role":role})).unwrap();
            assert_eq!(serde_json::to_value(value).unwrap()["role"], role);
        }
        let legacy: PortInfo = serde_json::from_value(serde_json::json!({"id":"p", "name":"P"})).unwrap();
        assert!(serde_json::to_value(legacy).unwrap().get("role").is_none());
        assert!(serde_json::from_value::<PortInfo>(serde_json::json!({"id":"p", "name":"P", "role":"future"})).is_err());
    }

    #[test]
    fn port_info_preserves_document_paths_and_deletion_fence_generation() {
        ...
        assert_eq!(serialized["syncGeneration"], "7");
        assert_eq!(serialized["sourcePortSyncGeneration"], "6");

        let explicit_clears: PortInfo = serde_json::from_value(serde_json::json!({
            "id": "cleared-project", "name": "Cleared", "port": 0,
            "commandPath": "", "terminalCommand": "", "folderPath": "", "deployUrl": "",
            "githubUrl": "", "githubUrls": [], "manualPath": "", "logFilePath": "",
            "category": "", "description": "", "syncGeneration": "3"
        })).expect("existing empty-value contracts should deserialize");
        let cleared = serde_json::to_value(explicit_clears).expect("explicit clears should serialize");
        assert_eq!(cleared["port"], 0);
        assert_eq!(cleared["githubUrls"], serde_json::json!([]));
        for field in ["commandPath", "terminalCommand", "folderPath", "deployUrl", "githubUrl",
            "manualPath", "logFilePath", "category", "description"] {
            assert_eq!(cleared[field], "", "lost explicit clear for {field}");
        }
        assert_eq!(cleared["syncGeneration"], "3");

        let legacy_number: PortInfo = serde_json::from_value(serde_json::json!({
            "id": "legacy-generation",
            "name": "Legacy",
            "syncGeneration": 3
        })).expect("legacy safe integer generation should migrate");
        assert_eq!(
            serde_json::to_value(legacy_number).unwrap()["syncGeneration"],
            "3",
        );
        assert!(serde_json::from_value::<PortInfo>(serde_json::json!({
            "id": "unsafe-generation",
            "name": "Unsafe",
            "syncGeneration": "9223372036854775808"
        })).is_err());
    }
    ...
    #[test]
    fn orca_launch_surface_is_creator_neutral_and_git_path_exact() {
        let listing = serde_json::json!({
            "result": { "worktrees": [
                { "path": "/repo" },
                { "path": "/Users/me/orca/workspaces/repo/task" }
            ] }
        });
        assert!(!orca_external_worktree_requires_floating(
            Some("/repo"), Some("/repo"), Some(&listing),
        ));
        assert!(!orca_external_worktree_requires_floating(
            Some("/Users/me/.codex/worktrees/registered-main"),
            Some("/Users/me/.codex/worktrees/registered-main"),
            Some(&listing),
        ));
        assert!(!orca_external_worktree_requires_floating(
            Some("/repo"), Some("/Users/me/orca/workspaces/repo/task"), Some(&listing),
        ));
        assert!(orca_external_worktree_requires_floating(
            Some("/repo"), Some("/repo/worktrees/agentstoz-task"), Some(&listing),
        ));
        assert!(orca_external_worktree_requires_floating(
            Some("/repo"), Some("/Users/me/.codex/worktrees/task"), None,
        ));
        // An unavailable Orca listing is not proof that a normal external path
        // is absent; keep the exact-selector attempt and its existing fallback.
        assert!(!orca_external_worktree_requires_floating(
            Some("/repo"), Some("/elsewhere/plain-git-task"), None,
        ));
    }

    #[test]
    fn orca_cli_failure_reads_the_json_error_it_prints_on_stdout() {
        // Orca 1.4.x exits 1 and prints its `{ok:false}` document on stdout with an
        // empty stderr. Reducing that to "unknown" hid terminal_handle_stale, so one
        // closed floating tab blocked every later 「Claude 실행」 (2026-09-12).
        let stale = r#"{"id":"x","ok":false,"error":{"code":"terminal_handle_stale","message":"terminal_handle_stale"}}"#;
        let message = orca_cli_failure_message(stale, "");
        assert_eq!(message, "terminal_handle_stale");
        assert!(is_orca_stale_terminal_error(&message));

        // The worktree -> Floating fallback matches on the code, which a message may omit.
        let missing = r#"{"ok":false,"error":{"code":"selector_not_found","message":"No worktree matches path:/tmp/x"}}"#;
        let message = orca_cli_failure_message(missing, "");
        assert!(message.contains("selector_not_found") && message.contains("No worktree matches"));
        assert!(orca_should_fall_back_to_floating(&message));

        assert_eq!(orca_cli_failure_message("", "daemon refused"), "daemon refused");
        assert_eq!(orca_cli_failure_message("not json", ""), "unknown");
        assert!(orca_should_fall_back_to_floating(&orca_cli_failure_message("", "")));
    }
    ...
    fn remembered_orca_handle_must_still_be_in_the_floating_workspace() {
        ...
        assert!(!find_orca_floating_terminal_handle_in_value(&exited, "terminal-live"));
        assert!(!find_orca_floating_terminal_handle_in_value(&orphaned, "terminal-live"));
    }

    #[test]
    fn floating_title_fallback_ignores_exited_and_orphaned_terminals() {
        let marker = "[ATZ:claude:12345678]";
        let legacy_title = "project · Claude";
        let listing = serde_json::json!({
            "result": {
                "terminals": [
                    {
                        "handle": "terminal-exited",
                        "worktreeId": "global-floating-terminal",
                        "title": format!("project · Claude · {}", marker),
                        "connected": false,
                        "orphaned": false
                    },
                    {
                        "handle": "terminal-orphaned",
                        "worktreeId": "global-floating-terminal",
                        "title": legacy_title,
                        "connected": true,
                        "orphaned": true
        ...
        let mut latest_handle = None;
        let mut legacy_handles = Vec::new();
        find_orca_managed_floating_terminal_in_value(
            &listing,
            marker,
            legacy_title,
            &mut latest_handle,
        ...
        assert_eq!(latest_handle.as_deref(), Some("terminal-live"));
        assert!(legacy_handles.is_empty());
    }
    ...
    #[test]
    fn list_worktrees_treats_missing_origin_as_a_successful_local_refresh() {
        let root = temp_test_dir("worktree-no-origin");
        std::fs::create_dir_all(&root).unwrap();
        git_ok(&root, &["init"]);
        ...
        let worktrees = list_git_worktrees(root.to_string_lossy().to_string(), Some(true))
            .expect("a repository without origin still has a valid local status");
        std::fs::remove_dir_all(&root).unwrap();

        assert!(!worktrees.is_empty());
        assert!(worktrees.iter().all(|worktree| worktree.remote_refresh_error.is_none()));
        assert!(worktrees.iter().all(|worktree| {
            worktree.remote_refresh_state.as_deref() == Some("no-origin")
        }));
    }
