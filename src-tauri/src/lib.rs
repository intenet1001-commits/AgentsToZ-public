...
fn is_allowed_agent_runtime_route(method: &str, path: &str) -> bool {
    if matches!(path, "/api/agent-runtime/terminals" | "/api/agent-runtime/terminals/access" | "/api/agent-runtime/terminals/memory" | "/api/agent-runtime/quick-labels") { return method == "POST"; }
    if matches!(path,
