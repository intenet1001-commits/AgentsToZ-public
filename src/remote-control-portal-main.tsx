...
function RemoteControlPortalApp() {
  ...
  const [remotePane,setRemotePane]=useState<'projects'|'workroom'>(()=>new URLSearchParams(window.location.search).get('tab')==='workroom'?'workroom':'projects');
  ...
  const navigateWorkspace = (tab: WorkspaceTab) => {
    ...
    if (tab !== workspaceTab) history.pushState({ ...history.state, workspaceTab: tab }, '');
    setWorkspaceTab(tab);
  };
  useEffect(() => {
  ...
  const openOps = async () => {
    ...
    const controller = selectedController;
    const hostId = selectedHostId;
    const host = status.hostName ?? '연결된 Mac';
    if (!window.confirm(`${host}\n\nAgentsToZ OPS 운영 패널을 Mac 화면에 열까요?`)) return;
    setError('');
    setNotice(`${host}: AgentsToZ OPS 열기 요청을 보냈습니다.`);
    try {
      const result = await controller.sendAction('ops.open');
      if (!result?.ok || !('ops' in result)) throw new Error('OPS 열기 결과를 확인하지 못했습니다.');
      opsStatusByHostRef.current.set(hostId, result.ops);
      if (selectedHostIdRef.current === hostId) {
        setOpsStatus(result.ops);
        setNotice(`${host}: AgentsToZ OPS 운영 패널을 열었습니다.`);
      }
      syncHosts();
    } catch (opsError) {
      if (selectedHostIdRef.current === hostId) { setNotice(''); reportError(opsError); }
    }
  };
  ...
  const reviewOpsCandidate = async (candidateId:string,accept:boolean) => {
    ...
    try{
      ...
      syncHosts();
    }catch(reviewError){if(selectedHostIdRef.current===hostId)reportError(reviewError);}
  };
  ...
  return (
    <main className="remote-shell" data-workspace-tab={workspaceTab} data-remote-pane={remotePane} data-testid="internet-remote-controller">
      ...
      <header className="remote-header">
        ...
        {workspaceTab==='workroom'&&<span className={`remote-state remote-state--${status.state}`}>{controllerStateLabels[status.state]}</span>}
      ...
      {supabase && authState === 'signed-in' && <div className="workspace-prompt-guides"><SharedPromptGuideBar key={email} client={supabase}/></div>}
      <PortalRemoteQrScanner open={showScanner} onClose={() => setShowScanner(false)} onDetected={acceptPairing} />
      {(workspaceTab==='home'||workspaceTab==='workroom')&&<button className="remote-portal-scan" type="button" data-testid="remote-portal-scan" onClick={() => setShowScanner(true)}><ScanLine aria-hidden="true" />작업 기기 연결 · QR 스캔</button>}
      ...
      <details className="workspace-connection-details"><summary>연결 정보</summary>
        ...
        <section className="remote-security-note"><ShieldCheck aria-hidden="true" /><p>승인한 작업과 기록은 암호화된 연결을 통해 이용합니다. 터미널 입력과 출력에는 프로젝트 경로와 계정 정보가 포함될 수 있습니다.</p></section>
          ...
          {terminalOpen && <div className="remote-panel remote-workroom-content" key={selectedHostId??'none'}><AiTerminalPanel remote visible={workspaceTab==='workroom'&&remotePane==='workroom'&&status.state==='online'} entry={terminalEntry} sessionScope={selectedHostId??'none'} projects={status.projects.map(p=>({targetId:p.controlId,label:p.name}))} transport={terminalTransport} workspaceTransport={workroomTransport} onWhatISaid={()=>{setRecordMode('said');navigateWorkspace('records')}} onRememberSession={targetId=>{setManagementTarget(targetId);setRemotePane('projects');navigateWorkspace('workroom')}}/></div>}
      ...
      {waiting ? (
        ...
        <section className="remote-panel remote-centered" hidden={workspaceTab==='home'}>
          <Smartphone aria-hidden="true" />
          <h2>작업할 기기를 연결하세요</h2>
          <p>Mac의 AgentsToZ에서 원격제어 QR을 스캔하면 여기에서 작업할 수 있습니다. 북마크와 계정 자료도 위 메뉴에서 이용하세요.</p>
        </section>
        ...
        <section className="remote-panel remote-centered">
          ...
          <button className="remote-primary" type="button" disabled={loginBusy} onClick={() => void login(false)}>
            {loginBusy ? <Loader2 className="remote-spin" aria-hidden="true" /> : <LockKeyhole aria-hidden="true" />}
            Google 계정으로 계속
          </button>
        </section>
        ...
        <>
          ...
          <section className="remote-panel remote-ops-control" data-testid="remote-ops-controls">
            <h2>AgentsToZ OPS</h2>
            <p>{opsStatus
              ? `${{ready:'운영기억 연결됨',preparing:'준비 중','needs-attention':'연결 확인 필요',unconfigured:'프로필 준비 필요'}[opsStatus.state]} · 저장 후보 ${opsStatus.pendingCount}개${opsStatus.lastSavedAt ? ` · 마지막 저장 ${new Date(opsStatus.lastSavedAt).toLocaleString()}` : ''}`
              : '운영기억 상태를 확인하면 이 Mac의 OPS 준비 상태와 저장 후보 수를 표시합니다.'}</p>
            <div className="workspace-quick-actions">
              <button type="button" className="remote-secondary" disabled={status.busy} onClick={() => void readOpsStatus()}>운영기억 상태 확인</button>
              <button type="button" className="remote-secondary" disabled={status.busy} onClick={() => void readOpsCandidates()}>저장 후보 확인</button>
              <button type="button" className="remote-primary" disabled={status.busy} onClick={() => void openOps()}>AgentsToZ OPS 열기</button>
            </div>
            <p>후보 접수는 저장이 아닙니다. 공유 OPS 기억은 SAS 승인된 이 화면에서 사람이 저장 또는 거절해야 합니다.</p>
            {opsCandidates && (opsCandidates.candidates.length ? (
              <div className="remote-task-list" data-testid="remote-ops-memory-candidates">
                {opsCandidates.candidates.map(candidate=><article className="remote-task-card" key={candidate.id}>
                  <strong>{candidate.title}</strong><p>{candidate.body}</p><small>근거: {candidate.evidence}</small>
                  <div className="workspace-quick-actions">
                    ...
                    <button type="button" className="remote-primary" disabled={status.busy} onClick={()=>void reviewOpsCandidate(candidate.id,true)}>검토 후 저장</button>
                  </div>
                </article>)}
              </div>
            ) : <p className="remote-task-empty">검토할 OPS 운영기억 후보가 없습니다.</p>)}
          </section>
...
root.render(<RemoteControlPortalApp />);
