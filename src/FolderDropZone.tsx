  ...
  return (
    <div
      ...
      className={[
        ...
        dragging
          ...
          : 'border-zinc-700 bg-zinc-900/55 text-zinc-400',
    ...
    >
      {inlineInput ? (
        <>
          {prefixLabel && (
            <span className="flex w-24 flex-none items-center border-r border-zinc-700 bg-[rgb(var(--surface-shade-rgb))]/20 px-3 text-[10px] font-medium text-zinc-500">
              {prefixLabel}
            </span>
          )}
          <input
            ...
            className="min-w-0 flex-1 bg-transparent px-3 py-2 text-xs text-zinc-100 outline-none placeholder:text-[var(--text-dim)]"
          />
          <button
            type="button"
            onClick={event => {
            ...
            title={`${label} · ${hint}`}
            aria-label={`${label} · ${hint}`}
            className="flex w-10 flex-none items-center justify-center border-l border-zinc-700 bg-[rgb(var(--surface-highlight-rgb))]/[0.03] text-zinc-400 transition-colors hover:bg-teal-500/10 hover:text-teal-300 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {pathKind === 'folder'
