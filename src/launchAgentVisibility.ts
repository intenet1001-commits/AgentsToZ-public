...
export type LaunchAgent = BuiltinAgentRuntimeId;

/** 표시 순서의 정본. 헤더 체크박스와 버튼 줄이 같은 순서를 쓴다. */
export const ALL_LAUNCH_AGENTS: readonly LaunchAgent[] = BUILTIN_AGENT_RUNTIME_IDS;

export const LAUNCH_AGENT_LABELS: Readonly<Record<LaunchAgent, string>> = BUILTIN_AGENT_RUNTIME_LABELS;
...
function isLaunchAgent(value: unknown): value is LaunchAgent {
  return isBuiltinAgentRuntimeId(value);
}
