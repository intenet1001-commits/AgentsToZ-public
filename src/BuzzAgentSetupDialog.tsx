...
export function BuzzAgentSetupDialog({
  ...
  }, [deviceName, runtime, status?.control, status?.project, status?.serviceMemory]);
  ...
  const copySettings = async () => {
    const targetPath = status?.project?.canonicalPath;
    if (!targetPath || !instructions) return;
    try {
      await copyAgentsToZPrompt(buildBuzzAgentSetupClipboard({
        agentName,
        runtime,
        instructions,
        project: status?.project,
      ...
      onToast("Buzz USE 서비스 Agent 생성 설정을 복사했습니다.", "success");
    } catch (nextError) {
  ...
  const targetReady = !!status?.project && !!status.serviceMemoryStatus.ready && !!status.serviceMemory;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[10090] flex items-center justify-center bg-[var(--dialog-scrim)] p-4 backdrop-blur-sm" onMouseDown={event => { if (event.target === event.currentTarget && !opening && !ensuringServiceMemory && !installingCodexControl) onClose(); }}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="buzz-agent-setup-title" className="max-h-[calc(var(--ui-viewport-height,100dvh)-2rem)] w-full max-w-2xl overflow-y-auto rounded-2xl border border-teal-300/20 bg-[var(--bg-elevated)] shadow-[var(--dialog-shadow)]">
        <div className="sticky top-0 z-10 flex items-start gap-3 border-b border-[rgb(var(--surface-highlight-rgb))]/[0.07] bg-[rgb(var(--bg-elevated-rgb))]/95 px-5 py-4 backdrop-blur">
          ...
          <div className="min-w-0">
            <div className="mb-1 flex flex-wrap items-center gap-2">
              <h2 id="buzz-agent-setup-title" className="m-0 text-base font-semibold text-zinc-100">Buzz 앱용 USE 서비스 Agent 만들기</h2>
              <span className="rounded-full border border-violet-300/20 bg-violet-300/[0.06] px-2 py-0.5 text-[10px] text-violet-200">USE 전용</span>
            </div>
            <p className="m-0 text-xs leading-5 text-zinc-400">프로젝트를 직접 개발하는 Agent가 아니라, 제품을 사용하는 사람과 대화하는 USE 서비스 Agent입니다. DEV 프로젝트 기억은 읽기 전용이며 운영지식은 별도 USE 기억에 보관합니다.</p>
          </div>
          <button ref={closeButtonRef} type="button" aria-label="Buzz Agent 설정 닫기" onClick={onClose} disabled={opening || ensuringServiceMemory || installingCodexControl} className="ml-auto min-h-11 min-w-11 shrink-0 rounded-lg p-2 text-zinc-400 hover:bg-[rgb(var(--surface-highlight-rgb))]/5 hover:text-zinc-100 disabled:opacity-40"><X className="h-4 w-4" /></button>
        </div>

        <div className="space-y-4 p-5">
          <div className="rounded-xl border border-teal-300/15 bg-teal-300/[0.04] p-3 text-[11px] leading-5 text-zinc-300">
            <strong className="text-teal-200">역할 분리</strong> · 프로젝트 영역의 <span className="text-cyan-200">Buzz 채널</span>은 대화 공간을 열거나 프로젝트와 연결만 합니다. 이 화면은 선택한 프로젝트를 <span className="text-cyan-200">Buzz 앱에서 대화할 USE 서비스 Agent</span>로 만들고, DEV 기억과 분리된 USE 운영기억을 준비합니다. GitHub 저장소는 Agent 생성의 전제가 아니며 나중에 추가되어도 현재 Git 상태를 사용합니다.
          </div>
          ...
          {(
            <label className="block text-[11px] text-zinc-300">서비스 Agent로 만들 프로젝트
              <select
                ...
                className="mt-1 min-h-11 w-full rounded-lg border border-[rgb(var(--surface-highlight-rgb))]/10 bg-[rgb(var(--surface-shade-rgb))]/25 px-3 py-2 text-xs text-zinc-100 outline-none focus:border-teal-300/40"
              >
          ...
          {loading && !status ? (
            <div className="flex items-center justify-center gap-2 py-12 text-sm text-zinc-400"><Loader2 className="h-4 w-4 animate-spin" />DEV 프로젝트와 USE 운영기억 확인 중…</div>
          ) : status && (
            <>
              <div className="rounded-xl border border-[rgb(var(--surface-highlight-rgb))]/[0.07] bg-[rgb(var(--surface-highlight-rgb))]/[0.02] p-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button type="button" onClick={() => void refresh()} disabled={loading || opening} className="ml-auto inline-flex min-h-9 items-center gap-1 rounded-md border border-[rgb(var(--surface-highlight-rgb))]/10 px-2 text-[10px] text-zinc-400 hover:text-zinc-100 disabled:opacity-40"><RefreshCw className={`h-3 w-3 ${loading ? "animate-spin" : ""}`} />다시 확인</button>
                </div>
                {status.serviceMemoryStatus.problem && <p className="mb-0 mt-2 text-[10px] text-amber-300">{status.serviceMemoryStatus.problem}</p>}
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="block text-[11px] text-zinc-300">Agent 이름 · 단말명 포함
                  <input value={agentName} onChange={event => { agentNameTouchedRef.current = true; setAgentName(event.target.value); }} maxLength={64} className="mt-1 min-h-11 w-full rounded-lg border border-[rgb(var(--surface-highlight-rgb))]/10 bg-[rgb(var(--surface-shade-rgb))]/25 px-3 py-2 text-xs text-zinc-100 outline-none focus:border-teal-300/40" />
                </label>
                <label className="block text-[11px] text-zinc-300">첫 실행기
                  <select data-testid="buzz-agent-runtime" value={runtime} onChange={event => { agentNameTouchedRef.current = false; setRuntime(event.target.value as BuzzAgentRuntime); }} className="mt-1 min-h-11 w-full rounded-lg border border-[rgb(var(--surface-highlight-rgb))]/10 bg-[rgb(var(--surface-shade-rgb))]/25 px-3 py-2 text-xs text-zinc-100 outline-none focus:border-teal-300/40">
              ...
              {(
                <div className="space-y-3 rounded-xl border border-violet-300/15 bg-violet-300/[0.035] p-3">
                  <div>
                    ...
                    <p className="mb-0 mt-1 text-[10px] leading-4 text-zinc-500">소스·Git·배포·제품 개선은 DEV 담당자 또는 개발 Agent가 담당합니다. USE Agent는 이 영역을 수정하지 않습니다.</p>
                  </div>

                  <div className="border-t border-[rgb(var(--surface-highlight-rgb))]/[0.07] pt-3">
                    <div className="flex flex-wrap items-center gap-2">
                  ...
                  {status.control && (
                    <div data-testid="agentstoz-use-control-capabilities" className="border-t border-[rgb(var(--surface-highlight-rgb))]/[0.07] pt-3">
                      <div className="flex flex-wrap items-center gap-2">
                      ...
                      <p className="mb-0 mt-2 text-[10px] leading-4 text-zinc-400">Buzz 대화에서 작업 루트를 고른 새 프로젝트 생성·등록, 현재 수동 채널과 등록 프로젝트 연결, GitHub 생성, 등록 프로젝트와 워크룸 세션 조회, 최근 출력 확인, AgentsToZ 화면 열기, Codex·Claude·Hermes로 프로젝트 열기, 연결된 DEV 채널의 Buzz 앱 열기를 요청할 수 있습니다. 새 프로젝트를 만들 때는 GitHub도 만들지와 Private/Public을 먼저 확인합니다.</p>
                      <p className="mb-0 mt-1 text-[10px] leading-4 text-zinc-500">폴더 경로·셸 명령은 받지 않습니다. 새 프로젝트의 Git·장기기억 초기화, 현재 Buzz 채널의 등록 프로젝트 연결, 사용자가 Private/Public을 명시한 GitHub 저장소 생성은 앱의 제한형 절차로 수행하지만, 임의 Git 작업·채널 생성·해제·삭제·빌드·배포·소스 수정은 허용하지 않습니다. Codex의 전체 접근 권한을 켜지 않고, 고정된 13개 MCP 도구만 이 단말의 로컬 sidecar에 연결합니다.</p>
                      <div className="mt-2 flex items-center justify-between gap-2 rounded-lg border border-[rgb(var(--surface-highlight-rgb))]/[0.07] bg-[rgb(var(--surface-shade-rgb))]/20 px-3 py-2">
                  ...
                  <div className="border-t border-[rgb(var(--surface-highlight-rgb))]/[0.07] pt-3">
                    ...
                    <textarea readOnly value={instructions} aria-label="Buzz USE 서비스 Agent system prompt" placeholder="USE 운영기억을 만든 뒤 안전 경계가 포함된 설정이 준비됩니다." className="mt-2 h-44 w-full resize-y rounded-lg border border-[rgb(var(--surface-highlight-rgb))]/10 bg-[rgb(var(--surface-shade-rgb))]/30 p-3 font-mono text-[10px] leading-4 text-zinc-300 outline-none placeholder:text-[var(--text-dim)]" />
                  </div>
                </div>
              )}

              <div className="rounded-xl border border-amber-300/15 bg-amber-300/[0.035] p-3 text-[10px] leading-4 text-amber-100/90">
                Buzz CLI는 최초 Agent를 직접 저장하지 않습니다. 아래 버튼은 Buzz 앱을 열고 설정을 복사하며, Agents → New agent에서 이름·실행기·Agent instructions를 붙여 넣은 뒤 <strong>Buzz Desktop에서 최종 생성 승인</strong>해야 합니다. Buzz에는 현재 Agent별 working-directory 입력란이 없으므로 경로는 instructions가 절대경로와 로컬 연결표로 검증합니다. 같은 서비스 Agent가 이미 있으면 중복 생성하지 마세요.
              </div>
