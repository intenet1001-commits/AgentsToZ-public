import {automaticMemoryObservationBoundary,migrateAutomaticMemoryPolicy,readAutomaticMemoryPolicy,changeAutomaticMemoryPolicy,refreshAutomaticMemoryProvider,assertLegacyAttemptAllowed,checkAutomaticMemoryAdmission,reserveAutomaticMemoryAdmission,type AutomaticMemoryPolicyInput} from './memorySaveAutomaticPolicy';
...
import {parseMemorySaveFailure,type MemorySaveFailure} from './memorySaveFailure';
import {migrateMemorySaveRecovery,readMemorySaveRecoveryCandidate,readMemorySaveRecoveryApproval,prepareMemorySaveRecoveryApproval,admitMemorySaveRecoverySuccessor} from './memorySaveRecoveryStore';
import {migrateMemoryProviderRecovery,readRecoveryProviderApproval,prepareRecoveryProviderApproval,claimRecoveryProviderProbe,finishRecoveryProviderProbe,
 readRecoveryProviderTransition,readLatestRecoveryProviderTransition,readRecoveryProviderProbeForApproval} from './memorySaveProviderTransitionStore';
import type {MemoryProviderRecoveryBinding,MemoryProviderRecoveryClaim,MemoryProviderRecoveryCompletion} from './memorySaveProviderTransitionContract';
import { canonicalSaveSource, memorySaveSourceKey, MEMORY_SAVE_PAGE_LIMIT, MEMORY_SAVE_SCHEMA_VERSION, MemorySaveError,
  saveDigest, saveHash, saveInteger, saveToken, type MemorySaveSource, type MemorySaveJob,
  type MemorySaveCommitReceipt, type MemorySaveAttemptIntent, type MemorySaveHostBinding,
  type MemorySaveRecoveryBinding,type MemorySaveRecoveryAdmission } from './memorySaveContract';
...
export interface MemorySaveStatusSnapshot {
  policy:ReturnType<typeof readAutomaticMemoryPolicy>;provider:MemoryProviderBinding|null;job:MemorySaveJob|null;
  failure:MemorySaveFailure|null;binding:{sessionPlanId:string;rootDigest:string}|null;bindingUnavailable:boolean;
...
export class MemorySaveStore {
  ...
  /** One bounded read transaction. Status never creates, migrates or chmods a
   * database; supported legacy diagnostics remain unknown until a writer upgrades. */
  statusSnapshot(memoryId:string):MemorySaveStatusSnapshot|null {
    if(!saveToken(memoryId))throw new MemorySaveError('INVALID_INPUT');
    let stamp;
    try{stamp=lstatSync(this.path);}catch(error:any){if(error?.code==='ENOENT')return null;throw new MemorySaveError('STORAGE_UNAVAILABLE');}
    if(!stamp.isFile()||stamp.isSymbolicLink())throw new MemorySaveError('STORAGE_UNAVAILABLE');
    ...
    try{
      ...
      return db.transaction(()=>{
        ...
        if(version<7||version>MEMORY_SAVE_SCHEMA_VERSION)throw new MemorySaveError('UNSUPPORTED_VERSION');
        const policySize=db!.query('SELECT length(CAST(payload AS BLOB)) AS size FROM save_auto_policy WHERE singleton=1').get() as {size:number}|null;
        if(!policySize||!saveInteger(policySize.size)||policySize.size>150000)throw new MemorySaveError('STORAGE_UNAVAILABLE');
        const policy=readAutomaticMemoryPolicy(db!);
        const providerRow=db!.query('SELECT CASE WHEN length(CAST(payload AS BLOB))<=4096 THEN payload ELSE NULL END AS payload FROM save_provider_binding WHERE singleton=1').get() as {payload:string|null}|null;
        if(providerRow&&!providerRow.payload)throw new MemorySaveError('STORAGE_UNAVAILABLE');
        const provider=providerRow?canonicalMemoryProviderBinding(JSON.parse(providerRow.payload!)):null;
        const job=db!.query(`SELECT ${jobProjection} FROM save_jobs WHERE memoryId=? ORDER BY sequence DESC LIMIT 1`).get(memoryId) as MemorySaveJob|null;
        if(job&&(!saveToken(job.saveId)||job.memoryId!==memoryId||!saveInteger(job.sequence)||!saveInteger(job.policyEpoch)||!saveHash(job.coverageDigest)
          ||(job.attemptId!==null&&!saveToken(job.attemptId))||!['prepared','summarizing','recovery-required','local-saved','superseded-unknown'].includes(job.phase)))throw new MemorySaveError('STORAGE_UNAVAILABLE');
        let failure:MemorySaveFailure|null=null,binding:MemorySaveStatusSnapshot['binding']=null,bindingUnavailable=false;
        if(job){
          if(version>=8)try{
            const row=db!.query(`SELECT CASE WHEN length(CAST(payload AS BLOB))<=512 THEN payload ELSE NULL END AS payload FROM save_failure_diagnostics
              WHERE saveId=? AND attemptId IS ?`).get(job.saveId,job.attemptId) as {payload:string|null}|null;
            if(row?.payload)failure=parseMemorySaveFailure(JSON.parse(row.payload));
          }catch{/* Missing/unreadable diagnostics never invent a cause. */}
          try{
            const row=db!.query(`SELECT CASE WHEN length(sessionPlanId)<=128 THEN sessionPlanId ELSE NULL END AS id,
              CASE WHEN length(CAST(payload AS BLOB))<=4096 THEN CASE WHEN json_valid(payload) THEN json_extract(payload,'$.rootDigest') END END AS rootDigest,
              CASE WHEN length(CAST(payload AS BLOB))<=4096 THEN CASE WHEN json_valid(payload) THEN json_extract(payload,'$.sessionPlanId') END END AS payloadId
              FROM save_host_sessions WHERE saveId=?`).get(job.saveId) as {id:string|null;rootDigest:unknown;payloadId:unknown}|null;
            if(row){if(!saveToken(row.id)||!saveHash(row.rootDigest)||row.payloadId!==row.id)bindingUnavailable=true;
              else binding={sessionPlanId:row.id,rootDigest:row.rootDigest};}
          }catch{bindingUnavailable=true;}
        }
        ...
        return {policy,provider,job,failure,binding,bindingUnavailable,backup:{pending:rows.slice(0,128).filter(r=>r.state==='pending').length,
          blocked:rows.slice(0,128).filter(r=>r.state==='blocked').length,hasMore:rows.length>128}};
      })();
  ...
  #db<T>(fn: (db: Database) => T): T {
    ...
    try {
      ...
      const version = (db.query('PRAGMA user_version').get() as {user_version:number}).user_version;
      if (version > MEMORY_SAVE_SCHEMA_VERSION) throw new MemorySaveError('UNSUPPORTED_VERSION');
      ...
      if(version<8)db.transaction(()=>{
        db!.run('CREATE TABLE save_failure_diagnostics (saveId TEXT PRIMARY KEY, attemptId TEXT, payload TEXT NOT NULL CHECK(length(payload)<=512))');
        db!.run('PRAGMA user_version=8');
      }).immediate();
      if(version<9)db.transaction(()=>{migrateMemorySaveRecovery(db!);db!.run('PRAGMA user_version=9');}).immediate();
      if(version<10)db.transaction(()=>{migrateMemoryProviderRecovery(db!);db!.run('PRAGMA user_version=10');}).immediate();
      return fn(db);
    } catch (error) {
  ...
  /** Explicit review reads are also non-creating and non-migrating. Versions 7
   * and 8 can be inspected before the user's review action upgrades the writer. */
  #recoveryRead<T>(read:(db:Database,version:number)=>T):T|null {
    let db:Database|undefined;
    try{
      let stamp;try{stamp=lstatSync(this.path);}catch(error:any){if(error?.code==='ENOENT')return null;throw error;}
      if(!stamp.isFile()||stamp.isSymbolicLink())throw new MemorySaveError('STORAGE_UNAVAILABLE');
      db=new Database(join(realpathSync(dirname(this.path)),basename(this.path)),constants.SQLITE_OPEN_READONLY|constants.SQLITE_OPEN_NOFOLLOW);
      db.run('PRAGMA busy_timeout=100');db.run('PRAGMA cache_size=-256');
      return db.transaction(()=>{
        const version=(db!.query('PRAGMA user_version').get() as {user_version:number}).user_version;
        if(version<7||version>MEMORY_SAVE_SCHEMA_VERSION)throw new MemorySaveError('UNSUPPORTED_VERSION');
        return read(db!,version);
      })();
    }catch(error){if(error instanceof MemorySaveError)throw error;throw new MemorySaveError('STORAGE_UNAVAILABLE');}finally{db?.close();}
  }
  recoveryCandidate(parentSaveId:string){
    if(!saveToken(parentSaveId))throw new MemorySaveError('INVALID_INPUT');
    return this.#recoveryRead((db,version)=>readMemorySaveRecoveryCandidate(db,parentSaveId,version));
  }
  recoveryApproval(approvalId:string){
    if(!saveToken(approvalId))throw new MemorySaveError('INVALID_INPUT');
    return this.#recoveryRead((db,version)=>version>=9?readMemorySaveRecoveryApproval(db,approvalId):null);
  }
  prepareRecoveryApproval(binding:MemorySaveRecoveryBinding){
    return this.#db(db=>db.transaction(()=>prepareMemorySaveRecoveryApproval(db,binding,this.now())).immediate());
  }
  admitRecoverySuccessor(input:MemorySaveRecoveryAdmission){
    return this.#db(db=>db.transaction(()=>admitMemorySaveRecoverySuccessor(db,input,this.now())).immediate());
  }
  prepareRecoveryProviderApproval(binding:MemoryProviderRecoveryBinding){
    return this.#db(db=>db.transaction(()=>prepareRecoveryProviderApproval(db,binding,this.now())).immediate());
  }
  claimRecoveryProviderProbe(input:MemoryProviderRecoveryClaim){
    return this.#db(db=>db.transaction(()=>claimRecoveryProviderProbe(db,input,this.now())).immediate());
  }
  finishRecoveryProviderProbe(transitionId:string,result:MemoryProviderRecoveryCompletion){
    return this.#db(db=>db.transaction(()=>finishRecoveryProviderProbe(db,transitionId,result,this.now())).immediate());
  }
  recoveryProviderApproval(approvalId:string){
    if(!saveToken(approvalId))throw new MemorySaveError('INVALID_INPUT');
    return this.#recoveryRead((db,version)=>version>=10?readRecoveryProviderApproval(db,approvalId):null);
  }
  recoveryProviderProbeForApproval(approvalId:string){
    if(!saveToken(approvalId))throw new MemorySaveError('INVALID_INPUT');
    return this.#recoveryRead((db,version)=>version>=10?readRecoveryProviderProbeForApproval(db,approvalId):null);
  }
  recoveryProviderTransition(transitionId:string){
    if(!saveToken(transitionId))throw new MemorySaveError('INVALID_INPUT');
    return this.#recoveryRead((db,version)=>version>=10?readRecoveryProviderTransition(db,transitionId):null);
  }
  latestRecoveryProviderTransition(parentSaveId:string){
    if(!saveToken(parentSaveId))throw new MemorySaveError('INVALID_INPUT');
    return this.#recoveryRead((db,version)=>version>=10?readLatestRecoveryProviderTransition(db,parentSaveId):null);
  }
  readyRecoveryProviderTransition(parentSaveId:string){
    if(!saveToken(parentSaveId))throw new MemorySaveError('INVALID_INPUT');
    return this.#recoveryRead((db,version)=>version>=10?readLatestRecoveryProviderTransition(db,parentSaveId,true):null);
  }
  ...
  reserve(memoryId: string, policyEpoch: number, keys: readonly string[]): MemorySaveJob {
    ...
    return this.#db(db => db.transaction(() => {
      ...
      if (db.query("SELECT 1 FROM save_jobs WHERE memoryId=? AND phase NOT IN ('local-saved','superseded-unknown')").get(memoryId)) throw new MemorySaveError('COVERAGE_RESERVED');
  ...
  openJob(memoryId:string):MemorySaveJob|null {
    if(!saveToken(memoryId))throw new MemorySaveError('INVALID_INPUT');
    return this.#db(db=>db.query(`SELECT ${jobProjection} FROM save_jobs WHERE memoryId=? AND phase NOT IN ('local-saved','superseded-unknown') LIMIT 1`).get(memoryId) as MemorySaveJob|null);
  }
  ...
  bindHostSession(saveId:string,attemptId:string,raw:MemorySaveHostBinding):void {
    ...
    this.#db(db=>db.transaction(()=>{
      ...
      if(job.phase==='superseded-unknown')throw new MemorySaveError('REVISION_CONFLICT');
  ...
  #providerRevalidation(db:Database,expectedRevision:number,configurationId:string):MemoryProviderBinding {
    if(!saveInteger(expectedRevision)||!saveHash(configurationId))throw new MemorySaveError('INVALID_INPUT');
    const policy=readAutomaticMemoryPolicy(db);
    if(!policy.enabled||policy.revision!==expectedRevision||policy.providerBindingDigest!==configurationId)throw new MemorySaveError('POLICY_CHANGED');
    // Includes prepared jobs, whose exact old binding must survive interrupted staging.
    if(db.query("SELECT 1 FROM save_jobs WHERE phase NOT IN ('local-saved','superseded-unknown') LIMIT 1").get())throw new MemorySaveError('RECOVERY_REQUIRED');
    const row=db.query('SELECT CASE WHEN length(payload)<=4096 THEN payload ELSE NULL END AS payload FROM save_provider_binding WHERE singleton=1').get() as {payload:string|null}|null;
    if(!row?.payload)throw new MemorySaveError('STORAGE_UNAVAILABLE');
    const binding=canonicalMemoryProviderBinding(JSON.parse(row.payload));
    if(saveDigest(binding)!==configurationId)throw new MemorySaveError('POLICY_CHANGED');
    return binding;
  }
  assertProviderRevalidation(expectedRevision:number,configurationId:string):MemoryProviderBinding {
    return this.#db(db=>this.#providerRevalidation(db,expectedRevision,configurationId));
  }
  revalidateProviderBinding(expectedRevision:number,configurationId:string,raw:MemoryProviderBinding):void {
    const next=canonicalMemoryProviderBinding(raw);
    this.#db(db=>db.transaction(()=>{
      const prior=this.#providerRevalidation(db,expectedRevision,configurationId);
      assertSameMemoryProviderIdentity(prior,next);
      refreshAutomaticMemoryProvider(db,expectedRevision,configurationId,saveDigest(next),this.now());
      db.query('UPDATE save_provider_binding SET payload=? WHERE singleton=1').run(JSON.stringify(next));
    }).immediate());
  }
  ...
  /** First failure only, scoped to its original job/attempt. This records no
   * execution authority and never changes coverage, quota, consent or receipts. */
  recordFailure(saveId:string,attemptId:string|null,raw:Omit<MemorySaveFailure,'recordedAt'>):void {
    if(!saveToken(saveId)||(attemptId!==null&&!saveToken(attemptId)))throw new MemorySaveError('INVALID_INPUT');
    const failure=parseMemorySaveFailure({...raw,recordedAt:this.now()});
    if(!failure)throw new MemorySaveError('INVALID_INPUT');
    this.#db(db=>db.transaction(()=>{
      const job=this.#job(db,saveId);if(job.attemptId!==attemptId||job.phase==='superseded-unknown')throw new MemorySaveError('REVISION_CONFLICT');
      db.query('INSERT OR IGNORE INTO save_failure_diagnostics(saveId,attemptId,payload) VALUES (?,?,?)').run(saveId,attemptId,JSON.stringify(failure));
    }).immediate());
  }
  failure(saveId:string):MemorySaveFailure|null {
    if(!saveToken(saveId))throw new MemorySaveError('INVALID_INPUT');
    return this.#db(db=>{
      const row=db.query(`SELECT CASE WHEN length(d.payload)<=512 THEN d.payload ELSE NULL END AS payload
        FROM save_failure_diagnostics d JOIN save_jobs j ON j.saveId=d.saveId
        WHERE d.saveId=? AND d.attemptId IS j.attemptId`).get(saveId) as {payload:string|null}|null;
      if(!row?.payload)return null;
      try{return parseMemorySaveFailure(JSON.parse(row.payload));}catch{return null;}
    });
  }
