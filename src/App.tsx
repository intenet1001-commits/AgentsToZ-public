import {ControlProfilePanel, requestControlProfile} from './ControlProfilePanel';
import {ProjectRoleBadge, ProjectRoleFilters} from './ProjectRoleLabels';
import {resolveProjectRoles, projectRoleWithoutBinding, PROJECT_ROLE_LABELS, type ProjectRole, type ProjectRoleView} from './projectRole';
...
const API = {
  ...
  async createControlCenter(workspaceRootId: string): Promise<{
    ...
    if (!response.ok || result.success !== true || !result.project?.projectId) {
      throw new Error(result.error || 'AgentsToZ OPS 운영 폴더를 만들지 못했습니다.');
    }
    return result;
  },
...
type TopLevelTab = 'ports' | 'terminal' | 'portal' | 'memory' | 'what-i-said';
const TOP_LEVEL_TABS: readonly TopLevelTab[] = ['ports', 'terminal', 'portal', 'memory', 'what-i-said'];
...
function App() {
  const openWorkroomDraft=(targetId:string,title:string,prompt:string)=>{setTerminalEntry({nonce:++runtimeEntryNonceRef.current,targetId,title,prompt});setActiveTab('terminal');};
  const requestSessionMemory=(targetId:string)=>openWorkroomDraft(targetId,'세션 기억하기','이 프로젝트의 remember-session 스킬을 실행해 완료된 작업과 검증된 결정을 장기기억에 저장하세요. 실제 저장소와 기존 기억을 먼저 확인하고, 진행 중인 작업이나 잠금이 있으면 강제로 해제하거나 덮어쓰지 말고 현재 상태를 설명하세요.');
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      try {
        const response = await fetch(`${isTauri() ? 'http://127.0.0.1:3001' : ''}/api/agentstoz-use/workroom-navigation`, { cache: 'no-store' });
        ...
        if (typeof navigation.nonce === 'string' && navigation.panel === 'ops') {
          lastUseWorkroomNavigation.current = navigation.nonce;
          setShowControlProfile(true);
          return;
        }
  ...
  const [setupInitialPage, setSetupInitialPage] = useState<'choose' | 'infrastructure'>('choose');
  ...
  const [testerOpenPort,setTesterOpenPort]=useState<string|null>(null);
  const [testerRevealKey,setTesterRevealKey]=useState(0);
  ...
  const [legacyNameWorkVisible, setLegacyNameWorkVisible] = useState(false);
  ...
  const [showControlProfile, setShowControlProfile] = useState(false);
  const [opsBinding, setOpsBinding] = useState<{loaded: boolean; projectId: string | null}>({loaded:false, projectId:null});
  useEffect(() => {
    // Read existing identity only. Closing the profile panel refreshes an attach;
    // no polling, preparation, migration, or memory write is introduced here.
    if (showControlProfile) return;
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 15_000);
    setOpsBinding(current => ({...current, loaded:false}));
    void requestControlProfile('status', undefined, controller.signal).then(result => {
      if (controller.signal.aborted || !result.profile) return;
      const id = result.profile.projectId;
      if (id !== null && typeof id !== 'string') return;
      setOpsBinding({loaded:true, projectId:id});
    }).catch(() => { /* Memory editor stays closed until identity can be read. */ });
  ...
  const confirmCopyProjectFolderRenamePrompt = async (direct=false) => {
    ...
    try {
      ...
      if(direct){openWorkroomDraft(item.id,folderChangeMode === 'move' ? '프로젝트 폴더 위치 이동' : '프로젝트 폴더명 변경',prompt);}
  ...
  const handleCopyBranchHygienePrompt = useCallback(async (item: PortInfo, direct=false) => {
    ...
    try {
      ...
      if(direct){openWorkroomDraft(item.id,'브랜치 정리',prompt);return;}
  ...
  const createControlCenterAtRoot = async (workspaceRootId: string) => {
    ...
    showToast(
      result.performed
        ? 'AgentsToZ OPS 운영 폴더(AgentsToZ-Control)와 Git·장기기억을 만들었습니다. 개인 GitHub 저장소 연결은 프로젝트의 GitHub 메뉴에서 이어가세요.'
        : '기존 AgentsToZ OPS 운영 폴더(AgentsToZ-Control)를 열었습니다.',
      'success',
    );
    return result;
  };
  ...
  const handleCreateProjectFolder = async () => {
    ...
    if (newProjectKind === 'control-center') {
      try {
        ...
        showToast(`AgentsToZ OPS 운영 폴더 생성 실패: ${error instanceof Error ? error.message : String(error)}`, 'error');
      }
      return;
    }
  ...
  const primaryProject = useMemo(() => resolvePrimaryProject(
    ports.filter(project => project.id !== opsBinding.projectId), AGENTSTOZ_PUBLIC_REPOSITORY_URL,
  ), [ports, opsBinding.projectId]);
  const controlCenterProject = useMemo(() => ports.find(project => project.id === opsBinding.projectId)
    ?? resolveControlCenterProject(ports), [ports, opsBinding.projectId]);
  const projectRoles = useMemo(() => resolveProjectRoles(ports, {
    opsProjectId:opsBinding.projectId, devProjectId:primaryProject?.id,
  }), [ports, opsBinding.projectId, primaryProject?.id]);
  const memoryRoleFor = (project: PortInfo): ProjectRoleView => {
    const role = projectRoles.get(project.id) ?? 'unknown';
    return role === 'ops' || opsBinding.loaded ? role : 'unknown';
  };
  const handleOnboardingControlCenter = async (choice: 'create' | 'restore') => {
    if (controlCenterSetupBusy) return;
    if (choice === 'create') { setShowSetupWizard(false); setShowControlProfile(true); return; }
    if (controlCenterProject) {
      ...
      showToast('기존 AgentsToZ OPS 운영 폴더(AgentsToZ-Control)를 열었습니다.', 'success');
      return;
    }
    if (choice === 'restore') {
      // Restore must not inherit a previous plain-folder or manual memory-join form.
      ...
      showToast('AgentsToZ OPS의 Private 저장소(AgentsToZ-Control) GitHub 주소를 넣으면 같은 memoryId의 기억을 먼저 복원합니다.', 'warning');
      return;
    }
    setShowSetupWizard(false);
    setShowControlProfile(true);
  ...
  const sendTesterToWorkroom = (project:PortInfo) => async(agent:AiTerminalAgent,prompt:string,workspaceTargetId?:string) => {
    const inventory=await new AgentRuntimeClient().targets();
    const targetId=workspaceTargetId??project.id;
    if(!inventory.targets.some(t=>t.targetId===targetId&&(t.targetId===project.id||t.projectTargetId===project.id)))throw new Error('검사 대상 프로젝트·워크트리를 다시 확인하세요.');
    const direct=agent==='codex'||agent==='claude';
    if(!direct)await navigator.clipboard.writeText(prompt);
    const {session}=await localAiTerminalTransport({operation:'start',requestId:crypto.randomUUID(),targetId,agent,cols:100,rows:28,...(direct?{prompt}:{})});
    if(!session||session.state!=='running')throw new Error('워크룸 실행 결과를 확인하지 못했습니다.');
    await localAiTerminalTransport({operation:'read',requestId:crypto.randomUUID(),sessionId:session.id,after:0});
    setTerminalEntry({nonce:++runtimeEntryNonceRef.current,targetId,agent,sessionId:session.id});setActiveTab('terminal');
    if(!direct)showToast('인계문을 복사했습니다. CLI 준비 상태를 확인한 뒤 워크룸에 붙여넣으세요.','success');
  };
  ...
  const openAgentsToZConversation = useCallback((project: PortInfo) => {
    ...
    setTerminalEntry({nonce:++runtimeEntryNonceRef.current,targetId:project.id});
    setActiveTab('terminal');
  ...
  const v3Ports = useMemo(() => {
    ...
    if (sidebarSection.startsWith('role:')) list = list.filter(p => !p.worktreePath && !p.worktreeParentId && projectRoles.get(p.id) === sidebarSection.slice(5));
    else if (sidebarSection === 'running') list = list.filter(p => p.isRunning);
    ...
    return list;
  }, [searchFilteredPorts, sidebarSection, lastVisits, gitActivity, filteredCurrentWorktreeParentIds, ports, projectRoles]);
      ...
      >
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,flexWrap:'wrap'}}>
          ...
          <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
            ...
            <button type="button" data-testid={`edit-ai-request-${context}`} disabled={!canUseAi} onClick={()=>void handleAiSuggestEdit()} className="rounded-md border border-[var(--line)] px-2 py-1 text-xs">AI로 생성</button>
          </div>
        </div>
      ...
      return (
        <div key={item.id} style={{padding:12,background:'var(--bg-card)',border:'1px solid rgb(var(--surface-highlight-rgb) / 0.12)',borderRadius:8,display:'flex',flexDirection:'column',gap:6}}>
          ...
          <Suspense fallback={null}>
            <ProjectMemoryPanel
              projectRole={memoryRoleFor(item)}
              onOpenOps={() => setShowControlProfile(true)}
    ...
    return (
      ...
      >
        <div style={{display:'flex',alignItems:'center',gap:5}}>
          ...
          <ProjectRoleBadge role={projectRoles.get(item.id) ?? 'unknown'}/>
        ...
        <ProjectTesterPanel portId={item.id} projectName={item.name} onSend={sendTesterToWorkroom(item)}/>
        {/* Worktree panel */}
      ...
      {worktreeLoadErrors[portItem.id] ? (
        ...
        (worktreeLists[portItem.id] ?? []).map(wt => {
          ...
          const prepareGitMergeSync = async () => {
            ...
            openWorkroomDraft(candidates[0]!.targetId,'AI 머지·싱크',buildGitMergeSyncPrompt({projectName:portItem.name,projectPath:portItem.folderPath||wt.path,worktreePath:wt.path,branch:wt.branch,isMainWorktree:wt.is_main}));
          ...
          const copyPublicRepositoryUpdatePrompt = async (direct=false) => {
            ...
            try {
              ...
              if(direct){openWorkroomDraft(portItem.id,'공개 저장소 업데이트',prompt);return;}
              await copyAgentsToZPrompt(prompt);
    ...
    return (
      <div className="workspace-project-layout" style={{flex:1,display:'flex',overflow:'hidden'}}>
        ...
        {workspaceSidebarHost && createPortal(<div className="workspace-project-sidebar" style={{
          ...
          <ProjectRoleFilters projects={searchFilteredPorts} roles={projectRoles} section={sidebarSection} onSelect={setSidebarSection}/>
        ...
        {(!isMobile || sel) && (sel ? (
          ...
          <div className="workspace-project-detail" style={{flex:1,overflowY:'auto',padding:'28px 32px'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:6,flexWrap:'wrap' as const}}>
              ...
              <ProjectRoleBadge role={projectRoles.get(sel.id) ?? 'unknown'}/>
            ...
            <ProjectTesterPanel initialOpen={testerOpenPort===sel.id} revealKey={testerRevealKey} portId={sel.id} projectName={sel.name} onSend={sendTesterToWorkroom(sel)}/>
            {/* 워크트리 패널 */}
      ...
      <div className="workspace-shell flex-1 min-h-0" data-mobile={isMobile ? 'true' : 'false'} data-sidebar-only={isMobile && activeTab === 'ports' && !v4SelectedId ? 'true' : 'false'}>
        ...
        >
          ...
          <div data-testid="top-toolbar-project-actions" className="workspace-project-tools">
            ...
            <button type="button" data-testid="control-profile-open" className="min-h-11 rounded-xl border border-[var(--line)] px-3 text-sm font-semibold" onClick={()=>setShowControlProfile(true)}>AgentsToZ OPS · 운영 프로필</button>
            {controlCenterProject ? <button
              type="button"
              data-testid="control-center-project-shortcut"
              onClick={() => void handleOnboardingControlCenter('restore')}
              title="AgentsToZ OPS 운영 폴더 열기 · 실제 폴더명 AgentsToZ-Control"
              className="flex items-center gap-1.5 rounded-xl border border-[var(--line)] px-3 py-2 text-xs font-semibold"
            >
              <Network className="w-3 h-3" />
              AgentsToZ OPS 운영 폴더 열기
            </button> : <details data-testid="control-center-project-shortcut" className="rounded-xl border border-[var(--line)] p-3 text-xs">
              <summary className="cursor-pointer font-semibold">AgentsToZ OPS 운영 폴더 · 이 Mac에 미등록</summary>
              <p className="mt-2 max-w-sm leading-relaxed">AgentsToZ OPS 운영 폴더의 실제 이름은 AgentsToZ-Control입니다. 앱 저장소를 Pull하거나 앱을 업데이트해도 별도 운영 저장소는 복제되지 않습니다. 다른 Mac에서 사용 중이면 복원하세요.</p>
              <div className="mt-2 flex flex-wrap gap-2">
                <button type="button" disabled={controlCenterSetupBusy} className="min-h-11 rounded-lg border border-[var(--line)] px-3" onClick={() => void handleOnboardingControlCenter('restore')}>다른 Mac의 AgentsToZ OPS 복원</button>
                <button type="button" disabled={controlCenterSetupBusy} className="min-h-11 rounded-lg border border-[var(--line)] px-3" onClick={() => void handleOnboardingControlCenter('create')}>처음 사용 · AgentsToZ OPS 준비</button>
              </div>
            </details>}
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              onClick={() => { setSetupInitialPage('infrastructure'); setShowSetupWizard(true); }}
              title={lang === 'ko' ? '내 기기와 연결 · 설치·로그인·설정' : 'My device and connections · Setup'}
              aria-label={lang === 'ko' ? '내 기기와 연결' : 'My device and connections'}
              className="p-1.5 bg-[var(--bg-card)] hover:bg-[var(--bg-elevated)] text-zinc-500 hover:text-[rgb(var(--text-primary-rgb))]/90 rounded-lg border border-zinc-800/40 hover:border-zinc-700/60 transition-all"
            >
        ...
        {quickLabelItems&&<QuickProjectNames items={quickLabelItems} onClose={()=>{setQuickLabelItems(null);quickLabelExpected.current.clear();}} onApply={async results=>{
        ...
        {legacyNameWork&&!legacyNameWorkVisible&&<button type="button" data-testid="legacy-name-work-reopen" onClick={()=>setLegacyNameWorkVisible(true)} className="fixed bottom-5 right-5 z-[149] rounded-lg bg-zinc-800 px-4 py-3 text-sm text-white shadow-lg">별명·카테고리 작업 다시 열기</button>}
        {legacyNameWork&&legacyNameWorkVisible&&<div role="dialog" aria-modal="true" aria-label="프로젝트 별명·카테고리 작업" className="fixed inset-0 z-[150] flex items-center justify-center bg-black/35 p-4"><div className="max-h-[85vh] w-full max-w-2xl overflow-auto"><LegacyProjectNameWork work={legacyNameWork} onApplyDraft={applyNameDraft}
            onReturn={()=>{setActiveTab('ports');setLegacyNameWorkVisible(false);}}
            onDismiss={()=>{publishNameWork(null);setLegacyNameWorkVisible(false);nameDraftBase.current=null;}}
            onStopAfterCurrent={()=>{stopNameEnrichment.current=true;const work=legacyNameWorkRef.current;if(work)publishNameWork({...work,state:'stopping'});}}/></div></div>}
        ...
        {activeTab === 'memory' && (
          <div id="top-level-memory-panel" role="tabpanel" aria-labelledby="tab-memory" data-testid="top-level-memory-view" className="flex-1 min-h-0 overflow-auto flex flex-col">
            ...
            <div className="m-3 rounded-xl border border-[var(--line)] p-3 text-sm" data-testid="memory-role-boundaries">
              <p>AgentsToZ OPS 운영 기억 · AgentsToZ DEV 개발 기억 · 관리 프로젝트별 기억은 각각 별도입니다. 각 프로젝트의 USE/DEV 봇 구분은 바꾸지 않습니다.</p>
              <button type="button" className="mt-2 min-h-11 rounded-lg border border-[var(--line)] px-3" onClick={() => setShowControlProfile(true)}>AgentsToZ OPS · 운영 기억 확인</button>
            </div>
        ...
        {showInternetQrRemoteControl && (
          <Suspense fallback={null}>
            <InternetQrRemoteControlDialog
              ...
              onOpenWorkroom={() => { setShowInternetQrRemoteControl(false); setActiveTab('terminal'); }}
            ...
            >
              <div className="bg-[var(--bg-card)] border border-teal-400/25 rounded-2xl shadow-[var(--dialog-shadow)] w-full max-w-md p-6" onClick={event => event.stopPropagation()}>
                ...
                <div className="flex flex-wrap gap-2">
                  <button data-testid="folder-rename-prompt-workroom" disabled={!!problem} className="rounded-xl border border-zinc-700 px-3 text-sm" onClick={()=>void confirmCopyProjectFolderRenamePrompt(true)}>워크룸에서 검토·실행…</button>
        ...
        {showControlProfile && <ControlProfilePanel onOpenProject={id=>{const p=ports.find(p=>p.id===id);if(!p){showToast('프로젝트 목록을 새로고침하세요.','warning');return;}setShowControlProfile(false);setActiveTab('ports');setSearchQuery('');setSidebarSection('all');setV4SelectedId(id);setTesterOpenPort(id);setTesterRevealKey(v=>v+1);}} onClose={()=>setShowControlProfile(false)} onOpenWork={()=>{setShowControlProfile(false);setActiveTab('terminal');}} onOpened={result=>{
          showToast([result.message,...result.warnings].join('\n'),result.warnings.length?'warning':'success');
          if(result.session){setShowControlProfile(false);setTerminalEntry({nonce:++runtimeEntryNonceRef.current,targetId:result.session.targetId,agent:result.session.agent,sessionId:result.session.id});setActiveTab('terminal');}
        }} />}
        {showSetupWizard && (
          <Suspense fallback={null}>
          <SetupWizard
            initialPage={setupInitialPage}
            onOpenFirstTask={() => {
              setShowSetupWizard(false); setActiveTab('terminal');
            }}
            onStartProject={(choice) => {
              ...
              setNewProjectKind('project');
              // Set optional effects before the modal's first paint; its normal
              // defaults belong to returning users, not this first-project flow.
              setInitializeNewProjectGit(false);
              setEnableNewProjectMemory(false);
              setBackupNewProjectMemory(false);
              setRunNewProjectAfterCreate(false);
              ...
              <div data-testid="project-main-header" style={{
                ...
                <h1 data-help-key="header-section-title" style={{margin:0,fontSize:18,fontWeight:600,letterSpacing:-0.3,color:'var(--text-primary)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',minWidth:0}}>
                  {sidebarSection === 'all' ? t(lang,'sectionAll')
                    ...
                    : sidebarSection.startsWith('role:') ? PROJECT_ROLE_LABELS[sidebarSection.slice(5) as ProjectRoleView]
      ...
      {showNewProjectModal && (
        <div className="fixed inset-0 bg-[var(--dialog-scrim)] backdrop-blur-sm z-50 flex items-center justify-center p-4"
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="new-project-title" data-testid="new-project-dialog" className="workspace-new-project bg-[var(--bg-card)] rounded-xl border border-zinc-800/40 w-full max-w-lg max-h-[calc(var(--ui-viewport-height,100dvh)-2rem)] overflow-y-auto p-6 space-y-4"
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h2 id="new-project-title" className="text-base font-semibold text-[var(--text-primary)]">{newProjectKind === 'control-center' ? 'AgentsToZ OPS 운영 폴더 준비' : '프로젝트 추가'}</h2>
            ...
            {newProjectKind === 'control-center'
              ? <p role="status" className="rounded-xl bg-[var(--accent-soft)] p-3 text-sm leading-relaxed text-[var(--ink)]">작업 루트만 선택하면 AgentsToZ OPS 운영 폴더(<code>AgentsToZ-Control</code>), Git 초기 커밋, 프로젝트 등록과 장기기억 백업을 한 번에 준비합니다.</p>
              : onboardingProject && <p role="status" className="rounded-xl bg-[var(--accent-soft)] p-3 text-sm leading-relaxed text-[var(--ink)]">폴더 위치와 이름을 확인한 뒤 등록하세요. Git 저장소와 장기기억은 아래에서 원할 때 켤 수 있습니다. 등록 후 프로젝트 카드에서 워크룸으로 AI 작업을 시작할 수 있습니다.</p>}
            {/* 탭 선택 */}
                  ...
                  <>
                    ...
                    {registerAsProject && (
                      ...
                      <div className="rounded-lg border border-teal-500/20 bg-teal-500/5 p-3 text-xs leading-relaxed text-teal-100">
                        생성 항목: AgentsToZ OPS 운영 문서 · Git 저장소와 초기 커밋 · AgentsToZ 프로젝트 등록 · Codex 장기기억과 Supabase 백업
                      </div>
                    )}
                    <div className="flex gap-2 pt-1">
                      ...
                      <button onClick={handleCreateProjectFolder}
                        ...
                        {newProjectKind === 'control-center' ? 'AgentsToZ OPS 운영 폴더 만들기' : newProjectCloneRequest ? 'clone해서 만들기' : '만들기'}
                      </button>
                    </div>
                  </>
                )}
            ...
            {projectModalTab === 'existing' && (
              <>
                ...
                <Suspense fallback={null}>
                  <ProjectMemoryPanel
                    projectRole={(() => {
                      const registered = ports.find(p => p.folderPath === existingFolderPath);
                      return registered ? memoryRoleFor(registered) : opsBinding.loaded ? projectRoleWithoutBinding({folderPath:existingFolderPath}) : 'unknown';
                    })()}
                    onOpenOps={() => setShowControlProfile(true)}
