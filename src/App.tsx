...
const API = {
  ...
  async openCodeApp(
    ...
    if (isTauri() && mode === 'reopen' && agent === 'codex') {
    ...
    const response = await fetch(`${baseUrl}/api/open-code-app`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ agent, folderPath, mode }),
    });
    const result = await response.json().catch(() => ({}));
...
const mergePorts = (local: PortInfo[], remote: PortInfo[]): PortInfo[] => {
  ...
  const merged = local.map(p => {
    ...
    return {
      ...
      // A pre-migration Supabase row has no provenance field. Preserve the
      // local positive marker instead of letting remote `undefined` revive the
      // generated worktree card as a normal project.
      worktreeParentId: p.worktreeParentId ?? r.worktreeParentId,
      // The server generation is authoritative for this exact row. Keeping a
      // stale local generation would make the next fenced Push fail closed.
      syncGeneration: r.syncGeneration ?? p.syncGeneration ?? '0',
...
function App() {
  ...
  const discoverAndPersistDeletedPortFences = useCallback(async (
    ...
    if (!propagation.available || propagation.markerIds.length === 0) return [];
  ...
  const confirmCopyProjectFolderRenamePrompt = async (direct=false) => {
    ...
    try {
      ...
      if(direct){setTerminalEntry({nonce:Date.now(),targetId:item.id,title:folderChangeMode === 'move' ? '프로젝트 폴더 위치 이동' : '프로젝트 폴더명 변경',prompt});setActiveTab('terminal');}
  ...
  const handleCopyBranchHygienePrompt = useCallback(async (item: PortInfo, direct=false) => {
    ...
    try {
      ...
      if(direct){setTerminalEntry({nonce:Date.now(),targetId:item.id,title:'브랜치 정리',prompt});setActiveTab('terminal');return;}
  ...
  const openProjectCodeApp = async (
    ...
    try {
      const result = await API.openCodeApp(agent, targetPath, mode);
      ...
      if (agent === 'claude') {
        showToast(
          result.reusedActiveSession
            ? `기존 Claude Code 대화를 다시 열었습니다: ${targetName}`
            : `Claude Code 대화를 시작하고 앱에서 열었습니다: ${targetName}`,
          'success',
        );
      } else if (agent === 'codex') {
        showToast(
          mode === 'new'
            ? `새 Codex 프로젝트 대화를 열었습니다: ${targetName}`
            : `가장 최근 Codex 프로젝트 대화를 다시 열었습니다: ${targetName}`,
          'success',
        );
      } else {
        showToast(
          mode === 'open'
            ? `프로젝트 전용 Hermes Desktop 창을 열었습니다: ${targetName}`
            : result.selectionVerified === true
              ? `가장 최근 Hermes 프로젝트 대화를 다시 열었습니다: ${targetName}`
              : `Hermes Desktop 실행을 확인하고 최근 프로젝트 대화를 여는 요청을 전달했습니다. 앱에서 대화 선택을 확인하세요: ${targetName}`,
          'success',
        );
      }
    } catch (error) {
          ...
          if (portalData?.supabaseUrl && portalData?.supabaseAnonKey) {
            ...
            try {
              ...
              if (!error && remoteData && remoteData.length > 0) {
                ...
                const merged = mergePorts(updatedData, visibleRemoteRows);
                updatedData = merged;
                // 메모 복원
                ...
                remoteData.forEach((row: any) => {
                  if (visibleRemoteIds.has(row.id) && row.memo != null) {
  ...
  const saveEdit = async () => {
    if (editingId && editName) {
      ...
      const nextPort = original ? {
        ...original,
        name: editName,
        port: editPort ? parseInt(editPort) : undefined,
        commandPath: editCommandPath || undefined,
        terminalCommand: editTerminalCommand || undefined,
        folderPath: autoFolderPath || undefined,
        deployUrl: editDeployUrl || undefined,
        ...githubRepositoryUrlFields(editGithubUrl),
        worktreePath: editWorktreePath || undefined,
        manualPath: editManualPath || undefined,
        logFilePath: editLogFilePath || undefined,
  ...
  const handleCopyEditAiNamePrompt = useCallback(async () => {
    ...
    try {
      await copyAgentsToZPrompt(buildSingleAiNameChatPrompt({
        id: editingId,
        name: editName.trim(),
    ...
    try {
      ...
      const restoreResult = await withPortalSafetyLease(portalData, async lease => {
        ...
        if (isOtherDevice) {
          ...
          merged = mergePorts(currentPorts, mergeRemoteRows);
        }
  ...
  const openProjectTerminal = async (project: PortInfo, agent: AiTerminalAgent, worktree?: Pick<WorktreeInfo, 'path' | 'is_main'>, prompt?: string) => {
    try {
      ...
      const result = await localAiTerminalTransport({operation: 'start', requestId: crypto.randomUUID(), targetId, agent, cols: 100, rows: 28, ...(prompt ? {prompt} : {})});
      if (!result.session) throw new Error('터미널 실행 결과를 확인하지 못했습니다. AI 터미널 탭에서 세션 목록을 확인하세요.');
      setTerminalEntry({nonce: ++runtimeEntryNonceRef.current, targetId, agent, sessionId: result.session.id});
      ...
      return (
        <div key={item.id} style={{padding:12,background:'var(--bg-card)',border:'1px solid rgb(var(--surface-highlight-rgb) / 0.12)',borderRadius:8,display:'flex',flexDirection:'column',gap:6}}>
          <div style={{display:'flex',gap:6}}>
            ...
            <button onClick={saveEdit} style={{padding:'5px 8px',background:'rgb(var(--ok-rgb) / 0.14)',border:'1px solid rgb(var(--ok-rgb) / 0.3)',borderRadius:6,cursor:'pointer',display:'flex',alignItems:'center'}}>
              <Check className="w-3.5 h-3.5" style={{color:'var(--ink-4ade80)'}} />
            </button>
            <button aria-label="프로젝트 수정 취소" onClick={cancelEdit} style={{padding:'5px 8px',background:'transparent',border:'1px solid rgb(var(--surface-highlight-rgb) / 0.07)',borderRadius:6,cursor:'pointer',display:'flex',alignItems:'center'}}>
      ...
      {worktreeLoadErrors[portItem.id] ? (
        ...
        (worktreeLists[portItem.id] ?? []).map(wt => {
          ...
          const prepareGitMergeSync = async () => {
            ...
            setTerminalEntry({nonce:Date.now(),targetId:candidates[0]!.targetId,title:'AI 머지·싱크',prompt:buildGitMergeSyncPrompt({projectName:portItem.name,projectPath:portItem.folderPath||wt.path,worktreePath:wt.path,branch:wt.branch,isMainWorktree:wt.is_main})});
            setActiveTab('terminal');
          ...
          const copyPublicRepositoryUpdatePrompt = async (direct=false) => {
            ...
            try {
              ...
              if(direct){setTerminalEntry({nonce:Date.now(),targetId:portItem.id,title:'공개 저장소 업데이트',prompt});setActiveTab('terminal');return;}
              await copyAgentsToZPrompt(prompt);
            ...
            <div key={wt.path} style={{padding:'5px 6px',borderRadius:5,background:wt.is_main?'rgb(var(--accent-rgb) / 0.04)':'rgb(var(--surface-highlight-rgb) / 0.02)',border:'1px solid rgb(var(--surface-highlight-rgb) / 0.05)',borderLeft:wt.is_main?'2px solid rgb(var(--accent-rgb) / 0.35)':'1px solid rgb(var(--surface-highlight-rgb) / 0.05)',display:'flex',flexDirection:'column',gap:4}}>
              ...
              <div className="worktree-action-groups" data-testid="worktree-action-groups" data-worktree-path={wt.path}>
                <div className="worktree-action-group" data-testid="worktree-internal-ai">
                  ...
                  <ProjectTerminalLauncher testId="worktree-terminal-launcher" onOpen={agent => openProjectTerminal(portItem, agent, wt)}/>
      ...
      <div className="workspace-project-layout" style={{flex:1,display:'flex',overflow:'hidden'}}>
        ...
        {(!isMobile || sel) && (sel ? (
          editingId === sel.id ? (
            ...
            <div style={{flex:1,overflowY:'auto',padding:'28px 32px',display:'flex',flexDirection:'column',gap:6}}>
              ...
              <div style={{display:'flex',gap:6}}>
                ...
                <button onClick={saveEdit} style={{padding:'5px 8px',background:'rgb(var(--ok-rgb) / 0.14)',border:'1px solid rgb(var(--ok-rgb) / 0.3)',borderRadius:6,cursor:'pointer',display:'flex',alignItems:'center'}}><Check className="w-3.5 h-3.5" style={{color:'var(--ink-4ade80)'}}/></button>
                <button aria-label="프로젝트 수정 취소" onClick={cancelEdit} style={{padding:'5px 8px',background:'transparent',border:'1px solid rgb(var(--surface-highlight-rgb) / 0.07)',borderRadius:6,cursor:'pointer',display:'flex',alignItems:'center'}}><XIcon className="w-3.5 h-3.5" style={{color:'var(--text-dim)'}}/></button>
              </div>
              <FolderDropZone
                compact
                pathKind="file"
          ...
          <div className="workspace-project-detail" style={{flex:1,overflowY:'auto',padding:'28px 32px'}}>
            ...
            <div data-testid="terminal-agent-panel" style={{marginBottom:8,padding:'8px 10px',borderRadius:7,background:'rgb(var(--surface-highlight-rgb) / 0.018)',border:'1px solid rgb(var(--surface-highlight-rgb) / 0.06)'}}>
              <div style={{fontSize:10,color:'var(--text-secondary)',fontWeight:600,marginBottom:2}}>{terminalApp === 'internal' ? '앱 내부 AI 터미널에서 실행' : '선택한 터미널에서 AI 실행'}</div>
            ...
            <div data-testid="desktop-project-app-panel" style={{marginBottom:10,padding:'8px 10px',borderRadius:7,background:'rgb(var(--violet-rgb) / 0.035)',border:'1px solid rgb(var(--violet-rgb) / 0.12)'}}>
              <div style={{fontSize:10,color:'var(--ink-d8b4fe)',fontWeight:600,marginBottom:2}}>AgentsToZ에서 작업</div>
              <div style={{fontSize:9,color:'var(--text-dim)',marginBottom:6,lineHeight:1.5}}>
                현재 프로젝트에서 실행할 AI를 선택하세요. 터미널은 앱 안에서 열립니다.
              </div>
              <ProjectTerminalLauncher testId="detail-terminal-launcher" onOpen={agent => openProjectTerminal(sel, agent)}/>
              <button
                type="button"
            ...
            <div data-testid="detail-provider-apps" className="project-provider-apps">
              <div style={{fontSize:9,color:'var(--text-muted)',marginBottom:6}}>외부 공급자 앱에서 열기 · 확인된 최근 대화가 있으면 정확히 다시 연결</div>
              <div style={{display:'flex',gap:6,flexWrap:'wrap' as const}}>
                {agentShown('claude') && (
                  <button
    ...
    <div className="h-screen flex flex-col overflow-hidden" style={{background:'var(--bg-deep)'}}>
      ...
      <div className="workspace-shell flex-1 min-h-0" data-mobile={isMobile ? 'true' : 'false'} data-sidebar-only={isMobile && activeTab === 'ports' && !v4SelectedId ? 'true' : 'false'}>
          ...
          <div data-testid="top-toolbar-project-actions" className="workspace-project-tools">
            ...
            {activeTab === 'ports' && (
              <>
                ...
                <button className="rounded-xl border border-zinc-700 bg-[var(--bg-card)] px-3 py-2 text-xs" disabled={!displayedPorts.length} onClick={()=>setQuickLabelItems(displayedPorts.slice(0,30).map(p=>({id:p.id,name:p.name.slice(0,120),description:(p.description??'').slice(0,500)})))}>AI 이름 추천</button>
        ...
        <div className="workspace-content">
          ...
          {quickLabelItems&&<QuickProjectNames items={quickLabelItems} onClose={()=>setQuickLabelItems(null)} onApply={async results=>{
            const before=portsRef.current;const proposals=new Map(results.map(r=>[r.id,r]));
            const next=before.map(p=>{const r=proposals.get(p.id);const input=quickLabelItems.find(i=>i.id===p.id);return r&&input&&p.name===input.name?{...p,aiName:r.name,category:r.category}:p});
            await API.savePorts([...next,...otherPlatformPortsRef.current]);setPorts(next);portsRef.current=next;showToast('추천 이름과 카테고리를 적용했습니다.','success');
          }}/>}
        <div id="top-level-terminal-panel" role="tabpanel" aria-labelledby="tab-terminal" hidden={activeTab!=='terminal'} className={activeTab==='terminal'?'flex-1 min-h-0 overflow-auto flex flex-col':''}>
          {activeTab==='terminal'&&<Suspense fallback={<p>AI 터미널을 불러오는 중…</p>}><AiTerminalPanel projects={agentRuntimeProjects} entry={terminalEntry} onManageProject={openAgentRuntimeProjectManager} onWhatISaid={()=>setActiveTab('what-i-said')}/></Suspense>}
        </div>
        <div
      ...
      {showNewProjectModal && (
        <div className="fixed inset-0 bg-[var(--dialog-scrim)] backdrop-blur-sm z-50 flex items-center justify-center p-4"
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="new-project-title" data-testid="new-project-dialog" className="workspace-new-project bg-[var(--bg-card)] rounded-xl border border-zinc-800/40 w-full max-w-lg max-h-[calc(var(--ui-viewport-height,100dvh)-2rem)] overflow-y-auto p-6 space-y-4"
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h2 id="new-project-title" className="text-base font-semibold text-[var(--text-primary)]">프로젝트 추가</h2>
