...
import { Server, Trash2, Plus, ExternalLink, Terminal, ArrowUpDown, Pencil, Check, X as XIcon, Play, Square, Rocket, FolderOpen, Upload, Download, Folder, FilePlus, Package, RefreshCw, FileText, RotateCw, Globe, Github, SquareTerminal, Info, Monitor, BookMarked, Cloud, CloudUpload, CloudDownload, Search, Sparkles, Settings, GitPullRequest, Copy, GitBranch, GitCommit, Star, Pin, BookOpen, ChevronDown, ChevronUp, StickyNote, Clock, Zap, History, Laptop, Keyboard, LayoutList, LayoutGrid, MoreHorizontal, Lock, Gauge, Mic, SlidersHorizontal, MessageSquarePlus, Archive } from 'lucide-react';
import { invoke } from '@tauri-apps/api/core';
import { open as openDialog } from '@tauri-apps/plugin-dialog';
import { getSupabaseClient, describeSupabaseError } from './lib/supabaseClient';
import { signInBrowserSupabase, signInNativeSupabase } from './nativeSupabaseAuth';
...
import { canOpenRegisteredPort, projectExecutionKind, shouldAutoDetectProjectStart } from './projectExecution';
import { shouldFetchWorktreeRemote } from './worktreeRemoteRefresh';
import {
...
function App() {
  ...
  const submitVoc = useCallback(async (input: { anchor: unknown; comment: string }) => {
    try {
      ...
      showToast('개선 요청을 저장했습니다', 'success');
      return true;
    } catch (e) {
  ...
  const loadWorktrees = useCallback((
    ...
    const key = `${portId}\u0000${folderPath}\u0000${fetchRemote ? 'remote' : 'local'}`;
    const existing = worktreeLoadInflightRef.current.get(key);
    if (existing && options.force !== true) return existing;
    const generation = (worktreeLoadGenerationRef.current.get(key) ?? 0) + 1;
    worktreeLoadGenerationRef.current.set(key, generation);
    ...
    const task = (async () => {
      ...
      // 더 최신 목록을 덮어쓰지 못한다.
      if (worktreeLoadGenerationRef.current.get(key) !== generation) return;
      ...
      const remoteError = fetchRemote
        ? list.find(wt => wt.remoteRefreshError)?.remoteRefreshError
        : undefined;
      if (fetchRemote && !remoteError) {
        worktreeRemoteRefreshAtRef.current.set(portId, Date.now());
      }
      if (fetchRemote && options.showResult !== false) {
        if (remoteError) showToast(`원격 상태 확인 실패: ${remoteError}`, 'error');
        else showToast('원격 Fetch 후 Git 상태를 갱신했습니다', 'success');
      }
      ...
      setWtActualPorts(prev => {
        ...
        if (worktreeLoadGenerationRef.current.get(key) === generation) {
          setWorktreeLoading(prev => ({ ...prev, [portId]: false }));
        }
      }
    })();

    worktreeLoadInflightRef.current.set(key, task);
    void task.finally(() => {
      if (worktreeLoadInflightRef.current.get(key) === task) {
        worktreeLoadInflightRef.current.delete(key);
      }
    });
    return task;
  }, []);
  ...
  const callOrcaLocalhost = async (item: PortInfo, worktreePath?: string): Promise<string> => {
    ...
    // 로컬 미리보기도 AI 실행과 **같은 규칙**을 따른다 — 플로팅이면 탭도 Floating Workspace에.
    const floating = shouldUseOrcaFloatingTerminal(
      worktreePath ?? item.worktreePath ?? item.folderPath,
      orcaLaunchMode,
    );
    // 숨김 경로 경고는 워크트리 내부 모드에서만 의미가 있다(플로팅은 경로에 매이지 않는다).
    if (!floating && targetPath && hasHiddenOrcaPathSegment(targetPath)) {
      throw new Error('이 워크트리는 숨김 경로라 Orca 화면에 브라우저 탭이 표시되지 않을 수 있습니다. WORKTREES의 “새 경로로 옮기기”를 먼저 실행하거나, 상단에서 “플로팅”을 선택하세요. 탭은 생성하지 않았습니다.');
    }
    return runOrcaOperation(async () => {
      const body = {
        port: item.port,
  ...
  const validateProjectDocumentPath = async (rawPath: string): Promise<string> => {
    const normalizedPath = rawPath.trim();
    if (!isAbsolutePath(normalizedPath)) {
      throw new Error('문서 파일의 절대경로를 확인하지 못했습니다.');
    }
    if (!/\.(html?|md|markdown|txt|log|json|csv|pdf)$/i.test(normalizedPath)) {
      throw new Error('HTML, Markdown, TXT, LOG, JSON, CSV, PDF 파일을 추가할 수 있습니다.');
    }
    await fileExistsAtPath(normalizedPath, '드롭한 경로가 파일이 아닙니다.');
    return normalizedPath;
  };
    ...
    try {
      ...
      if (isTauri()) {
        ...
        const selected = await open({
          ...
          filters: [{
            name: 'Documents',
            extensions: ['html', 'htm', 'md', 'markdown', 'txt', 'log', 'json', 'csv', 'pdf'],
          }],
        });
  ...
  const matchesSearch = (p: PortInfo, q: string): boolean => {
    ...
    return (
      ...
      (p.manualPath || '').toLowerCase().includes(q) ||
      (p.logFilePath || '').toLowerCase().includes(q)
    );
  };
      ...
      return (
        <div key={item.id} style={{padding:12,background:'#111113',border:'1px solid rgba(255,255,255,0.12)',borderRadius:8,display:'flex',flexDirection:'column',gap:6}}>
          ...
          <FolderDropZone
            ...
            onInputKeyDown={handleEditKeyPress}
            prefixLabel="매뉴얼"
            placeholder="매뉴얼 파일 (HTML·MD·PDF 등) · 드래그 또는 선택"
          ...
          <FolderDropZone
            ...
            onInputKeyDown={handleEditKeyPress}
            prefixLabel="로그 관리"
            placeholder="로그 관리 파일 (HTML·MD·LOG 등) · 드래그 또는 선택"
          ...
          <div style={{display:'flex',gap:6,alignItems:'stretch'}}>
            <GitHubUrlInputs
              ...
              onOpen={url => { void openBrowserWithDiagnostics(item, url); }}
              onKeyDown={handleEditKeyPress}
              inputStyle={inpV3}
            />
            <button
    ...
    return (
      ...
      >
        ...
        <div style={{marginTop:8,display:'flex',flexWrap:'wrap',gap:3}}>
          ...
          {item.port && (
            <button data-help-key="card-chrome" aria-label={`localhost:${item.port}`} onClick={e=>{e.stopPropagation(); void openBrowserWithDiagnostics(item, `http://localhost:${item.port}`);}} style={{...btnBase,padding:'4px 6px'}} title={`localhost:${item.port}`}>
          ...
          {itemGithubUrls.map((url, index) => (
            <button
              ...
              onClick={e=>{e.stopPropagation(); void openBrowserWithDiagnostics(item, url);}}
        ...
        {item.port && !isWindows() && (
          <div style={{display:'flex', gap:3, marginTop:3}} onClick={e=>e.stopPropagation()}>
            ...
            <button data-testid="card-orca-localhost" onClick={e=>{e.stopPropagation(); openOrcaLocalhost(item);}} style={{
              padding:'3px 8px',borderRadius:3,fontSize:9,
              background:'rgba(56,189,248,0.1)',color:'#38bdf8',
              border:'1px solid rgba(56,189,248,0.18)',cursor:'pointer',fontFamily:'inherit',
            }} title={`이 프로젝트의 Orca 브라우저 탭으로 localhost:${item.port} 열기`}>
              Orca :{item.port}
            </button>
          </div>
        )}
          ...
          return (
            <div key={wt.path} style={{padding:'5px 6px',borderRadius:5,background:wt.is_main?'rgba(94,234,212,0.04)':'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.05)',borderLeft:wt.is_main?'2px solid rgba(94,234,212,0.35)':'1px solid rgba(255,255,255,0.05)',display:'flex',flexDirection:'column',gap:4}}>
              ...
              {wt.is_main ? <>
                ...
                <button disabled={!portItem.port || !portItem.isRunning} onClick={e=>{e.stopPropagation(); if (portItem.port && portItem.isRunning) void openBrowserWithDiagnostics(portItem, `http://localhost:${portItem.port}`, wt.path);}} style={{...miniBtn,...(!portItem.port||!portItem.isRunning?{opacity:0.45,cursor:'not-allowed'}:{})}} title={!portItem.port?'프로젝트 수정에서 포트를 먼저 설정하세요':portItem.isRunning?`localhost:${portItem.port} 열기`:'포트가 실행되면 사용할 수 있습니다'}><Globe style={{width:9,height:9}}/>localhost</button>
                {!isWindows() && <button disabled={!portItem.port || !portItem.isRunning} onClick={e=>{e.stopPropagation(); if (portItem.isRunning) openCmuxLocalhost(portItem);}} style={{...miniBtn,color:'#2dd4bf',borderColor:'rgba(45,212,191,0.2)',...(!portItem.port||!portItem.isRunning?{opacity:0.45,cursor:'not-allowed'}:{})}} title={!portItem.port?'프로젝트 수정에서 포트를 먼저 설정하세요':portItem.isRunning?`cmux localhost:${portItem.port}`:'포트가 실행되면 사용할 수 있습니다'}><Terminal style={{width:9,height:9}}/></button>}
                {!isWindows() && <button data-testid="worktree-orca-localhost" data-worktree-path={wt.path} disabled={!portItem.port || !portItem.isRunning} onClick={e=>{e.stopPropagation(); if (portItem.isRunning) openOrcaLocalhost(portItem, wt.path);}} style={{...miniBtn,color:'#38bdf8',borderColor:'rgba(56,189,248,0.25)',...(!portItem.port||!portItem.isRunning?{opacity:0.45,cursor:'not-allowed'}:{})}} title={!portItem.port?'프로젝트 수정에서 포트를 먼저 설정하세요':portItem.isRunning?`이 프로젝트의 Orca 브라우저 탭으로 localhost:${portItem.port} 열기`:'포트가 실행되면 사용할 수 있습니다'}><Globe style={{width:9,height:9}}/>Orca 탭</button>}
                ...
                <button data-testid="worktree-open-localhost" data-worktree-path={wt.path} disabled={!isWtRunning} onClick={e=>{e.stopPropagation(); if (isWtRunning) void openBrowserWithDiagnostics({...portItem, port:wtPort, worktreePath: wt.path}, `http://localhost:${wtPort}`, wt.path);}} style={{...miniBtn,...(!isWtRunning?{opacity:0.45,cursor:'not-allowed'}:{})}} title={isWtRunning?`localhost:${wtPort} 열기`:'워크트리 포트가 실행되면 사용할 수 있습니다'}><Globe style={{width:9,height:9}}/>localhost</button>
                {!isWindows() && <button disabled={!isWtRunning} onClick={e=>{e.stopPropagation(); if (isWtRunning) openCmuxLocalhost({...portItem,port:wtPort,worktreePath:wt.path});}} style={{...miniBtn,color:'#2dd4bf',borderColor:'rgba(45,212,191,0.2)',...(!isWtRunning?{opacity:0.45,cursor:'not-allowed'}:{})}} title={isWtRunning?`cmux localhost:${wtPort}`:'워크트리 포트가 실행되면 사용할 수 있습니다'}><Terminal style={{width:9,height:9}}/></button>}
                <button data-testid="worktree-orca-localhost" data-worktree-path={wt.path} disabled={!isWtRunning} onClick={e=>{e.stopPropagation(); if (isWtRunning) openOrcaLocalhost({...portItem,port:wtPort,worktreePath:wt.path}, wt.path);}} style={{...miniBtn,color:'#38bdf8',borderColor:'rgba(56,189,248,0.25)',...(!isWtRunning?{opacity:0.45,cursor:'not-allowed'}:{})}} title={isWtRunning?`이 워크트리의 Orca 브라우저 탭으로 localhost:${wtPort} 열기`:'워크트리 포트가 실행되면 사용할 수 있습니다'}><Globe style={{width:9,height:9}}/>Orca 탭</button>
          ...
          {item.port && (
            <div data-testid="sidebar-process-controls" style={{display:'flex',alignItems:'center',gap:3,flexShrink:0}}>
              ...
              <button
                ...
                data-project-id={item.id}
                disabled={!canOpenRegisteredPort(item, !!item.isRunning)}
                aria-label={`${item.name} localhost:${item.port} 열기`}
                title={item.isRunning ? `localhost:${item.port} 열기` : '포트가 실행되면 사용할 수 있습니다'}
                onClick={e => {
                  e.stopPropagation();
                  if (canOpenRegisteredPort(item, !!item.isRunning)) void openBrowserWithDiagnostics(item, `http://localhost:${item.port}`);
                }}
                style={{
                  width:22,height:22,padding:0,borderRadius:4,flexShrink:0,
                  ...
                  cursor:canOpenRegisteredPort(item, !!item.isRunning) ? 'pointer' : 'not-allowed',opacity:canOpenRegisteredPort(item, !!item.isRunning) ? 1 : 0.55,
                }}
          ...
          <div style={{flex:1,overflowY:'auto'}}>
            ...
            {v3Idle.length > 0 && <>
              ...
              {v3IdleRecent.length > 0 && <>
                ...
                {v3IdleRecent.map(item => renderRow(item))}
              </>}
              {v3IdleAging.length > 0 && <>
                <div style={{padding:'6px 14px 2px',fontSize:9.5,fontFamily:monoFont,color:'#71717a',letterSpacing:0.3}}>
                  가끔 사용 (7~30일) · {v3IdleAging.length}
                </div>
                {v3IdleAging.map(item => renderRow(item))}
              </>}
              {v3IdleStale.length > 0 && <>
                ...
                {v3IdleStale.map(item => renderRow(item))}
              </>}
            </>}
            {v3Ports.length === 0 && (
            ...
            <div style={{flex:1,overflowY:'auto',padding:'28px 32px',display:'flex',flexDirection:'column',gap:6}}>
              ...
              <div style={{display:'flex',gap:6,alignItems:'stretch'}}>
                <GitHubUrlInputs
                  value={editGithubUrl}
                  onChange={setEditGithubUrl}
                  onOpen={url => { void openBrowserWithDiagnostics(sel, url); }}
                  onKeyDown={handleEditKeyPress}
                  inputStyle={inpV3}
                />
                <button
          ...
          <div style={{flex:1,overflowY:'auto',padding:'28px 32px'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:5,marginBottom:24,fontSize:12,fontFamily:monoFont}}>
              ...
              {sel.folderPath && (
                ...
                action={<button type="button" data-testid="meta-open-folder" onClick={event => { event.stopPropagation(); API.openFolder(sel.folderPath!).catch(e=>showToast(`폴더 열기 실패: ${e.message}`, 'error')); }} style={{padding:'2px 7px',background:'rgba(255,255,255,0.025)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:4,color:'#a1a1aa',cursor:'pointer',fontSize:10,fontFamily:'inherit',display:'flex',alignItems:'center',gap:3,whiteSpace:'nowrap',flexShrink:0}}><FolderOpen style={{width:10,height:10}}/>폴더 열기</button>}
                />
              )}
              {sel.commandPath && (
                <MetaEditableRow
            ...
            <div data-testid="local-preview-panel" style={{marginBottom:8,padding:'8px 10px',borderRadius:7,background:'rgba(56,189,248,0.035)',border:'1px solid rgba(56,189,248,0.12)'}}>
              ...
              <div style={{fontSize:9,color:'#52525b',marginBottom:6}}>실행 중인 localhost를 어느 브라우저에서 볼지 선택</div>
              <div style={{display:'flex',gap:6,flexWrap:'wrap' as const}}>
                {sel.port && <button data-testid="detail-browser-localhost" disabled={!sel.isRunning} onClick={() => { if (sel.isRunning) void openBrowserWithDiagnostics(sel, `http://localhost:${sel.port}`); }} style={{...rowBtn,...(!sel.isRunning?{opacity:0.45,cursor:'not-allowed'}:{})}} title={sel.isRunning?`기본 브라우저로 localhost:${sel.port} 열기`:'포트가 실행되면 사용할 수 있습니다'}><Laptop style={{width:11,height:11}}/>기본 브라우저</button>}
                {sel.port && !isWindows() && <button data-testid="detail-cmux-localhost" disabled={!sel.isRunning} onClick={() => { if (sel.isRunning) openCmuxLocalhost(sel); }} style={{...rowBtn,color:'#2dd4bf',borderColor:'rgba(45,212,191,0.2)',...(!sel.isRunning?{opacity:0.45,cursor:'not-allowed'}:{})}} title={sel.isRunning?`cmux로 localhost:${sel.port} 열기 (macOS 전용)`:'포트가 실행되면 사용할 수 있습니다'}><Terminal style={{width:11,height:11}}/>cmux localhost</button>}
                {sel.port && <button data-testid="detail-orca-localhost" disabled={!sel.isRunning} onClick={() => { if (sel.isRunning) openOrcaLocalhost(sel); }} style={{...rowBtn,color:'#38bdf8',borderColor:'rgba(56,189,248,0.28)',background:'rgba(56,189,248,0.07)',...(!sel.isRunning?{opacity:0.45,cursor:'not-allowed'}:{})}} title={sel.isRunning?`이 프로젝트의 Orca 브라우저 탭으로 localhost:${sel.port} 열기`:'포트가 실행되면 사용할 수 있습니다'}><Globe style={{width:11,height:11}}/>Orca localhost</button>}
              </div>
            </div>
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden" style={{background:'#09090b'}}>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        {/* 탭 네비게이션 */}
        <div className="flex items-center gap-2 px-6 py-2.5 shrink-0 flex-wrap" style={{borderBottom:'1px solid rgba(255,255,255,0.07)'}}>
          <div className="flex items-center gap-2">
            ...
            {activeTab === 'ports' && (
              <>
                ...
                <div className="flex items-center rounded-xl border border-stone-800/40 overflow-hidden">
                  <button data-help-key="btn-supabase-push" onClick={handlePushToSupabase} disabled={isPushingPorts} title="Supabase Push" className="px-2.5 py-1.5 bg-[#111113] hover:bg-[#18181b] text-[#f4f4f5]/90 text-sm border-r border-stone-800/40 transition-all flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed">
                    <CloudUpload className={`w-3.5 h-3.5 ${isPushingPorts ? 'animate-pulse' : 'text-indigo-400'}`} />
                    <span className="text-xs font-medium">Push</span>
                  </button>
                  <button data-help-key="btn-supabase-pull" onClick={handleRestoreFromSupabase} disabled={isRestoring} title="Supabase Pull" className="px-2.5 py-1.5 bg-[#111113] hover:bg-[#18181b] text-[#f4f4f5]/90 text-sm transition-all flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          {!isDeployedWeb() && browserProfiles.length > 0 && (
            <select
              ...
              title={selectedDeploymentBrowserProfile
                ? `배포 URL을 ${browserProfileOptionLabel(selectedDeploymentBrowserProfile)} 프로필로 엽니다. CDP 연결 없이 기존 로그인 세션을 사용합니다.`
                : '배포 URL을 Chrome 기본 프로필로 엽니다.'}
              style={{
                maxWidth: 220,
                padding: '2px 6px',
                borderRadius: 6,
            ...
            >
              <option value="">배포 · Chrome 기본</option>
              {browserProfiles.map(profile => (
                <option key={profile.id} value={profile.id}>
                  {`배포 · ${browserProfileOptionLabel(profile)}`}
                </option>
              ))}
            </select>
          )}
          ...
          {!isDeployedWeb() && browserProfiles.length > 0 && (() => {
            ...
            const target = selected?.deployUrl ? selected : null;
            const deployable = ports
              .filter(p => p.deployUrl && p.id !== target?.id)
            ...
            const openDeployment = (item: PortInfo) => {
            ...
            return (
              <div style={{position:'relative',display:'flex',marginLeft:4}}>
                <button
                  ...
                  title={empty
                    ? '배포 주소가 등록된 프로젝트가 없습니다. 프로젝트 상세의 deploy 줄에 입력하세요.'
                    : target
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:10}}>
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:4}}>
                      ...
                      <GitHubUrlInputs
                        ...
                        onOpen={url => { void API.openInChrome(url).catch(error => showToast(`GitHub 열기 실패: ${error instanceof Error ? error.message : String(error)}`, 'error')); }}
                        inputStyle={inpStyle}
                      />
                    </div>
                    <label style={{display:'flex',flexDirection:'column'}}>
        ...
        {vocMode && (
          <Suspense fallback={null}>
            <VocOverlay onClose={() => setVocMode(false)} onSubmit={submitVoc} />
          </Suspense>
        )}
        {showMemoryArchive && (
          <Suspense fallback={null}>
          ...
          <SetupWizard
            onComplete={async ({ supabaseUrl, supabaseAnonKey, deviceName, deviceId }) => {
              ...
              try {
                ...
                if (isTauri()) {
                  const { invoke } = await import('@tauri-apps/api/core');
                  await invoke('save_portal', { data: next });
                } else {
                ...
                if (!currentAuth.session) {
                  ...
                  if (isTauri()) {
                    await signInNativeSupabase(supabase, authOptions);
                  } else {
                ...
                showToast('설정 또는 Google 로그인 실패: ' + e, 'error');
                throw e;
              }
            }}
            onSkip={() => {
          ...
          <div style={{flex:1,display:'flex',minHeight:0,overflow:'hidden'}}>
            ...
            <div style={{flex:1,display:'flex',flexDirection:'column',minHeight:0,overflow:'hidden'}}>
              ...
              <div style={{
                ...
                <h1 data-help-key="header-section-title" style={{margin:0,fontSize:18,fontWeight:600,letterSpacing:-0.3,color:'#f4f4f5'}}>
                ...
                <span data-help-key="header-project-count" style={{fontSize:12,color:'#a1a1aa'}}>{v3Ports.length} {t(lang,'projects')}</span>
                <div style={{flex:1}} />
                <BulkUpgradeButton
