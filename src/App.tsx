import {ControlProfilePanel} from './ControlProfilePanel';
      ...
      <div className="workspace-shell flex-1 min-h-0" data-mobile={isMobile ? 'true' : 'false'} data-sidebar-only={isMobile && activeTab === 'ports' && !v4SelectedId ? 'true' : 'false'}>
        ...
        >
          ...
          <div data-testid="top-toolbar-project-actions" className="workspace-project-tools">
            ...
            <button type="button" data-testid="control-profile-open" className="min-h-11 rounded-xl border border-[var(--line)] px-3 text-sm font-semibold" onClick={()=>setShowControlProfile(true)}>아젠투지 · 운영 프로필</button>
            {controlCenterProject ? <button
              type="button"
              data-testid="control-center-project-shortcut"
              onClick={() => void handleOnboardingControlCenter('restore')}
              title="검색 없이 AgentsToZ-Control 프로젝트 열기"
            ...
            >
              ...
              <summary className="cursor-pointer font-semibold">Control 준비 · 이 Mac에 미등록</summary>
              <p className="mt-2 max-w-sm leading-relaxed">앱 저장소를 Pull하거나 앱을 업데이트해도 별도 Control 저장소는 복제되지 않습니다. 다른 Mac에서 쓰던 Control이 있으면 복원하세요.</p>
              <div className="mt-2 flex flex-wrap gap-2">
                <button type="button" disabled={controlCenterSetupBusy} className="min-h-11 rounded-lg border border-[var(--line)] px-3" onClick={() => void handleOnboardingControlCenter('restore')}>다른 Mac의 Control 복원</button>
                <button type="button" disabled={controlCenterSetupBusy} className="min-h-11 rounded-lg border border-[var(--line)] px-3" onClick={() => void handleOnboardingControlCenter('create')}>처음 사용 · 새 Control 만들기</button>
              </div>
            </details>}
        ...
        >
          ...
          {(runtimeHasMounted || activeTab === 'runtime') && (
            <Suspense fallback={<div className="text-xs text-zinc-500">{lang === 'ko' ? 'AI 작업을 불러오는 중…' : 'Loading AI Tasks…'}</div>}>
              <>
              <div className="mb-3 rounded-xl border border-[var(--line)] p-3 text-sm"><strong>AgentsToZ · 나의 관제</strong><p className="my-2">프로젝트를 바꾸거나 다른 AI를 사용해도 같은 운영 기억으로 이어갑니다.</p><button type="button" className="min-h-11 rounded-lg border border-[var(--line)] px-3" onClick={()=>setShowControlProfile(true)}>운영 프로필과 기억 확인</button></div>
