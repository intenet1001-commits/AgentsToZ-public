...
import { getSupabaseClient, type SupabaseClientFactory } from './lib/supabaseClient';
...
export default function RemoteDeviceManager({
  ...
  async function copyRuntimePreparationPrompt(device: RemoteDeviceRow) {
    try {
      await copyAgentsToZPrompt(buildAwsRuntimePreparationPrompt(device));
  ...
  return (
    <section className={compact ? 'border-t border-zinc-800' : ''} data-testid="remote-device-manager">
      ...
      {showRegister && (
        <div className="mx-4 mb-4 space-y-3 rounded-xl border border-emerald-900/60 bg-emerald-500/5 p-3">
          {!commandInfo ? <>
            ...
            <button onClick={createEnrollment} disabled={creating || !deviceName.trim() || !workspaceRoot.trim()}
              className="min-h-11 w-full rounded-lg bg-emerald-400 px-3 py-2 text-xs font-semibold text-[var(--text-on-accent)] hover:bg-emerald-300 disabled:cursor-not-allowed disabled:opacity-40">
            ...
            <pre className="max-h-44 overflow-auto whitespace-pre-wrap break-all rounded-lg border border-zinc-800 bg-[var(--bg-input)] p-3 text-[10px] leading-relaxed text-zinc-300">{commandInfo.command}</pre>
      ...
      {projectDevice && (
        <div data-testid="remote-project-setup" className="mx-4 mb-4 space-y-3 rounded-xl border border-sky-900/60 bg-sky-500/5 p-3">
          <div className="flex items-start justify-between gap-3">
            <div>
          ...
          {!projectCommand ? <>
            ...
            <button onClick={createProjectCommand} disabled={!projectName.trim() || (projectAction === 'clone' && !repositoryUrl.trim()) || (projectAction === 'memory' && !memoryId)} className="min-h-11 w-full rounded-lg bg-sky-400 px-3 py-2 text-xs font-semibold text-[var(--text-on-accent)] disabled:opacity-40">프로젝트 명령 만들기</button>
          </> : <>
            <pre className="max-h-44 overflow-auto whitespace-pre-wrap break-all rounded-lg border border-zinc-800 bg-[var(--bg-input)] p-3 text-[10px] leading-relaxed text-zinc-300">{projectCommand}</pre>
      ...
      {runtimeHelpDevice && (() => {
        ...
        return <div data-testid="remote-runtime-preparation" className="mx-4 mb-4 space-y-3 rounded-xl border border-amber-800/60 bg-[var(--bg-warning)] p-3">
          <div className="flex items-start justify-between gap-3">
            <div>
      ...
      {error && <div className="mx-4 mb-3 rounded-lg border border-red-900/60 bg-[var(--bg-danger)] px-3 py-2 text-[11px] text-red-300">{error}</div>}
      {loading ? <div className="px-4 pb-4 text-[11px] text-zinc-400">클라우드 단말을 확인하는 중…</div> : activeDevices.length === 0 ? (
        ...
        {activeDevices.map(device => {
          ...
          {editing?.device_id === device.device_id ? <>
            <input value={editName} onChange={event => setEditName(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') void saveName(); if (event.key === 'Escape') setEditing(null); }} autoFocus className="min-w-0 flex-1 rounded border border-zinc-600 bg-zinc-800 px-2 py-1 text-xs text-[var(--text-primary)] outline-none" />
          ...
          {needsAgentUpdate && <div className="ml-6 mt-2 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-amber-700/50 bg-[var(--bg-warning)] p-3">
          ...
          {pendingRevokeId === device.device_id && <div className="ml-6 mt-2 rounded-lg border border-red-800/60 bg-[var(--bg-danger)] p-3">
            ...
            <div className="mt-2 flex justify-end gap-2">
              ...
              <button onClick={() => void revoke(device)} disabled={savingId === device.device_id} className="min-h-11 rounded border border-red-700 bg-[var(--bg-danger)] px-3 py-2 text-[11px] font-medium text-red-200 disabled:opacity-40">접속 자격 폐기</button>
            </div>
          </div>}
          {projects.filter(project => project.device_id === device.device_id).length > 0 && <div className="ml-6 mt-2 space-y-1.5 border-l border-zinc-800 pl-3">
            {projects.filter(project => project.device_id === device.device_id).map(project => <div key={project.project_path} className={`rounded-md border px-2.5 py-2 ${project.present ? 'border-zinc-800 bg-zinc-950/60' : 'border-amber-900/40 bg-[var(--bg-warning)] opacity-70'}`}>
