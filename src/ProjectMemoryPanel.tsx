...
import { buildHermesProjectHandoffPrompt } from './hermesProjectHandoffPrompt';
import { hermesProfileDisplayName } from './hermesProfileOnboardingPrompt';
// legacy UI test marker: newDeviceHermesSetupPrompt is intentionally replaced by projectHermesHandoffPrompt.
import { buildTelegramBotOnboardingPrompt, buildTelegramProfileHandoffPrompt } from './newDeviceTelegramBotSetupPrompt';
...
export function ProjectMemoryPanel({
  ...
  const [hermesProfileName, setHermesProfileName] = useState('');
  const [hermesProfiles, setHermesProfiles] = useState<Array<{ name: string; configPresent: boolean }>>([]);
  const [hermesProfilesLoading, setHermesProfilesLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase()}/api/hermes/profiles`, { cache: 'no-store' })
      .then(response => response.ok ? response.json() : Promise.reject(new Error(`profile 조회 실패 (${response.status})`)))
      .then(data => {
        if (cancelled) return;
        const profiles = Array.isArray(data?.profiles) ? data.profiles.filter((profile: any) => typeof profile?.name === 'string') : [];
        setHermesProfiles(profiles);
      })
      .catch(() => { if (!cancelled) setHermesProfiles([]); })
      .finally(() => { if (!cancelled) setHermesProfilesLoading(false); });
  ...
  const projectHermesHandoffPrompt = buildHermesProjectHandoffPrompt({
    projectName,
  ...
  const telegramProfileHandoffPrompt = buildTelegramProfileHandoffPrompt({
    profileName: hermesProfileName,
    projectName,
    canonicalPath: folderPath,
    memoryId: status?.config?.memoryId,
  });
  const telegramBotOnboardingPrompt = buildTelegramBotOnboardingPrompt();
        ...
        {projectIdentityCopy ? (
          <button
            ...
            style={{
              ...
              fontSize: 9.5,
              lineHeight: 1.6,
              color: copiedChatCommand === 'project-memory-identity' ? '#5eead4' : '#a1a1aa',
        ...
        <>
          ...
          {hermesZoom && (
            <div
              ...
              style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.72)', zIndex: 9700 }}
            />
          )}
          <details
            ...
            style={hermesZoom
              ? {
                  ...
                  width: 'min(780px, 94vw)', maxHeight: '88vh', overflow: 'auto', zIndex: 9701,
                  padding: '14px 16px', borderRadius: 10, border: '1px solid rgba(125,211,252,0.32)',
                  background: '#111113', boxShadow: '0 20px 60px rgba(0,0,0,0.66)',
                  fontSize: 13, lineHeight: 1.65,
                }
              : { marginBottom: 8, padding: '7px 8px', borderRadius: 6, border: '1px solid rgba(125,211,252,0.13)', background: 'rgba(14,116,144,0.045)', fontSize: compact ? 10 : 11 }}
          >
            <summary
              onClick={e => { e.preventDefault(); if (!hermesZoom) setHermesZoom(true); }}
              style={{ cursor: hermesZoom ? 'default' : 'pointer', color: '#7dd3fc', fontWeight: 600, listStyle: hermesZoom ? 'none' : 'revert', marginBottom: hermesZoom ? 8 : 0 }}
            >
              Hermes 명령 <span style={{ fontWeight: 400 }}>— Telegram 대화에 붙여넣는 것</span>
              <span style={{ color: '#52525b', fontWeight: 400, marginLeft: 5 }}>기기당 한 번 설치 · 눌러서 크게 보기</span>
              ...
              <button
                ...
                style={{
                  ...
                  border: '1px solid rgba(125,211,252,0.3)', background: 'rgba(14,116,144,0.16)',
                  color: '#7dd3fc', cursor: 'pointer', fontSize: 'inherit', fontFamily: 'inherit', fontWeight: 600,
                }}
              >
          ...
          <details data-testid="project-memory-setup-prompts" open={compact} style={{ marginBottom: 8, padding: '5px 8px', borderRadius: 6, border: '1px solid rgba(161,161,170,0.12)' }}>
            ...
            <span>·</span>
            <select
              data-testid="hermes-project-profile-name"
              value={hermesProfileName}
              onChange={(event) => setHermesProfileName(event.target.value)}
              aria-label="연결할 Hermes profile 선택"
              disabled={hermesProfilesLoading || hermesProfiles.length === 0}
              title="현재 로컬 Hermes에서 read-only 조회한 profile 목록입니다."
              style={{ minWidth: 180, padding: '3px 6px', borderRadius: 4, border: '1px solid rgba(167,139,250,0.28)', background: 'rgba(24,24,27,0.8)', color: '#ddd6fe', fontSize: 'inherit', fontFamily: 'inherit' }}
            >
              <option value="">{hermesProfilesLoading ? 'Hermes profile 조회 중…' : hermesProfiles.length ? 'Hermes profile 선택' : '조회된 profile 없음'}</option>
              {hermesProfiles.map(profile => <option key={profile.name} value={profile.name}>{hermesProfileDisplayName(profile.name)}{profile.configPresent ? '' : ' · config 없음'}</option>)}
            </select>
            <span>·</span>
            <button
              type="button"
              data-testid="copy-new-device-hermes-setup"
              onClick={() => void copyChatCommand('project-hermes-handoff', projectHermesHandoffPrompt, '프로젝트 Hermes profile 연동 안내')}
              title="선택한 프로젝트와 Hermes profile을 연동하는 프로젝트 전용 handoff prompt를 복사합니다. Telegram 그룹은 만들지 않습니다."
              style={{
                display:'inline-flex',alignItems:'center',gap:4,padding:'2px 5px',
                borderRadius:4,border:'1px solid rgba(167,139,250,0.28)',
                background:'rgba(91,33,182,0.10)',color:'#c4b5fd',
                cursor:'pointer',fontSize:'inherit',fontFamily:'inherit',
              }}
            >
              <span>선택 프로젝트 → Hermes profile 연동</span>
              {copiedChatCommand === 'project-hermes-handoff' ? <Check style={{width:9,height:9}}/> : <Copy style={{width:9,height:9}}/>}
            </button>
            <span>·</span>
            <button
              type="button"
              data-testid="copy-telegram-profile-handoff"
              disabled={!telegramProfileHandoffPrompt}
              onClick={() => void copyChatCommand('telegram-profile-handoff', telegramProfileHandoffPrompt, 'Hermes profile → Telegram Bot 연동 안내')}
              title="선택한 Hermes profile을 Telegram Bot에 연결하는 secret-free prompt를 복사합니다. Telegram 그룹은 수동으로 만듭니다."
              style={{ display:'inline-flex',alignItems:'center',gap:4,padding:'2px 5px', borderRadius:4,border:'1px solid rgba(45,212,191,0.28)', background:'rgba(13,148,136,0.10)',color:'#99f6e4', cursor:'pointer',fontSize:'inherit',fontFamily:'inherit' }}
            >
              <span>선택 profile → Telegram Bot 연동</span>
              {copiedChatCommand === 'telegram-profile-handoff' ? <Check style={{width:9,height:9}}/> : <Copy style={{width:9,height:9}}/>}
            </button>
            <span>·</span>
            <button
              type="button"
              data-testid="copy-telegram-bot-onboarding"
              onClick={() => void copyChatCommand('telegram-bot-onboarding', telegramBotOnboardingPrompt, '고급 Telegram Bot 3개 일괄 온보딩 안내')}
              title="단말별 Telegram Bot 3개 생성·이름 지정·Hermes 연결·단체톡 생성 절차를 복사합니다. Bot token은 포함하지 않습니다."
              style={{
                display:'inline-flex',alignItems:'center',gap:4,padding:'2px 5px',
                borderRadius:4,border:'1px solid rgba(45,212,191,0.28)',
                background:'rgba(13,148,136,0.10)',color:'#99f6e4',
                cursor:'pointer',fontSize:'inherit',fontFamily:'inherit',
              }}
            >
              <span>고급: Telegram Bot 3개 일괄 온보딩</span>
              {copiedChatCommand === 'telegram-bot-onboarding' ? <Check style={{width:9,height:9}}/> : <Copy style={{width:9,height:9}}/>}
            </button>
            </div>
          </details>
          ...
          {!compact && (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4, fontSize: 10, color: '#a1a1aa', fontWeight: 600 }}>
                <ShieldCheck style={{ width: 11, height: 11, color: '#5eead4' }} />
                Private GitHub 재해복구 보관 <span style={{ color: '#52525b', fontWeight: 400 }}>(선택)</span>
              </div>
              <p style={{ margin: '0 0 6px', color: '#71717a', fontSize: 9.5, lineHeight: 1.55 }}>
                Supabase가 일상 동기화 정본입니다. 실제 Private·쓰기 권한을 매 Push 직전에 다시 확인하고, 검증된 기억 파일만
                <code style={{ marginLeft: 3, color: '#a1a1aa' }}>agentstoz-memory-v1</code> branch에 별도 보관합니다. 토큰·비밀키·원본 대화는 제외합니다.
              </p>
              {privateGitHubArchive?.enabled ? (
                <>
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', marginBottom: 6, fontSize: 9.5 }}>
                    <span style={{ color: '#86efac' }}>● 자동 보관 켬</span>
                    <span style={{ color: '#52525b', overflowWrap: 'anywhere' }}>{privateGitHubArchive.repositoryUrl}</span>
                    {privateGitHubArchive.lastSuccessAt && <span style={{ color: '#71717a' }}>최근 성공 {formatDateTimeSeconds(privateGitHubArchive.lastSuccessAt)}</span>}
                    {privateGitHubArchive.lastError && <span role="alert" style={{ color: '#fca5a5' }}>최근 실패: {privateGitHubArchive.lastError}</span>}
                  </div>
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <button
                      type="button"
              ...
              <p style={{ margin: '6px 0 0', color: '#52525b', fontSize: 9 }}>
