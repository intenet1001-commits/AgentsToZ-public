...
/**
 * 한 줄 복사 형식: #프로젝트명 [로컬프로젝트해시: HASH]
 * Telegram/Hermes는 선두의 exact project selector를 읽고 보조 해시를 작업에서 제외한다.
 * 로컬 해시는 사람과 이 단말의 보조 식별값일 뿐, Telegram 라우팅에는 필수가 아니다.
 */
export const projectIdentityClipboard = (name: string, id: string): string => [
  `#${name.trim()}`,
  `[로컬프로젝트해시: ${projectCode(id)}]`,
].join(' ');
