...
export class FileCodexFirstConversationPendingStore implements CodexFirstConversationPendingStore {
  ...
  save(projectKey: string, threadId: string): void {
    ...
    registry.entries[projectFingerprint(projectKey)] = {
      threadId,
      createdAt: new Date().toISOString(),
    };
    const ordered = Object.entries(registry.entries)
      .sort((left, right) => Date.parse(right[1].createdAt) - Date.parse(left[1].createdAt))
      .slice(0, MAX_ENTRIES);
    registry.entries = Object.fromEntries(ordered);
    this.#write(registry);
  }
