...
import { parseOnboardingHandoff } from './onboardingHandoff';
import { SCHEMA_TABLE_COUNT, migrationSqlForAllowedEmails, portalSqlForAllowedEmails } from './schemaSql';

interface SetupWizardProps {
  onComplete: (config: { supabaseUrl: string; supabaseAnonKey: string; deviceName: string; deviceId?: string }) => void | Promise<void>;
  onSkip: () => void;
}
...
const REPO_URL: string = (import.meta as any).env?.VITE_REPO_URL
  ?? 'https://github.com/intenet1001-commits/AgentsToZ-public';
...
type Mode = 'choose' | 'first' | 'first_cli' | 'additional' | 'portal' | 'windows_env' | 'mac_env' | 'dev_env' | 'terminal_tools' | 'one_click';
type OS = 'mac' | 'windows';
...
npm install -g supabase`;
  ...
  const stepContent = [
    ...
    <div key={7} className="space-y-4">
      ...
      <InfoBox color="amber">
        <p className="text-xs">다음 단계: 마법사 완료 후 <code>http://127.0.0.1:9000/portal.html</code>에서 Google 로그인하고, root 앱으로 돌아와 Push/Pull을 확인하세요.</p>
      </InfoBox>
    </div>,
  ];
...
/**
 * 첫 단말에서는 Supabase CLI, Rust, GitHub, Vercel을 전부 설치할 필요가 없다.
 * Dashboard의 SQL Editor와 URL/key만으로 동기화를 준비하고, CLI 전체 흐름은
 * 명시적으로 원하는 사용자에게만 고급 경로로 제공한다.
 */
function FirstSetupWizard({
  ...
  const stepContent = [
    ...
    <div key={2} className="space-y-4">
      ...
      <InfoBox color="blue">
        <p className="text-xs">완료 후 포털에서 Google 로그인 → root 앱에서 첫 Push를 확인하세요. 두 번째 단말은 포털의 “새 기기” 한 번 복사로 연결할 수 있습니다.</p>
      </InfoBox>
    </div>,
  ];
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 min-h-0">
        <WizardLayout
          ...
          step={step}
          setStep={setStep}
          canNext={[allowedEmailValid && schemaApplied, testResult === 'ok', !!deviceName.trim()]}
          onBack={onBack}
          onComplete={() => onComplete({ supabaseUrl, supabaseAnonKey, deviceName: deviceName.trim() })}
          canComplete={testResult === 'ok' && !!deviceName.trim()}
        >
          {stepContent[step]}
        </WizardLayout>
      </div>
...
function AdditionalDeviceWizard({ onComplete, onBack }: { onComplete: SetupWizardProps['onComplete']; onBack: () => void }) {
  ...
  const [adoptDeviceId, setAdoptDeviceId] = useState(false); // paste된 device_id는 명시적으로 선택할 때만 이어받는다
  ...
  async function handlePasteSetup() {
    ...
    try {
      ...
      if (payload.deviceId) {
        setPastedDeviceId(payload.deviceId);
        setAdoptDeviceId(false);
      }
...
bun run start`;
  ...
  const stepContent = [
    ...
    <div key={2} className="space-y-4">
      ...
      {authRequired && (
        <InfoBox color="blue">
          <p className="text-xs">마법사 완료 후 <code>http://127.0.0.1:9000/portal.html</code>에서 Google 로그인해야 Push/Pull을 사용할 수 있습니다.</p>
        </InfoBox>
      )}
        ...
        <div className="border-t border-zinc-800 px-4 py-3 md:px-8 md:py-4 flex items-center justify-between shrink-0">
          ...
          {isLast ? (
            <button onClick={() => void handleComplete()} disabled={!canComplete || completionBusy}
              ...
              {completionBusy ? 'Google 로그인 대기 중…' : '완료 및 동기화'} <Check className="w-4 h-4" />
            </button>
          ) : (
...
5. bun run start로 API(3001)와 Vite(9000)를 함께 실행하고 http://localhost:9000 이 열리는지 확인해줘.
   vite를 직접 실행하지 마라.
6. 첫 화면에서는 로컬로 바로 시작할 수 있다. 다기기 동기화가 필요할 때만 초기 설정 마법사의
   “첫 단말 · 동기화 설정” 또는 “두 번째·추가 기기 연결” 경로를 안내해줘.
   Supabase SQL은 src/schemaSql.ts의 MIGRATION_SQL을 정본으로 쓰고, URL/anon key를 사용자의
   파일에 직접 기록하지 마라. Push/Pull은 portal.html에서 Google OAuth 세션의 authenticated JWT로만
   동작하며, anon key 단독 접근은 RLS가 거부해야 한다. 동기화 전 포털 로그인을 반드시 확인해줘.
7. GitHub는 포크/푸시·Actions용, Vercel은 웹 포털 배포용, Rust는 데스크톱 번들용,
          ...
          {mode === 'choose' && (
            <div className="h-full flex flex-col items-center p-4 sm:p-8 gap-5 overflow-y-auto justify-start pt-6">
              <div className="text-center space-y-2">
                ...
                <p className="text-zinc-400 text-sm">필요한 경로 하나만 고르면 됩니다. 나머지는 나중에 설정할 수 있습니다.</p>
                {detectedOs && (
              ...
              <div className="grid sm:grid-cols-3 gap-3 w-full max-w-4xl">
                ...
                <button onClick={() => setMode('first')} className="group bg-zinc-900 hover:bg-zinc-800 border-2 border-blue-500/40 hover:border-blue-500/70 rounded-2xl p-5 text-left transition-all">
                  ...
                  <p className="text-xs text-zinc-400 leading-relaxed">Supabase Dashboard 3단계<br />CLI 없이 보안 연결까지 검사</p>
                  <div className="flex items-center gap-1 text-blue-400 text-xs mt-4">설정 시작 <ChevronRight className="w-3.5 h-3.5" /></div>
                </button>
                <button onClick={() => setMode('additional')} aria-label="두 번째·추가 기기 연결" className="group bg-zinc-900 hover:bg-zinc-800 border-2 border-violet-500/40 hover:border-violet-500/70 rounded-2xl p-5 text-left transition-all">
                  ...
                  <h3 className="text-base font-semibold text-white mb-1">두 번째·추가 기기 연결</h3>
                  <p className="text-xs text-zinc-400 leading-relaxed">1st 포털에서 “새 기기” 복사<br />붙여넣고 연결 확인</p>
                  <div className="flex items-center gap-1 text-violet-400 text-xs mt-4">연결 시작 <ChevronRight className="w-3.5 h-3.5" /></div>
                </button>
              </div>

              <div className="w-full max-w-4xl bg-zinc-900/60 border border-zinc-700 rounded-xl px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2 min-w-0">
                  <Zap className="w-4 h-4 text-yellow-400 shrink-0" />
                  <div><p className="text-xs text-zinc-200 font-medium">Codex·Claude 온보딩 에이전트로 설치</p><p className="text-[10px] text-zinc-500">OS와 Bun·Git만 진단하고, 필요한 기능만 단계별로 안내합니다.</p></div>
                </div>
                <button onClick={() => { navigator.clipboard.writeText(QUICK_INSTALL_PROMPT); setQuickInstallCopied(true); setTimeout(() => setQuickInstallCopied(false), 2000); }} className={`flex items-center gap-1 px-3 py-1.5 text-[11px] rounded-md border shrink-0 ${quickInstallCopied ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-zinc-800 hover:bg-zinc-700 border-zinc-700 text-zinc-300'}`}>
                  {quickInstallCopied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}{quickInstallCopied ? '복사됨' : '에이전트 프롬프트 복사'}
                </button>
              </div>
  ...
  async function runStep(id: string) {
    ...
    try {
      ...
      if (id === 'finish_setup') {
        ...
        updateStep(id, { status: 'running', detail: 'Google 로그인과 DB 접근 권한 확인 중…' });
        await onComplete({
          supabaseUrl: sbUrl,
          supabaseAnonKey: sbKey,
          deviceName: deviceName.trim(),
        ...
        updateStep(id, { status: 'done', detail: 'Google 로그인 · DB 권한 · 로컬 설정 완료' });
        setAllDone(true);
        return;
      }
    } catch (e: any) {
