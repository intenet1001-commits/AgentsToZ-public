...
type Mode = 'choose' | 'first' | 'additional' | 'portal' | 'windows_env' | 'mac_env' | 'dev_env' | 'terminal_tools' | 'one_click';
type OS = 'mac' | 'windows';
...
6. 첫 화면에서는 로컬로 바로 시작할 수 있다. 다기기 동기화가 필요할 때만 초기 설정 마법사의
   “처음 사용” 또는 “기존 Supabase 프로젝트 빠른 연결” 경로를 안내해줘.
        ...
        <div className="flex-1 overflow-hidden">
          {mode === 'choose' && (
            <div className="h-full flex flex-col items-center p-4 sm:p-8 gap-6 overflow-y-auto justify-start pt-6">
              ...
              {/* 기존 Supabase 프로젝트가 있는 경우의 빠른 연결 */}
              <button onClick={() => setMode('one_click')}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-base font-bold text-white">기존 Supabase 프로젝트 빠른 연결</p>
                    <span className="text-[10px] font-semibold text-amber-400 bg-amber-400/10 border border-amber-400/20 px-2 py-0.5 rounded-full">선택</span>
                  </div>
                  <p className="text-sm text-zinc-400 mt-0.5">이미 만든 Supabase 프로젝트를 CLI로 연결합니다. GitHub·Vercel은 필요한 경우에만 추가하세요.</p>
                </div>
              ...
              <div className="w-full max-w-4xl bg-blue-500/5 border border-blue-500/20 rounded-xl px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-2 text-xs text-zinc-400">
                ...
                <span>
                  포트 관리 기능은 <strong className="text-zinc-200">Supabase 없이도</strong> 바로 사용 가능합니다. 먼저 로컬로 시작하고, 다기기 동기화가 필요할 때만 <span className="text-blue-400">🆕 처음 사용</span>을 선택하세요.
                </span>
                <button onClick={onSkip} className="shrink-0 px-3 py-1.5 rounded-lg border border-blue-500/30 hover:bg-blue-500/10 text-blue-300 font-medium transition-colors">
                  로컬로 바로 시작
                </button>
              </div>
              ...
              <div className="w-full max-w-4xl bg-zinc-900/60 border border-zinc-700 rounded-xl px-4 py-2.5 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-xs text-zinc-300 font-medium truncate">에이전트 온보딩 프롬프트</span>
                  <span className="text-[10px] text-zinc-500 hidden sm:inline">— 현재 OS를 진단하고 로컬 실행에 필요한 항목만 안내</span>
                </div>
                <button
                  onClick={() => {
              ...
              {/* CLI 토큰은 로컬에만 보관하고, 새 기기에는 연결 설정만 전달 */}
              <div className="w-full max-w-4xl grid sm:grid-cols-2 gap-3">
                <div className="bg-zinc-900 border border-emerald-500/30 rounded-2xl p-5 text-left">
                  ...
                  <h3 className="text-sm font-semibold text-white mb-1">🔒 CLI 토큰은 이 기기에만 보관</h3>
                  <p className="text-xs text-zinc-500">보안을 위해 GitHub · Vercel · Supabase CLI 토큰의 클라우드 백업과 복원을 사용하지 않습니다.</p>
                  <p className="text-xs text-emerald-400/80 mt-3">새 기기에서는 각 CLI에 직접 로그인하세요.</p>
                </div>
                <button onClick={() => setMode('additional')}
                  ...
                  <h3 className="text-sm font-semibold text-white mb-1">📥 새 기기 설정 연결</h3>
                  <p className="text-xs text-zinc-500">Supabase URL · Anon Key와 기기 설정만 연결합니다. CLI 로그인 토큰은 포함하지 않습니다.</p>
