import { normalizeMentionAlias } from './projectMemoryMentionAlias';
 ...
 */
export function parseAiMemoryRecommendation(raw: string): { displayName: string; mentionAlias: string | null } | null {
  const object = raw.trim().match(/\{[\s\S]*\}/)?.[0];
  if (!object) return null;
  try {
    const parsed = JSON.parse(object) as { displayName?: unknown; mentionAlias?: unknown };
    const displayName = parseAiMemoryAlias(JSON.stringify({ displayName: parsed.displayName }));
    if (!displayName) return null;
    const mentionAlias = typeof parsed.mentionAlias === 'string'
      ? normalizeMentionAlias(parsed.mentionAlias)
      : null;
    return {
      displayName,
      mentionAlias,
