...
export const MACOS_BASE_API_SIDECAR_ENTITLEMENTS = '<?xml version="1.0" encoding="UTF-8"?>\n<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n<plist version="1.0"><dict><key>com.apple.security.cs.allow-jit</key><true/></dict></plist>\n';
const RESOURCE_ROOT = 'Contents/Resources/resources/';
export const MACOS_BASE_CODEX_ACCESSIBILITY_HELPER_IDENTIFIER =
  'com.intenet.agentstozbycs.helper.codex-accessibility' as const;
export const MACOS_BASE_CODE = Object.freeze([
  ...
  { path: RESOURCE_ROOT + 'agentstoz-codex-accessibility-helper', identifier: MACOS_BASE_CODEX_ACCESSIBILITY_HELPER_IDENTIFIER },
...
export interface MacOSBaseSigningCommand {
  ...
  readonly entitlements: 'apiSidecar' | 'empty';
  readonly args: readonly string[];
}

export interface MacOSBaseEntitlements {
  readonly apiSidecar: string;
  readonly empty: string;
}

export function planMacOSBaseAppSigning(identity: Readonly<MacOSRuntimeProductionBuildIdentityResolution>, app: string, entitlements: Readonly<MacOSBaseEntitlements>): readonly MacOSBaseSigningCommand[] {
  if (!validMacOSRuntimeProductionIdentity(identity)) throw new Error('base signing identity rejected');
  if (!canonicalPath(app) || !app.endsWith('/AgentsToZ_byCS.app')
    || !canonicalPath(entitlements.apiSidecar) || !canonicalPath(entitlements.empty)
    || entitlements.apiSidecar === entitlements.empty
    || [entitlements.apiSidecar, entitlements.empty].some(path => path === app || path.startsWith(app + '/'))) {
    throw new Error('base signing path rejected');
  }
  ...
  return Object.freeze(MACOS_BASE_CODE.flatMap(code => {
    ...
    const entitlementKind = code.path === RESOURCE_ROOT + 'agentstoz-api-sidecar' ? 'apiSidecar' : 'empty';
    const entitlementPath = entitlements[entitlementKind];
    ...
    return [
      { operation: 'sign' as const, code, entitlements: entitlementKind, args: ['--force', '--sign', key.certificateFingerprint, '--options', 'runtime', '--timestamp', '--identifier', code.identifier, '--entitlements', entitlementPath, target] },
      { operation: 'verify' as const, code, entitlements: entitlementKind, args: ['--verify', '--strict', '--all-architectures', ...(target === app ? ['--deep'] : []), '-R', requirement, target] },
      { operation: 'inspect' as const, code, entitlements: entitlementKind, args: ['--display', '--verbose=4', '--entitlements', '-', '--xml', target] },
    ].map(command => Object.freeze({ ...command, args: Object.freeze(command.args) })) as MacOSBaseSigningCommand[];
  }));
}
...
export function verifyMacOSBaseSignatureInspection(output: string, app: string, code: typeof MACOS_BASE_CODE[number], identity: Readonly<MacOSRuntimeProductionBuildIdentityResolution>): void {
  ...
  const plists = [...output.matchAll(/<plist\b[^>]*>([\s\S]*?)<\/plist>/gu)];
  const entitlementBody = plists.length === 1 ? plists[0]![1]!.trim() : '';
  const apiSidecar = code.path === RESOURCE_ROOT + 'agentstoz-api-sidecar';
  const entitlementsValid = apiSidecar
    ? /^<dict>\s*<key>com\.apple\.security\.cs\.allow-jit<\/key>\s*<true\s*\/>\s*<\/dict>$/u.test(entitlementBody)
    : /^(?:<dict\s*\/\s*>|<dict>\s*<\/dict>)$/u.test(entitlementBody);
  // The compiled Bun computation/SQLite/file probe passes without exceptions.
  // Full sidecar and app runtime smoke remains a separate release gate. Only
  // the API sidecar's Bun FFI may use JIT; inherited debug/broker grants fail.
...
export async function signMacOSBaseApp(identity: Readonly<MacOSRuntimeProductionBuildIdentityResolution>, app: string, entitlements: Readonly<MacOSBaseEntitlements>,
  ...
  for (const command of commands) {
    if (command.operation === 'sign') {
      inspectMacOSBaseBundle(app);
      const entitlementPath = entitlements[command.entitlements];
      const expected = command.entitlements === 'apiSidecar'
        ? MACOS_BASE_API_SIDECAR_ENTITLEMENTS
        : MACOS_BASE_EMPTY_ENTITLEMENTS;
      if (realpathSync(entitlementPath) !== entitlementPath || !lstatSync(entitlementPath).isFile()
        || readFileSync(entitlementPath, 'utf8') !== expected) throw new Error('base entitlements rejected');
    }
  ...
  return Object.freeze({ schemaVersion: 1, mode: 'developer-id-base', result: 'signed-awaiting-notarization',
    appSigned: true, nestedCodeSigned: 4, bundleDigest: digest.digest('hex'),
