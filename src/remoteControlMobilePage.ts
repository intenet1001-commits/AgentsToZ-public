import {mountLanTester} from './mobileTesterLan';
...
<html lang="ko" data-app-theme="gray">
  ...
  <body>
    <main>
      ...
      <section class="panel" id="ops-panel" hidden>
        <h2>AgentsToZ OPS</h2>
        <p id="ops-status">운영기억 상태를 아직 확인하지 않았습니다.</p>
        <button type="button" class="secondary" id="ops-refresh" data-action="ops.status" data-available="true">운영기억 상태 확인</button>
        <button type="button" class="secondary" id="ops-candidates-refresh" data-action="ops.memory.pending" data-available="true">저장 후보 확인</button>
        <button type="button" id="ops-open" data-action="ops.open" data-available="true">AgentsToZ OPS 열기</button>
        <p>후보 접수는 저장이 아닙니다. 같은 Wi-Fi 화면에서는 조회만 가능하며, 저장 승인은 SAS 승인된 개인 포털 또는 Mac OPS 패널에서 진행하세요.</p>
        <div id="ops-candidates"></div>
      </section>
      ...
      <section id="tester-panel" class="panel" hidden></section>
...
const opsOpen = document.querySelector("#ops-open");
...
let testerSupported=false;
let testerCapabilityRequest='';
function probeTester(){
  testerSupported=false;testerCapabilityRequest=nextActionId();
  socket.send(JSON.stringify({type:'action.request',protocolVersion:PROTOCOL_VERSION,sessionToken,actionId:testerCapabilityRequest,action:'protocol.capabilities'}));
}
const lanTester=(${mountLanTester.toString()})(document.querySelector('#tester-panel'),request=>terminalRequest(request),()=>({online:connection.classList.contains('online'),owner:sessionToken,supported:testerSupported,projects}));
...
function sendAction(action, controlId, remoteConfirmed, page) {
  if (!socket || socket.readyState !== WebSocket.OPEN || !sessionToken || inFlight) return;
  const targetName = action.indexOf("ops.") === 0 ? "AgentsToZ OPS" : projectName(controlId);
...
function handleActionResult(message) {
  ...
  if (message.ok) {
    if (message.ops) {
      const labels={ready:"운영기억 연결됨",preparing:"준비 중","needs-attention":"연결 확인 필요",unconfigured:"프로필 준비 필요"};
      const saved=message.ops.lastSavedAt?" · 마지막 저장 "+new Date(message.ops.lastSavedAt).toLocaleString():"";
      opsStatus.textContent=(labels[message.ops.state]||"상태 확인 필요")+" · 저장 후보 "+message.ops.pendingCount+"개"+saved;
      notify(context + " 완료");
      return;
    }
    if(message.opsCandidates){
      const container=document.querySelector("#ops-candidates");container.replaceChildren();
      if(!message.opsCandidates.candidates.length){const empty=document.createElement("p");empty.textContent="검토할 OPS 운영기억 후보가 없습니다.";container.append(empty);}
      for(const candidate of message.opsCandidates.candidates){const article=document.createElement("article"),title=document.createElement("strong"),body=document.createElement("p"),evidence=document.createElement("small");title.textContent=candidate.title;body.textContent=candidate.body;evidence.textContent="근거: "+candidate.evidence;article.append(title,body,evidence);container.append(article);}
      notify(context+" 완료");return;
    }
...
function connect() {
  ...
  socket.addEventListener("message", (event) => {
    ...
    if (message.type === "action.result") {
      if(message.actionId===testerCapabilityRequest){testerSupported=message.ok&&Array.isArray(message.supportedFeatures)&&message.supportedFeatures.includes('tester-v1');lanTester.update();return;}
...
opsRefresh.addEventListener("click", () => sendAction("ops.status", "", false));
document.querySelector("#ops-candidates-refresh").addEventListener("click", () => sendAction("ops.memory.pending", "", false));
opsOpen.addEventListener("click", () => sendAction("ops.open", "", true));
...
body[data-workspace-tab="projects"] #terminal-panel,body[data-workspace-tab="workroom"] #projects,body[data-workspace-tab="workroom"] #filter,body[data-workspace-tab="workroom"] #project-create,body[data-workspace-tab="workroom"] #tester-panel{display:none;}
