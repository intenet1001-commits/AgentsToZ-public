...
import { MACOS_BASE_API_SIDECAR_ENTITLEMENTS, MACOS_BASE_EMPTY_ENTITLEMENTS, MACOS_BASE_REQUIRED_FILES, inspectMacOSBaseBundle, signMacOSBaseApp, type MacOSBaseEntitlements } from './macOSBaseAppSigning';
...
export interface MacOSBasePipelineDependencies {
  ...
  sign(identity: Readonly<MacOSRuntimeProductionBuildIdentityResolution>, app: string, entitlements: Readonly<MacOSBaseEntitlements>): ReturnType<typeof signMacOSBaseApp>;
  now(): number;
}
...
export async function executeMacOSBaseAppPipelineForTest(projectRoot: string, dependencies: MacOSBasePipelineDependencies) {
  ...
  const entitlements = Object.freeze({
    apiSidecar: join(targetDir, 'base-api-sidecar-entitlements.plist'),
    empty: join(targetDir, 'base-empty-entitlements.plist'),
  });
  writeFileSync(entitlements.apiSidecar, MACOS_BASE_API_SIDECAR_ENTITLEMENTS, { mode: 0o600, flag: 'wx' });
  writeFileSync(entitlements.empty, MACOS_BASE_EMPTY_ENTITLEMENTS, { mode: 0o600, flag: 'wx' });
  ...
  if (signed.mode !== 'developer-id-base' || signed.result !== 'signed-awaiting-notarization'
    || !signed.appSigned || signed.nestedCodeSigned !== 4 || !/^[0-9a-f]{64}$/u.test(signed.bundleDigest)
  ...
  const receipt = Object.freeze({ ...signed, sourceSha: source.headSha, appBundlePath: app,
    signingIdentity: Object.freeze({ ...identity.identity! }),
