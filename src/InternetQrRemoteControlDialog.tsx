...
import {RemoteConnectionLabel} from './RemoteConnectionLabel';
...
type WorkroomConnection = { id: string; allowed: boolean; durable?: boolean; expiresAt?: string | null };
...
export function InternetQrRemoteControlDialog({
  ...
  const [pendingWorkroom,setPendingWorkroom]=useState<Record<string,string>>({});
  const [workroomConnections, setWorkroomConnections] = useState<WorkroomConnection[] | null>(null);
  useEffect(()=>{
    if(!open){setWorkroomApprovals({});setWorkroomScopes({});setWorkroomConnections(null);return;}
    setWorkroomOptions([]);
    let stopped=false;
  ...
  const grantWorkroom=async(sessionId:string,scope:string)=>{
    const result=await terminalLocalRequest(AI_TERMINAL_PREFIX+'/access',{owner:'internet:'+sessionId,enabled:true,rememberDevice:true,...workroomApprovalScope(scope,workroomOptions)});
  ...
  const handleApprove = async (session: InternetRemoteControlSession) => {
    ...
    try {
      ...
      ),workroomApprovals[session.sessionId]?()=>grantWorkroom(session.sessionId,workroomScope!):undefined);
      applyStatus(approval.status);
      if(approval.workroomError)setPendingWorkroom(current=>({...current,[session.sessionId]:workroomScope!}));
      ...
      setNotice(approval.workroomError?{kind:'error',message:`연결 승인은 완료됐습니다. 워크룸 권한 저장에 실패했습니다: ${approval.workroomError}. 아래에서 권한 저장만 다시 시도하세요.`}:{ kind: 'success', message: `${session.controllerName} 연결을 이 Mac에서 승인했습니다.${workroomApprovals[session.sessionId]?' 워크룸 권한도 최대 30일 유지됩니다.':''}` });
    } catch (error) {
      setNotice({ kind: 'error', message: readableError(error) });
    } finally {
      setBusy(null);
    ...
    >
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5 sm:py-5">
          ...
          {Object.entries(pendingWorkroom).map(([sessionId,scope])=><div key={sessionId} role="alert" className="internet-workroom-retry">연결 승인 완료 · 워크룸 권한 저장 필요<button type="button" disabled={busy!==null} onClick={async()=>{setBusy('scopes');try{await grantWorkroom(sessionId,scope);setPendingWorkroom(current=>{const next={...current};delete next[sessionId];return next});setNotice({kind:'success',message:'워크룸 권한을 저장했습니다.'})}catch(error){setNotice({kind:'error',message:readableError(error)})}finally{setBusy(null)}}}>권한 저장 다시 시도</button></div>)}
            {notice && (
            <div
                            ...
                            return (
                              <div key={session.sessionId} className="min-w-0 rounded-xl border border-[rgb(var(--surface-highlight-rgb))]/[0.07] bg-[rgb(var(--surface-shade-rgb))]/20 p-3">
                                ...
                                <div className="internet-workroom-status" data-testid="approved-internet-workroom-status">
                                  ...
                                  {onOpenWorkroom && <button type="button" onClick={onOpenWorkroom}>워크룸 열기 · 기기 권한 관리</button>}
                                </div>
