...
export function InternetQrRemoteControlDialog({
  ...
  useEffect(()=>{
    if(!open){setWorkroomApprovals({});setWorkroomScopes({});return;}
    setWorkroomOptions([]);
    let stopped=false;
    Promise.all([terminalLocalRequest(AI_TERMINAL_PREFIX+'/access',{}),new AgentRuntimeClient().targets()]).then(([access,inventory])=>{
      if(stopped)return;
  ...
  const grantWorkroom=(sessionId:string,scope:string)=>terminalLocalRequest(AI_TERMINAL_PREFIX+'/access',{owner:'internet:'+sessionId,enabled:true,rememberDevice:true,...workroomApprovalScope(scope,workroomOptions)});
