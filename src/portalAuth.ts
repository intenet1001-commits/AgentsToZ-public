...
export async function preflightPortalGoogleOAuth(options: {
  ...
  try {
    const response = await Promise.race([
      fetchImpl(new URL('/auth/v1/settings', origin), {
        method: 'GET',
        headers: {
  ...
  if (error instanceof PortalOAuthStartError) {
    switch (error.failure) {
      case 'PREFLIGHT_TIMEOUT':
        return 'Supabase 로그인 서버가 8초 안에 응답하지 않았습니다. 현재 화면은 유지했습니다. 잠시 후 다시 시도하고, 계속되면 Supabase Auth 상태를 확인해 주세요.';
      case 'PREFLIGHT_NETWORK':
        return 'Supabase 로그인 서버에 연결하지 못했습니다. 네트워크와 Supabase Auth 상태를 확인한 뒤 다시 시도해 주세요.';
