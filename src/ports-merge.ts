// This module is imported directly by the Node smoke runner as well as bundled
// by Bun/Vite. Keep the TypeScript extension so native ESM resolves both paths.
import {isProjectRole} from './projectRole.ts';

export type PortRecord = {
  id: string;
  /** Untrusted disk/input value: known writers use ProjectRole; unknown values survive round trips. */
  role?: unknown;
};
...
function preserveOmittedRole<T extends PortRecord>(desired: T, current?: T): T {
  return desired.role === undefined && current?.role !== undefined
    ? {...desired, role: current.role} : desired;
}

function validateRoleChanges<T extends PortRecord>(merged: T[], current: readonly T[]): T[] {
  const currentById = new Map(current.map(port => [port.id, port]));
  for (const port of merged) {
    if (port.role !== undefined && !isProjectRole(port.role)
      && !jsonEqual(port.role, currentById.get(port.id)?.role)) {
...
function changedFromBase(base: PortRecord, desired: PortRecord): boolean {
  ...
  for (const key of keys) {
    if (key === 'role' && desiredRecord[key] === undefined) continue;
...
export function mergePortSnapshots<T extends PortRecord>(
  ...
  for (const desiredPort of desired) {
    ...
    if (!basePort) {
      ...
      mergedById.set(desiredPort.id, preserveOmittedRole(desiredPort, currentPort));
      continue;
    }
  ...
  return validateRoleChanges([
    ...desiredAddedIds,
    ...concurrentAddedIds,
    ...remainingDesiredIds,
    ...remainingIds,
  ].map(id => mergedById.get(id)!), current);
}
...
export function mergeLegacyPortSave<T extends PortRecord>(
  ...
  const currentById = new Map(current.map(port => [port.id, port]));
  return validateRoleChanges([
    ...desired.map(port => preserveOmittedRole(port, currentById.get(port.id))),
