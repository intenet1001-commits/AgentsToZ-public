...
import {TERMINAL_MEMORY_HISTORY_KEY,canMoveMemoryToHistory,memoryHistoryKey,readMemoryHistory,moveMemoryToHistory,reuseUnchangedMemoryJobs} from './terminalMemoryHistory';
import type {WorkroomMemoryReviewTarget} from './workroomMemoryReview';
...
const labels: Record<TerminalMemoryState, string> = {
  ...
  unchanged: '새로 저장할 활동 없음 · 정상', unavailable: '장기기억 초기화 필요',
  'backup-pending': '로컬 저장 완료 · Supabase 백업 확인 필요', failed: '세션 기억 저장 실패 · 로컬 저장 원인 확인',
  retrying: '작업공간 사용 중 · 잠시 후 다시 저장',
  'recovery-required': '이전 작업·저장 결과 확인 후 복구 필요 · 자동 재실행 안 함',
};
...
export function TerminalMemoryStatus({visible, projects, onReview, onManageProject, observationTargetId}: {
  ...
  onReview: (target: WorkroomMemoryReviewTarget) => void;
  ...
  const displayedOffset = useRef<number | null>(null);
  const [page, setPage] = useState<{total: number; unresolved: number; nextOffset: number | null} | null>(null);
  useEffect(() => {
    if (!visible) return;
    let stopped = false;
    const refresh = async () => {
      if (displayedOffset.current !== offset) setPageLoading(true);
      ...
      try {
        ...
        if (!stopped) {
          displayedOffset.current=offset;
          ...
          setJobs(previous=>reuseUnchangedMemoryJobs(previous,result.jobs)); setLoaded(true); setError(false);
          ...
          if(observationTargetId&&!requestedObservation&&observationSupported.current)setObservation(null);
          else if(observationTargetId)setObservation(previous=>{
            const next={targetId:observationTargetId,value:value?.version===1&&value.targetId===observationTargetId
            ...
            return previous?.targetId===next.targetId&&JSON.stringify(previous.value)===JSON.stringify(next.value)?previous:next;
          });
          setPage(previous=>{
            const next=Number.isSafeInteger(result.total) && Number.isSafeInteger(result.unresolved)
              && (result.nextOffset === null || Number.isSafeInteger(result.nextOffset))
              ? {total: result.total as number, unresolved: result.unresolved as number, nextOffset: result.nextOffset as number|null} : null;
            return previous?.total===next?.total&&previous?.unresolved===next?.unresolved&&previous?.nextOffset===next?.nextOffset?previous:next;
          });
        }
  ...
  const row = (job: MemoryJobGroup, isPrevious = false) => {
    const project = projects.find(project => project.targetId === job.targetId);
    const groupJobs = current.filter(item => item.targetId === job.targetId && item.state === job.state);
    const canTidy = !isPrevious && groupJobs.some(canMoveMemoryToHistory);
    return <li key={job.key} className="ai-terminal-memory-row">
      ...
      {['failed', 'backup-pending', 'recovery-required'].includes(job.state) && <button className="ai-terminal-btn" onClick={() => onReview({targetId:job.targetId,state:job.state as WorkroomMemoryReviewTarget['state'],count:job.count})}>새 AI로 원인 조사 요청</button>}
      ...
      {canTidy && <button className="ai-terminal-btn" onClick={() => savePrevious(moveMemoryToHistory(previous,groupJobs))}>이 항목 이전 기록으로 정리</button>}
    </li>;
  };
  ...
  return <section className="ai-terminal-memory" aria-label="세션 저장">
    ...
    <div className="ai-terminal-memory-heading">
      <h3>워크룸 종료 후 장기기억 저장 결과</h3>
      <div data-testid="workroom-memory-purpose" className="ai-terminal-hint"><b>이곳은 대화 입력창이 아니라 저장 결과판입니다.</b> 각 줄에서 로컬 장기기억 저장과 Supabase 백업이 어디까지 끝났는지 확인합니다. 실패·백업 대기는 원본을 지우지 않으며 자동으로 성공 처리되지 않습니다.</div>
      <p className="ai-terminal-hint" data-testid="workroom-memory-guide">지금 저장하려면 상단 ‘세션 기억하기…’에서 요청을 확인하고 실행하세요. 아래 ‘새 AI로 원인 조사 요청’은 기존 세션을 재개하지 않고 새 조사 초안만 열며 저장을 자동 재실행하지 않습니다. 불필요한 표시 기록은 항목별로 접을 수 있고 원본 저장 상태는 유지됩니다. 완료 대화 감지와 장기기억 저장 완료도 서로 다른 상태입니다.</p>
      <details className="ai-terminal-memory-help"><summary>언제 저장되고 무엇이 남나요?</summary><p className="ai-terminal-hint">자동 세션 기억이 켜져 있으면 세션 종료 시 미저장 활동을 로컬 장기기억에 반영한 뒤 Supabase 백업을 시도합니다. 앱 종료 시 아직 시작하지 않은 저장은 다음 실행에서 이어집니다. 저장 도중 종료됐거나 이전 작업의 잠금이 남으면 완료 여부를 먼저 확인하고, 자동으로 중복 실행하지 않습니다. 창 숨기기·탭 이동은 세션 종료가 아닙니다.</p></details>
    </div>
    ...
    {!!attention.length && <div data-testid="workroom-memory-attention">
      ...
      <ul className="ai-terminal-memory-list">{attentionGroups.slice(0, attentionLimit).map(job=>row(job))}</ul>
    ...
    {!!history.length && <details data-testid="workroom-memory-history" className="ai-terminal-memory-history" open={historyOpen} onToggle={event => setHistoryOpen(event.currentTarget.open)}>
      ...
      {historyOpen && <><ul className="ai-terminal-memory-list">{historyGroups.slice(0, historyLimit).map(job=>row(job))}</ul>
    ...
    {!!older.length&&<details data-testid="workroom-memory-previous" className="ai-terminal-memory-history">
      ...
      <ul className="ai-terminal-memory-list">{groupTerminalMemoryJobs(older).map(job=>row(job,true))}</ul>
