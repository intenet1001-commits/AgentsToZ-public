...
export class RemoteControlInternetAgent {
  ...
  constructor(options: CreateRemoteControlInternetAgentOptions) {
    ...
    const supportedFeatures = [
      ...(options.terminalGateway && options.workspaceSupported ? ['workspace-v1',...(options.testerSupported?['tester-v1']:[])] : []),
  ...
  async #handleDelivery(delivery: RemoteControlInternetReceivedEnvelope): Promise<void> {
    ...
    if (!decision.ok&&!sequenceGap) throw new RemoteControlError('RELAY_SEQUENCE_REJECTED', '원격 요청 순번이나 만료 상태가 올바르지 않습니다.', 409);
    ...
    if (!prepared) {
      ...
      if(sequenceGap) {
        // Only an approved controller's authenticated, valid pairing
        // checkpoint may bridge an expired delivery. Never admit actions,
        // terminal input or tasks across a missing sequence.
        const checkpoint=parseRemoteControlClientJson(serialized);
        if(checkpoint.type!=='controller.pair'||!this.#isActivePairingSecret(checkpoint.token))
          throw new RemoteControlError('RELAY_SEQUENCE_REJECTED','원격 요청 순번이 올바르지 않습니다.',409);
      }
      ...
      if (terminalRequest) {
        try {
          if(!state.coreSessionToken || !sameOpaqueSecret(terminalRequest.sessionToken,state.coreSessionToken)) {
            ...
            const body=await this.#terminalGateway(terminalRequest.request,bindings,'internet:'+state.row.sessionId,()=>!this.#stopped&&this.#sessionCrypto.get(state.row.sessionId)===state&&state.row.approvalState==='approved'&&!this.#locallyRevokedSessions.has(state.row.sessionId));
        ...
        try {
          if (request.type === 'controller.pair') {
            ...
            if (request.type === 'session.restore') {
              ...
              response = await this.#core.perform(request,{controllerId:'internet:'+state.row.controllerId,expiresAt:state.row.expiresAt});
              responseType = 'result';
            }
          }
        } catch (error) {
