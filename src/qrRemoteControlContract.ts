...
export interface QrRemoteControlSession {
  ...
  label: string;
  /**
   * Whether a socket is attached right now. Being listed is not the same as being connected:
   * a paired phone stays listed while its screen is locked so the operator can still see and
   * revoke it, but anything that grants access — the AI terminal above all — must key off this
   * flag, never off mere presence in the list.
   */
  connected: boolean;
  pairedAt: string;
...
export interface QrRemoteControlOpsStatus {
...
export interface QrRemoteControlOpsCandidate {
  id: string;
  title: string;
  body: string;
  evidence: string;
...
export interface QrRemoteControlOpsCandidates {
  revision: string;
  candidates: QrRemoteControlOpsCandidate[];
}
...
const INTERNAL_ID_RE_FOR_OPS = /^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/;
...
function normalizeSession(value: unknown): QrRemoteControlSession {
  ...
  return {
    ...
    // An older Mac that predates the flag reports no field at all. Treating that as connected
    // preserves its behaviour; only an explicit false marks a phone as away.
...
export function normalizeQrRemoteControlOpsStatus(payload: unknown): QrRemoteControlOpsStatus {
  const outer = asObject(payload) ?? invalid();
  const raw = asObject(outer.ops) ?? outer;
  const keys = ['state', 'backend', 'pendingCount', 'lastSavedAt', 'syncState'];
  if (Object.keys(raw).length !== keys.length || keys.some(key => !Object.prototype.hasOwnProperty.call(raw, key))) invalid();
  if (!['unconfigured', 'preparing', 'ready', 'needs-attention'].includes(String(raw.state))) invalid();
  if (raw.backend !== null && raw.backend !== 'app-data' && raw.backend !== 'control-folder') invalid();
  if (!Number.isInteger(raw.pendingCount) || (raw.pendingCount as number) < 0 || (raw.pendingCount as number) > 500) invalid();
  if (raw.syncState !== null && !['current', 'needs-attention', 'not-configured'].includes(String(raw.syncState))) invalid();
  const lastSavedAt = raw.lastSavedAt === null ? null : isoDate(raw.lastSavedAt);
  if (raw.lastSavedAt !== null && !lastSavedAt) invalid();
  return {
    state: raw.state as QrRemoteControlOpsStatus['state'],
    backend: raw.backend as QrRemoteControlOpsStatus['backend'],
    pendingCount: raw.pendingCount as number,
    lastSavedAt,
    syncState: raw.syncState as QrRemoteControlOpsStatus['syncState'],
  };
}

export function normalizeQrRemoteControlOpsCandidates(payload: unknown): QrRemoteControlOpsCandidates {
  const outer = asObject(payload) ?? invalid();
  const raw = asObject(outer.opsCandidates) ?? outer;
  if (Object.keys(raw).length !== 2 || !Object.prototype.hasOwnProperty.call(raw, 'revision') || !Object.prototype.hasOwnProperty.call(raw, 'candidates')) invalid();
  if (typeof raw.revision !== 'string' || !/^[a-f0-9]{64}$/.test(raw.revision) || !Array.isArray(raw.candidates) || raw.candidates.length > 50) invalid();
  const candidates = raw.candidates.map(value => {
    const item = asObject(value) ?? invalid();
    const keys = ['id','title','body','evidence','baseRevision','createdAt'];
    if (Object.keys(item).length !== keys.length || keys.some(key => !Object.prototype.hasOwnProperty.call(item,key))) invalid();
    const id = boundedString(item.id,128),title=boundedString(item.title,200),body=boundedString(item.body,4000),evidence=boundedString(item.evidence,1000),createdAt=isoDate(item.createdAt);
    if(!id||!INTERNAL_ID_RE_FOR_OPS.test(id)||!title||!body||!evidence||typeof item.baseRevision!=='string'||!/^[a-f0-9]{64}$/.test(item.baseRevision)||!createdAt)invalid();
    return {id,title,body,evidence,baseRevision:item.baseRevision,createdAt};
  });
  return {revision:raw.revision,candidates};
}
