...
export const CODEX_DESKTOP_ACCESSIBILITY_HELPER_NAME =
  'agentstoz-codex-accessibility-helper' as const;

function verifiedExecutable(path: string): string | null {
  try {
    const canonical = realpathSync(path);
    const info = lstatSync(canonical);
    return canonical === path && info.isFile() && (info.mode & 0o111) !== 0 ? canonical : null;
  } catch {
    return null;
  }
}

export function resolveCodexDesktopAccessibilityHelper(input: {
  execPath?: string;
  moduleDir?: string;
  ...
  const moduleDir = input.moduleDir ?? import.meta.dir;
  const exists = input.exists ?? existsSync;
  const candidates = /^agentstoz-api-sidecar$/u.test(basename(execPath))
    ? [join(dirname(execPath), CODEX_DESKTOP_ACCESSIBILITY_HELPER_NAME)]
    : [join(moduleDir, '..', 'src-tauri', 'resources', CODEX_DESKTOP_ACCESSIBILITY_HELPER_NAME)];
  for (const candidate of candidates) {
    if (exists(candidate)) return verifiedExecutable(candidate);
  }
  return null;
}

export function buildCodexDesktopPidProbeAppleScript(): string {
  return [
    ...
    '  if (count of matchingProcesses) is 0 then error "CODEX_DESKTOP_PROCESS_NOT_FOUND"',
    '  if (count of matchingProcesses) is not 1 then error "CODEX_DESKTOP_SUBMISSION_UNCERTAIN"',
    '  return unix id of item 1 of matchingProcesses as text',
    'end tell',
  ].join('\n');
}
...
export function describeCodexDesktopComposerAutomationFailure(
  ...
  return 'Codex 프로젝트의 새 대화 입력창을 확인하지 못해 첫 메시지를 보내지 않았습니다. Mac의 Codex에서 선택한 프로젝트와 연결 확인 문구가 맞는지 확인하세요. 문구를 직접 전송한 경우 AgentsToZ의 연결 상태를 새로고침하면 생성된 대화를 확인할 수 있습니다.';
}
