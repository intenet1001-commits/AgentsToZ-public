...
export function buildMemoryDirectory(
  ...
  const physicalNames = canonicalPhysicalDevices(physicalDeviceRows, resolveDeviceId);
  const physicalNameByCanonical = new Map([...physicalNames].map(([id, row]) => [id, {name: (row.name ?? '').trim()}]));
...
export function buildMemoryDeviceFilters(
  ...
  for (const [deviceId, row] of canonicalPhysicalDevices(physicalDeviceRows, resolveDeviceId)) {
...
/** Explicit identity links alone collapse installations. Prefer the representative
 * name deterministically, even when an old installation reports more recently. */
function canonicalPhysicalDevices(rows: readonly PhysicalDeviceRow[], resolve: (id: string) => string): Map<string, PhysicalDeviceRow> {
  const result = new Map<string, PhysicalDeviceRow>();
  for (const row of rows) {
    const id = resolve((row.id ?? '').trim());
    if (!id) continue;
    const current = result.get(id);
    const rank = (value: PhysicalDeviceRow) => Number(!!value.name?.trim()) * 2 + Number(value.id?.trim() === id);
    if (!current || rank(row) > rank(current) || (rank(row) === rank(current)
      && (compareCreatedAt(row.last_push_at, current.last_push_at) > 0
        || (compareCreatedAt(row.last_push_at, current.last_push_at) === 0 && (row.id ?? '') < (current.id ?? ''))))) result.set(id, row);
  }
  return result;
}
 ...
 *
 * 포털은 현재 Google 세션과 `portmgr_is_member()` 결과를 모두 확인하지만, 세션이
 * 이후 만료되면 진행 중이던 쿼리가 anon으로 나가 42501(permission denied)을 받을 수
 * 있다. **세션 유무를 모른 채 "정책을 다시 실행하라"고 하면 멀쩡한 DB를 건드리게
 * 만드는 오답이다** — 그래서 세션 상태를 받아 둘을 갈라 말한다.
 */
 ...
 *
 * 최신 `portmgr_ports.memory_id`가 있으면 그 값을 정본으로 쓴다. 아직 그 열을 채우지 않은
 * 구버전 행만 저장소 주소와 이름으로 잇는데, **저장소가 먼저**다 — 이름은 바뀌어도 저장소는
 * 그 프로젝트의 계보 키(`github_url`)이기 때문이다(실측 2026-08-14: 이 기기 9개 중
 * 저장소로 4개, 이름으로 4개가 더 붙어 8개 연결).
 *
