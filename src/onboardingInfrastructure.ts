...
export type OnboardingRuntimeMode = 'packaged' | 'source' | 'remote';
...
export const ONBOARDING_SCENARIOS: OnboardingScenario[] = [
  ...
  },
  {
    ...
    label: '첫 단말 + 동기화',
    shortLabel: '첫 단말',
    description: '이 PC에 Supabase 동기화를 연결합니다. GitHub·Vercel은 필요할 때만 추가합니다.',
...
export const ONBOARDING_TOOLS: OnboardingToolDefinition[] = [
  ...
  {
    id: 'api', label: 'AgentsToZ 로컬 API', description: '설치 앱에 포함되거나 소스와 함께 실행되는 localhost:3001 서비스', why: '설치·연결 현황, 동기화, AWS 프로젝트 연결을 실제로 처리합니다. 설치 앱 사용자는 별도 Bun 명령으로 시작하지 않습니다.',
  ...
  },
  {
    ...
    officialUrl: 'https://cli.github.com/', platforms: ['mac', 'windows', 'linux'], requirements: req('optional', 'optional', 'optional', 'recommended'),
  ...
  },
  {
    ...
    officialUrl: 'https://vercel.com/docs/cli', platforms: ['mac', 'windows', 'linux'], requirements: req('optional', 'optional', 'optional', 'optional'),
    install: {
...
export function toolRequirement(
  tool: OnboardingToolDefinition,
  scenario: OnboardingScenarioId,
  platform: OnboardingPlatform,
  runtimeMode: OnboardingRuntimeMode = platform === 'linux' ? 'remote' : 'source',
): OnboardingRequirement {
  if (!tool.platforms.includes(platform)) return 'not-applicable';
  if (runtimeMode === 'packaged') {
    if (tool.id === 'bun' || tool.id === 'dependencies') return 'not-applicable';
    if (tool.id === 'git') return 'optional';
  }
...
export function toolsForScenario(
  ...
  return ONBOARDING_TOOLS
    .filter(tool => toolRequirement(tool, scenario, platform, runtimeMode) !== 'not-applicable')
    .sort((a, b) => rank[toolRequirement(a, scenario, platform, runtimeMode)] - rank[toolRequirement(b, scenario, platform, runtimeMode)]);
}
...
export function toolInstallCommand(
  ...
  if (runtimeMode === 'packaged' && (tool.id === 'bun' || tool.id === 'git' || tool.id === 'dependencies' || tool.id === 'api')) {
    return undefined;
  }
  return tool.install[platform];
}
...
function safeDiagnosticContext(diagnostics: OnboardingToolDiagnostic[]): Array<Record<string, string | boolean>> {
  return diagnostics.map(item => ({
    id: item.id,
    state: item.state,
    ...(typeof item.installed === 'boolean' ? { installed: item.installed } : {}),
    ...(typeof item.authenticated === 'boolean' ? { authenticated: item.authenticated } : {}),
    ...(item.authenticationEvidence === 'cached' ? { authenticationEvidence: 'cached' } : {}),
  }));
}
...
export function buildOnboardingAgentPrompt(input: {
  ...
  const runtimeMode = input.runtimeMode ?? (input.platform === 'linux' ? 'remote' : 'source');
  ...
  const scenarioRules: Record<OnboardingScenarioId, string> = {
    local: runtimeMode === 'packaged'
      ? '클라우드 계정이나 GitHub clone을 만들지 않는다. 설치된 앱과 포함된 localhost API가 열리는지만 확인한다. Bun·node_modules는 설치하지 않는다.'
      : '클라우드 계정은 만들지 말고 소스 실행에 필요한 Bun·Git·앱 구성요소·localhost API까지만 준비한다.',
  ...
  const readOnlyChecks = runtimeMode === 'packaged'
    ? '현재 OS, 실행 중인 설치 앱의 정확한 경로, GET http://127.0.0.1:3001/api/onboarding/status, GET /api/onboarding/tools 결과를 확인하세요.'
    : '현재 OS, 저장소 루트, GET http://127.0.0.1:3001/api/onboarding/status, GET /api/onboarding/tools 결과를 확인하세요.';
...
2. ${readOnlyChecks} unknown을 미설치로 단정하지 마세요. authenticationEvidence가 cached면 저장된 로그인 정보만 확인한 것이며 실제 인증 성공과 구분하세요.
3. runtimeMode가 packaged면 Bun·node_modules·GitHub clone을 설치 앱의 필수 조건으로 제시하지 마세요. source일 때만 소스 실행 도구를 요구하세요. “필수 / 지금은 선택 / 나중에 가능” 세 묶음으로 짧게 설명하고, 지금 필요한 다음 행동은 항상 하나만 제시하세요.
4. 설치, 로그인, 권한 변경, 재부팅, 파일 덮어쓰기, 클라우드 리소스 생성·배포·과금 작업은 사용자가 요청한 범위에서는 진행하세요. 새 과금·공개 범위·덮어쓰기처럼 아직 결정하지 않은 사항은 영향과 정확한 대상을 보여주고 확인하세요.
...
7. Claude Code, Codex, Hermes, Buzz, Telegram, Antigravity는 선택 기능입니다. 사용자가 원한 기능에 필요한 것만 공식 문서(${ONBOARDING_GUIDE_URL})와 설치·연결 현황판 기준으로 안내하세요.
