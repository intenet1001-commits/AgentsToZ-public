...
export class AiTerminalService {
  ...
  #activity(cwd: string) { try { this.dependencies.activity?.(cwd); } catch { /* Capture must not break PTY input or exit. */ } }
  ...
  async perform(value: unknown, authority?: AiTerminalAuthority): Promise<AiTerminalResponse> {
    ...
    if (authority && (!this.remoteAllowed(authority.owner) || authority.isActive?.()===false)) throw new Error('Mac의 AI 터미널 화면에서 이 원격 연결의 터미널 접근을 허용하세요.');
    ...
    if (request.operation === 'read') {
      const available = session!.chunks.filter(c => c.seq > request.after!);
      const chunks: {seq:number;text:string}[]=[];let bytes=0;
      for(const c of available){const size=new TextEncoder().encode(JSON.stringify(c)).length;if(chunks.length>=4 || bytes+size>8500)break;chunks.push(c);bytes+=size;}
      const hasMore=chunks.length<available.length;
      return {session: this.#summary(session!), chunks, hasMore, nextCursor: chunks.at(-1)?.seq ?? request.after!, truncated: (session!.chunks[0]?.seq ?? 1) > request.after! + 1};
    }
  ...
  async #mutate(r: AiTerminalRequest, session: Session | null | undefined, authority?: AiTerminalAuthority, epoch?: number): Promise<AiTerminalResponse> {
    if (authority && (!this.remoteAllowed(authority.owner) || authority.isActive?.()===false || this.#grantEpoch.get(authority.owner) !== epoch)) throw new Error('원격 터미널 권한이 해제되었습니다.');
    if(this.#closed)throw new Error('터미널 서버가 종료 중입니다.');
    ...
    {
      if (session!.summary.state !== 'running') throw new Error('종료된 터미널입니다. 새 세션을 여세요.');
      if (r.operation === 'input') { session!.child.terminal.write(r.data!); this.#activity(cwd); }
