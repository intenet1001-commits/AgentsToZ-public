...
export class AiTerminalService {
  ...
  inspectSession(id:string,targetId:string) {
    ...
    return {...s.summary,cwd:s.cwd,inputRevision:s.inputRevision};
  }
  ...
  async #mutate(r: AiTerminalRequest, session: Session | null | undefined, authority?: AiTerminalAuthority, epoch?: number): Promise<AiTerminalResponse> {
    ...
    if (r.operation === 'start') {
      ...
      const record = {summary,cwd,inputRevision:0,chunks:[],next:0,buffered:0,decoder:new TextDecoder(),requestKeys:new Set<string>(),revalidate:target.revalidate} as unknown as Session;
      ...
      const args = [executable,...(r.agent==='claude'?['--session-id',summary.id]:r.agent==='codex'?['-c','tui.status_line=["context-remaining"]']:[]),...(r.prompt?['--',r.prompt]:[])];
    ...
    {
      if (session!.summary.state !== 'running') throw new Error('종료된 터미널입니다. 새 세션을 여세요.');
      if (r.operation === 'input') { session!.suppressMemoryOnExit=false;session!.inputRevision++; await writeAiTerminalInput(session!.summary.agent,session!.child.terminal,r.data!); this.#activity(cwd,session!.summary.agent); }
