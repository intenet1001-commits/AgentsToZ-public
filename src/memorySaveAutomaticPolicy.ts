...
/** Inside the provider-binding transaction only. A binary refresh cannot renew
 * consent, move the observation boundary, reset quota or change project scope. */
export function refreshAutomaticMemoryProvider(db:Database,expectedRevision:number,expectedDigest:string,nextDigest:string,now:number):AutomaticMemoryPolicy {
 if(!saveInteger(expectedRevision)||!saveHash(expectedDigest)||!saveHash(nextDigest)||!saveInteger(now))throw new MemorySaveError('INVALID_INPUT');
 const prior=read(db);
 if(!prior.enabled||prior.revision!==expectedRevision||prior.providerBindingDigest!==expectedDigest||!saveInteger(prior.revision+1))throw new MemorySaveError('POLICY_CHANGED');
 if(now<prior.lastClock)throw new MemorySaveError('CLOCK_ROLLBACK');
 const next={...prior,revision:prior.revision+1,providerBindingDigest:nextDigest};
...
export function checkAutomaticMemoryAdmission(db:Database,job:Pick<MemorySaveJob,'memoryId'|'policyEpoch'>,expectedRevision:number,intent:Pick<MemorySaveAttemptIntent,'providerBindingDigest'>,sources:readonly {sequence:number;payload:string}[],now:number):void {
  checkAdmission(db,job,expectedRevision,intent,sources,now);
}
function checkAdmission(db:Database,job:Pick<MemorySaveJob,'memoryId'|'policyEpoch'>,expectedRevision:number,intent:Pick<MemorySaveAttemptIntent,'providerBindingDigest'>,sources:readonly {sequence:number;payload:string}[],now:number,replaceParent?:string):void {
  ...
  if(replaceParent
    ?db.query("SELECT 1 FROM save_jobs WHERE phase IN ('summarizing','recovery-required') AND saveId<>? LIMIT 1").get(replaceParent)
    :db.query("SELECT 1 FROM save_jobs WHERE phase IN ('summarizing','recovery-required') LIMIT 1").get())throw new MemorySaveError('EXECUTION_BUSY');
...
/** Internal to the successor transaction only. The original attempt remains in
 * the rolling budget; its active fence is replaced by the new job before commit. */
export function checkAutomaticMemoryRecoveryAdmission(db:Database,parent:MemorySaveJob,expectedRevision:number,intent:MemorySaveAttemptIntent,sources:readonly {sequence:number;payload:string}[],now:number):void {
  if(parent.phase!=='recovery-required'||!saveToken(parent.saveId)||!saveToken(parent.attemptId))throw new MemorySaveError('RECOVERY_REQUIRED');
  checkAdmission(db,parent,expectedRevision,intent,sources,now,parent.saveId);
}
export function reserveAutomaticMemoryRecoveryAdmission(db:Database,parent:MemorySaveJob,successorSaveId:string,expectedRevision:number,intent:MemorySaveAttemptIntent,attemptId:string,sources:readonly {sequence:number;payload:string}[],now:number):void {
  if(!saveToken(successorSaveId)||!saveToken(attemptId)||successorSaveId===parent.saveId||attemptId===parent.attemptId)throw new MemorySaveError('INVALID_INPUT');
  checkAutomaticMemoryRecoveryAdmission(db,parent,expectedRevision,intent,sources,now);
  const p=read(db);
  db.query('INSERT INTO save_auto_attempts VALUES (?,?,?,?,?)').run(successorSaveId,attemptId,parent.memoryId,p.revision,now);
