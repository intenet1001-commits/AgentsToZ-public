      ...
      <main className="portal-login-card">
        ...
        {error && (
          <div className="portal-login-recovery">
            ...
            <button type="button" onClick={resetStoredLogin}>이 기기 로그인 초기화</button>
          </div>
        )}
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800 sticky top-0 bg-zinc-900">
          <span className="text-sm font-semibold text-[var(--text-primary)]">{mode === 'add' ? '프로젝트 추가' : '프로젝트 수정'}</span>
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>(
    () => readInitialPortalTab() ?? (readInitialSelectedDeviceId() ? 'ports' : 'bookmarks')
  );
    ...
    <div className="portal-shell" style={webThemeVars} onClick={() => { setShowDevicePicker(false); setShowMoreMenu(false); }}>
      ...
      <div className="portal-workspace">
        ...
        <main className={`portal-main ${isFullLayout ? 'portal-main--full' : ''}`}>
          ...
          {activeTab === 'memories' && (
            creds ? (
              <PortalMemoryDirectory supabaseUrl={creds.url} supabaseKey={creds.key} showToast={showToast} requiresUserSession={true} supabaseClientFactory={getAuthenticatedSupabaseClient} />
            ) : (
              <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
