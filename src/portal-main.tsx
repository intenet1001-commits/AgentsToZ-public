...
import {
  ...
  Plus, Check, Brain, Copy, Star, Cloud, ScanLine,
} from 'lucide-react';
import { type SupabaseClient } from '@supabase/supabase-js';
import { getAuthenticatedSupabaseClient, describeAuthenticatedPortalSupabaseError } from './lib/supabaseClient';
import { isDeployedWeb, isLocalWeb } from './lib/env';
...
function getDefaultSupabaseClient(url: string, key: string): SupabaseClient {
  return getAuthenticatedSupabaseClient(url, key);
}
...
function readInitialPortalTab(): Tab | null {
  const value = new URLSearchParams(window.location.search).get('tab');
  return value === 'ports' || value === 'bookmarks' || value === 'memories' ? value : null;
}
...
;(function applyUrlParams() {
  ...
  if (!url || !key) return;
  // Credential handoff is a localhost-only onboarding convenience. A hosted
  // link must never be able to replace the OAuth/Supabase authority chosen at
  // build time, even when that deployment is missing its environment values.
  if (!isLocalWeb()) {
...
interface PortRow {
  id: string;
  name: string;
  /** PostgreSQL bigint를 JS number로 반올림하지 않는 canonical decimal text. */
...
function canonicalPortRow(value: unknown): PortRow {
  if (!value || typeof value !== 'object') throw new Error('PORT_ROW_INVALID');
  const row = value as Record<string, unknown>;
  if (typeof row.id !== 'string' || !row.id || typeof row.name !== 'string') {
    throw new Error('PORT_ROW_INVALID');
  }
  return {
    ...row,
    sync_generation: portFenceGeneration(row.sync_generation ?? 0),
  } as unknown as PortRow;
}

class PortalPortSchemaError extends Error {
  constructor(readonly schemaError: unknown) {
    super('PORT_DURABLE_FENCE_SCHEMA_REQUIRED');
    this.name = 'PortalPortSchemaError';
  }
}

function describePortalPortMutationError(error: unknown): string {
  if (error instanceof PortalPortSchemaError) {
    return '안전 동기화 DB 컬럼이 준비되지 않았습니다. AgentsToZ의 최신 Supabase 마이그레이션을 적용한 뒤 다시 시도하세요.';
  }
  if (error instanceof PortDurableFenceError) {
    if (error.failure === 'unavailable') {
      return '안전 동기화 DB 업데이트가 필요합니다. AgentsToZ의 최신 Supabase 마이그레이션을 적용한 뒤 다시 시도하세요.';
    }
    if (error.mutationMayHaveCommitted) {
      return '네트워크 응답이 끊겨 저장 결과를 확정할 수 없습니다. 먼저 새로고침해 현재 상태를 확인하세요.';
    }
    return '다른 기기에서 프로젝트 상태가 바뀌었거나 안전 조건이 맞지 않아 저장을 멈췄습니다. 새로고침 후 다시 시도하세요.';
  }
  return describeAuthenticatedPortalSupabaseError(error);
}
...
function getSupabaseCreds(): { url: string; key: string } | null {
  ...
  // Hosted portals fail closed when their build-time pair is incomplete.
  // localStorage is mutable and may contain a legacy or link-injected project.
  if (isDeployedWeb()) return null;
  try {
...
function GoogleAuthGate({ onVerified }: { onVerified: () => void }) {
  ...
  const [recovery, setRecovery] = useState<{ kind: 'session' | 'oauth'; error: unknown } | null>(null);
  ...
  useEffect(() => {
    ...
    const failSessionCheck = (sessionError: unknown) => {
      if (cancelled) return;
      stalled = true;
      setChecking(false);
      setLoading(false);
      setRecovery({ kind: 'session', error: sessionError });
      setError(
        `저장된 Google 로그인 상태 확인이 끝나지 않았습니다. 다시 확인하거나 이 기기의 로그인만 초기화해 주세요. (${portalAuthErrorMessage(sessionError)})`,
      );
    };

    const verifySession = async (email: string, expectedRevision: number) => {
      const membership = await verifyPortalMembership(sb as unknown as PortalMembershipRpcClient);
      ...
      setError(portalMembershipFailureMessage(membership, email));
      setRecovery(membership.state === 'error' ? { kind: 'session', error: membership } : null);
    };

    const observeSession = (email: string) => {
      if (stalled) return;
      ...
      setChecking(true);
      // Supabase warns against awaiting another client request inside its auth
      // callback. Run the DB authority check after that callback releases.
      window.setTimeout(() => void verifySession(email, expectedRevision), 0);
    };
    ...
    const { data: { subscription } } = sb.auth.onAuthStateChange((_event, session) => {
      if (!cancelled && !stalled) observeSession(session?.user?.email ?? '');
    });

    return () => { cancelled = true; revision += 1; subscription.unsubscribe(); };
  }, [retryNonce]);

  async function handleLogin(continueAfterConnectionFailure = false) {
    if (loginInFlight.current || checking) return;
    if (!sbUrl || !sbKey) { setError('Supabase 설정이 없습니다'); return; }
    const continueDirectly = continueAfterConnectionFailure && canContinuePortalGoogleOAuth(recovery?.error);
    ...
    try {
      ...
      setRecovery({ kind: 'oauth', error: loginError });
      setError(portalOAuthStartErrorMessage(loginError));
    } finally {
  ...
  return (
    <div className="portal-login-shell">
      ...
      <main className="portal-login-card">
        ...
        {error && recovery && (
          <div className="portal-login-recovery">
            <button type="button" disabled={loading || checking} onClick={() => recovery.kind === 'oauth'
              ? void handleLogin() : setRetryNonce(value => value + 1)}>
              <RefreshCw className="h-3.5 w-3.5" />{recovery.kind === 'oauth' ? '로그인 다시 시도' : '로그인 상태 다시 확인'}
            </button>
            {canContinuePortalGoogleOAuth(recovery.error) && <button type="button" disabled={loading || checking}
              onClick={() => void handleLogin(true)}>Google 로그인 페이지로 이동</button>}
            {recovery.kind === 'session' && <button type="button" disabled={loading || checking}
              onClick={resetStoredLogin}>이 기기 로그인 초기화</button>}
          </div>
        )}
...
function PortsView({ deviceId, deviceIds, creds, showToast, onSwitchDevice, onRemoteWork }: {
  deviceId: string;
  deviceIds: string[];
  creds: { url: string; key: string };
  showToast: (msg: string, type: 'success' | 'error') => void;
  onSwitchDevice?: () => void;
  onRemoteWork?: (project?: {name: string; deviceId: string}) => void;
}) {
  ...
  const sb = useCallback(() => getAuthenticatedSupabaseClient(creds.url, creds.key), [creds.url, creds.key]);

  // PostgREST는 jsonb_populate_record에 주어진 알 수 없는 키를 무시할 수 있다.
  // 그러므로 쓰기 직전에 최신 포털이 다루는 모든 컬럼을 명시적으로 요구해,
  // 구버전 DB가 카테고리·설명·복수 GitHub 주소를 조용히 버리지 못하게 한다.
  async function requireDurablePortMutationSchema() {
    const { error } = await sb().from('portmgr_ports')
      .select('id,name,device_id,sync_generation,port,deploy_url,github_url,github_urls,category,description')
      .limit(0);
    if (error) throw new PortalPortSchemaError(error);
  }
  ...
  const portsLoadRevision=useRef(0);
  async function loadPorts() {
    if (!deviceId) return;
    const revision=++portsLoadRevision.current;
    setLoading(true);
    try {
      const { data, error } = await sb().from('portmgr_ports').select('*').in('device_id', deviceIds).order('name');
      if (revision!==portsLoadRevision.current) return;
      if (error) throw error;
      setPorts((data ?? []).map(canonicalPortRow));
    } catch (e: unknown) {
      if (revision!==portsLoadRevision.current) return;
      // 기기 전환 직후 조회가 실패했을 때 이전 기기의 행을 현재 기기 행으로
      // 오해해 수정·삭제하지 못하게 실패 시 목록을 fail-closed로 비운다.
      setPorts([]);
      const msg = (e as any)?.message ?? JSON.stringify(e);
      showToast('포트 로드 실패: ' + msg, 'error');
    } finally {
      if (revision===portsLoadRevision.current) setLoading(false);
    }
  }

  useEffect(() => {
    loadPorts();
    return ()=>{portsLoadRevision.current++};
  ...
  async function saveInlineUrl(id: string, field: 'deploy_url' | 'github_url', value: string) {
    ...
    if (!current) {
      showToast('저장 대상이 새로고침된 목록과 다릅니다. 다시 시도하세요.', 'error');
      await loadPorts();
      return;
    }
    ...
    const body = field === 'github_url'
      ...
      : { deploy_url: trimmed };
    try {
      await requireDurablePortMutationSchema();
      const [result] = await upsertPortsWithDurableFence(sb(), [{
        ...body,
        id: current.id,
        device_id: current.device_id ?? null,
        name: current.name,
        sync_generation: current.sync_generation,
      }], crypto.randomUUID());
      setPorts(prev => prev.map(port => port.id === id
        ? { ...port, ...body, sync_generation: result!.generation }
        : port));
      showToast(`${field === 'deploy_url' ? '배포 주소' : 'GitHub 저장소 주소'} 저장됨 ✓`, 'success');
    } catch (error) {
      showToast('저장 실패: ' + describePortalPortMutationError(error), 'error');
      await loadPorts();
    }
  }
  ...
  async function createProject() {
    ...
    try {
      ...
      await requireDurablePortMutationSchema();
      await upsertPortsWithDurableFence(sb(), [{
        ...payload,
        id: newId,
        device_id: deviceId,
        sync_generation: '0',
      ...
      showToast('추가 실패: ' + describePortalPortMutationError(error), 'error');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function updateProject() {
    ...
    if (!payload.name) { showToast('이름은 필수입니다', 'error'); return; }
    const current = ports.find(port => port.id === showEdit.id);
    if (!current) {
      showToast('수정 대상이 새로고침된 목록과 다릅니다. 다시 시도하세요.', 'error');
      setShowEdit(null);
      await loadPorts();
      return;
    }
    ...
    try {
      ...
      showToast('수정 실패: ' + describePortalPortMutationError(error), 'error');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function deleteProject(id: string) {
    const current = ports.find(port => port.id === id);
    if (!current) {
      showToast('삭제 대상이 새로고침된 목록과 다릅니다. 다시 시도하세요.', 'error');
    ...
    try {
      await requireDurablePortMutationSchema();
      await deletePortsWithDurableFence(sb(), [{
        id: current.id,
        device_id: current.device_id ?? null,
        name: current.name,
        sync_generation: current.sync_generation,
      ...
      showToast('삭제 실패: ' + describePortalPortMutationError(error), 'error');
    } finally {
      setDeletingId(null);
    }
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <p className="text-xs text-zinc-500">{ports.length}개 프로젝트</p>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          placeholder="프로젝트 검색..."
          className="w-full pl-8 pr-3 py-1.5 bg-zinc-900/60 border border-zinc-800/60 rounded-lg text-xs text-zinc-300 placeholder:text-[var(--text-dim)] focus:outline-none focus:border-zinc-600 transition-colors"
        />
      </div>
      ...
      {!loading && ports.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
          <Server className="w-10 h-10 text-zinc-700" />
          <p className="text-sm text-zinc-500">동기화된 프로젝트가 없습니다</p>
          <p className="text-xs text-zinc-600">로컬 앱에서 Push하거나 위 버튼으로 목록에 등록하세요</p>
          ...
          {ports.filter(p => {
            ...
            <div key={p.id} className="flex flex-wrap items-center gap-2 sm:gap-3 py-3 px-1 hover:bg-zinc-800/30 rounded-lg transition-colors group">
              ...
              <div className="flex items-center gap-3 sm:gap-1.5 flex-wrap">
                ...
                {memoryPill(p)}
                {onRemoteWork&&<button type="button" className="portal-mini-button" onClick={()=>onRemoteWork({name:p.name,deviceId})}>이 프로젝트에서 작업</button>}
                <button
                  onClick={() => openEdit(p)}
                  title="수정"
                ...
                {pendingDeleteId === p.id ? (
                  ...
                  >
                    <Trash2 className="w-2.5 h-2.5" />목록에서 제거
                  </button>
                )}
              </div>
            </div>
...
function RemoteProjectsView({ device, creds, showToast, onManageDevice }: {
  ...
  const loadProjects = useCallback(async () => {
    ...
    try {
      ...
      setProjects([]);
      showToast(`AWS 프로젝트를 불러오지 못했습니다: ${describeAuthenticatedPortalSupabaseError(error)}`, 'error');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div data-testid="portal-remote-projects-view" className="min-w-0 overflow-hidden">
      ...
      <div className="relative mb-4">
        ...
        <input value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder="AWS 프로젝트 검색..." className="min-h-11 w-full rounded-lg border border-zinc-800/60 bg-zinc-900/60 pl-8 pr-3 py-2 text-xs text-zinc-300 outline-none placeholder:text-[var(--text-dim)] focus:border-zinc-600" />
      </div>
      {loading ? (
...
function PortFormModal({ mode, draft, setDraft, submitting, onSubmit, onClose }: {
  ...
  const inpCls = 'w-full px-3 py-2 bg-zinc-950 border border-zinc-700 rounded-md text-zinc-200 placeholder:text-[var(--text-dim)] focus:outline-none focus:border-blue-500';
  return (
    <div
      className="fixed inset-0 z-50 bg-[var(--dialog-scrim)] backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4"
      onClick={onClose}
    >
      <div
        className="bg-zinc-900 border border-zinc-700 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md max-h-[calc(var(--ui-viewport-height,100dvh)*0.88)] overflow-y-auto shadow-[var(--dialog-shadow)]"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800 sticky top-0 bg-zinc-900">
          <span className="text-sm font-semibold text-[var(--text-primary)]">{mode === 'add' ? '프로젝트 목록에 등록' : '프로젝트 정보 수정'}</span>
        ...
        <div className="px-5 py-4 border-t border-zinc-800 flex justify-end gap-2 sticky bottom-0 bg-zinc-900">
          ...
          <button
            ...
            className="px-5 py-2 text-sm font-medium text-[var(--text-on-accent)] bg-emerald-400 hover:bg-emerald-300 rounded-md transition-colors disabled:bg-emerald-700 disabled:text-zinc-500 disabled:cursor-not-allowed"
...
function DeviceManagerModal({ devices, creds, onClose, onUpdate, showToast }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--dialog-scrim)] backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-xl max-h-[calc(var(--ui-viewport-height,100dvh)*0.88)] overflow-y-auto bg-zinc-900 border border-zinc-700 rounded-2xl shadow-[var(--dialog-shadow)]" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
          <div>
            <span className="text-sm font-semibold text-[var(--text-primary)]">기기 관리 · 사용 중 {devices.length + remoteActiveCount}대</span>
              ...
              {editingId === d.id ? (
                <>
                  ...
                  <button onClick={() => saveEdit(d.id)} disabled={saving}
                    className="shrink-0 px-2.5 py-2 sm:px-2 sm:py-1 text-[11px] bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-[var(--text-on-solid)] rounded transition-colors">저장</button>
        ...
        <section className="border-t border-zinc-800 px-4 py-3" aria-label="Mac·Windows 연결">
          ...
          {showDesktopPairing && (
            <div className="mt-3 space-y-2 rounded-lg border border-blue-900/50 bg-blue-950/10 p-3">
              ...
              <button onClick={() => void copyDesktopPairing()} disabled={pairingBusy || !desktopDeviceName.trim()} className="min-h-11 w-full rounded-lg bg-blue-500 px-3 py-2 text-xs font-semibold text-[var(--text-on-solid)] hover:bg-blue-400 disabled:cursor-not-allowed disabled:opacity-40">
        ...
        <RemoteDeviceManager
          ...
          supabaseClientFactory={getAuthenticatedSupabaseClient}
          showToast={showToast}
...
export function PortalCatalog({ tab, visible = true, onRemoteWork }: { tab: Tab; visible?: boolean; onRemoteWork?: (project?: {name: string; deviceId: string}) => void }) {
  useLayoutEffect(() => { setActiveTab(tab); }, [tab]);
  // Never trust a localStorage marker for authorization. The login gate opens
  // only after the current Supabase session passes portmgr_is_member().
  const [pwOk, setPwOk] = useState(false);
  const [memoryVisited,setMemoryVisited]=useState(tab==='memories'&&visible);
  const [portsVisited,setPortsVisited]=useState(tab==='ports'&&visible);
  useEffect(()=>{if(visible&&tab==='memories')setMemoryVisited(true);if(visible&&tab==='ports')setPortsVisited(true)},[tab,visible]);
  ...
  // `?scan=remote` opens the scanner directly. The remote-control screen tells
  // the user to scan a new QR to add another Mac, and links here rather than
  // duplicating the camera stack on that page.
  const [showRemoteQrScanner, setShowRemoteQrScanner] = useState(
    () => new URLSearchParams(window.location.search).get('scan') === 'remote',
  );
  ...
  useEffect(() => {
    if (!pwOk || !creds) return;
    const sb = getDefaultSupabaseClient(creds.url, creds.key);
    let cancelled = false;
    let revision = 0;
    const enforce = async (email: string, expectedRevision: number) => {
      if (!email) { if (!cancelled) setPwOk(false); return; }
      const membership = await verifyPortalMembership(sb as unknown as PortalMembershipRpcClient);
      if (!cancelled && expectedRevision === revision && membership.state !== 'member') setPwOk(false);
    };
    const observe = (email: string) => {
      const expectedRevision = ++revision;
      if (!email) { setPwOk(false); return; }
      window.setTimeout(() => void enforce(email, expectedRevision), 0);
    };
    void sb.auth.getSession().then(({ data, error }) => {
      if (cancelled) return;
      if (error) { setPwOk(false); return; }
      observe(data.session?.user?.email ?? '');
    });
    const { data: { subscription } } = sb.auth.onAuthStateChange((_event, session) => {
      if (!cancelled) observe(session?.user?.email ?? '');
    });
    return () => { cancelled = true; revision += 1; subscription.unsubscribe(); };
  }, [pwOk, creds?.url, creds?.key]);
  ...
  const isFullLayout = false && windowWidth >= 768
  ...
  async function loadDevices() {
    if (!creds) return;
    try {
      const sb = getAuthenticatedSupabaseClient(creds.url, creds.key);
      ...
      if (devError) {
        setDevices([]);
        showToast(`기기 목록을 불러오지 못했습니다: ${describeAuthenticatedPortalSupabaseError(devError)}`, 'error');
        return;
      }
      ...
      if (remoteDeviceResult.error || remoteProjectResult.error) {
        showToast(`AWS 단말 목록을 불러오지 못했습니다: ${describeAuthenticatedPortalSupabaseError(remoteDeviceResult.error || remoteProjectResult.error)}`, 'error');
      }
  ...
  function selectDevice(id: string, name?: string, autoSwitch = true) {
    ...
    // Device selection changes the catalog filter; the workspace owns navigation.
  }
  ...
  async function openPortsHistory() {
    ...
    try {
      ...
      showToast(`히스토리를 읽지 못했습니다: ${describeAuthenticatedPortalSupabaseError(e)}`, 'error');
    } finally {
  ...
  const webThemeVars = {
    '--pm-bg': 'var(--bg)',
    '--pm-bg-input': 'var(--bg-input)',
    '--pm-bg-hover': 'var(--bg-hover)',
    '--pm-border': 'rgb(var(--surface-highlight-rgb) / 0.075)',
    '--pm-border-hover': 'rgb(var(--surface-highlight-rgb) / 0.14)',
    '--pm-border-mid': 'rgb(var(--surface-highlight-rgb) / 0.10)',
    '--pm-border-faint': 'rgb(var(--surface-highlight-rgb) / 0.045)',
    '--pm-border-strong': 'rgb(var(--surface-highlight-rgb) / 0.20)',
    '--pm-text': 'var(--text-primary)',
    '--pm-text-muted': 'var(--text-secondary)',
    '--pm-text-faint': 'var(--text-dim)',
  } as React.CSSProperties;

  return (
    <div className="portal-shell portal-catalog" style={webThemeVars} onClick={() => { setShowDevicePicker(false); setShowMoreMenu(false); }}>
      {/* ── Header ── */}
      <header className="portal-app-header" hidden>
        <div className="portal-app-header__inner">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              type="button"
              data-testid="open-portal-remote-qr-scanner"
              aria-label="원격제어 QR 스캔"
              onClick={() => setShowRemoteQrScanner(true)}
              className={`${btnCls} portal-toolbar-button--remote-qr`}
              title="원격제어 QR 스캔"
            >
            ...
            <a
              data-testid="open-active-remote-control"
              href="/remote/"
              className={`${btnCls} portal-toolbar-button--remote-control`}
              title="기존 원격제어 세션 열기"
            >
              <Smartphone className="h-4 w-4" aria-hidden="true" />
              <span className="hidden md:inline">원격제어</span>
            </a>
              ...
              {showMoreMenu && (
                <div className="portal-header-menu" onClick={() => setShowMoreMenu(false)}>
                  ...
                  <button onClick={() => {
                    ...
                    // portal_google_verified is removed only as legacy cleanup;
                    // it is never read as current authorization evidence.
      ...
      <PortalRemoteQrScanner
        open={showRemoteQrScanner}
        onClose={() => setShowRemoteQrScanner(false)}
      />

      <div className="portal-catalog-toolbar">
        {activeTab==='ports'&&<div><span>조회 기기</span>{devicePickerEl}</div>}
        <button type="button" onClick={() => setOpenSettings(true)}>{activeTab==='bookmarks'?'북마크 설정':'계정 자료 설정'}</button>
        {activeTab==='ports'&&<button type="button" onClick={()=>setShowDeviceManager(true)}>기기 관리</button>}
      </div>
      {activeTab==='ports'&&<section className="workspace-inventory-note"><p>{selectedDevice ? `${selectedDevice.name} · ${formatDeviceLastPushAt(selectedDevice.last_push_at)}` : '조회할 기기를 선택하세요.'}</p><p>동기화된 프로젝트 현황입니다. 원격 연결 없이 조회할 수 있으며, 이후 Mac·PC에서 변경한 내용은 아직 반영되지 않았을 수 있습니다.</p>{onRemoteWork&&<button type="button" onClick={()=>onRemoteWork()}>원격 작업으로 이동</button>}</section>}
      {/* ── Body ── */}
      <div className="portal-workspace">
        ...
        <main className={`portal-main ${isFullLayout ? 'portal-main--full' : ''}`}>
          <PortalManager showToast={showToast} openSettings={openSettings}
            ...
            supabaseClientFactory={getAuthenticatedSupabaseClient}
            describeSupabaseFailure={describeAuthenticatedPortalSupabaseError}
            onSettingsClosed={() => setOpenSettings(false)} actionsRef={actionsRef}
            isVisible={visible && activeTab === 'bookmarks'}
            onSyncStateChange={setBookmarkSyncState}
            />
          {memoryVisited && <div hidden={activeTab!=='memories'}>{
            creds ? (
              <PortalMemoryDirectory supabaseUrl={creds.url} supabaseKey={creds.key} showToast={showToast} requiresUserSession={true} supabaseClientFactory={getAuthenticatedSupabaseClient} visible={visible&&activeTab==='memories'} />
            ) : (
              <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
          ...
          {portsVisited && <div hidden={activeTab!=='ports'}>{
            creds && selectedDeviceId ? (
              selectedDevice?.kind === 'remote' ? (
                ...
                <PortsView onRemoteWork={onRemoteWork} deviceId={selectedDeviceId} deviceIds={selectedDevice?.sourceIds ?? [selectedDeviceId]} creds={creds} showToast={showToast} onSwitchDevice={() => setShowDevicePicker(true)} />
              )
            ) : (
              <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
      ...
      {showPortsHistory && (
        <div className="fixed inset-0 z-50 bg-[var(--dialog-scrim)] backdrop-blur-sm flex items-center justify-center p-4"
          onClick={() => setShowPortsHistory(false)}>
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-md shadow-[var(--dialog-shadow)]"
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
              <div className="flex items-center gap-2.5">
                <Clock className="w-4 h-4 text-zinc-400" />
                <span className="text-sm font-semibold text-[var(--text-primary)]">프로젝트 Push 히스토리</span>
              </div>
