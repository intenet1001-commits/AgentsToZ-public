 ...
 */
export function withPortalFileLockCoordinator<T>(lockPath: string, operation: () => T): T {
...
export interface FileLockOwnerDescription {
  pid: number;
  recoveryClass: 'legacy' | 'guarded' | 'manual';
  alive: boolean;
}

export function describeFileLockOwner(owner: string): FileLockOwnerDescription | null {
  const identity = parsePortalFileLockOwner(owner);
  if (!identity) return null;
  return {
    ...identity,
    alive: identity.pid === process.pid || portalFileLockOwnerProcessAlive(owner),
  };
}

export type DeadManualFileLockRecovery =
  | 'recovered'
  | 'missing'
  | 'owner-changed'
  | 'owner-alive'
...
/**
 * The only deletion path for a dead `manual` owner, and it never runs on its
 * own: a person must first confirm that no detached AI/Git descendant of that
 * owner is still writing. The owner check and unlink share one coordinator
 * transaction, so a new owner that appeared after the listing is never removed.
 */
export function recoverDeadManualFileLock(
  lockPath: string,
  expectedPid: number,
): DeadManualFileLockRecovery {
  ...
  return withPortalFileLockCoordinator(lockPath, () => {
    ...
    if (!identity || identity.pid !== expectedPid) return 'owner-changed';
    if (identity.recoveryClass !== 'manual') return 'not-manual';
    if (identity.alive) return 'owner-alive';
    return releasePortalFileLockIfOwnedInCoordinator(lockPath, owner) ? 'recovered' : 'missing';
  });
}
