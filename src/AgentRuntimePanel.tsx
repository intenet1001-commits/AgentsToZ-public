...
export function AgentRuntimePanel({
  ...
  useEffect(() => {
    if (!entryRequest || appliedEntryNonceRef.current === entryRequest.nonce) return;
    if(entryRequest.surface==='first-task'){
      appliedEntryNonceRef.current=entryRequest.nonce;
      setSurfaceMode('conversations');
      // Only open the composer; never replace a draft or submit a test automatically.
      return;
    }
    setSurfaceMode('conversations');
  ...
  return (
    ...
    >
      ...
      >
        {entryRequest?.surface==='first-task'&&<div role="status" className="m-4 rounded-xl border border-[var(--accent-line)] bg-[var(--accent-soft)] p-4 text-sm leading-relaxed">
          <strong>Codex 첫 응답 확인</strong>
          <p className="mt-2">프로젝트와 모델을 확인한 뒤 작은 질문을 직접 보내세요. 이 대화는 읽기 전용이며 파일을 수정하지 않습니다. 예: “이 프로젝트에서 먼저 확인할 내용을 알려줘.”</p>
          <p className="mt-2">응답이 끝나면 아래 대화에서 결과를 확인하세요. 로그인 정보 확인이나 대화 열기는 성공 판정이 아닙니다. 이후 파일을 수정하는 개발 작업은 워크룸에서 시작할 수 있습니다.</p>
        </div>}
