...
export async function executeAutomaticMemorySave(c:MemorySaveExecution){
 ...
 let reservedSaveId:string|undefined,stage:MemorySaveFailure['stage']='input-binding',providerCallPossible=false;
 try{
  ...
  const materialized=await materializeMemorySaveInput({sources:c.sources,expectedCoverageDigest:coverageDigest,validateRegistrationAndLease:valid,projection:'conversation-v1'});
  ...
  return await commitAutomaticMemoryProposal(c,prepared,raw,attempted,valid,value=>{stage=value;});
 }catch(error){
  ...
  if(reservedSaveId){try{c.saves.recordFailure(reservedSaveId,attempted?.attemptId??null,{version:1,stage,code:memorySaveFailureCode(error),providerCallPossible});}catch{/* Missing diagnostics mean unknown; never replace the original failure or replay. */}}
  throw error;
 }finally{key?.fill(0);prompt?.fill(0);evidence?.fill(0);active.delete(c.stagingLease);}
}

/** Shared validation and host-only commit; the caller must already own the
 * unique admitted attempt. This helper creates no attempt or retry authority. */
export async function commitAutomaticMemoryProposal(c:MemorySaveExecution,prepared:ReturnType<typeof prepareProjectMemoryUpdate>,raw:string,
 attempted:{saveId:string;attemptId:string},valid:()=>Promise<boolean>,onStage:(value:MemorySaveFailure['stage'])=>void){
 const memoryId=c.sources[0]!.source.memoryId;
  onStage('proposal-validation');
  ...
  onStage('host-plan');
  const owner=c.saves.get(attempted.saveId);
  if(owner.attemptId!==attempted.attemptId||!['summarizing','recovery-required'].includes(owner.phase))throw new MemorySaveError('REVISION_CONFLICT');
