...
export type ClaudeRemoteConversationErrorCode =
  ...
  | 'CLAUDE_REMOTE_SESSION_PERMISSION_MODE_MISMATCH';
...
/** Mirrors the desktop launch-options bypass toggle; no other Claude mode is exposed. */
export type ClaudeRemotePermissionMode = 'default' | 'bypassPermissions';

function permissionModeLabel(mode: ClaudeRemotePermissionMode): string {
  return mode === 'bypassPermissions' ? 'ON' : 'OFF';
}

/**
 * One live session per project is the ownership rule, so a session running in
 * another mode cannot simply be reused: that would quietly hand an approval-free
 * conversation to someone who turned bypass off, or the reverse.
 */
function permissionModeMismatch(
  running: ClaudeRemotePermissionMode,
  requested: ClaudeRemotePermissionMode,
): ClaudeRemoteConversationError {
  return new ClaudeRemoteConversationError(
    'CLAUDE_REMOTE_SESSION_PERMISSION_MODE_MISMATCH',
    `이 프로젝트에는 권한 우회 ${permissionModeLabel(running)} 상태로 시작한 Claude 대화가 이미 실행 중입니다. `
      + `그 대화를 이어가려면 실행 옵션의 권한 우회를 ${permissionModeLabel(running)}로 맞추고, `
      + `권한 우회 ${permissionModeLabel(requested)}로 새로 시작하려면 AgentsToZ를 다시 시작해 그 대화를 종료한 뒤 다시 누르세요.`,
  );
}
...
interface ActiveClaudeRemoteConversation {
  sessionId: string;
  sessionUrl: string;
  permissionMode: ClaudeRemotePermissionMode;
...
export class ClaudeRemoteConversationManager {
  ...
  readonly #inflightPermissionModeByProject = new Map<string, ClaudeRemotePermissionMode>();
  ...
  startAndOpen(input: {
    ...
    /** Omitted by callers that do not own the choice (the phone): reuse whatever is live. */
    permissionMode?: ClaudeRemotePermissionMode;
    ...
    if (active && !active.exited) {
      if (input.permissionMode && input.permissionMode !== active.permissionMode) {
        return Promise.reject(permissionModeMismatch(active.permissionMode, input.permissionMode));
      }
      return this.#openExisting(active, input.openSession);
    }
    const inflight = this.#inflightByProject.get(input.projectKey);
    if (inflight) {
      const inflightMode = this.#inflightPermissionModeByProject.get(input.projectKey) ?? 'default';
      if (input.permissionMode && input.permissionMode !== inflightMode) {
        return Promise.reject(permissionModeMismatch(inflightMode, input.permissionMode));
      }
      return inflight;
    }
    ...
    const task = this.#startAndOpen({ ...input, permissionMode }).finally(() => {
      if (this.#inflightByProject.get(input.projectKey) === task) {
        this.#inflightByProject.delete(input.projectKey);
        this.#inflightPermissionModeByProject.delete(input.projectKey);
      }
    });
    this.#inflightByProject.set(input.projectKey, task);
    this.#inflightPermissionModeByProject.set(input.projectKey, permissionMode);
    return task;
  }
