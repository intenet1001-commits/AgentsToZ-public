...
import {CompletedMemoryTurnReader,MEMORY_OBSERVATION_MAX_RECORD_BYTES,type MemoryObservationBinding} from './memorySaveObservation';
import {MEMORY_INPUT_LIMIT} from './memorySaveInputStore';
import {MemoryConversationProjector,MemoryProjectionError,MEMORY_CONVERSATION_INPUT_LIMIT,MEMORY_CONVERSATION_PROJECTION} from './memorySaveConversationProjection';
...
export class MemoryMaterializationError extends Error {
 constructor(readonly code:'INVALID_INPUT'|'SOURCE_CHANGED'|'REGISTRATION_CHANGED'|'SOURCE_OVERSIZED'|'INPUT_OVERSIZED'|'UNSUPPORTED_CONTENT'|'UNAVAILABLE'){super(`Memory materialization: ${code}`);}
}
 ...
 * Raw mode preserves whole JSONL; conversation-v1 retains every canonical text
 * block and explicitly describes omitted runtime metadata. This is not a prompt,
...
export async function materializeMemorySaveInput(input:{
 ...
 /** Opt-in versioned text evidence; the default preserves whole raw JSONL. */
 projection?:typeof MEMORY_CONVERSATION_PROJECTION;
 ...
 try{
  ...
  if(input.projection!==undefined&&input.projection!==MEMORY_CONVERSATION_PROJECTION)fail('INVALID_INPUT');
  const projected=input.projection===MEMORY_CONVERSATION_PROJECTION;
  if(projected&&input.sources.length>8)fail('INVALID_INPUT');
  const inputLimit=projected?MEMORY_CONVERSATION_INPUT_LIMIT:MEMORY_INPUT_LIMIT;
  ...
  if(items.reduce((n,i)=>n+i.source.endByte-i.source.startByte,0)>(projected?8*1024*1024:MEMORY_INPUT_LIMIT))fail(projected?'SOURCE_OVERSIZED':'INPUT_OVERSIZED');
  if(!await input.validateRegistrationAndLease())fail('REGISTRATION_CHANGED');
  const stamps:Stamp[]=[],turns:Record<string,unknown>[]=[];
  const envelope=()=>projected?{version:2,projection:MEMORY_CONVERSATION_PROJECTION,coverage:'exact-completed-raw-range',content:'canonical-conversation-text-only',turns}:{version:1,turns};
  for(const item of items){
   ...
   try{
    ...
    try{
     let start=0;while(start<range.length){const end=range.indexOf(10,start);if(end<0)fail('SOURCE_CHANGED');if(end+1-start>MEMORY_OBSERVATION_MAX_RECORD_BYTES)fail('SOURCE_OVERSIZED');reader.record(range.subarray(start,end+1),s.startByte+start);start=end+1;}
     ...
     const identity={sourceKey:memorySaveSourceKey(s),agent:s.agent,sessionId:s.sessionId,turnId:s.turnId};
     if(projected){
      const projector=new MemoryConversationProjector(s.agent as 'claude'|'codex');
      let offset=0;while(offset<range.length){const end=range.indexOf(10,offset);projector.record(JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(range.subarray(offset,end+1))));offset=end+1;}
      turns.push({...identity,sourceDigest:s.sourceDigest,...projector.finish()});
     }else {
      ...
      turns.push({...identity,jsonl});
     }
     if(Buffer.byteLength(JSON.stringify(envelope()))>inputLimit)fail('INPUT_OVERSIZED');
  ...
  output=Buffer.from(JSON.stringify(envelope()));
  if(output.length>inputLimit)fail('INPUT_OVERSIZED');
  return {plaintext:output,inputDigest:hash(output),coverageDigest,sourceCount:items.length,bytesRead};
 }catch(e){output?.fill(0);if(e instanceof MemoryMaterializationError)throw e;if(e instanceof MemoryProjectionError)throw new MemoryMaterializationError(e.code);throw new MemoryMaterializationError('UNAVAILABLE');}
}
