...
import OnboardingAssistantHandoff from './OnboardingAssistantHandoffPanel';
...
export default function OnboardingPreparation({onOpenFirstTask}:{onOpenFirstTask?:()=>void}) {
  ...
  return <section aria-label="이어서 준비하기" className="rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-5 text-[var(--ink)]">
    ...
    <OnboardingAssistantHandoff progress={progress} disabled={busy || !!error || !loaded || editing || !!progress?.operation} onRecheck={() => void check()} />
  </section>;
}
