...
import {saveHash,MEMORY_SAVE_SCHEMA_VERSION} from './memorySaveContract';
...
export class MemorySaveKeyLifecycle {
 ...
 async #virgin(){
  ...
  try{try{const st=lstatSync(path);if(!st.isFile()||st.nlink!==1)throw new Error();}catch(e:any){if(e?.code==='ENOENT')return;throw e;}
   ...
   if(version<1||version>MEMORY_SAVE_SCHEMA_VERSION)throw new Error();if(db.query('SELECT 1 FROM save_jobs LIMIT 1').get())throw new MemorySaveKeyLifecycleError('HISTORY_EXISTS');
