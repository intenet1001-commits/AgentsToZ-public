import {readCommonBookmarkRows} from './portalCommonBookmarks';
...
import { getSupabaseClient, describeSupabaseError, type SupabaseClientFactory } from './lib/supabaseClient';
...
export interface PortalData {
  ...
  /** 이 Mac에서만 숨긴 port IDs. Supabase에는 전송하지 않는다. */
  localOnlyDeletedPortIds?: string[];
  /** Supabase에서 명시적으로 삭제되어 다시 Pull되지 않아야 하는 port IDs. */
  remoteDeletedPortIds?: string[];
  /** live Git 증거로 한 번 확인한 pre-column generated worktree IDs. */
...
// 카테고리 팔레트. `dot` 한 값이 레일·칩·카드 타일·섹션 제목의 색 점을 모두 칠한다.
...
const PortalAPI = {
  ...
  async save(data: PortalData): Promise<PortalData> {
    return runPortalDataWriteExclusive(async () => {
      const authoritative = await PortalAPI.load();
      const safeData = preservePortalLocalMetadata(data, authoritative) as PortalData;
      // Deployed web: persist to localStorage only
      if (isDeployedWeb()) {
        localStorage.setItem(PORTAL_WEB_KEY, JSON.stringify(safeData));
        return safeData;
      }
      if (isTauri()) {
        await invoke('save_portal', { data: safeData });
        return safeData;
      }
      // Always persist to localStorage (Vercel / offline support)
      localStorage.setItem('portalData', JSON.stringify(safeData));
      if (safeData.supabaseUrl || safeData.supabaseAnonKey) {
        localStorage.setItem('portalCreds', JSON.stringify({
          supabaseUrl: safeData.supabaseUrl,
          supabaseAnonKey: safeData.supabaseAnonKey,
          deviceId: safeData.deviceId,
        }));
      }
      try {
  ...
  async openUrl(url: string): Promise<void> {
    await openPreferredBrowser(url);
  },
...
interface Props {
  ...
  /** Hosted portal shells pass the direct OAuth/JWT client explicitly. */
  supabaseClientFactory?: SupabaseClientFactory;
  describeSupabaseFailure?: (error: unknown) => string;
  /** True only for browser portal shells that own a Google JWT. */
...
function AdvancedSettings({ deviceId, deviceName, viewingDeviceId, knownDevices, isFetchingDevices, onFetchDevices, onSelectDevice, onResetDevice, onCopyDeviceId, handoffNote, onChangeHandoffNote, onSaveHandoffNote, isSavingHandoffNote }: AdvancedSettingsProps) {
  ...
  return (
    <div className="mb-3">
      <button
        onClick={() => setOpen(o => !o)}
        className="portal-disclosure"
      >
        <span className="flex items-center gap-1.5">
          <Settings2 className="w-3.5 h-3.5" style={{ color: 'var(--ink-3)' }} />
          <span>고급 설정</span>
          {viewingDeviceId && viewingDeviceId !== deviceId && (
            <span style={{ color: 'var(--warn)', fontSize: 10.5 }}>• 다른 기기 보는 중</span>
          )}
        </span>
        <ChevronDown className={`w-3.5 h-3.5 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div className="portal-disclosure__body space-y-3">
          {/* Device ID */}
          <div>
            <label className="portal-field-label">Device ID</label>
            <div className="flex items-center gap-1.5">
              <input readOnly value={deviceId ? `${deviceId.slice(0, 16)}…` : '—'}
                className="portal-field portal-mono flex-1 cursor-default" style={{ height: 28, minHeight: 28, color: 'var(--ink-2)' }} />
              <button onClick={onCopyDeviceId}
                className="portal-mini-button">복사</button>
            </div>
            <p className="portal-hint">설치마다 자동 생성되는 신원입니다. 다른 PC의 ID로 바꾸지 마세요. 같은 컴퓨터가 재설치 후 두 개로 보이면 장기기억의 ‘중복 단말 정리’에서 데이터 삭제 없이 한 단말로 묶습니다.</p>
          </div>
          {/* Handoff note — 이 기기 전용 (조회 중인 기기가 아닌, 항상 현재 기기의 메모) */}
          <div>
            <label className="portal-field-label">핸드오프 메모 (이 기기)</label>
            <textarea
              value={handoffNote}
              onChange={e => onChangeHandoffNote(e.target.value)}
              placeholder="예: X 작업 중, Y 확인 필요…"
              rows={2}
              className="portal-field portal-field--textarea"
            />
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="portal-field-label" style={{ margin: 0 }}>다른 기기 데이터 보기</label>
              ...
              <>
                ...
                {viewingDeviceId && viewingDeviceId !== deviceId && (
                  <div className="mt-1.5 space-y-1.5">
                    <p className="portal-hint" style={{ margin: 0, color: 'var(--warn)', fontWeight: 600 }}>⚠ Pull 시 선택한 기기 데이터 적용됨</p>
                    {viewingDevice?.handoff_note && (
                      <div className="portal-banner portal-banner--warn">
                        <p style={{ color: 'var(--ink)', whiteSpace: 'pre-wrap' }}>{viewingDevice.handoff_note}</p>
                        {viewingDevice.handoff_updated_at && (
                          <p style={{ color: 'var(--warn)', fontSize: 9.5 }}>{relativeTimeFromNow(viewingDevice.handoff_updated_at)}</p>
                        )}
                      </div>
                    )}
                    <div className="flex items-center gap-2">
                      <button onClick={onResetDevice} className="portal-link-button">내 기기로 복귀</button>
                    </div>
                  </div>
                )}
              </>
            ) : (
              <p className="portal-hint" style={{ margin: 0 }}>단말 조회 버튼으로 다른 기기 목록을 불러오세요</p>
            )}
          </div>
        </div>
      )}
...
- 포털은 로그인 후 portmgr_is_member()를 호출한다. 실제 접근은 portmgr_allowed_members에 등록된 계정만 허용한다.
  ...
  const steps = [
    {
      ...
      content: (
        <div className="space-y-2">
          <p className="portal-hint" style={{ margin: 0, color: 'var(--ink-2)', fontSize: 11.5 }}>Supabase CLI + MCP 방식으로 처음 설정하는 경우</p>
          <pre className="portal-pre">{CLI_FIRST_SETUP}</pre>
          <div className="flex gap-2">
            <button
              onClick={() => { void copyAgentsToZPrompt(CLI_FIRST_SETUP).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }); }}
              className="portal-mini-button portal-mini-button--grow"
            >
            ...
            <button
              ...
              className="portal-mini-button portal-mini-button--info portal-mini-button--grow"
            >
    ...
    },
    {
      ...
      content: (
        <div className="space-y-2">
          <p className="portal-hint" style={{ margin: 0, color: 'var(--ink-2)', fontSize: 11.5 }}>기존 Supabase 프로젝트에 새 맥/PC를 추가하는 경우</p>
          <pre className="portal-pre">{CLI_ADDITIONAL_DEVICE}</pre>
          <button
            onClick={() => { void copyAgentsToZPrompt(CLI_ADDITIONAL_DEVICE).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }); }}
            className="portal-mini-button portal-mini-button--block"
          >
            <Database className="w-3 h-3" />
            가이드 복사
          </button>
    ...
    },
    {
      ...
      content: (
        <div className="space-y-2">
          <p className="portal-hint" style={{ margin: 0, color: 'var(--ink-2)', fontSize: 11.5 }}>Claude Code + Supabase MCP로 테이블 자동 생성</p>
          <input
            type="email"
            value={allowedEmail}
          ...
          <ol className="portal-steps">
            ...
            <li>Supabase MCP 연결 확인 (<code className="portal-code">/mcp-setup</code>)</li>
            <li>아래 프롬프트 복사 → Claude Code에 붙여넣기</li>
            <li>AI가 {SCHEMA_TABLE_COUNT}개 테이블과 로그인 사용자용 RLS 정책을 함께 적용</li>
          </ol>
          <pre className="portal-pre portal-pre--short">{aiTablePrompt}</pre>
          <button
            disabled={!allowedEmailValid}
            onClick={() => { void copyAgentsToZPrompt(aiTablePrompt).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }); }}
            className={`portal-mini-button portal-mini-button--block ${copied ? 'portal-mini-button--ok' : ''}`}
          >
    ...
    },
    {
      ...
      content: (
        <div className="space-y-2">
          <p className="portal-hint" style={{ margin: 0, color: 'var(--ink-2)', fontSize: 11.5 }}>포털 북마크를 웹에 배포 — 앱 없이 브라우저에서 접근 가능</p>
          <pre className="portal-pre">{VERCEL_GUIDE}</pre>
          <button
            onClick={() => { void copyAgentsToZPrompt(VERCEL_GUIDE).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }); }}
            className={`portal-mini-button portal-mini-button--block ${copied ? 'portal-mini-button--ok' : ''}`}
          >
    ...
    <div className="mb-4">
      ...
      >
        <span className="flex items-center gap-1.5">
          ...
          <span style={{ color: 'var(--ink)' }}>초기 설정 가이드</span>
          <span style={{ color: 'var(--ink-3)', fontWeight: 500 }}>— 처음 사용 시</span>
        </span>
        <ChevronDown className={`w-3.5 h-3.5 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div className="portal-disclosure__body portal-disclosure__body--flush">
          {/* Step tabs */}
          <div className="portal-step-tabs">
            {steps.map((s, i) => (
              <button
                key={i}
                onClick={() => setStep(i)}
                className={`portal-step-tab ${step === i ? 'is-active' : ''}`}
              >
                {s.title}
              </button>
            ))}
          ...
          {/* Step content */}
          <div className="text-xs" style={{ padding: '0 12px 12px' }}>
            {steps[step]?.content}
            <div className="flex gap-2 mt-3">
              {step > 0 && (
                <button onClick={() => setStep(s => s - 1)} className="portal-mini-button portal-mini-button--grow">← 이전</button>
              )}
              {step < steps.length - 1 && (
                <button onClick={() => setStep(s => s + 1)} className="portal-mini-button portal-mini-button--info portal-mini-button--grow">다음 →</button>
              )}
            </div>
          </div>
        </div>
...
export default function PortalManager({ showToast, openSettings, onSettingsClosed, actionsRef, isVisible = true, onChangeDevice, onManageDevices, onOpenDeployUrl, selectedProjectDevice, onSyncStateChange, supabaseClientFactory = getSupabaseClient, describeSupabaseFailure = describeSupabaseError, requiresUserSession }: Props) {
  ...
  useEffect(() => {
    const syncLocalMetadata = (event: Event) => {
      const detail = (event as CustomEvent<PortalLocalMetadata>).detail ?? {};
      const next = preservePortalLocalMetadata(dataRef.current, detail) as PortalData;
      dataRef.current = next;
      setData(next);
    };
    window.addEventListener(PORTAL_LOCAL_METADATA_EVENT, syncLocalMetadata);
    return () => window.removeEventListener(PORTAL_LOCAL_METADATA_EVENT, syncLocalMetadata);
  }, []);
  ...
  const refreshSettingsIdentity = useCallback(async () => {
    ...
    if (!deployedWeb && resolvedUrl && resolvedKey) {
      try {
        const supabase = supabaseClientFactory(resolvedUrl, resolvedKey);
        if (requiresUserSession) await supabase.auth.getSession();
    ...
    (async () => {
      try {
        ...
        if (!loaded.deviceName && resolvedUrl && resolvedKey && loaded.deviceId) {
          try {
            const sb = supabaseClientFactory(resolvedUrl, resolvedKey);
  ...
  useEffect(() => {
    let active = true;
    if (!requiresUserSession || !showSettings || !sbUrl || !sbKey) {
      setAuthenticatedSessionEmail(null);
      return () => { active = false; };
    }
    try {
      const supabase = supabaseClientFactory(sbUrl, sbKey);
    ...
    return () => { active = false; };
  }, [requiresUserSession, showSettings, sbUrl, sbKey, supabaseClientFactory]);

  const loginNativeSupabase = async () => {
    if (!requiresUserSession) {
      showToast('로컬 앱은 sidecar 연결을 사용하므로 Google 로그인이 필요하지 않습니다.', 'success');
      return;
    }
    if (isDeployedWeb()) {
    ...
    try {
      ...
      if (error || isMember !== true) {
        throw new Error(
          `DB 허용 이메일 정책 거부${error ? `: ${describeSupabaseFailure(error)}` : ': 현재 계정이 server member가 아닙니다.'}`,
        );
      }
  ...
  const persist = useCallback(async (
    ...
    let localNext = preservePortalLocalMetadata(
      withPendingCloudDeletes(next, pendingCloudDeletes),
      previous,
    ) as PortalData;
    ...
    try {
      localNext = await PortalAPI.save(localNext);
      dataRef.current = localNext;
      setData(localNext);
    } catch (error) {
    ...
    try {
      const supabase = supabaseClientFactory(localNext.supabaseUrl!, localNext.supabaseAnonKey!);
      ...
      if (itemRows.length > 0) {
        ...
        if (error) throw new Error(describeSupabaseFailure(error));
      }
      if (catRows.length > 0) {
      ...
      if (pendingCloudDeletes) {
        ...
        try {
          const savedNext = await PortalAPI.save(syncedNext);
          dataRef.current = savedNext;
          setData(savedNext);
        } catch {
  ...
  async function openPortalHistory() {
    ...
    try {
      ...
      showToast(`히스토리를 읽지 못했습니다: ${describeSupabaseFailure(e)}`, 'error');
    } finally {
  ...
  async function syncSupabase() {
    ...
    try {
      ...
      if (itemsRes.error) throw new Error(describeSupabaseFailure(itemsRes.error));
      if (catsRes.error) throw new Error(describeSupabaseFailure(catsRes.error));
  ...
  async function pullFromSupabase(opts?: { skipConfirm?: boolean; targetDeviceId?: string }) {
    ...
    try {
      ...
      // Only desktop legacy browsing uses a selected device. The authenticated
      // web catalog reads all accessible bookmark rows without changing ownership.
      const targetDeviceId = isDeployedWeb() ? ownDeviceId : (opts?.targetDeviceId ?? viewingDeviceId ?? ownDeviceId);
      const [itemsRes, catsRes] = await Promise.all([
        isDeployedWeb()
          ? readCommonBookmarkRows<any>(async (from,to)=>{
              const page=await supabase.from('portmgr_portal_items').select('*').order('id').range(from,to);
              if(page.error)throw new Error(describeSupabaseFailure(page.error));
              return page.data??[];
  ...
  const portalUI = isVisible ? (
    ...
    <div className="portal-bookmarks__layout">
      ...
      <nav className="portal-category-tabs md:hidden" aria-label="북마크 카테고리">
        ...
        {sortedCategories.map(cat => {
          ...
          return (
            ...
            >
              <span className={`portal-category-dot ${c.dot}`} />
      ...
      <div className="portal-bookmark-content">
        ...
        <div className="portal-bookmark-toolbar">
          ...
          {viewingDeviceId && viewingDeviceId !== data.deviceId && (() => {
            ...
            return (
              <div className="portal-device-banner">
                <div className="portal-device-banner__row">
                  <span className="portal-device-banner__name">
                    📱 {viewedDevice?.device_name ?? viewingDeviceId.slice(0, 6) + '…'}
                  </span>
                  <button
                    onClick={() => { setViewingDeviceId(''); setData(d => ({ ...d, viewingDeviceId: undefined })); }}
                    className="portal-device-banner__close"
                    title="내 기기로 복귀"
                  >✕</button>
                </div>
                {viewedDevice?.handoff_note && (
                  <div className="portal-device-banner__note">
                    <p>{viewedDevice.handoff_note}</p>
                    {viewedDevice.handoff_updated_at && (
                      <p>{relativeTimeFromNow(viewedDevice.handoff_updated_at)}</p>
                    )}
                  </div>
                )}
              </div>
        ...
        {pinnedItems.length > 0 && (
          <section className="portal-bookmark-section" aria-labelledby="portal-category-pinned">
            <div className="portal-bookmark-section__heading">
              <Star className="portal-bookmark-section__star" />
              <h2 id="portal-category-pinned">고정됨</h2>
              <span className="portal-bookmark-section__count">{pinnedItems.length}</span>
            </div>
            ...
            <>
              ...
              {uncategorizedItems.length > 0 && (
                <section className="portal-bookmark-section" aria-labelledby="portal-category-uncategorized">
                  <div className="portal-bookmark-section__heading">
                    <span className="portal-category-dot bg-zinc-500" />
      ...
      {showItemModal && (
        ...
        >
          ...
          <label htmlFor="portal-bookmark-name" className="portal-field-label">이름 *</label>
          <input
            id="portal-bookmark-name"
            type="text"
            value={form.name}
            onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
            className="portal-field mb-3"
            placeholder="이름을 입력하세요"
            autoFocus
            data-dialog-initial
          />

          <label htmlFor="portal-bookmark-url" className="portal-field-label">URL *</label>
          <input
          ...
          <label htmlFor="portal-bookmark-category" className="portal-field-label">카테고리</label>
          <select
          ...
          <label htmlFor="portal-bookmark-description" className="portal-field-label">설명 (선택)</label>
          <input
          ...
          <label htmlFor="portal-bookmark-pinned" className="portal-check">
      ...
      {showCatModal && (
        ...
        >
              ...
              {sortedCategories.map(category => {
                ...
                return (
                  <div key={category.id} className="portal-category-manager-list__row">
                    <span className={`portal-category-dot ${color.dot}`} />
          ...
          <label htmlFor="portal-category-name" className="portal-field-label">이름 *</label>
          <input
            id="portal-category-name"
            type="text"
            value={catForm.name}
            onChange={e => setCatForm(f => ({ ...f, name: e.target.value }))}
            className="portal-field mb-4"
            placeholder="카테고리 이름"
            autoFocus
            data-dialog-initial
          />
          <span id="portal-category-color-label" className="portal-field-label">색상</span>
    ...
    <>
      ...
      {showSettings && (
        ...
        >
          ...
          {isDeployedWeb() ? (
            <div data-testid="deployed-settings-bookmark-context" className="portal-banner portal-banner--info" style={{marginBottom:16}}>
              <p className="portal-banner__lead">공통 북마크</p>
              <p>북마크와 분류는 기기 선택이나 원격 연결과 관계없이 함께 사용합니다. 프로젝트 조회 기기는 프로젝트 현황에서 선택하세요.</p>
            </div>
          ) : (
          <div style={{marginBottom:16}}>
            <div className="portal-field-label portal-field-label--row">
              <span>이 브라우저의 등록 이름{!deviceName && !data.deviceId && <span style={{color:'var(--accent)',fontWeight:700,marginLeft:4}}>* 필수</span>}</span>
              <div style={{flex:1}}/>
              {deviceName
                ? <span style={{fontSize:11,color:'var(--ok)',display:'inline-flex',alignItems:'center',gap:4}}><span style={{width:6,height:6,borderRadius:3,background:'var(--ok)',display:'inline-block'}}/>등록됨</span>
                : data.deviceId
                  ? <span style={{fontSize:11,color:'var(--accent)'}}>이름 미설정 — 입력 권장</span>
                  : null
              }
            </div>
            {selectedProjectDevice && selectedProjectDevice.id !== data.deviceId && (
              <div className="portal-banner portal-banner--warn" style={{marginBottom:8,fontSize:11}}>
                상단에서 보는 기기는 <strong style={{color:'var(--warn)'}}>{selectedProjectDevice.name || selectedProjectDevice.id.slice(0, 8)}</strong>이고,
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:6}}>
              <p className="portal-hint portal-mono" style={{margin:0}}>Device ID: {data.deviceId ? data.deviceId.slice(0, 16) + '…' : '자동생성'}</p>
              {onChangeDevice && (
                <button
          ...
          {/* ── 2. Supabase 연결 ─────────────────────────────────────────────── */}
          {isDeployedWeb() ? (
            <div data-testid="deployed-settings-connection" className="portal-banner portal-banner--info" style={{marginBottom:16}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:10}}>
                <div style={{minWidth:0}}>
                  <p>공유 데이터 연결</p>
                  <p className="portal-banner__lead" style={{fontSize:12.5,color:sbUrl && sbKey ? 'var(--ok)' : 'var(--warn)'}}>
                    {sbUrl && sbKey ? 'Supabase 연결됨' : '연결 정보 확인 필요'}
                  </p>
                </div>
                <span className={`portal-pill ${syncOk ? 'portal-pill--ok' : ''}`}>
          ...
          <div style={{marginBottom:16}}>
            <label className="portal-field-label">Project URL</label>
            <input
              type="text"
              value={sbUrl}
            ...
            />
            <label className="portal-field-label">Anon Key</label>
            <input
              type="password"
              value={sbKey}
              onChange={e => setSbKey(e.target.value)}
              className="portal-field portal-mono"
              placeholder="eyJ..."
            />
            {requiresUserSession && !isDeployedWeb() && (
              <div className="portal-banner portal-banner--accent" style={{marginTop:12}}>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
                  <div style={{minWidth:0}}>
                    <p style={{fontWeight:600,color:authenticatedSessionEmail?'var(--ok)':'var(--warn)'}}>
                    ...
                    <p className="portal-hint" style={{marginTop:4}}>
                      같은 origin의 <code className="portal-code">/portal.html</code> popup에서 session을 완료합니다.
                    </p>
                  </div>
                  <button
                    ...
                    className="portal-mini-button portal-mini-button--accent"
                    style={{flexShrink:0}}
                  >
            ...
            {/* 앱/source UI의 Supabase 읽기·쓰기는 로컬 sidecar의 service_role 경로로 수행한다.
                포털 엔트리포인트만 위 Google authenticated session을 사용한다.
                service_role은 portal.json에 저장하지 않고 로컬 서버 전용 파일(0600)에 따로 둔다. */}
            {!isDeployedWeb() && (
              <div style={{marginTop:14,paddingTop:12,borderTop:'1px solid var(--line)'}}>
                <label className="portal-field-label">
                  service_role Key
                  <span style={{marginLeft:6,color:serviceKeyPresent ? 'var(--ok)' : 'var(--warn)'}}>
                ...
                <div style={{display:'flex',gap:6,marginTop:8,flexWrap:'wrap'}}>
                  <button
                    ...
                    className="portal-mini-button portal-mini-button--info"
                  >
                  ...
                  {serviceKeyPresent && (
                    <button
                      ...
                      className="portal-mini-button portal-mini-button--danger"
                    >
                      삭제
                    </button>
                  )}
            ...
            {!isDeployedWeb() && (
              <div data-testid="voc-admin-settings" className="portal-banner portal-banner--warn" style={{marginTop:14}}>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
                  <div>
                    <p style={{fontWeight:700,color:'var(--warn)'}}>공개 VOC 수집 관리</p>
                    <p className="portal-hint" style={{marginTop:3}}>공개 설치본의 개발자 전송 한도와 스팸 단말을 관리합니다.</p>
                  </div>
                  {vocAdminBusy && <span style={{fontSize:10.5,color:'var(--ink-2)'}}>처리 중…</span>}
                </div>
                {!serviceKeyPresent ? (
                  <p className="portal-hint" style={{marginTop:8,color:'var(--warn)'}}>위 service_role 키를 저장하면 관리 기능이 열립니다.</p>
                ) : (
                  <>
                    {vocAdminError && <p className="portal-hint" style={{marginTop:8,color:'var(--danger)'}}>{vocAdminError}</p>}
                    <div style={{display:'flex',alignItems:'center',gap:8,marginTop:9,flexWrap:'wrap'}}>
                      <label className="portal-check" style={{gap:5,fontSize:11,color:'var(--ink-2)'}}>
                        <input type="checkbox" checked={vocAccepting} onChange={e => setVocAccepting(e.target.checked)} style={{accentColor:'var(--warn)'}} />
                        VOC 접수 켜기
                      </label>
                      <label className="portal-check" style={{gap:5,fontSize:11,color:'var(--ink-2)'}}>
                        단말당 하루
                        <input
                          ...
                          className="portal-field portal-mono"
                          style={{width:64,height:28,minHeight:28,padding:'0 6px',fontSize:11}}
                        />회
                      </label>
                      <button
                        ...
                        className="portal-mini-button portal-mini-button--warn"
                      >설정 저장</button>
                    </div>
                    <div style={{marginTop:10,paddingTop:9,borderTop:'1px solid var(--line)'}}>
                      <p style={{marginBottom:6,fontSize:10.5}}>스팸 단말 차단 · VOC DB의 <code className="portal-code">device_hash</code> 사용</p>
                      <input
                      ...
                      <div style={{display:'flex',gap:6,marginTop:6,flexWrap:'wrap'}}>
                        <select value={vocBlockScope} onChange={e => setVocBlockScope(e.target.value === 'app' ? 'app' : 'voc')} className="portal-field" style={{width:'auto',fontSize:11}}>
                          <option value="voc">VOC 전송만 차단</option>
                          <option value="app">앱 사용 차단</option>
                        </select>
                        <input type="datetime-local" value={vocBlockExpires} onChange={e => setVocBlockExpires(e.target.value)} title="비우면 무기한" className="portal-field" style={{width:'auto',fontSize:11}} />
                      </div>
                      <input value={vocBlockNote} onChange={e => setVocBlockNote(e.target.value)} maxLength={500} placeholder="운영자 메모 (사용자에게 노출되지 않음)" className="portal-field" style={{marginTop:6,fontSize:11}} />
                      <div style={{display:'flex',gap:6,marginTop:6}}>
                        <button disabled={vocAdminBusy || !/^[0-9a-f]{64}$/i.test(vocBlockHash)} onClick={() => void saveVocBlock()} className="portal-mini-button portal-mini-button--danger">차단 적용</button>
                        <button disabled={vocAdminBusy || !/^[0-9a-f]{64}$/i.test(vocBlockHash)} onClick={() => void clearVocBlock()} className="portal-mini-button">차단 해제</button>
                      </div>
                    </div>
                  </>
                )}
            ...
            {sbUrl && sbKey && !isDeployedWeb() && (
              <div style={{display:'flex',flexDirection:'column',gap:6,marginTop:10}}>
                ...
                <button
                  onClick={async () => {
                    ...
                    if (onOpenDeployUrl) {
                      ...
                      await PortalAPI.openUrl(portalUrl);
                    }
                  }}
                  className="portal-mini-button portal-mini-button--ok portal-mini-button--block"
                  style={{height:32}}
                >
                  🔗 Vercel 포털 열기 (자동 인증)
                </button>
                <button
                  ...
                  className="portal-mini-button portal-mini-button--info portal-mini-button--block"
                  style={{height:32}}
                >
                  📋 새 단말 등록 링크 복사
                </button>
                  ...
                  <div data-testid="personal-portal-not-configured" className="portal-banner portal-banner--warn" style={{fontSize:11}}>
                    개인 포털 주소가 아직 연결되지 않았습니다. 포털 열기·새 단말 링크만 사용할 수 없고 로컬 기능과 Supabase 동기화는 정상 작동합니다. 포털 자동 배포를 완료하거나, 운영자가 공개 소스가 아닌 배포 환경에 <code className="portal-code">VITE_PORTAL_URL</code>을 설정하세요.
                  </div>
                )}
              </div>
            )}
      ...
      {/* ── Push 히스토리 모달 — 포털 아이템 ─────────────────────────────────── */}
      {showPortalHistory && (
        <div className="portal-modal-backdrop" style={{zIndex:60}} onClick={() => setShowPortalHistory(false)}>
          <div className="portal-modal-panel portal-modal-panel--history" onClick={e => e.stopPropagation()}>
            <div className="portal-modal-panel__header">
              <Clock className="w-4 h-4" style={{color:'var(--accent)',flexShrink:0}} />
              <h2>Push 히스토리 — 포털 북마크</h2>
              <button onClick={() => setShowPortalHistory(false)} className="portal-icon-button"><X className="w-4 h-4" /></button>
            </div>
            <div className="portal-modal-panel__body portal-modal-panel__body--flush" style={{maxHeight:360}}>
              {portalHistoryLoading ? (
                <div style={{display:'flex',alignItems:'center',justifyContent:'center',padding:'40px 0'}}>
                  <RefreshCw className="w-5 h-5" style={{color:'var(--ink-3)',animation:'spin 1s linear infinite'}} />
                </div>
              ) : portalHistoryList.length === 0 ? (
                <div className="portal-history-empty">
                  <Clock className="w-8 h-8" />
                  <p>저장된 히스토리가 없습니다</p>
                  <p>Push 시 자동으로 스냅샷이 저장됩니다</p>
                </div>
              ) : portalHistoryList.map((snap, i) => (
                <div key={snap.id} className="portal-history-row">
                  <div>
                    <p className="portal-history-row__time">{new Date(snap.created_at).toLocaleString('ko-KR')}</p>
                    <p className="portal-history-row__meta">
                      {snap.row_count}개 항목{snap.device_name ? ` · ${snap.device_name}` : ''}
                      {i === 0 && <span className="portal-pill portal-pill--ok" style={{height:18,fontSize:10}}>최신</span>}
                    </p>
                  </div>
                  <button
            ...
            <div className="portal-modal-panel__footer portal-modal-panel__footer--note">
              <p>복원 시 현재 Supabase 북마크 데이터를 선택한 시점으로 되돌립니다</p>
            </div>
          </div>
        </div>
      )}
...
// 30px 타일. 파비콘이 있으면 중립 타일에 얹고, 없거나 폴더면 카테고리 색 타일에 이니셜/아이콘.
function FaviconBadge({ item, tint }: { item: PortalItem; tint?: string }) {
  const [errored, setErrored] = useState(false);

  if (item.type !== 'web') {
    return (
      <div className={`portal-bookmark-card__icon ${tint ?? 'bg-zinc-500'}`}>
  ...
  if (host && !errored) {
    return (
      <div className="portal-bookmark-card__icon portal-bookmark-card__icon--favicon">
        <img
  ...
  return (
    ...
    >
      ...
      <div className="portal-bookmark-card__header">
        <FaviconBadge item={item} tint={c.dot} />
      ...
      <div className="portal-bookmark-card__footer">
        <div className="portal-bookmark-card__category">
          <span className={`portal-category-dot ${cat ? c.dot : 'bg-zinc-500'}`} />
