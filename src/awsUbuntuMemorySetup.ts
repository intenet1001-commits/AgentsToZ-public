...
export function buildAwsUbuntuMemorySetupPrompt(input: AwsUbuntuMemorySetupInput): string {
  ...
  return [
    ...
    `   curl --fail-with-body -sS -X POST http://127.0.0.1:3001/api/project-memory/install-hermes-adapter | python3 -c 'import json,sys; d=json.load(sys.stdin); req={"remember-session","memory-link","memory-sync","memory-status","memory-unlink","memory-start","project-start","project-clone","project-from-memory"}; assert d.get("ok") and d.get("externalDirRegistered") and req<=set(d.get("installed",[])) and d.get("installedVersion",0)>=${CURRENT_PROJECT_MEMORY_VERSION}, d; print("Hermes AgentsToZ adapter: PASS")'`,
    ...
    '   /project_start는 작업 루트 아래에 실제 프로젝트 폴더를 만들고 git 저장소·초기 커밋·장기기억까지 함께 만든다. 이 호스트는 GUI가 없어 작업 루트를 등록하기 어려우므로, 등록된 루트가 없으면 ~/projects/<이름>에 만들고 만든 경로를 함께 알려준다. 루트를 등록해 두면 그 아래에 만들고, 둘 이상이면 어디에 만들지 되묻는다.',
    '   /project_clone <GitHub URL> [이름]은 저장소를 ~/projects 아래에 clone하고 장기기억·AgentsToZ 등록·현재 topic 연결까지 처리한다.',
    '   /project_from_memory <memoryId> <이름>은 기존 장기기억을 새 프로젝트 폴더로 복원하고 로컬 Git·AgentsToZ 등록·현재 topic 연결까지 처리한다. 소스 코드는 기억이 아니라 GitHub에 있으므로 저장소가 있으면 /project_clone을 우선한다.',
    '',
    ...
    '   새 명령을 설치한 직후에는 Telegram 대화에서 /reload_skills 를 한 번 실행한다. 실행 중인 gateway 는 스킬 목록을 캐시하므로 그 전까지는 새 명령이 인식되지 않는다. ⚠️ 셸로 `hermes gateway restart` 를 돌리지 말 것 — Hermes 가 막는다(그 대화를 돌리는 것이 그 gateway 라 SIGTERM 이 명령 자신에게 전파된다). 슬래시 명령 /restart 는 정상 동작하지만 봇이 몇 초 끊기므로 무중단인 /reload_skills 를 먼저 쓴다. 어느 쪽이든 Telegram 자동완성 목록은 클라이언트가 캐시해서 즉시 바뀌지 않는다 — 등록 여부는 /commands 로 확인하고, 목록에 없어도 명령을 직접 입력하면 실행된다.',
    '',
    ...
    '   ⚠️ 새 명령이 늘어난 갱신이면 여기서 끝내지 말 것 — 실행 중이던 gateway 는 스킬 목록을 캐시하므로 재기동하지 않으면 새 명령을 모른다. `gateway install --force` 가 재기동을 겸하지만, 재기동 없이 확인만 하려면 Telegram 대화에서 /reload_skills 를 한 번 실행한다(무중단). 등록 여부는 /commands 로 확인하고, 자동완성 목록에 없어도 명령을 직접 입력하면 실행된다.',
