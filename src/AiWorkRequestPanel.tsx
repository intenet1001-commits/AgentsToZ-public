...
export function AiWorkRequestPanel({projects:suppliedProjects, entry, onStarted, onOpenWorkroom, visible}: {
  ...
  return <section data-testid="ai-work-request" className="shrink-0 rounded-xl border border-[var(--line)] bg-[var(--surface)] p-4 text-[var(--ink)]">
    ...
    <div className="mb-2 flex flex-wrap gap-2">
      ...
      {mode==='mission'&&<select aria-label="AI 작업 관제 전략" className="max-w-full bg-[var(--sunken)] p-2" value={policy} disabled={busy} onChange={event=>{revision.current+=1;policyEdited.current=true;setPolicy(event.target.value as AiWorkOrchestrationPolicy);}}><option value="agentstoz">AgentsToZ 기본 관제</option><option value="cs-ceo">AgentsToZ + CS-CEO</option></select>}
    </div>
    ...
    {mode==='mission'&&!controlProject&&!profileReady&&<p role="alert" className="my-2 text-xs text-red-400">AgentsToZ 운영 프로필 연결이 필요합니다. 운영 프로필에서 준비 상태를 확인하세요.</p>}
