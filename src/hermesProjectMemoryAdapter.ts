...
/**
 * 명령을 깐 **직후에 남는 한 걸음**을 알리는 문구. 세 표면(패널 설치 버튼·헤더 배지·
 * 서버 자동 맞춤 로그)이 이 하나를 함께 쓴다.
 *
 * 이 문구가 없으면 "설치는 성공했는데 Telegram 이 명령을 모르는" 상태에 갇힌다. 실행 중인
 * 게이트웨이가 스킬 목록을 캐시하기 때문이다(`get_skill_commands()`).
 *
 * ⚠️ **"Hermes 를 다시 시작하세요"라고 쓰지 말 것.** 예전 문구가 그랬고, 그것이 사용자를
 * 두 번 잘못된 길로 보냈다: 셸로 `hermes gateway restart` 를 돌리면 Hermes 가 막고
 * (SIGTERM 이 명령 자신에게 전파된다), 슬래시 `/restart` 는 되지만 봇을 몇 초 끊는다.
 * 무중단으로 되는 길은 `/reload_skills` 다.
 *
 * 자동완성 목록은 이것으로도 즉시 바뀌지 않는다 — Telegram **클라이언트**가 캐시하기
 * 때문이고, 표시의 문제일 뿐 직접 입력하면 실행된다. 그래서 그 사실을 함께 적는다.
 * 이 두 문장이 없으면 사용자는 잘 깔린 설치를 고장으로 읽는다(실제로 그랬다).
 */
export const HERMES_POST_INSTALL_HINT =
  'Telegram 대화에서 /reload_skills 를 한 번 실행하면 바로 쓸 수 있습니다. '
  + '자동완성 목록은 늦게 갱신되니, 안 보여도 명령을 직접 입력하면 실행됩니다.';
...
export function hermesPendingWork(status: HermesInstallState): string | null {
  ...
  if (status.outputStyleInstalled === false) return '공통 응답 규칙 설치';
