...
export class RemoteControlRelayController {
   ...
   */
  async #waitForPollToRelease(attempts=REMOTE_CONTROL_BUSY_WAIT_ATTEMPTS,pollMs=REMOTE_CONTROL_BUSY_WAIT_POLL_MS): Promise<void> {
  ...
  async sendTerminal(request: AiTerminalRequest|MobileWorkspaceRequest): Promise<AiTerminalResponse|MobileWorkspaceResult> {
    ...
    if (this.#terminalQueued >= 32) throw new Error('대기 중인 원격 입력이 많습니다. 잠시 후 다시 시도하세요.');
    this.#terminalQueued++;
    ...
    try {
      ...
      return await this.#sendTerminalRequest(request);
    } finally {
      this.#terminalQueued--;
  ...
  async #sendTerminalRequest(request: AiTerminalRequest|MobileWorkspaceRequest): Promise<AiTerminalResponse|MobileWorkspaceResult> {
...
const REMOTE_CONTROL_INTERACTIVE_INPUTS = new Set(['1','2','3','4','5','y','Y','n','N','\x1b','\t','\x1b[A','\x1b[B','\x1bOA','\x1bOB','\x03','\x0c','\r']);
