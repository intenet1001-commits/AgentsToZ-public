...
export function normalizeAiTerminalRequest(value: unknown): AiTerminalRequest {
  ...
  const keys: Record<string, string[]> = {
    ...
    input: ['sessionId', 'data'], resize: ['sessionId', 'cols', 'rows'], close: ['sessionId','memoryPolicy','saveRequestId'],
  };
  ...
  if (r.prompt !== undefined && (aiInitialPromptError(r.prompt) || !['codex', 'claude'].includes(r.agent!))) throw new Error('이 AI에 전달할 수 없는 작업 요청입니다.');
  return { ...r };
}
