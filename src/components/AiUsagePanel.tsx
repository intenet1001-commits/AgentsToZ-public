...
import { AutoRememberEvidence } from './AutoRememberEvidence';
...
function UsageBar({ percent }: { percent: number | null }) {
  if (typeof percent !== 'number') {
    return (
      <div title="이 Voice 대화에는 컨텍스트 사용량 기록이 없습니다." style={{ height: 6, background: 'rgb(var(--surface-highlight-rgb) / 0.07)', borderRadius: 3 }} />
    );
  }
  const pct = Math.max(0, Math.min(100, percent));
  const color = pct >= 90 ? 'var(--ink-f87171)' : pct >= 70 ? 'var(--ink-fbbf24)' : 'var(--ink-5eead4)';
  return (
    <div style={{ height: 6, background: 'rgb(var(--surface-highlight-rgb) / 0.07)', borderRadius: 3, overflow: 'hidden' }}>
...
const voiceBindingBadge = (state: ProjectCodexVoiceBindingState | undefined): {
  ...
  switch (state) {
    case 'execution-confirmed':
      return { label: '프로젝트 실행 확인', title: '최근 Voice 실행 경로와 프로젝트 배정이 모두 이 프로젝트로 확인되었습니다. 마이크 활성 상태는 별도로 확인해야 합니다.', tone: 'var(--ink-99f6e4)' };
    case 'assigned-awaiting-execution':
      return { label: '프로젝트 배정됨', title: 'Voice 대화는 프로젝트에 배정되었지만, 실제 실행 경로가 담긴 Voice turn은 아직 기록되지 않았습니다.', tone: 'var(--ink-bfdbfe)' };
    case 'move-pending':
      return { label: '이동 적용 대기', title: '프로젝트 이동 요청은 기록됐지만 Voice의 실제 작업 폴더는 아직 바뀌지 않았습니다.', tone: 'var(--ink-fde68a)' };
    case 'scope-conflict':
      return { label: '실행 경로 불일치', title: 'Voice 대화의 프로젝트 배정과 최근 실행 경로가 다릅니다. 대상 프로젝트 장기기억으로 연결하지 않습니다.', tone: 'var(--ink-fca5a5)' };
    case 'not-associated':
      return { label: '프로젝트 미연결', title: '이 Voice 대화의 로컬 Codex 프로젝트 배정 기록을 찾지 못했습니다.', tone: 'var(--text-secondary)' };
    case 'unverifiable':
      return { label: '연결 확인 불가', title: 'ChatGPT Desktop의 로컬 프로젝트 또는 Voice 기록을 읽을 수 없어 연결 여부를 판단하지 않았습니다.', tone: 'var(--text-secondary)' };
    default:
      return null;
  }
};
...
export function AiUsagePanel({
  ...
  const [autoRemember, setAutoRemember] = useState<{
    loading: boolean;
    data?: CodexAutoRememberStatus;
    error?: string;
  }>({ loading: true });
  const [updatingAutoRemember, setUpdatingAutoRemember] = useState(false);
  ...
  const loadAutoRemember = useCallback(async () => {
    try {
      const res = await fetch(`${apiBase()}/api/project-memory/auto-checkpoint/status`);
      const json = await res.json();
      if (!res.ok) throw new Error(res.status === 404
        ? '이 Mac의 AgentsToZ 앱이 자동 체크포인트를 지원하지 않습니다. 앱을 업데이트하고 완전히 다시 열어 주세요.'
        : json.error || `요청 실패 (${res.status})`);
      setAutoRemember({ loading: false, data: json });
    } catch (e: any) {
      setAutoRemember({ loading: false, error: e?.message || String(e) });
    }
  }, []);

  const toggleAutoRemember = useCallback(async () => {
    if (updatingAutoRemember) return;
    setUpdatingAutoRemember(true);
    try {
      const enabled = autoRemember.data?.settings.enabled !== true;
      const res = await fetch(`${apiBase()}/api/project-memory/auto-checkpoint/settings`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ enabled }),
      });
      const json = await res.json();
      if (!res.ok || json.success === false) throw new Error(json.error || `요청 실패 (${res.status})`);
      setAutoRemember({ loading: false, data: json });
    } catch (e: any) {
      setAutoRemember(previous => ({ ...previous, loading: false, error: e?.message || String(e) }));
    } finally {
      setUpdatingAutoRemember(false);
    }
  }, [autoRemember.data?.settings.enabled, updatingAutoRemember]);
  ...
  useEffect(() => {
    ...
    void loadCodex();
    void loadCtx();
    void loadAutoRemember();
  }, [loadAutoRemember, loadClaude, loadCodex, loadCtx]);
  ...
  useEffect(() => {
    const id = setInterval(() => { void loadAutoRemember(); }, 15_000);
    return () => clearInterval(id);
  }, [loadAutoRemember]);
  ...
  const cardStyle: React.CSSProperties = {
    border: '1px solid rgb(var(--surface-highlight-rgb) / 0.08)',
  ...
  const refreshBtn: React.CSSProperties = {
    marginLeft: 'auto', background: 'transparent', border: '1px solid rgb(var(--surface-highlight-rgb) / 0.1)',
    borderRadius: 5, color:'var(--text-secondary)', cursor: 'pointer', padding: '2px 6px',
  ...
  const latestAutoRememberSession = autoRemember.data?.sessions.find(session => (
    session.phase === 'saving' || session.phase === 'failed' || session.phase === 'retrying'
      || session.phase === 'recovery-required'
  )) ?? autoRemember.data?.sessions[0] ?? null;
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'var(--dialog-scrim)', zIndex: 9500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      onClick={onClose}
    >
      <div
        onClick={e => e.stopPropagation()}
        role="dialog" aria-modal="true" aria-label="AI 사용량"
        style={{
          background: 'var(--bg-deep)', border: '1px solid rgb(var(--surface-highlight-rgb) / 0.1)', borderRadius: 12,
          width: '100%', maxWidth: 560, maxHeight: 'calc(var(--ui-viewport-height, 100dvh) * 0.82)', display: 'flex', flexDirection: 'column',
          boxShadow: 'var(--dialog-shadow)', fontFamily: 'inherit',
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 14px', borderBottom: '1px solid rgb(var(--surface-highlight-rgb) / 0.08)' }}>
          <Gauge style={{ width: 15, height: 15, color:'var(--ink-5eead4)' }} />
          <span style={{ fontSize: 13, fontWeight: 600, color:'var(--text-primary)' }}>AI 사용량</span>
          <button onClick={onClose} aria-label="닫기" style={{ marginLeft: 'auto', background: 'transparent', border: 'none', color:'var(--text-dim)', cursor: 'pointer', display: 'flex', padding: 2 }}>
        ...
        <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 12, overflowY: 'auto' }}>
          ...
          <div style={cardStyle}>
            <div style={headStyle}>
              <span style={{ color:'var(--ink-5eead4)' }}>최근 AI 세션 컨텍스트</span>
              <span style={{ fontSize: 10, color:'var(--text-dim)', fontWeight: 400 }}>Claude statusLine · Codex/Voice 세션 기록 · Voice는 토큰 수치 없이도 표시 · Orca/cmux 표면은 런타임 확인</span>
            </div>
            <div
              data-testid="codex-auto-remember-status"
              style={{
                marginBottom: 9, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap',
                fontSize: 10.5, lineHeight: 1.45, borderRadius: 6, padding: '7px 8px',
                background: autoRemember.data?.settings.enabled ? 'rgb(var(--accent-rgb) / 0.08)' : 'rgb(var(--surface-highlight-rgb) / 0.035)',
                color:autoRemember.error ? 'var(--ink-fca5a5)' : autoRemember.data?.settings.enabled ? 'var(--ink-99f6e4)' : 'var(--text-secondary)',
              }}
            >
              <span style={{ fontWeight: 600 }}>
                Claude·Codex 프로젝트 기억 자동 체크포인트 {autoRemember.loading
                  ? '확인 중…'
                  : autoRemember.error
                    ? '확인 실패'
                    : autoRemember.data?.settings.enabled ? '켜짐' : '꺼짐'}
              </span>
              {!autoRemember.error && (
                <span>
                  원문 저장 아님 · 실행 앱과 무관 · {CODEX_AUTO_REMEMBER_THRESHOLDS.join('·')}% · 실제 턴 종료 뒤 · 검증된 요약만 · 등록 프로젝트만 · Claude 상태 표시줄 필요 · Hermes·Antigravity 미지원
                </span>
              )}
              {latestAutoRememberSession && autoRemember.data?.settings.enabled && (
                <span title={latestAutoRememberSession.backupWarning ?? undefined}>
                  최근 상태: {latestAutoRememberSession.projectName
                    ? `${latestAutoRememberSession.projectName} · `
                    : ''}{latestAutoRememberSession.message}
                </span>
              )}
              {!autoRemember.error && autoRemember.data && <AutoRememberEvidence status={autoRemember.data} />}
              {!!autoRemember.data?.backups?.length && (
                <span>자동 백업: {autoRemember.data.backups.some(job => job.state === 'blocked')
                  ? '내용·연결 확인이 필요한 작업이 있습니다. 프로젝트의 싱크 상태를 확인해 주세요.'
                  : '저장한 기억을 백업 대기 중입니다. AI를 다시 실행하지 않습니다.'}</span>
              )}
              {autoRemember.error && <span>{autoRemember.error}</span>}
              {!autoRemember.loading && !autoRemember.error && (
                <button
                  type="button"
                  data-testid="codex-auto-remember-toggle"
                  disabled={updatingAutoRemember}
                  onClick={() => { void toggleAutoRemember(); }}
                  style={{
                    marginLeft: 'auto', border: '1px solid rgb(var(--accent-rgb) / 0.3)', borderRadius: 5,
                    background: 'rgb(var(--accent-rgb) / 0.08)', color:'var(--ink-ccfbf1)', cursor: updatingAutoRemember ? 'wait' : 'pointer',
                    padding: '3px 7px', fontSize: 10, fontFamily: 'inherit', opacity: updatingAutoRemember ? 0.7 : 1,
                  }}
                >
                  {updatingAutoRemember
                    ? '변경 중…'
                    : autoRemember.data?.settings.enabled ? '자동 체크포인트 끄기' : '자동 체크포인트 켜기'}
                </button>
              )}
            </div>
            {apiVersion.state === 'outdated' && (
              <div
                ...
                role="status"
                style={{
                  fontSize: 10.5, color:'var(--ink-fdba74)', lineHeight: 1.5,
                  background: 'rgb(var(--warn-rgb) / 0.09)', borderRadius: 6, padding: '6px 8px',
                }}
              >
            ...
            {hiddenUnverifiedSurfaceCount > 0 && (
              <div style={{ fontSize: 10.5, color:'var(--ink-fbbf24)', lineHeight: 1.45 }}>
                Orca/cmux 세션 {hiddenUnverifiedSurfaceCount}개는 실행 표면을 확인할 수 없어 목록에서 제외했습니다.
              </div>
            )}
            {ctx.loading ? (
              <div style={{ fontSize: 11.5, color:'var(--text-secondary)' }}>조회 중…</div>
            ) : ctx.error ? (
              <div style={{ fontSize: 11.5, color:'var(--ink-fca5a5)' }}>{ctx.error}</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {projectVoiceHistory.length > 0 && (
                  <div style={{ fontSize: 10.5, color:'var(--ink-99f6e4)', lineHeight: 1.45 }}>
                ...
                {displayedSessions.map(s => {
                  ...
                  const warn = (s.usedPercent ?? 0) >= CONTEXT_WARNING_THRESHOLD;
                  ...
                  const memoryBackground = voiceMemoryBlocked
                    ? s.voiceBindingState === 'scope-conflict'
                      ? 'rgb(var(--danger-rgb) / 0.09)'
                      : 'rgb(var(--warn-rgb) / 0.09)'
                    : savedMemory
                    ? 'rgb(var(--accent-rgb) / 0.08)'
                    : memoryAction === 'conflict' || memoryAction === 'retry'
                      ? 'rgb(var(--danger-rgb) / 0.09)'
                      : memoryAction === 'pull'
                        ? 'rgb(var(--info-rgb) / 0.09)'
                        : memoryAction === 'ephemeral'
                          ? 'rgb(var(--surface-highlight-rgb) / 0.05)'
                          : 'rgb(var(--warn-rgb) / 0.09)';
                  ...
                  return (
                    ...
                    <div
                      ...
                      style={{
                        ...
                        background: 'rgb(var(--surface-highlight-rgb) / 0.032)',
                        border: '1px solid rgb(var(--surface-highlight-rgb) / 0.075)',
                        borderRadius: 10,
                        padding: '9px 11px',
                      }}
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                        <span
                          ...
                          style={{ minWidth: 0, flex: 1, fontSize: 12.5, color:'var(--ink-fafafa)', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                        >
                        ...
                        <span style={{
                          ...
                          color:warn ? 'var(--ink-fbbf24)' : 'var(--text-primary)', fontFamily: "'JetBrains Mono', monospace",
                        }}>
                      ...
                      <div
                        ...
                        style={{ marginTop: 2, fontSize: 10.5, color:'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                      >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 4, marginBottom: 5, flexWrap: 'wrap' }}>
                        <span style={originBadge(
                          s.sourceAgent === 'claude' ? 'rgb(var(--violet-rgb) / 0.12)' : 'rgb(var(--info-rgb) / 0.12)',
                          s.sourceAgent === 'claude' ? 'var(--ink-c4b5fd)' : 'var(--ink-93c5fd)',
                        )}>
                          {s.clientLabel}
                        </span>
                        <span
                          title={s.surfaceDetail ? `${s.surfaceLabel} · ${s.surfaceDetail}` : s.surfaceLabel}
                          style={originBadge('rgb(var(--surface-highlight-rgb) / 0.06)', 'var(--text-secondary)')}
                        >
                          {s.surfaceLabel}
                        </span>
                        {isVoiceChat && (
                          <span title="ChatGPT Codex 보이스 채팅에서 시작한 세션" style={originBadge('rgb(var(--accent-rgb) / 0.1)', 'var(--ink-99f6e4)')}>
                            Codex 보이스 채팅
                          </span>
                        )}
                        {voiceBinding && (
                          <span
                            ...
                            style={originBadge('rgb(var(--accent-rgb) / 0.08)', voiceBinding.tone)}
                          >
                            {voiceBinding.label}
                          </span>
                        )}
                        {surfaceBadge && (
                          <span title={surfaceBadge.title} style={originBadge('rgb(var(--accent-rgb) / 0.08)', surfaceBadge.tone)}>
                            {surfaceBadge.label}
                          </span>
                        )}
                        <span style={{ marginLeft: 'auto', fontSize: 10, color:'var(--text-dim)', whiteSpace: 'nowrap' }}>
                          {s.modelName}
                      ...
                      {projectMovePending && (
                        <div
                          ...
                          style={{ marginTop: 6, fontSize: 10, lineHeight: 1.45, color:'var(--ink-fde68a)', background: 'var(--bg-warning)', borderRadius: 5, padding: '5px 7px' }}
                        >
                          {s.voiceBindingState === 'scope-conflict'
                      ...
                      {!projectMovePending && projectMoveConfirmed && (
                        <div
                          ...
                          style={{ marginTop: 6, fontSize: 10, lineHeight: 1.45, color:'var(--ink-bfdbfe)', background: 'rgb(var(--info-rgb) / 0.09)', borderRadius: 5, padding: '5px 7px' }}
                        >
                      ...
                      <div style={{ marginTop: 5, display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 6, minHeight: 21, flexWrap: 'wrap' }}>
                        {projectAvailable && (
                          <button
                            ...
                            style={{
                              ...
                              border: '1px solid rgb(var(--info-rgb) / 0.3)', background: 'rgb(var(--info-rgb) / 0.08)',
                              color:'var(--ink-bfdbfe)', fontSize: 10, fontFamily: 'inherit', padding: '3px 7px', cursor: 'pointer',
                            }}
                          >
                        ...
                        {navigation?.available ? (
                          <button
                            ...
                            style={{
                              display: 'inline-flex', alignItems: 'center', gap: 4, borderRadius: 5,
                              border: '1px solid rgb(var(--accent-rgb) / 0.28)', background: 'rgb(var(--accent-rgb) / 0.08)',
                              color:'var(--ink-99f6e4)', fontSize: 10, fontFamily: 'inherit', padding: '3px 7px',
                      ...
                      {rowFocusResult && (
                        <div style={{
                          ...
                          color:rowFocusResult.success ? 'var(--ink-99f6e4)' : 'var(--ink-fca5a5)',
                        }} role="status">
                      ...
                      {(voiceMemoryBlocked || warn || memoryAction !== 'unavailable') && (
                        <div style={{
                          ...
                          {voiceMemoryBlocked ? (
                            ...
                            <span style={{ fontSize: 10, color:'var(--text-secondary)' }}>기억 상태 확인 중…</span>
                          ) : memoryAction === 'missing' ? (
                            <span
                            ...
                            <button
                              ...
                              style={{
                                ...
                                border: '1px solid rgb(var(--danger-rgb) / 0.5)', background: 'var(--bg-danger)',
                                color:'var(--ink-fecaca)', fontSize: 10, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer',
                              }}
                            >
                              기억 상태 다시 확인
                            </button>
                            ...
                            <span
                              ...
                              style={{ fontSize: 10, color:'var(--ink-99f6e4)', fontWeight: 600 }}
                            >
                              세션 기억 완료
                            </span>
                          ) : syncNeedsAttention ? (
                            ...
                            <button
                              ...
                              style={{
                                display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 7px', borderRadius: 5,
                                border: '1px solid rgb(var(--warn-rgb) / 0.65)', background: 'var(--bg-warning)',
                                color:'var(--ink-fde68a)', fontSize: 10, fontWeight: 600, fontFamily: 'inherit',
                            ...
                            <span style={{ fontSize: 10, color:'var(--text-secondary)' }}>프로젝트 경로 없음</span>
                          )}
                        </div>
                      )}
                      {rowRememberResult && (
                        <div
                          ...
                          style={{
                            marginTop: 4, fontSize: 10, lineHeight: 1.45,
                            color:rowRememberResult.success ? 'var(--ink-99f6e4)' : 'var(--ink-fca5a5)',
                          }}
                          role="status"
                        >
                ...
                <div style={{ fontSize: 10, color:'var(--text-dim)', lineHeight: 1.5, borderTop: '1px solid rgb(var(--surface-highlight-rgb) / 0.06)', paddingTop: 7 }}>
          ...
          <div style={cardStyle}>
            <div style={headStyle}>
              <span style={{ color:'var(--ink-5eead4)' }}>Claude</span>
              <span style={{ fontSize: 10, color:'var(--text-dim)', fontWeight: 400 }}>claude -p "/usage" · 실시간</span>
              <button style={refreshBtn} onClick={() => void loadClaude()} disabled={claude.loading}>
                <RefreshCw style={{ width: 9, height: 9 }} /> 새로고침
              </button>
            </div>
            {claude.loading ? (
              <div style={{ fontSize: 11.5, color:'var(--text-secondary)' }}>조회 중… (몇 초 걸립니다)</div>
            ) : claude.error ? (
              <div style={{ fontSize: 11.5, color:'var(--ink-fca5a5)', whiteSpace: 'pre-wrap' }}>{claude.error}</div>
            ) : (
              <pre style={{
                margin: 0, fontSize: 11, lineHeight: 1.6, color:'var(--ink-c8d2d0)', whiteSpace: 'pre-wrap',
          ...
          <div style={cardStyle}>
            <div style={headStyle}>
              <span style={{ color:'var(--ink-93c5fd)' }}>Codex</span>
            ...
            {codex.loading ? (
              ...
              <div style={{ fontSize: 11.5, color:'var(--ink-fca5a5)', whiteSpace: 'pre-wrap' }}>{codex.error}</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
                {codexRows.length === 0 && (
                  <div style={{ fontSize: 11.5, color:'var(--text-secondary)' }}>사용량 항목이 기록되지 않았습니다.</div>
                )}
                {codexRows.map(({ label, limit }) => (
                  <div key={label}>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 4 }}>
                      ...
                      <span style={{ marginLeft: 'auto', fontSize: 12, fontWeight: 600, color:'var(--text-primary)', fontFamily: "'JetBrains Mono', monospace" }}>
