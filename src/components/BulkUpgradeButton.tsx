...
export default function BulkUpgradeButton({
  ...
  const [state, setState] = useState<BulkUpgradeState>({ memory: emptyTarget(), workflow: emptyTarget(), hermes: null, missing: [], checked: 0 });
  const [open, setOpen] = useState(false);
  const [busy, setBusy] = useState<BulkUpgradeTarget | 'hermes' | 'all' | 'github' | null>(null);
  ...
  const versionPending = state.memory.folderPaths.length + state.workflow.folderPaths.length + (state.hermes ? 1 : 0);
      ...
      {open && (
        ...
        >
          ...
          {state.hermes && (
            ...
            >
              <div style={{ minWidth: 0 }}>
                <div style={{ color: '#e4e4e7', fontSize: 11.5, fontWeight: 600 }}>{HERMES_LABEL}</div>
                <div style={{ color: '#a1a1aa', fontSize: 10.5, fontVariantNumeric: 'tabular-nums' }}>
