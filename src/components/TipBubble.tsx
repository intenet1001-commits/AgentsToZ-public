      ...
      {rich ? (
        <>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: tip.body ? 7 : 0 }}>
            <span style={{ flex: 1, fontWeight: 600, color:'var(--ink-5eead4)', fontSize: 13.5, letterSpacing: -0.1 }}>
              {tip.title}
            </span>
            {tip.source === 'auto' && (
              <span
                style={{
                  display: 'flex', alignItems: 'center', gap: 3, fontSize: 9.5, color:'var(--ink-5b7a75)',
                  border: '1px solid rgb(var(--accent-rgb) / 0.2)', borderRadius: 4, padding: '1px 5px',
            ...
            {pinned && (
              <span style={{ display: 'flex', alignItems: 'center', color:'var(--ink-5b7a75)', flexShrink: 0, marginTop: 2 }}>
                <Pin size={10} />
              </span>
            )}
            {pinned && onClose && (
              <button
                ...
                aria-label="닫기"
                style={{
                  background: 'transparent', border: 'none', color:'var(--ink-5b6663)', cursor: 'pointer',
          ...
          {tip.body && <div style={{ color:'var(--ink-b5c4c1)', marginBottom: tip.hint ? 9 : 0 }}>{tip.body}</div>}
          {tip.hint && (
            <div
              style={{
                display: 'flex', alignItems: 'flex-start', gap: 7,
                background: 'rgb(var(--accent-rgb) / 0.06)', borderLeft: '2px solid rgb(var(--accent-rgb) / 0.45)',
                borderRadius: 4, padding: '7px 10px', color:'var(--ink-8fd3c7)', fontSize: 11.5, lineHeight: 1.5,
              }}
            >
        ...
        <span style={{ color:'var(--ink-d5e3e0)' }}>{tip.title}</span>
      )}
      ...
      {arrowVisible && (
        <div
          style={{
            ...
            left: arrowLeft,
            ...(placement === 'below'
              ? { top: -5, borderBottom: '5px solid rgb(var(--accent-rgb) / 0.28)' }
              : { bottom: -5, borderTop: '5px solid rgb(var(--accent-rgb) / 0.28)' }),
            width: 0, height: 0,
