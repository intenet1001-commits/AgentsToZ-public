...
export function buildAiWorkMissionPrompt(input:{title:string;goal:string;targetLabel:string;workers:readonly AiTerminalAgent[];policy:AiWorkOrchestrationPolicy}):string{
 const method=input.policy==='cs-ceo'
  ? 'AgentsToZ 사용자 운영 프로필(기존 AgentsToZ-Control 기억)과 대상 프로젝트의 장기기억을 회상한 뒤, cs-ceo의 Goal Gate와 작업 분해·검토 절차를 적용하세요. cs-ceo는 계획자와 검토자이며 실제 세션·상태·재개는 AgentsToZ 미션이 소유합니다.'
  : 'AgentsToZ 사용자 운영 프로필(기존 AgentsToZ-Control 기억)과 대상 프로젝트의 장기기억을 회상해 운영 원칙, 활성 제약, 프로젝트 문맥을 적용하세요.';
 return `당신은 AgentsToZ 관제 에이전트입니다. 직접 구현을 독점하지 말고 등록된 하위 에이전트에게 미션을 배정하고 결과를 검증하세요.

${method}
먼저 agentstoz_use_get_control_profile과 agentstoz_use_recall_control_context로 현재 폴더와 무관한 운영 기억을 확인하세요. 연결이 없으면 임의로 새 프로필을 만들지 말고 준비 상태를 알려 주세요.
