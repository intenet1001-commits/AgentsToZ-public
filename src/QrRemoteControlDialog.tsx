              ...
              <section className="rounded-2xl border border-[rgb(var(--surface-highlight-rgb))]/[0.08] bg-[rgb(var(--surface-highlight-rgb))]/[0.02] p-4 sm:p-5" aria-labelledby="qr-remote-sessions-title">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <h3 id="qr-remote-sessions-title" className="m-0 text-sm font-semibold text-zinc-100">
                    {/* A phone whose screen is locked stays paired but is not connected. Counting it
                        as connected would claim a live link that is not there; it is still listed
                        below, with its own label, so it can be revoked. */}
                    연결된 기기 · {status.sessions.filter(session => session.connected).length}
                    {status.sessions.some(session => !session.connected) && (
                      <span className="ml-2 font-normal text-zinc-400">
                        연결 대기 {status.sessions.filter(session => !session.connected).length}
                      </span>
                    )}
                  </h3>
                ...
                {status.sessions.length === 0 ? (
                  ...
                  <ul className="m-0 mt-3 space-y-2 p-0" role="list">
                    {status.sessions.map(session => (
                      <li key={session.id} data-testid={session.connected ? 'qr-remote-session-connected' : 'qr-remote-session-away'} className="flex min-w-0 flex-col gap-3 rounded-xl border border-[rgb(var(--surface-highlight-rgb))]/[0.07] bg-[rgb(var(--surface-shade-rgb))]/10 p-3 sm:flex-row sm:items-center">
                        <div className="min-w-0 flex-1">
