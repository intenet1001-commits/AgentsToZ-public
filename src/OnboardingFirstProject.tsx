  ...
  return <section aria-label="첫 프로젝트 시작" className="rounded-2xl border border-[var(--accent-line)] bg-[var(--accent-soft)] p-5">
    ...
    {onStartControl && <div data-testid="onboarding-control-center" className="mt-4 rounded-xl border border-[var(--accent-line)] bg-[var(--surface)] p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-extrabold text-[var(--ink)]">먼저 AgentsToZ OPS 운영 프로필 준비</p>
          <p className="mt-1 text-xs leading-relaxed text-[var(--ink-2)]">프로젝트와 AI를 바꿔도 같은 운영 방식과 공통 결정을 기억합니다. 기존 운영 폴더(AgentsToZ-Control)의 기억도 연결할 수 있습니다.</p>
        </div>
        <span className="rounded-full bg-[var(--accent-soft)] px-2 py-1 text-[10px] font-bold text-[var(--accent)]">권장</span>
      </div>
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        <button type="button" data-testid="onboarding-create-control" className={`${button} bg-[var(--accent)] text-[var(--on-accent)]`} onClick={()=>onStartControl('create')}>내 AgentsToZ OPS 준비</button>
        <button type="button" data-testid="onboarding-restore-control" className={`${button} bg-[var(--surface)] text-[var(--ink)]`} onClick={()=>onStartControl('restore')}>다른 Mac의 AgentsToZ OPS 복원</button>
      </div>
      <p className="mt-2 text-[11px] leading-relaxed text-[var(--ink-3)]">앱 저장소 Pull만으로 운영 폴더는 복제되지 않습니다. 처음이면 AgentsToZ OPS 프로필을 준비하고, 다른 Mac에서 사용 중이면 기존 Private GitHub 저장소를 복원한 뒤 프로필에 연결하세요.</p>
    </div>}
