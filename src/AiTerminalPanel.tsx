...
export interface AiTerminalEntry {nonce:number;targetId:string;prompt?:string;title?:string;agent?:AiTerminalAgent;sessionId?:string}
export function AiTerminalPanel({visible=true,projects:suppliedProjects,transport=localAiTerminalTransport,remote=false,entry,onManageProject,onWhatISaid,sessionScope='local'}: {
 visible?:boolean;projects:{targetId:string;label:string}[];transport?:AiTerminalTransport;remote?:boolean;entry?:AiTerminalEntry|null;onManageProject?:(id:string)=>void;onWhatISaid?:()=>void;sessionScope?:string;
}) {
 ...
 const [target,setTarget]=useState('');const [agent,setAgent]=useState<AiTerminalAgent>('codex');
 ...
 const canStart=!!targetProject&&(!prompt.trim()||supportsPrompt)&&!busy;
 const [connections,setConnections]=useState<{id:string;label:string;allowed:boolean}[]>([]);
 ...
 const refreshConnections=useMemo(()=>singleFlight(()=>terminalLocalRequest(AI_TERMINAL_PREFIX+'/access',{}).then(r=>setConnections(r.connections??[])).catch(()=>{})),[]);
 useEffect(()=>{if(!target && projects.length)setTarget(projects[0]!.targetId)},[target,projects]);
 useEffect(()=>{if(entry && applied.current!==entry.nonce){applied.current=entry.nonce;setTarget(entry.targetId);setPrompt(entry.prompt??'');setRequestTitle(entry.title??'');setComposerOpen(!!entry.prompt);if(entry.prompt)setFocusRequest(value=>value+1);setSelected(entry.sessionId??'');if(entry.agent)setAgent(entry.agent);}},[entry]);
 const refresh=useMemo(()=>singleFlight(async(clearError=true)=>{const epoch=inventoryEpoch.current;try{const r=await request({operation:'list'});if(epoch!==inventoryEpoch.current)return;setSessions(old=>(r.sessions??[]).map(s=>old.find(prior=>prior.id===s.id&&prior.state==='exited')??s));setSelected(current=>current && !(r.sessions??[]).some(s=>s.id===current)?'':current);setConnectionError('');if(clearError)setError('');}catch(e){if(epoch===inventoryEpoch.current)setConnectionError(String(e instanceof Error?e.message:e));}}),[request]);
 useEffect(()=>{
   if(!showEnded&&sessions.some(s=>s.id===selected&&s.state==='exited')){
     setSelected(sessions.filter(s=>s.state==='running').at(-1)?.id??'');setError('');
   }
 },[sessions,selected,showEnded]);
 useEffect(()=>{if(!visible)return;void refresh();if(!remote){void refreshConnections();setTargetReload(value=>value+1)};const timer=!remote?setInterval(()=>void refreshConnections(),5000):undefined;return()=>clearInterval(timer);},[visible,refresh,remote,refreshConnections]);
 ...
 useEffect(()=>{
  ...
  let stopped=false,inFlight=false,wakePending=false;let timer:ReturnType<typeof setTimeout>;let releaseWrite:(()=>void)|undefined;let nextDelay=remote?700:120;let failures=0,idleReads=0;
  const poll=async()=>{
   ...
   try {const r=await request({operation:'read',sessionId:selected,after:cursor.current});
    ...
    idleReads=chunks.length?0:Math.min(5,idleReads+1);
    nextDelay=r.hasMore?0:Math.min(remote?5000:2000,(remote?700:120)*2**idleReads);
  ...
  };wakeRead.current=()=>{if(stopped)return;if(inFlight){wakePending=true;return;}clearTimeout(timer);timer=setTimeout(poll,0)};void poll();return()=>{stopped=true;wakeRead.current=null;clearTimeout(timer);releaseWrite?.()};
 },[selected,activeSession?.id,visible,request,remote]);
 const start=async()=>{if(!canStart)return;inventoryEpoch.current++;setBusy(true);setError('');try{const r=await request({operation:'start',targetId:target,agent,cols:100,rows:28,...(prompt.trim()?{prompt}: {})});if(r.session){inventoryEpoch.current++;setSessions(s=>[...s.filter(x=>x.id!==r.session!.id),r.session!]);setSelected(r.session.id);setPrompt('');setRequestTitle('');setComposerOpen(false);}}catch(e){setError(e instanceof Error?e.message:String(e));}finally{inventoryEpoch.current++;setBusy(false)}};
 const close=async()=>{
 ...
 const runningSessions=sessions.filter(s=>s.state==='running'), endedSessions=sessions.filter(s=>s.state==='exited');
 const visibleSessions=showEnded?sessions:runningSessions;
 ...
 const reviewMemory=(targetId:string)=>{setTarget(targetId);setSelected('');setAgent('codex');setComposerOpen(true);setFocusRequest(value=>value+1);setRequestTitle('세션 저장 확인');setPrompt('이 프로젝트의 워크룸 종료 후 세션 기억 저장 또는 백업이 완료되지 않았습니다. 실제 저장소 루트와 장기기억 상태를 읽고 원인을 확인하세요. 기존 기억과 Git 변경을 보존하고, 충돌은 양쪽 근거를 비교하여 초안을 제시하세요. 임의 force push, reset, 기억 덮어쓰기나 저장 재시도는 하지 말고 필요한 조치를 먼저 설명하세요.');};
 return <section className="ai-terminal-panel" data-testid="ai-terminal-panel">
  <header className="ai-terminal-head">
   ...
   <div className="ai-terminal-context-actions">
    ...
    {!remote&&onWhatISaid&&<button className="ai-terminal-btn" onClick={onWhatISaid}>내가 한 말</button>}
   </div>
  ...
  {(!activeSession||composerOpen)&&<div className="ai-terminal-prompt" data-testid="workroom-composer">
   ...
   <textarea ref={promptField} id={promptId} aria-label="터미널에 전달할 작업" value={prompt} disabled={busy} onChange={e=>setPrompt(e.target.value)} placeholder="작업을 적으면 선택한 AI에 전달합니다. 비워 두고 터미널만 열어도 됩니다." className="ai-terminal-textarea"/>
  ...
  <div className="ai-terminal-tabs" role="tablist" aria-label="터미널 세션">{visibleSessions.map(s=><button role="tab" aria-selected={s.id===selected} key={s.id} onClick={()=>{setError('');setSelected(s.id)}} className={`ai-terminal-tab${s.state==='running'?'':' ai-terminal-tab--ended'}`}><span className={`ai-terminal-dot${s.state==='running'?' ai-terminal-dot--running':''}`} aria-hidden="true"/>{projects.find(p=>p.targetId===s.targetId)?.label??'프로젝트'} · {s.agent}<span className="ai-terminal-tab-state">{s.state==='running'?'실행 중':`종료 ${s.exitCode??''}`}</span></button>)}</div></aside>
  <div className="ai-terminal-body">
   {activeSession?<>
   <div className="ai-terminal-current">{activeProject?.label??'프로젝트'} · {activeSession.agent}</div>
   {activeSession.state==='exited'&&<p className="ai-terminal-hint">종료된 세션의 기록입니다.</p>}
  ...
  {!remote&&connections.length>0&&<details className="ai-terminal-access"><summary>원격 연결의 터미널 접근</summary><p>허용하면 이 연결에서 CLI 입력과 터미널 출력에 접근합니다. 화면에 표시되는 경로·계정 정보도 전송될 수 있습니다. 앱 재시작 시 다시 허용해야 합니다.</p>{connections.map(c=><label key={c.id}><input type="checkbox" checked={c.allowed} onChange={async e=>{try{const r=await terminalLocalRequest(AI_TERMINAL_PREFIX+'/access',{owner:c.id,enabled:e.target.checked});setConnections(r.connections)}catch(e){setError(String(e))}}}/>{c.label}</label>)}</details>}
 </section>;
}
