...
import {localAiTerminalTransport,localWorkroomTransport,terminalLocalRequest,type AiTerminalTransport} from './aiTerminalClient';
...
export function AiTerminalPanel({visible=true,projects:suppliedProjects,transport=localAiTerminalTransport,remote=false,entry,onManageProject,onWhatISaid,onRememberSession,sessionScope='local',workspaceTransport}: {
 workspaceTransport?:WorkroomTransport;visible?:boolean;projects:{targetId:string;label:string}[];transport?:AiTerminalTransport;remote?:boolean;entry?:AiTerminalEntry|null;onManageProject?:(id:string)=>void;onWhatISaid?:()=>void;onRememberSession?:(id:string)=>void;sessionScope?:string;
}) {
 ...
 const refresh=useMemo(()=>singleFlight(async(clearError=true)=>{const epoch=inventoryEpoch.current;try{const r=await request({operation:'list'});if(epoch!==inventoryEpoch.current)return;setSessions(old=>reconcileTerminalSummaries(old,r.sessions??[]));
 ...
 useEffect(()=>{
  ...
  const contextListener=t.onWriteParsed(()=>{
    ...
    const b=t.buffer.active,lines:string[]=[];for(let i=Math.max(b.baseY,b.length-4);i<b.length;i++)lines.push(b.getLine(i)?.translateToString(true)??'');
  ...
  return()=>{if(revealFrame!==undefined)cancelAnimationFrame(revealFrame);sendInput.current=null;flushInput.current=null;flush();input.dispose();contextListener.dispose();resize.disconnect();t.dispose();terminal.current=null;};
 ...
 return <section className="ai-terminal-panel" data-testid="ai-terminal-panel">
  ...
  <div className="ai-terminal-body">
   ...
   <div className="ai-terminal-keys">{remote&&<button className="ai-terminal-key" disabled={activeSession.state!=='running'||busy} onClick={()=>terminal.current?.focus()}>키보드</button>}{['1','2','3','Esc','Tab','↑','↓','Ctrl+C','Ctrl+L','Enter'].map(key=><button disabled={activeSession.state!=='running'||busy} key={key} className="ai-terminal-key" onClick={()=>sendInput.current?.(({'1':'1','2':'2','3':'3','Esc':'\x1b','Tab':'\t','↑':terminal.current?.modes.applicationCursorKeysMode?'\x1bOA':'\x1b[A','↓':terminal.current?.modes.applicationCursorKeysMode?'\x1bOB':'\x1b[B','Ctrl+C':'\x03','Ctrl+L':'\x0c','Enter':'\r'} as Record<string,string>)[key]!)}>{key}</button>)}</div>
   <WorkroomSessionFooter key={sessionScope+':'+activeSession.id} session={activeSession} visible={visible} transport={memoryTransport} contextUsed={contextUsed} scope={sessionScope} onClose={close} onPause={pauseForSave}/>
