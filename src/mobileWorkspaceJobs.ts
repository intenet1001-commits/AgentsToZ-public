...
type Receipt={schema:1;id:string;target:string;at:number;completedAt?:number;binding?:number;evidence?:string;result:Outcome};
...
export class MobileWorkspaceJobs {
  ...
  private read(id:string):Receipt|null {
    ...
    if((r.evidence!==undefined&&(typeof r.evidence!=='string'||!/^[a-f0-9]{64}$/.test(r.evidence)))||(r.binding!==undefined&&(!Number.isSafeInteger(r.binding)||r.binding<0))||(r.completedAt!==undefined&&(!Number.isFinite(r.completedAt)||r.completedAt<r.at))||r.schema!==1||r.id!==id||!/^\w{64}$/.test(r.target)||!Number.isFinite(r.at)||!r.result||typeof r.result.state!=='string'||typeof r.result.message!=='string'||typeof r.result.localSaved!=='boolean'||typeof r.result.backupSaved!=='boolean')throw new Error('저장 작업 기록을 확인하세요.');
    return r;
  }
  ...
  receipt(owner:string,targetId:string,requestId:string) {
    const row=this.read(digest(JSON.stringify([owner,requestId])));
    if(!row||row.target!==digest(JSON.stringify([owner,targetId])))return null;
    return {result:this.outcome(row),completedAt:row.completedAt??null,binding:row.binding??null,evidence:row.evidence??null};
  }
  async start(owner:string,targetId:string,requestId:string,run:()=>Promise<Outcome>,binding?:number,evidence?:string):Promise<Outcome>{
    mkdirSync(this.directory,{recursive:true,mode:0o700});
    ...
    return withOwnedPortalFileLock(join(this.directory,'admission.lock'),()=>{
      ...
      const receipt:Receipt={schema:1,id,target,at:Date.now(),...(binding===undefined?{}:{binding}),...(evidence===undefined?{}:{evidence}),result:{state:'saving',localSaved:false,backupSaved:false,message:'세션 기억 저장 중입니다. 화면을 이동해도 작업은 계속됩니다.'}};
      writePrivateFileAtomically(this.directory,join(this.directory,id+'.json'),JSON.stringify(receipt));
      this.live.add(id);
      void Promise.resolve().then(run).then(result=>{receipt.result=result;receipt.completedAt=Date.now();writePrivateFileAtomically(this.directory,join(this.directory,id+'.json'),JSON.stringify(receipt));}).catch(()=>{
