...
/** Lexical UI validation; the execution prompt also requires realpath and volume checks. */
export function projectFolderMoveProblem(currentFolderPath: string, targetFolderPath: string): string | null {
  const current = withoutTrailingSeparators(currentFolderPath.trim());
  const target = withoutTrailingSeparators(targetFolderPath.trim());
  if (!target) return '이동할 전체 경로를 입력해주세요.';
  if (/[\u0000-\u001f\u007f]/.test(target)) return '경로에 제어 문자를 사용할 수 없습니다.';
  const windows = /^[a-z]:[\\/]/i.test(current) || current.startsWith('\\\\');
  if (windows ? !(/^[a-z]:[\\/]/i.test(target) || /^\\\\[^\\]+\\[^\\]+\\.+/.test(target)) : !target.startsWith('/'))
    return '현재 프로젝트와 같은 운영체제 형식의 절대경로를 입력해주세요.';
  const parts = target.split(windows ? /[\\/]/ : /\//);
  if (parts.some(part => part === '.' || part === '..')) return '`.` 또는 `..` 없이 전체 경로를 입력해주세요.';
  if (windows && parts.slice(target.startsWith('\\\\') ? 2 : 1).some(part => /[<>:"|?*]|[. ]$/.test(part))) return 'Windows에서 사용할 수 없는 경로입니다.';
  const normalize = (value: string) => windows ? value.replace(/\\/g, '/').toLowerCase().replace(/\/+/g, '/') : value.replace(/\/+/g, '/');
  const a = normalize(current), b = normalize(target);
  if (a === b) return '현재 폴더 경로와 같습니다.';
  if (b.startsWith(a + '/') || a.startsWith(b + '/')) return '현재 폴더의 상위 또는 하위 폴더로 이동할 수 없습니다.';
  if (windows && (/^[a-z]:$/i.test(target) || (target.startsWith('\\\\') && parts.filter(Boolean).length < 3))) return '드라이브나 공유 루트로 이동할 수 없습니다.';
  return null;
}
...
export function buildProjectFolderRenamePrompt(input: ProjectFolderRenamePromptInput): string {
  ...
  const moving = input.targetFolderPath !== undefined;
  const problem = moving ? projectFolderMoveProblem(currentFolderPath, input.targetFolderPath!) : projectFolderNameProblem(currentFolderPath, newFolderName);
  if (problem) throw new Error(problem);
  const targetFolderPath = moving ? withoutTrailingSeparators(input.targetFolderPath!.trim()) : renamedProjectFolderPath(currentFolderPath, newFolderName);
  ...
  return `아래 프로젝트의 **${moving ? '폴더 위치(필요하면 이름도)' : '폴더 이름만'}** 충돌과 경로 유실 없이 바꿔줘. Claude Code, Codex 등 어떤 코딩 에이전트에서도 실행 가능한 절차로 처리해.
...
1. 현재 경로가 실제 디렉터리이고 대상 경로가 존재하지 않는지 확인해. ${moving ? '대상 부모 폴더가 이미 존재하고 쓰기 가능하며, 두 경로의 실제 경로·심볼릭 링크를 해석해 서로의 상위·하위가 아닌지 확인해. 같은 파일시스템에서만 이동하고, 다른 볼륨이나 EXDEV이면 중단해. 복사 후 원본 삭제로 우회하지 마.' : '두 경로의 부모가 같아 **이름만 변경**되는지도 확인해.'}
...
4. 이 요청을 수행하는 워크룸/AI 세션도 현재 폴더를 사용 중일 수 있다. 자기 세션을 종료하거나 활성 cwd를 옮긴 채 진행하지 마. 먼저 변경 계획을 준비하고 프로젝트 외부의 검증된 실행 위치로 인계할 수 있을 때만 이동해. 인계할 수 없으면 준비 상태와 다음 절차를 보고해. 현재 경로를 cwd로 쓰는 개발 서버·터미널·AI 세션이 있으면 안전하게 종료 가능한 것만 종료해. 소유자를 판단할 수 없는 프로세스는 강제 종료하지 말고 중단해.
...
2. \`.agent-memory\`와 기존 기억 ID를 포함한 전체 프로젝트를 보존해. 새 기억을 초기화하거나 별도 프로젝트로 등록하지 마. 즉시 대상 경로로 작업 디렉터리를 다시 잡아. 옛 경로에서 후속 명령을 실행하지 마.
