...
export type RemoteControlMutationAction = RemoteControlProjectAction | 'project.create' | 'ops.open' | 'ops.memory.review';
...
export interface RemoteControlOpsDto {
  state: 'unconfigured' | 'preparing' | 'ready' | 'needs-attention';
  backend: 'app-data' | 'control-folder' | null;
  pendingCount: number;
  lastSavedAt: string | null;
  syncState: 'current' | 'needs-attention' | 'not-configured' | null;
}

export interface RemoteControlOpsAction {
  action: 'ops.open';
  actionId: string;
  authority: RemoteControlActionAuthority;
}

export interface RemoteControlOpsCandidateDto {
  id: string;
  title: string;
  body: string;
  evidence: string;
...
export interface RemoteControlOpsCandidatesDto {
  revision: string;
  candidates: RemoteControlOpsCandidateDto[];
}

export interface RemoteControlOpsReviewAction {
  action: 'ops.memory.review';
  actionId: string;
  candidateId: string;
...
export interface RemoteControlGateway {
  ...
  readOpsStatus?(): RemoteControlOpsDto | Promise<RemoteControlOpsDto>;
  executeOpsAction?(action: RemoteControlOpsAction): void | Promise<void>;
  listOpsMemoryCandidates?(): RemoteControlOpsCandidatesDto | Promise<RemoteControlOpsCandidatesDto>;
  reviewOpsMemoryCandidate?(action: RemoteControlOpsReviewAction): void | Promise<void>;
}
...
export type RemoteControlActionResult =
  ...
  | {
      ...
      actionId: string;
      ok: true;
      ops: RemoteControlOpsDto;
    }
  | {
      type: 'action.result';
      actionId: string;
      ok: true;
      opsCandidates: RemoteControlOpsCandidatesDto;
    }
  | {
      type: 'action.result';
      actionId: string;
...
export function parseRemoteControlClientMessage(value: unknown): RemoteControlClientMessage {
  ...
  if (!hasExactKeys(
    ...
    ['controlId', 'remoteConfirmed', 'page', 'input', 'workspaceRootId', 'candidateId', 'expectedRevision', 'accept'],
  )) {
  ...
  if (action === 'projects.list') {
    if (value.controlId !== undefined || value.remoteConfirmed !== undefined
      ...
      || value.candidateId !== undefined || value.expectedRevision !== undefined || value.accept !== undefined
    ...
    if (value.controlId !== undefined || value.page !== undefined
      || value.input !== undefined || value.workspaceRootId !== undefined
      || value.candidateId !== undefined || value.expectedRevision !== undefined || value.accept !== undefined
      || (value.remoteConfirmed !== undefined && value.remoteConfirmed !== true)) {
      throw new RemoteControlError('INVALID_ACTION', 'OPS 열기에는 휴대폰 확인만 사용할 수 있습니다.');
    }
  } else if (action === 'ops.memory.review') {
    if (value.controlId !== undefined || value.page !== undefined || value.input !== undefined || value.workspaceRootId !== undefined
      || value.remoteConfirmed !== true || typeof value.accept !== 'boolean'
      || typeof value.candidateId !== 'string' || !INTERNAL_ID_RE.test(value.candidateId)
      || typeof value.expectedRevision !== 'string' || !/^[a-f0-9]{64}$/.test(value.expectedRevision)) {
      throw new RemoteControlError('INVALID_ACTION', 'OPS 기억 검토에는 후보·기억 기준·사람 확인이 필요합니다.');
    }
  } else if (action === 'workspace-roots.list' || action === 'protocol.capabilities' || action === 'ops.status' || action === 'ops.memory.pending') {
    if (value.controlId !== undefined || value.remoteConfirmed !== undefined
      ...
      throw new RemoteControlError(
        ...
        action === 'protocol.capabilities'
          ...
          : action === 'ops.status'
            ? 'OPS 상태 조회에는 추가 입력을 사용할 수 없습니다.'
          : action === 'ops.memory.pending'
            ? 'OPS 기억 후보 조회에는 추가 입력을 사용할 수 없습니다.'
    ...
    if (value.candidateId !== undefined || value.expectedRevision !== undefined || value.accept !== undefined) throw new RemoteControlError('INVALID_ACTION', '프로젝트 기능 요청에는 OPS 기억 검토 값을 사용할 수 없습니다.');
...
export class RemoteControlCore {
  ...
  perform(request: RemoteControlActionRequest, authority?: RemoteControlActionAuthority): Promise<RemoteControlActionResult> {
    ...
    const fingerprint = `${request.action}\u0000${request.controlId ?? ''}\u0000${request.remoteConfirmed === true ? '1' : '0'}\u0000${request.page ?? ''}\u0000${request.input ?? ''}\u0000${request.workspaceRootId ?? ''}\u0000${request.candidateId ?? ''}\u0000${request.expectedRevision ?? ''}\u0000${request.accept === undefined ? '' : request.accept ? '1' : '0'}`;
  ...
  async #execute(
    ...
    try {
      ...
      if (request.action === 'ops.status') {
        if (!this.#gateway.readOpsStatus) {
          throw new RemoteControlError('ACTION_NOT_AVAILABLE', '이 설치본에서는 원격 OPS 상태를 사용할 수 없습니다.', 409);
        }
        return {type: 'action.result', actionId: request.actionId, ok: true, ops: await this.#gateway.readOpsStatus()};
      }
      if (request.action === 'ops.memory.pending') {
        if (!this.#gateway.listOpsMemoryCandidates) {
          throw new RemoteControlError('ACTION_NOT_AVAILABLE', '이 설치본에서는 OPS 기억 후보 조회를 사용할 수 없습니다.', 409);
        }
        return {type: 'action.result', actionId: request.actionId, ok: true, opsCandidates: await this.#gateway.listOpsMemoryCandidates()};
      }
      if (request.action === 'ops.memory.review') {
        if (!this.#gateway.reviewOpsMemoryCandidate || !this.#gateway.readOpsStatus) {
          throw new RemoteControlError('ACTION_NOT_AVAILABLE', '이 설치본에서는 원격 OPS 기억 검토를 사용할 수 없습니다.', 409);
        }
        if (!authority?.controllerId.startsWith('internet:')) {
          throw new RemoteControlError('OPS_REVIEW_SAS_REQUIRED', '공유 OPS 기억 저장 승인은 SAS 승인된 개인 포털에서만 할 수 있습니다.', 403);
        }
        await this.#gateway.reviewOpsMemoryCandidate({action:request.action,actionId:request.actionId,candidateId:request.candidateId!,expectedRevision:request.expectedRevision!,accept:request.accept!,authority});
        return {type:'action.result',actionId:request.actionId,ok:true,ops:await this.#gateway.readOpsStatus()};
      }
      if (request.action === 'ops.open') {
        if (!this.#gateway.executeOpsAction || !this.#gateway.readOpsStatus) {
          throw new RemoteControlError('ACTION_NOT_AVAILABLE', '이 설치본에서는 원격 OPS 열기를 사용할 수 없습니다.', 409);
        }
        const resolvedAuthority = authority ?? {
          controllerId: `lan:${createHash('sha256').update(`agentstoz-lan-pairing-v1:${session.sessionToken}`).digest('hex')}`,
          expiresAt: new Date(session.createdAt + this.#absoluteTtlMs).toISOString(),
        };
        await this.#gateway.executeOpsAction({action: 'ops.open', actionId: request.actionId, authority: resolvedAuthority});
        return {type: 'action.result', actionId: request.actionId, ok: true, ops: await this.#gateway.readOpsStatus()};
      }
