...
export interface RemoteControlGateway {
  ...
  createProject?(action: RemoteControlCreateProjectAction): void | Promise<void>;
}
...
export type RemoteControlClientMessage = RemoteControlPairRequest | RemoteControlRestoreRequest | RemoteControlActionRequest;
...
export class RemoteControlCore {
  ...
  perform(request: RemoteControlActionRequest): Promise<RemoteControlActionResult> {
  ...
  async #execute(session: SessionState, request: RemoteControlActionRequest): Promise<RemoteControlActionResult> {
    try {
    ...
    for (const project of registered) {
      ...
      if (!controlId) {
        ...
        for (let attempt = 0; attempt < 8; attempt += 1) {
          const candidate = opaqueToken(this.#randomBytes);
  ...
  #assertSessionStillActive(session: SessionState): void {
    if (!this.#enabled || this.#sessions.get(session.sessionToken) !== session) {
