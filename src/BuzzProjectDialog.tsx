...
function StatusDot({ ok, label }: { ok: boolean | null; label: string }) {
  const color = ok === true ? "text-emerald-300 border-emerald-400/25 bg-emerald-400/8"
    : ok === false ? "text-amber-300 border-amber-400/25 bg-amber-400/8"
      : "text-zinc-400 border-[rgb(var(--surface-highlight-rgb))]/10 bg-[rgb(var(--surface-highlight-rgb))]/[0.03]";
  return (
...
export function BuzzProjectDialog({ portId, projectName, onClose, onToast }: BuzzProjectDialogProps) {
  ...
  const content = (
    <div className="fixed inset-0 z-[10080] flex items-center justify-center bg-[var(--dialog-scrim)] p-4 backdrop-blur-sm" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose(); }}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label={`${projectName} Buzz 채널 연결`} className="max-h-[calc(var(--ui-viewport-height,100dvh)*0.90)] w-full max-w-2xl overflow-y-auto rounded-2xl border border-cyan-300/15 bg-[var(--bg-elevated)] shadow-[var(--dialog-shadow)]">
        <div className="sticky top-0 z-10 flex items-start justify-between border-b border-[rgb(var(--surface-highlight-rgb))]/[0.07] bg-[rgb(var(--bg-elevated-rgb))]/95 px-5 py-4 backdrop-blur">
          ...
          <button ref={closeButtonRef} type="button" onClick={onClose} disabled={!!busy} aria-label="닫기" className="min-h-11 min-w-11 rounded-lg p-2 text-zinc-400 hover:bg-[rgb(var(--surface-highlight-rgb))]/5 hover:text-zinc-200 disabled:opacity-40"><X className="h-4 w-4" /></button>
        </div>
          ...
          {loading && !status ? (
            ...
            <>
              {status && (
                <div className="rounded-xl border border-[rgb(var(--surface-highlight-rgb))]/[0.07] bg-[rgb(var(--surface-highlight-rgb))]/[0.02] p-3">
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button type="button" onClick={() => void refresh(relayUrl)} disabled={loading || !!busy} className="ml-auto inline-flex items-center gap-1 rounded-md border border-[rgb(var(--surface-highlight-rgb))]/10 px-2 py-1 text-[10px] text-zinc-400 hover:text-zinc-100 disabled:opacity-40"><RefreshCw className={`h-3 w-3 ${loading ? "animate-spin" : ""}`} />다시 확인</button>
                  </div>
              ...
              {status?.binding ? (
                <div className="space-y-3 rounded-xl border border-emerald-300/15 bg-emerald-300/[0.035] p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button
                      ...
                      disabled={!!busy}
                      className={`inline-flex min-h-11 items-center gap-1.5 rounded-lg border px-3 py-2 text-xs disabled:opacity-40 ${confirmUnlink ? "border-red-300/30 bg-red-300/[0.07] text-red-200" : "border-[rgb(var(--surface-highlight-rgb))]/10 text-zinc-400 hover:border-red-300/20 hover:text-red-300"}`}
                    >
                ...
                <div className="space-y-3 rounded-xl border border-[rgb(var(--surface-highlight-rgb))]/[0.07] bg-[rgb(var(--surface-highlight-rgb))]/[0.02] p-4">
                  <div role="tablist" aria-label="Buzz 채널 연결 방식" className="grid grid-cols-2 gap-1 rounded-lg bg-[rgb(var(--surface-shade-rgb))]/25 p-1">
                  ...
                  <div>
                    ...
                    <div className="mt-1 flex gap-2">
                      <input id="buzz-relay-url" value={relayUrl} onChange={event => setRelayUrl(event.target.value)} spellCheck={false} className="min-h-11 min-w-0 flex-1 rounded-lg border border-[rgb(var(--surface-highlight-rgb))]/10 bg-[rgb(var(--surface-shade-rgb))]/25 px-3 py-2 font-mono text-xs text-zinc-200 outline-none focus:border-cyan-300/35" />
                      <button type="button" onClick={() => void saveRelay()} disabled={!!busy || !relayUrl.trim()} className="min-h-11 shrink-0 rounded-lg border border-[rgb(var(--surface-highlight-rgb))]/10 px-3 text-xs text-zinc-300 hover:text-[var(--text-primary)] disabled:opacity-40">기본값 저장</button>
                    </div>
                  ...
                  {mode === "create" ? (
                    <>
                      <label className="block text-[11px] text-zinc-400">채널 이름
                        <input value={channelName} onChange={event => setChannelName(event.target.value)} maxLength={64} className="mt-1 w-full rounded-lg border border-[rgb(var(--surface-highlight-rgb))]/10 bg-[rgb(var(--surface-shade-rgb))]/25 px-3 py-2 text-xs text-zinc-200 outline-none focus:border-cyan-300/35" />
                      </label>
                  ...
                  <button data-testid="buzz-open-app" type="button" onClick={() => void run("open")} disabled={!!busy} className="inline-flex w-full items-center justify-center gap-1.5 rounded-lg border border-[rgb(var(--surface-highlight-rgb))]/10 px-3 py-2 text-xs text-zinc-400 hover:text-zinc-100 disabled:opacity-40"><ExternalLink className="h-3.5 w-3.5" />Buzz 앱 먼저 열기</button>
                </div>
              )}
              ...
              {status && (
                <div className="rounded-xl border border-violet-300/10 bg-violet-300/[0.025] p-3 text-[10px] leading-4 text-zinc-500">
                  ...
                  <p className="mb-0 mt-2">{status.projectContext.memorySkillsReady ? "프로젝트 장기기억·remember-session 스킬이 준비되어 있습니다." : "프로젝트 로컬 장기기억 스킬을 먼저 업데이트해야 합니다."} 이 화면은 Buzz 채널을 만들거나 기존 채널과 프로젝트를 연결하는 역할만 합니다. 서비스 Agent 생성은 장기기억 탭에서 별도로 진행합니다.</p>
                </div>
              )}
            </>
          )}
