...
/**
 * Finds the most recently active ordinary Codex Desktop task that is already
 * applied to the exact local project. `threadUpdatedAt` comes from ChatGPT's
 * compact session index and is a better recency signal than rollout mtime when
 * an older task was revisited. Voice tasks stay on their separate route.
 */
export function selectLatestProjectCodexThread(
  folderPath: string,
  candidates: readonly ProjectCodexVoiceCandidate[],
  metadata: ReadonlyMap<string, ContextSessionMetadata>,
  threadUpdatedAt: ReadonlyMap<string, number> = new Map(),
): string | null {
  let selected: { candidate: ProjectCodexVoiceCandidate; activity: number } | null = null;
  ...
  for (const candidate of candidates) {
    ...
    const assignedPath = projectHint.assignedPath ?? projectHint.path;
    if (!sameWorkspacePath(assignedPath, folderPath)) continue;
    if (projectHint.appliedPath && !sameWorkspacePath(projectHint.appliedPath, folderPath)) continue;

    const indexedActivity = threadUpdatedAt.get(candidate.sessionId);
    const activity = Number.isFinite(indexedActivity)
      ? Number(indexedActivity)
      : candidate.modifiedAtMs;
    if (!selected
      || activity > selected.activity
      || (activity === selected.activity && candidate.sessionId > selected.candidate.sessionId)) {
      selected = { candidate, activity };
    }
  }

  return selected?.candidate.sessionId ?? null;
}
