...
function writePrivateFileAtomically(appDataDir: string, path: string, content: string): void {
  ...
  try {
    ...
    if (!stat.isFile()
      ...
      throw new Error('REMOTE_CONTROL_HOST_RECORD_TEMP_UNSAFE');
    }
...
function unlinkPrivateFileStrict(appDataDir: string, path: string): boolean {
  let removed = false;
  try {
    unlinkSync(path);
    removed = true;
  ...
  if (pathEntryExists(path)) throw new Error('REMOTE_CONTROL_HOST_RECORD_DELETE_FAILED');
