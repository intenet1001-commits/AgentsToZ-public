...
function comparableCanonicalPath(path: string): string {
  const withoutMacPrivatePrefix = process.platform === "darwin" && path.startsWith("/private/") ? path.slice("/private".length) : path;
  return process.platform === "win32" ? withoutMacPrivatePrefix.replaceAll("/", "\\").toLowerCase() : withoutMacPrivatePrefix;
}
...
export function verifyAgentsToZProjectBinding(input: {
  ...
  try {
    ...
    if (!statSync(path).isDirectory() || comparableCanonicalPath(resolvedPath) !== comparableCanonicalPath(path)) {
  ...
  try {
    ...
    if (comparableCanonicalPath(gitRootRaw) !== comparableCanonicalPath(canonicalPath)) {
...
export function commitApprovedAgentsToZProject(input: {
  ...
  try {
    ...
    if (!statSync(path).isDirectory() || comparableCanonicalPath(resolved) !== comparableCanonicalPath(path)) return { ok: false, code: "PATH_INVALID" };
    const gitRootRaw = execFileSync("git", ["-C", path, "rev-parse", "--show-toplevel"], { encoding: "utf8" }).trim();
    if (comparableCanonicalPath(gitRootRaw) !== comparableCanonicalPath(path)) return { ok: false, code: "PATH_INVALID" };
...
export function planAgentsToZProjectWorker(input: {
  ...
  if (!/^[A-Za-z0-9_-]{1,64}$/.test(workerProfile) || workerProfile === "cs-ceo" || workerProfile === "cs_ceo") {
    return {ok:false,code:"WORKER_PROFILE_REQUIRED",error:"CS-CEO 자동 worker 연동은 종료되었습니다. 개발 작업은 워크룸에서 명시적으로 시작하세요."};
  }
  const prompt = [
