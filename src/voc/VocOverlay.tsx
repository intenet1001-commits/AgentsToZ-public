...
interface VocOverlayProps {
  onClose: () => void;
  onSubmit: (input: { anchor: VocAnchor; comment: string }) => Promise<boolean>;
}
 ...
 */
export function VocOverlay({ onClose, onSubmit }: VocOverlayProps) {
  ...
  const handleMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (start) {
      ...
      if (far || drag) {
        setDrag({ x0: start.x, y0: start.y, x1: e.clientX, y1: e.clientY });
        setHoverRect(null);
        return;
      }
    }
  ...
  const submit = useCallback(async () => {
    ...
    setSaving(true);
    const ok = await onSubmit({ anchor: picked.anchor, comment: comment.trim() });
    setSaving(false);
    // 실패하면 쓴 글을 지우지 않는다 — 다시 쓰게 만드는 것이 가장 나쁜 실패다.
    if (ok) reset();
  }, [picked, comment, saving, onSubmit, reset]);
  ...
  }, [picked, reset, onClose, submit]);

  const rect = picked?.rect ?? (drag ? normalizeBox(drag) : null) ?? hoverRect;
  const boxTop = rect ? Math.min(rect.top + rect.height + 10, window.innerHeight - 210) : 120;
  const boxLeft = rect ? Math.min(Math.max(rect.left, 12), window.innerWidth - 372) : 12;
    ...
    <>
      ...
      {picked && (
        <div
          ...
          style={{
            position: 'fixed', top: boxTop, left: boxLeft, width: 360, zIndex: GUIDE_Z.tooltip,
