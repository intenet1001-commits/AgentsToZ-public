...
export function parseAgentsToZUseCodeAppMode(agent: unknown, mode: unknown): "reopen" | "prepare" | undefined {
  if (mode === undefined) return undefined;
  if (agent !== "codex" || (mode !== "reopen" && mode !== "prepare")) {
    throw new AgentsToZUseControlError("Codex 앱 mode는 reopen 또는 prepare여야 합니다.", "AGENTSTOZ_USE_CODE_APP_MODE_INVALID");
  }
  return mode;
}

const OPS_SURFACE_ACTIONS: readonly string[] = [
  'open-dashboard', 'open-code-app', 'list-workroom-sessions', 'read-workroom-session',
  'start-workroom-session', 'send-workroom-instruction', 'connect-buzz-channel', 'open-buzz-dev',
];
...
export function parseAgentsToZUseSurfaceTarget(action: string, body: Record<string, unknown>): {target?: 'ops'; portId?: string} {
  ...
  return action === 'open-dashboard' ? {} : {portId: requiredId(body.portId, 'portId')};
}

export function parseAgentsToZUseLaunchOptions(body: Record<string, unknown>): {surface?: AgentsToZUseSurface; bypass?: boolean; mode?: 'reopen' | 'prepare'} {
  ...
  if (!['codex', 'claude', 'hermes', ...(orca ? ['agy'] : [])].includes(body.agent as string)) {
  ...
  return {
    ...
    ...(body.mode !== undefined ? {mode: parseAgentsToZUseCodeAppMode(body.agent, body.mode)} : {}),
  };
}
...
export function parseAgentsToZUseActionRequest(input: unknown): AgentsToZUseActionRequest {
  ...
  if (action !== 'open-code-app' && (body.surface !== undefined || body.bypass !== undefined || body.mode !== undefined)) {
  ...
  return { ...(overview?{testerOverview:overview}:{}), ...(tester?{tester}:{}), ...(controlQuery ? {controlQuery} : {}), ...(controlProposal ? {controlProposal} : {}), action, controllerPortId, portId, agent, projectName, workspaceRootId, channelId, channelName, visibility, archiveMemory, sessionId, after, requestId, instruction,
