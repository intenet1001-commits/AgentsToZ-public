...
export const WORKSPACE_LEASE_DIRECTORY = 'workspace-leases-v1';
...
/**
 * Every lock part this process currently owns. Shutdown waits for this set to
 * empty so an in-flight writer's `finally` can release before process.exit.
 * Exiting first is what left `manual` leases orphaned after app updates.
 */
const heldLeaseParts = new Set<OwnedWorkspaceLeasePart>();
const heldLeaseDrainWaiters = new Set<() => void>();

function forgetHeldLeasePart(part: OwnedWorkspaceLeasePart): void {
  if (!heldLeaseParts.delete(part) || heldLeaseParts.size > 0) return;
  for (const notify of [...heldLeaseDrainWaiters]) notify();
}

export function heldWorkspaceLeaseLockCount(): number {
  return heldLeaseParts.size;
}

/** Resolves true once no lease part is held, or false when the deadline passes first. */
export function waitForHeldWorkspaceLeases(timeoutMs: number): Promise<boolean> {
  if (heldLeaseParts.size === 0) return Promise.resolve(true);
  if (!(timeoutMs > 0)) return Promise.resolve(false);
  return new Promise(resolve => {
    const settle = (drained: boolean) => {
      clearTimeout(timer);
      heldLeaseDrainWaiters.delete(onDrained);
      resolve(drained);
    };
    const onDrained = () => settle(true);
    const timer = setTimeout(() => settle(false), timeoutMs);
    heldLeaseDrainWaiters.add(onDrained);
  });
}
...
function gitFamilyIdentitySource(canonicalCommonDirectory: string): string {
  return `git-family-v1\0${canonicalCommonDirectory}`;
}

/** Git's own markers: a `.git` directory, or a linked worktree's `gitdir:` file plus `commondir`. */
function gitCommonDirectoryFromMarkers(startDirectory: string): string | null {
  let cursor = startDirectory;
  while (true) {
    ...
    if (entry) {
      try {
        if (entry.isDirectory()) return realpathSync(marker);
        if (!entry.isFile()) return null;
        const gitDirectory = /^gitdir:\s*(.+?)\s*$/m.exec(readFileSync(marker, 'utf8'))?.[1];
        if (!gitDirectory) return null;
        const canonicalGitDirectory = realpathSync(resolve(cursor, gitDirectory));
        try {
          const common = readFileSync(join(canonicalGitDirectory, 'commondir'), 'utf8').trim();
          return realpathSync(resolve(canonicalGitDirectory, common));
        } catch {
...
/**
 * Display-only mapping from a registered folder to the lock files that would
 * guard it. Reads Git's markers instead of spawning Git because recovery walks
 * every registered project; a wrong answer only labels a lock "unknown" and
 * never grants deletion.
 */
export function workspaceLeaseKeysForPath(workspacePath: string): {
  ...
  try {
    canonicalWorkspacePath = canonicalExistingDirectory(workspacePath, 'workspace');
  } catch {
    return null;
  }
  const commonDirectory = gitCommonDirectoryFromMarkers(canonicalWorkspacePath);
  return {
    canonicalWorkspacePath,
    directoryKey: directoryWorkspaceIdentity(canonicalWorkspacePath).identity.key,
    familyKey: commonDirectory === null
      ? null
      : createHash('sha256').update(gitFamilyIdentitySource(commonDirectory)).digest('hex'),
  };
}
...
function resolveGitFamilyIdentity(
  ...
  if (probe.status === 0 && !probe.error) {
    ...
    const identitySource = gitFamilyIdentitySource(canonicalCommonDirectory);
    return {
      identitySource,
      identity: {
        kind: 'git-family',
...
async function acquireWorkspaceLeasePart(
  ...
  try {
    const part: OwnedWorkspaceLeasePart = {
      lockPath,
...
function operateEveryLeasePart(
  ...
  for (const part of parts) {
    try {
      ...
      // An attempted release is final: false means ownership was already lost
      // and a throw is surfaced to the caller, so nothing is left to wait for.
      if (operation === 'release') forgetHeldLeasePart(part);
    }
  }
