...
export class MemoryObservationCoordinator {
  ...
  #preferHint=true;
  #hints=new Map<string,'codex'|'claude'|undefined>();
  ...
  request(root:string,agent?:'codex'|'claude'):void {
    if(!this.deps.enabled()||!root||root.length>4096||/[\0\r\n]/.test(root))return;
    if(this.#hints.size>=64&&!this.#hints.has(root))this.#hints.delete(this.#hints.keys().next().value!);
    // A background continuation must not erase a pending Workroom agent hint.
    this.#hints.set(root,agent??this.#hints.get(root));
  }
  ...
  async tick():Promise<void>{
    ...
    try{
      ...
      for(const root of this.#hints.keys()){
      ...
      if(!selected)return;
      const hintedAgent=useHint?this.#hints.get(selected.projectRoot):undefined;
      ...
      // The existing per-tick byte budget remains unchanged. On a Workroom hint,
      // inspect that agent's newest changed source before older session hashes.
      // Ordinary ticks and partial-source continuations retain cursor fairness.
      const modified=(s:MemoryObservationCandidate)=>{
        const value=Number(s.stamp.split(':').at(-1));return Number.isFinite(value)?value:0;
      };
      const recent=hintedAgent?eligible.filter(s=>s.agent===hintedAgent)
        .sort((a,b)=>modified(b)-modified(a)||a.key.localeCompare(b.key))[0]:undefined;
      const source=recent??eligible.find(s=>!after||s.key.localeCompare(after)>0)??eligible[0];
      if(!source)return;
