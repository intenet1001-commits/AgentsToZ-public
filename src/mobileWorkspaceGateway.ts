...
export function createMobileWorkspaceGateway(deps: {
  ...
  perform(request:MobileWorkspaceRequest,targetId:string,owner:string,active:()=>boolean,testerAccess?:{canRun:boolean;executionAllowed:()=>boolean}):Promise<MobileWorkspaceResult>;
  ...
  return async (request,bindings,owner,transportActive=()=>true) => {
    ...
    const result=await deps.perform(request,target.runtimeTargetId,consent.requestOwner,allowed,tester?{canRun:consent.workspaceScopes.includes('tester.run'),executionAllowed:consent.executionAllowed??(()=>false)}:undefined);
