...
export interface WindowsSupervisedLaunchOptions {
  /**
   * Optional owner whose death must close the Job Object. This is used for
   * remote-control runtimes that must not outlive the local API sidecar.
   */
  parentPid?: number;
}
...
export function buildWindowsSupervisedLaunch(
  ...
  options: WindowsSupervisedLaunchOptions = {},
  ...
  if (options.parentPid !== undefined
    && (!Number.isSafeInteger(options.parentPid)
      || options.parentPid <= 1
      || options.parentPid > 0xffff_ffff)) {
    throw new Error('Windows supervisor parent PID must be a safe process identifier.');
  }
  return {
    cmd: [
      'powershell.exe',
    ...
    env: {
      ...
      ...(options.parentPid === undefined
        ? {}
        : { AGENTSTOZ_SUPERVISOR_PARENT_PID: String(options.parentPid) }),
    },
  };
}
