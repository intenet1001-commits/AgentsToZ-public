...
import {saveToken,MEMORY_SAVE_SCHEMA_VERSION} from './memorySaveContract';
...
export function readMemoryObservationSummary(path:string,memoryId:string):{completedTurns:number;hasMore:boolean} {
  ...
  try{
    ...
    if(version<2||version>MEMORY_SAVE_SCHEMA_VERSION)throw new Error('Unsupported observation store');
