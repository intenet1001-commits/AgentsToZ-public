...
const mcpUnixOutput = join(resourceDir, "agentstoz-use-mcp");
const mcpWindowsOutput = `${mcpUnixOutput}.exe`;
const mcpOutputPath = process.platform === "win32" ? mcpWindowsOutput : mcpUnixOutput;
...
for (const stale of process.platform === "win32"
  ? [unixOutput, mcpUnixOutput]
  : [windowsOutput, mcpWindowsOutput]) {
  rmSync(stale, { force: true });
}

console.log(`[build-sidecar] compiling ${process.platform}/${process.arch}`);
for (const [entrypoint, outfile] of [
  ["api-server.ts", outputPath],
  ["agentstoz-use-mcp-server.ts", mcpOutputPath],
] as const) {
  ...
  if (exitCode !== 0) {
    console.error(`[build-sidecar] ${entrypoint} compile failed (${exitCode})`);
...
if (process.platform !== "win32") {
  ...
  chmodSync(mcpOutputPath, 0o755);
}
console.log(`[build-sidecar] ready: ${outputPath}`);
console.log(`[build-sidecar] ready: ${mcpOutputPath}`);
